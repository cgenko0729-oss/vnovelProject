...
在立绘上叠一张透明局部图，强度 0~1 可以连续变化，**多层能同时存在**。

```
overlay 星野结衣 潮红 0.5              半强度潮红（0.35 秒淡入）
overlay 星野结衣 潮红 1 time:2         两秒慢慢红透
overlay 星野结衣 汗 0.8                再叠一层汗，潮红不受影响
overlay 星野结衣 潮红 0                单独退掉潮红
overlay 星野结衣 clear                 全部清空
```

- 强度省略 = 1；`time:` 省略 = 0.35 秒。命令**瞬发不等待**，要等就自己接 `wait`。
- 层在**角色资产** `VNCharacterDef.overlays` 里登记（层 id + 透明图 + maxAlpha）。
  图要保留与默认立绘完全相同的整张透明画布，只在该部位留像素 —— 和闭眼图/张嘴图同一个要求。
- `maxAlpha` 是这一层「强度 1.0」时的实际不透明度。潮红通常不需要满不透明，配 0.85 左右。
- **状态进存档**，读档后潮红还在。

**为什么不做成表情**：「表情 × 潮红三档」是乘法爆炸，每个组合都得画一张完整立绘；
叠加层是加法，一张潮红图配所有表情，而且强度能连续补间而不是跳变。

### imprint — 立绘痕迹（掌印 / 口红印 / 绳痕 …）

在立绘的指定位置印一枚痕迹，随时间褪色（红→粉→无）并自行消失。

```
imprint 星野结衣 手掌 pos:0.01,0.15          在脸颊印一个掌印
imprint 星野结衣 手掌 pos:0.1,0 life:15      存在 15 秒
imprint 星野结衣 手掌 pos:0,0 size:1.5 rot:30  放大一半、转 30 度
imprint 星野结衣 clear                       清掉全部痕迹
```

- `pos` 是**立绘归一化坐标**，(0,0) = 立绘中心，x/y 各取 -0.5~0.5 ——
  与部位框、`markAnchor` 完全同一套。留空 = 立绘正中。
- `life` 省略 = 用资产里配的默认时长。`size` 是倍率，`rot` 是基准角度
  （每枚还会自动叠一点随机旋转，否则连点出来一排一模一样的印子很假）。
- 痕迹在**角色资产** `VNCharacterDef.imprints` 里登记：id + 图 + 基准尺寸 +
  着色（alpha 就是印上瞬间的浓度）+ 默认时长 + 随机旋转范围。
- 同屏上限 20 枚，超了自动淘汰最旧的。
- **临时演出，不进存档** —— 它会自己消失，互动结束时也会被清掉。
...
camseq mode:depth               # mode：谁跟着 zoom 缩放（写在 camseq 行，整段一个模式）
> 亚里沙:head 1.8 1.5           # 立绘比背景多缩放一点 = 有纵深的推镜

camto 亚里沙:head 1.6 0.8       # 单段直达：目标点 [zoom] [秒] [ease:名]
camcut topright 1.8             # 瞬切到镜头状态（"一开始就在那里"）
```

**`mode:` 缩放模式**（`camseq` 行上写，默认 `both` 可省略；`camto`/`camcut` 没有这个参数）：

| mode | 背景 | 立绘 | 什么时候用 |
|---|---|---|---|
| `both` | 跟着 zoom | 跟着 zoom | **推拉镜（TU/TB），最常用的默认**。情绪升温、场面收束 |
| `depth` | 跟着 zoom | 比背景多缩一点 | **有纵深的推拉镜**。等比缩放其实是「数码变焦」，真镜头推近时近处的人放大得比远景快——想让推镜「像有摄影机」就用这档 |
| `bg` | 跟着 zoom | **尺寸不变** | **眩晕变焦**（希区柯克变焦）。世界观崩塌、震惊的临界点，**杀伤力极强，全篇用 1~2 次** |
| `char` | **纹丝不动** | 跟着 zoom | **变焦推**：强调某人的反应、突发惊愕。背景分辨率不够时靠它做特写而不把背景放糊 |

```
camseq mode:bg                  # 眩晕变焦：背景被拉扯，人却一动不动
> middle 1.5 1.2
> middle 1 0.8

camseq mode:char                # 只放大立绘，背景完全静止
> 亚里沙:head 1.6 0.25 ease:OutBack
> 亚里沙:head 1 0.4
```

- 模式作用于**整段**，不逐点写——逐点切换会让立绘尺寸在两点之间跳变
- `depth` 的视差系数固定 0.5（组件 `VNCamera.depthRatio` 可全局调）。
  数值本身不重要，**全篇一致才重要**
- 参与缩放的是**全部在场立绘**，由 VNStage 自动维护；同一层的人一起动，
  否则两个人站一起会一大一小
- 立绘的运镜倍率与说话者高亮的倍率是**两个通道相乘**，所以推完镜头再说话，
  尺寸不会被打回去
- **zoom 别写小于 1**：会露出画布边缘。「拉远」一律以 1.0 为底
...
| 配图 | 可选，一张 Sprite |
| 卡片位置 | 自动（躲开洞口）/ 上 / 中 / 下 / **自定义**（归一化坐标＝卡片中心，左下原点；编辑器画布上直接拖卡片即可，拖一下自动切成自定义） |
| 卡片尺寸 | 宽度（px，1920 基准）+ 整体缩放（字号·边距·配图一起等比变，高度仍由内容撑）；「整篇设置」给默认值，每步可覆盖（0 = 用整篇）；编辑器画布上拖卡片**右下角手柄**改宽度、**Shift+拖**改缩放。皮肤 prefab 的卡片同样受控 |
| `once` | 看过就不再自动播（默认开） |
...
鼠标变成道具图标，点或按住拖动去摸角色的部位；摸够了推进阶段，角色换表情、说台词、
出漫符、放语音。

```
show 星野结衣 at:center expr:默认        # event 之前必须先让角色出场
...
| 参数 | 说明 |
|---|---|
| `vs:` | 被互动的角色 id（**必须已经 show 出场**，模块只换表情不负责出场） |
| `id:` | 互动定义资产 `VNInteractionDef` 的 id |
| `items:` | 本场可用道具，逗号分隔；留空 = 资产里全部。玩家在右侧道具栏里切换 |
| `time:` | 限时（秒），留空 = 不限时 |
| `flag:` | 成绩 flag 前缀，覆盖资产里的设置 |
| `zones:on` | 把部位框画出来（**开发调试用**，正式剧本删掉） |

**`* 拒绝` 必须接住** —— 玩家摸了还没解禁的禁忌部位、次数到上限就会走这条，
不接的话会静默跳过（同 aitalk 的 `* 失败`）。

成绩写 flag：`<前缀>_兴奋度`、`<前缀>_阶段`、`<前缀>_拒绝数`、`<前缀>_<部位>次数`。

**操作**：按住左键在角色身上来回移动 = 抚摸（累计移动距离），单击也算一点点。
右下角「结束」可以随时收手，按当前进度判档。

**两份资产**
- `VNTouchZoneDef`（部位在哪）：一个角色一份。用
  **Tools → VN Effects → 预览 Preview → 部位区域编辑器** 在立绘上直接拖框画，
  别手填坐标。构图不同的立绘（坐下/躺下/近景）可以加「覆盖层」只重画动过的那几个框。
- `VNInteractionDef`（摸了会怎样）：道具、阶段阈值、每个部位的反馈池、禁忌部位的
  解禁阶段、结束条件。反馈既可以填字段（表情/漫符/情绪动作/叠加层/台词/语音池/音效/属性），
  也可以写**内嵌剧本行**跑任意演出（`fx` / `camseq` / `liquid` …）——
  字段配不出来的复杂演出走这里。
...
| `{zx}` `{zy}` | 当前部位的中心 |
| `{px}` `{py}` | 角色中心 |
| `{nx}` `{ny}` | 光标处的**立绘归一化坐标**（-0.5~0.5，`imprint` 用这个） |
| `{znx}` `{zny}` | 当前部位中心的立绘归一化坐标 |
| `{char}` | 本场互动的角色 id（模板资产不用写死名字） |
...
**点一下印一个掌印**：在「脸」的规则上限定道具（`itemId` 填 `手掌`），
反馈的 `trigger` 选「只在点击时」，`scriptLines` 写：

```
imprint {char} 手掌 pos:{nx},{ny} life:7
```

`{nx}{ny}` 是光标处的**立绘归一化坐标**（`imprint` 要的坐标系，和 `liquid` 的
viewport 比例不是一回事）。`trigger` 不设「只在点击时」的话，拖一下会连着印出一串。
...
quest claim 球场之王          # 剧本内强制领取（「她当场把奖励塞给你」）
quest offer 摄影社的委托      # 推上委托板，但不接取
quest abandon 球场之王        # 放弃，回未接取（不发惩罚）
quest reset 球场之王          # 清空全部状态含完成次数（剧情重置 / 调试）
```

- 任务状态存在 flag `任务_<id>`：`0` 未接取 / `1..98` 进行中阶段 /
  `100` 完成 / `-1` 失败——if 可直接判断。
  **阶段号上限是 98**（100 与 -1 是完成/失败的保留值），Lint 会卡。
- 玩家按 **J** 打开任务日志面板：**可领取 / 进行中 / 已完成 / 已失败** 四栏，
  可领取那栏置顶、金色高亮、带「领取」按钮。有可领取时右上角出现 `任务 ✦N` 角标。
- 任务配置在 **VNQuestDef 资产**（右键 **Create → VN → Quest Definition**）；
  **没有资产也能运作**（id 当标题，但没有条件与奖励）。

#### 自动判定：给阶段配完成条件

资产里每个阶段可以配若干**子目标**，每条子目标一个 flag 表达式。
**全部达成 → 任务转「可领取」**，玩家在 J 键面板点领取才发奖、才推进到下一阶段。

```
阶段 1「先赢下一场比赛」
  子目标：打赢一场羽毛球     条件 羽球_我方得分@最高>=21
  奖励：  金钱 +200
阶段 2「单场拿下 5000 分」
  子目标：单场得分达到 5000  条件 羽球_我方得分@最高>=5000
          进度条 flag 羽球_我方得分@最高，目标值 5000
  奖励：  金钱 +1000、flag 待触发_球王庆功 = 1
```

- 条件是普通的 flag 表达式，语法与 `if` 完全一样（`&&` `||` 比较 括号，中间不能有空格）。
- **一条带条件的子目标都没有的阶段不会自动判定**，仍靠剧本 `quest stage` / `quest done`
  推进——所以「自动任务」和「剧情任务」可以在同一套系统里共存。
- 子目标可以另外声明「进度条看哪个 flag / 目标值多少」，面板里就会画进度条与 `3/5`。

#### ⭐ 统计声明：小游戏只写「本次成绩」

**这是最容易卡住的一步。** 小游戏写回的是**本次成绩**——`羽球_我方得分` 打完一场是 21，
下一场直接覆盖，永远不会等于 5000。要写「单场 5000 分」「累计打 10 场」这类条件，
得先在 **VNGameConfig → 玩法页签 → 统计声明** 里登记源 flag：

| 声明 | 自动派生出的 flag |
|---|---|
| `羽球_我方得分` 勾 最高 / 累计 / 次数 | `羽球_我方得分@最高`、`@累计`、`@次数` |
| `照片_分数` 勾 最高 / 次数 | `照片_分数@最高`、`@次数` |

派生值本身也是普通 flag，随存档走，条件里直接写就行。两个语义要注意：

- **`@累计` 累的是「每次写入的值本身」**，适合成绩类（本场 21 + 上场 18 = 39）。
  `金钱` 这种**余额型** flag 累计没有意义，要总收入请另开一个只写增量的 flag。
- **`@次数` 数的是写入次数**，所以要求模块只在结束时写一次结果 flag。

> `@` 从此是引擎保留字：任务 / 属性 / 道具 id 都不能含它，Lint 会报错。

#### 奖励能给什么

| 种类 | 目标 | 效果 |
|---|---|---|
| 属性 | 属性 id | 走 `stat` 那条路，带钳制与 HUD 数字滚动、`+N` 上飘 |
| 道具 | 道具 id | `道具_<id> +N`，背包与装备系统自动接住 |
| flag | 任意 flag 名 | **剧情解锁走这条**（见下） |
| CG | CG id | 解锁鉴赏画廊条目（全局，不随存档回退） |
| 任务 | 任务 id | 连锁接取下一个任务，任务链用 |

**「触发特殊剧情」怎么落地**：奖励只写一个 flag（比如 `待触发_球王庆功 = 1`），
剧本自己在日程节点或章节开头判断——**绝不会打断当前演出**：

```
label 每月开头
if 待触发_球王庆功 jump 球王庆功
...

label 球王庆功
flag 待触发_球王庆功 0        # 用掉它，免得每月都触发
星野结衣: 恭喜你啊，球场之王。
```

#### 限时、日常、任务链

- **限时**：资产填「接取后 N 个月内完成」，超期转失败（可配惩罚）或自动放弃。
  时间基准是 `time pass` 维护的 **`月序`**（单调递增），
  **不是**会 1~12 循环的日历「月份」——所以剧本里必须有 `time` 命令，否则永不过期（Lint 会提醒）。
- **可重复日常**：勾「可重复」+ 冷却月数 + 总次数上限。完成后到月自动回到未接取，
  配合「自动接取」就是「每月自动来一个日常委托」。
- **任务链**：填「前置任务」列表，全部完成后本任务才可接。成环 Lint 会报错。

#### event questboard — 委托板

```
event questboard tag:社团 max:3
* 接取
    她把委托单递了过来。
* 离开
    今天没什么想接的。
...
每条显示标题、委托人、报酬预览、期限。玩家可以连着接好几个，点「离开」才结束。

| 参数 | 说明 |
|---|---|
| `tag:` | 只列这个分类标签的委托（资产里填）；留空 = 全部 |
| `max:` | 同时进行中的任务上限，超了「接下」按钮变灰；`0`/留空 = 不限 |
| `title:` | 板子标题，留空用默认「—— 委托板 ——」 |

结果名两个：至少接下一份是 `接取`，一份都没接是 `离开`。

> 装机：**Tools → VN Effects → 场景装机 Install To Scene → 委托板 Quest Board**。
> 顺带会补上 `VNQuestLog`、铺一套默认统计声明，工程里缺哪个示例任务就补哪个。
...
不是剧本命令，是**解锁后玩家随时能进的全局功能**（右上角相机图标）。剧本只管两件事：

```
flag 秘密相机=1        # 解锁：右上角出现相机图标（flag 归零即收回）
flag 道具_胶卷+10       # 给胶卷；或 event shop id:杂货店 让玩家去买（装机器会建这间店）
```

- **进入条件**：停在台词上（与存档同规则）、没有事件/面板/SNS 盖着、有胶卷。
  不满足时点图标会提示原因；没胶卷图标变灰
- **操作**：滚轮缩放（1.0～1.6x，以指针为中心）、左键拖动平移、底部快门或空格拍照、Esc 退出。
  进入后对话框与 HUD 隐藏，退出按原状态还原（`hideHUD keep` 的段落不会被弹回）
- **察觉**：镜头对着在场角色就涨，越近越快、停越久越多，**她不在取景框里不涨**；
  察觉条满 = 被发现：好感 -3（可配）、flag `偷拍_警惕_<角色>` +10（永久，下次涨得更快）、
  她做一个情绪动作 + 漫符 + 在对话框里说一句、强制退出。**被发现那一下按的快门照样扣胶卷**
- **照片**：整屏 16:9，存 `persistentDataPath/vn_secret_photos/`（独立于大头贴相册与存档），
  G 键画廊里多出「私密」页（解锁后或相册非空时才出现），可全屏翻页、删除
- **多人同场**锁定取景框中心最近的那个；场上没人可拍风景（不涨察觉、照样扣胶卷）
- **参数与台词**全在 `VNSecretPhotoDef` 资产（Create → VN → Secret Photo Settings，
  登记进 VNGameConfig「玩法」页 `secretPhoto`）；被发现台词三语、可按角色覆盖；
  首次进入自动播教程 `秘密相机`
- **装机**：Tools → VN Effects → 场景装机 Install To Scene → **秘密偷拍 Secret Photo**
  （建参数资产、教程资产、示例商店并登记，不动场景）
- 状态全在 flag，存档格式零改动；模式打开期间不能存读档
...
| VNQuestDef | `Assets/Art/VNEffects/Quests/`（可选） | 任务：文案 + 阶段子目标条件 + 奖励 + 限时 + 可重复日常 |
| VNSecretPhotoDef | `Assets/VNEffects/SecretPhoto/`（单份，可选） | 偷拍模式：解锁 flag / 胶卷 / 缩放范围 / 察觉涨速 / 惩罚 / 被发现台词 |
...
| | `quest-unknown-id` | `quest` 引用了未登记的任务 id，或任务的前置任务不存在 |
| | `quest-condition-syntax` | 任务资产里的条件表达式语法错（**错误级**） |
| | `quest-unknown-identifier` | **最要紧的一条**：条件里的 flag 名全项目从没人写过 —— 拼错会**静默求值为 0**，任务永远不完成且运行时零报错 |
| | `quest-tracker-missing` | 条件用了 `@最高` 这类统计值，但 VNGameConfig 里没声明这个统计源（永远读不到值） |
| | `quest-stage-range` | `quest stage` 的阶段号超出 1..98（**错误级**；100 与 -1 是完成/失败的保留值），或超过该资产的阶段数 |
| | `quest-reward-unknown-target` | 奖励指向未登记的属性 / 道具 / CG / 任务 id |
| | `quest-no-entry` | 任务配了完成条件，却没有任何接取出口（没人 `quest start`、不上委托板、不自动接、也没别的任务把它当奖励）→ 永远不会出现 |
| | `quest-deadline-no-time` | 用了限时，但整个剧本没有 `time` 命令 → `月序` 永远是 0，永不过期 |
| | `quest-requires-cycle` | 前置任务链成环（**错误级**），环上的任务全都永远接不了 |
| | `quest-reserved-char` | 任务 / 属性 / 道具 id 含保留字符 `@`（**错误级**） |
...
overlay <角色> <层|clear> [强度0~1] [time:秒]      情绪叠加层（潮红/汗/泪，进存档）
imprint <角色> <痕迹|clear> [pos:x,y] [size:] [life:] [rot:]  立绘痕迹（掌印等，会自己消退）
<角色> [表情]: 台词                              说话
move <角色> <位置> [秒]                          滑步换位
wait <秒>                                        停顿
...
event aitalk vs:<角色> [persona:] [turns:] [topic:] [place:] [me:] [stat: rate:] [flag:]  AI自由聊天
  结果 好感提升/普通/冷场/失败 —— * 失败 必须接住；event 前要先 show 角色
                                                 结果 全对/及格/失败
quest <start|stage|done|fail> <id> [阶段]        任务（阶段号 1..98）
quest <claim|offer|abandon|reset> <id>           领取 / 推上委托板 / 放弃 / 清空
event questboard [tag:] [max:] [title:]          委托板，结果 接取/离开
