...
- **贴图程序化生成**：先查 [`VNProceduralTextures`](../../../Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) 有没有现成形状，没有再加生成函数，零美术依赖。
...
- **要改亮度/饱和/色相 → 走 `SetGrade(VNGradeLayer.X, …)`，禁止直接写
  ...
  新效果要么复用现成通道，要么在 [`VNGrade.cs`](../../../Assets/Project/Scripts/VNEffects/VNGrade.cs) 的 `VNGradeLayer` 里加一条
  （加在 `Count` 之前即可，合并逻辑不用改）。
- **情绪色调 mood 不是全屏后处理**（一一〇章）：色彩逐层写进各自材质实例。
  新加的图层想被 mood 染色 → 注册进 [`VNMoodGrading`](../../../Assets/Project/Scripts/VNEffects/VNMoodGrading.cs) 的目标列表；想躲开 → 别注册。
...
- 文字一律 TextMeshPro + [`VNFont.Asset`](../../../Assets/Project/Scripts/VNEffects/Script/VNFont.cs)；输入一律新 Input System。

## 操作清单
- [ ] 参考同类组件写（组件分类见 CLAUDE.md「组件速查」表 / ProjectCodeGuide 八）
- [ ] 要接 `fx` 命令：VNStage 加引用 + [`Fx()`](../../../Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1614) 路由 + `_fxStates` + 快照，**三件套 + 存档一处不能少**
      （进存档细节走 [vn-save-compat]）
- [ ] 整屏运动类：确认挂在哪个容器层（SceneRoot/ZoomRoot/TiltRoot 各司其职，见 SetUpGuide 第 0/3 章），
      不要两种运动挤一层
- [ ] [`VNEffectsDemoSetup`](../../../Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) 生成器里创建/连线 → **Tools → VN Effects → 演示场景 Demo Scenes → 重建特效演示场景 Create Demo Scene 重建演示场景**
