...
| 存档 | **进存档** | **不进存档** |
| 起步模板 | Tools → VN Effects → UI 皮肤 UI Skins → 导出皮肤模板（默认+样例）Export Skin Prefabs | Tools → VN Effects → UI 皮肤 UI Skins → 系统主题：导出默认模板 System UI: Export Default Prefabs（11 个，含背包/排程面板/结算弹窗）；只重导排程+结算两项用 Export Event Panel Prefabs（不覆盖手改过的其他 prefab） |
...
- 编辑期创建、随场景/prefab 保存的 TMP 文字必须引用
  `VNFontAssetBuilder.EnsureFontAsset()`（[`Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs:80`](../../../Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs#L80)） 的持久化字体资产——运行时临时资产存进
  prefab 会变 Missing。
