...
| 文件 | 一句话 |
|---|---|
| `VNFogMask.cs` | 掩码层：擦除 Stamp / 线段补点 / 边缘权重图 / 回雾衰减 / 清晰度求和。**纯逻辑无 MonoBehaviour**，编辑器调参窗口跑的是同一份代码 |
| `VNFogScore.cs` | 阶段推进（只升不降）+ 三档判定（按峰值）。**纯静态可单测**，沿用 `VNTouchScore` / `VNPhotoScore` 的分层 |
| `VNFogWipeDef.cs` | 定义资产：雾外观九参 / 笔刷 / 回雾五参 / 规则 / 分阶段台词（三语 + 语音 + 属性奖励）/ 音效覆盖 |
| `VNFogWipeModule.cs` | 事件模块：自绘底图与雾层、输入轮询、HUD、台词条、倒计时、ESC 确认、结算、flag 回写 |
| `VNFogSfx.cs` | 四个代码合成音效；擦拭循环音的音量与音调跟鼠标速度联动 |
| `VNFogTextures.cs` | 四种道具光标的程序化 SDF 贴图（雨刷/手掌/抹布/手指），RGB 存明暗、A 存形状 |

**新增 —— 编辑器（`Assets/Project/Scripts/VNEffects/Editor/`）**

| 文件 | 一句话 |
|---|---|
| `VNFogWipeInstaller.cs` | 场景装机（Tools → VN Effects → 场景装机 → **擦雾 Fog Wipe**）：补禁用的 FogWipeTemplate（必须带 RectTransform）+ 造示例资产 + 登记进 VNGameConfig，可重复执行 |
| `VNFogTuneWindow.cs` | 调参窗口（Tools → VN Effects → 预览 → **擦雾调参 Fog Wipe Tuning**）：上半**算出**预计通关秒数并给手感评语，下半是可拖鼠标试擦的掩码预览，回雾照参数实时跑 |
...
| 文件 | 改了什么 |
|---|---|
| `Script/VNGameConfig.cs` | 「擦雾库」`fogWipes` |
| `Editor/VNGameConfigEditor.cs` | `fogWipes` 登记进「玩法」页 |
| `Editor/VNScenarioSchema.cs` | `wipefog` 的 EventVariants 一行（9 个参数）+ event 帮助文本 |
| `Editor/VNScenarioLinter.cs` | 结果名表 + `fogWipeIds` 扫描 + 三条检查 |
...
`VNInteractionModule` / `VNAiTalkModule` 那样需要破模块三铁律**，全部绘制都在自己的
...
**CG 解锁要在模块里补一次。** `VNCgUnlocks.Unlock` 本来是 `VNStage.ShowCg` 顺手做的，
...
**光标不用 `Cursor.SetCursor`。** 沿用 `VNTouchCursor`（硬件光标做不了摆动/按住震动/
...
| 5 | 关一次调参窗口泄漏一张贴图 | `VNFogMask.Destroy()` 用了 `Object.Destroy`，编辑模式下只打一条 error 什么都不销毁 → 按 `Application.isPlaying` 分路 |
...
`<前缀>_档位`（**整数** 2 完美 / 1 普通 / 0 失败——`VNFlags` 是整型字典，同 `VNPhotoBoothModule`）。
...
## 一三四、任务系统升级：条件自动判定 + 可领取 + 奖励发放（2026-09-02，分支 `feature/quest-system`）

### 需求 / 背景

原来的任务系统只有两个东西：`VNQuestDef`（纯文案资产：标题/描述/阶段文案）和
`VNQuestLog`（J 键日志面板 + `quest start|stage|done|fail` 命令），状态存 flag
`任务_<id>`。**全是手动的**——剧本不写 `quest done` 就永远不会完成，也没有任何奖励机制。

用户要的是：任务能配「羽毛球单场拿 5000 分」「在商店买到特定相机」这类**完成条件**，
达成后自动判定；奖励也能配——给钱、给非卖品道具、加好感、解锁 CG、触发特殊剧情。

三轮问答确定的方案：
判定走**半自动**（条件自动达成 → 标记「可领取」→ 玩家在 J 键面板点领取）；
任务来源**三条都要**（剧本发布 / 委托板接取 / 满足条件自动接取）；
剧情类奖励**只写 flag**，剧本在日程节点自己 `if` 接，绝不打断当前演出；
另加限时过期、前置任务链、可重复日常、多子目标带进度条。

### 文件改动清单

**新增（运行时）**

- `VNTracker.cs` —— 统计层。按 `VNGameConfig.trackers` 的声明，把源 flag 派生成
  `@最高 / @最低 / @累计 / @次数`。纯静态、可单测。
- `VNQuestEngine.cs` —— 引擎核心。条件求值、状态推进、领取发奖、超期判定、
  日常冷却重置。不建 UI、不碰舞台，纯逻辑可单测。
- `VNQuestReward.cs` —— 一条奖励的数据结构 + 发放器。五类：`Stat`（走
  `VNStatsHud.Apply`，带钳制与 HUD 飘字）/ `Item`（`道具_<id>`）/ `Flag`（任意 flag，
  剧情解锁走这条）/ `Cg`（`VNCgUnlocks`）/ `Quest`（连锁接取，任务链用）。
- `VNQuestBoardModule.cs` —— 委托板事件模块 `event questboard`，结果 `接取` / `离开`。

**新增（编辑器）**

- `VNQuestBoardInstaller.cs` —— 场景装机菜单
  **Tools → VN Effects → 场景装机 Install To Scene → 委托板 Quest Board**：
  注册模块模板、补 `VNQuestLog`、生成三个示例任务、铺一套默认统计声明。

**修改**

- `VNQuestDef.cs` —— 新增 `Stage` / `Objective` 两层结构（阶段文案 + 子目标条件 +
  进度声明 + 阶段奖励）、接取方式、前置任务、限时、可重复日常、隐藏任务等字段。
  **旧 `stages: List<string>` 原样保留**：`stageDefs` 为空时回退用它，老资产零改动
  照常工作，只是不会自动判定。
- `VNQuestLog.cs` —— 面板从三栏改四栏（**可领取置顶**、金色高亮、带「领取」按钮、
  子目标 ☑/☐ 逐条显示、带进度条），并负责驱动引擎求值与右上角角标。
- `VNFlags.cs` —— 新增带 key 的 `KeyChanged` 事件。
- `VNSaveSystem.cs` —— 读档期间挂起引擎与统计层，恢复后 `RecalculateSilently()`。
- `VNScriptRunner.cs` —— `time` 命令维护「月序」；调试重建期间同样挂起两者。
- `VNToast.cs` —— 新增独立角标槽位 `SetBadge()`。
- `VNGameConfig.cs` —— 新增 `trackers` 列表。
- 编辑器三处：`VNScenarioSchema`（`quest` 新子命令 + `event questboard` 参数格）、
  `VNScenarioLinter`（十条 quest 检查 + `questboard` 结果名）、
  `VNGameConfigEditor`（`trackers` 归入玩法页签）。
- `Resources/VNLocale/ui.{zh,en,ja}.txt` —— 新增 12 条 UI 字符串。

### 技术决策与取舍

**「可领取」不写进主 flag，走旁路 `任务_<id>@待领`**

最初的方案是 `任务_球场之王 = 99`。但主 flag 的语义是**阶段号**，塞 99 进去有三个问题：
剧本里的 `if 任务_xx==2` 会在可领取期间失效；多阶段任务丢失「是哪个阶段可领」的信息；
现有 `quest start <id> N` 走 `Mathf.Max(1, stage)` 且没有上限，写个 99 就撞上了。
改成主 flag 永远只存阶段号，另开 `@待领` 存待领取的阶段号（0 = 无）。
代价是阶段号合法范围收窄到 `1..98`（100 与 -1 是完成/失败的保留值），由 Lint 卡死。

**限时基准是新增的「月序」，不是日历「月份」**

`time pass` 的实现是「月份 +N 后在 1~12 内循环，剩余月数 -N」，所以 `月份` 是**日历月**
不是**绝对时间**：11 月接的 3 个月期限任务到期该是次年 2 月，而 `月份>=14` 永远不成立。
引擎自己维护一个单调递增的 `月序`（`time set` 只初始化不重置，`time pass` 每过一月 +1）。
没用现成的 `剩余月数`，是因为那个只有养成模式才会被 `time set … remain:` 设上，
纯剧情章节里恒为 0。

**读档必须挂起统计层与引擎**

读档是把整张 flag 字典逐个 `Set` 回去，每一次都会触发变化事件。不挂起的话，
读一次档 `@累计` 就被整份历史再加一遍、`@次数` 跟着涨——而派生值本来就存在档里，
重算纯属破坏。同一个开关顺带保护任务引擎，避免在半截状态上弹「任务完成」Toast。
调试重建（从选中行播放）走同一条路径。

**统计层靠 `KeyChanged` 而不是 diff 字典**

`VNFlags.Changed` 不带参数，拿不到「是哪个 flag 变了」。原本打算靠缓存上一帧快照做 diff，
但那会把「两场都打 21 分」算成一场（值没变），`@次数` 与 `@累计` 就错了。
所以给 `VNFlags` 加了一个带 key 的事件——值相同的重复 `Set` 也会触发，那确实是一次新的写入。

**`@` 成为引擎保留字**

已验证 `@` 不在 `VNExpression` 的标识符分隔符黑名单里（那是黑名单式：除分隔符外任何字符
都合法），所以 `羽球_我方得分@最高>=5000` 现在就能被求值器直接解析，**表达式解析器零改动**。
代价是任务 / 属性 / 道具 id 从此不得含 `@`，由 Lint 卡死。

**状态全落 flag，`VNSaveData` 零改动**

任务状态、统计派生值、`月序` 全是普通 flag，本来就随 flag 字典整份序列化——
`vn-save-compat` 那份「三处同步」清单这次一处都不用改。这是把状态压在 flag 上换来的最大红利。

**求值节奏：标脏 + 下一帧一次**

照抄属性 HUD 的做法，一帧最多算一次，且只算「进行中」的任务。几十个任务的量级
每帧几十次表达式求值毫无压力。幂等由状态机保证：奖励只在「`@待领` → 领取」这一次
跳变里发放，清 `@待领` 与写下一阶段在同一步完成。

### 修复记录

**装机跑了但示例任务一个都没生成**

`EnsureSampleQuests()` 里写的守卫是
`if (AssetDatabase.FindAssets("t:VNQuestDef").Length > 0) return;`——
「工程里一个任务资产都没有才造示例」。但工程里早就有 `告白大作战`，于是三个示例
一个都没造出来。症状有两个：剧本编辑器的 `quest` id 下拉搜不到「球场之王」（候选扫的
就是工程里的资产）；运行时任务日志只有光秃秃一个标题，没有描述与阶段文案（走了
「未登记任务用 id 当标题」的回退路径）。

改成**逐个按 id 判断存不存在**，缺哪个补哪个，已有的原样保留。顺带两处：
资产造在**既有任务资产所在的目录**（工程整理过，实际是 `Assets/Art/VNEffects/Quests`，
而代码里写死的 `Assets/VNEffects/Quests` 是 `VNEffectsDemoSetup` 的老常量、已与实际不符）；
装机结尾补上 `VNAssetLibraryEvents.RaiseChanged()`，让剧本编辑器立刻重建 id 下拉候选，
不用手点 `Refresh Sources`——这条是 CLAUDE.md 里写明的约定，第一版漏了。

### 验证方法

- **引擎自测 32 项断言全通过**（用 `script-execute` 在编辑器里跑）：统计派生、
  低分不覆盖最高、值相同也计次、接取→可领取→领取→下一阶段、重复领取不发奖（幂等）、
  末阶段完成、限时边界（第 2 个月末不过期 / 第 3 个月过期）、失败发惩罚、
  日常自动接取与跨月重置、挂起期间 `@累计`/`@次数` 不变。
- **Lint 跑通**，并在既有剧本里抓到一个真实问题：`quest stage 告白大作战 3`，
  但那个资产只有 2 个阶段（既有数据问题，本次未改动）。
- **既有资产兼容**：`告白大作战` 读出来 `旧stages=2 / StageCount=2 / 阶段文案正常 /
  有条件=False`——老资产照常工作、不自动判定，正是预期。
- Unity 运行时与编辑器程序集编译无错误。
