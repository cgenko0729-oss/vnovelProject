...
### 3. [`Assets/Scripts/VNEffects/VNProceduralTextures.cs`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) — 程序化贴图生成器
...
### 4. [`Assets/Scripts/VNEffects/VNImageEffectController.cs`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) — 单图特效控制器
...
### 5. [`Assets/Scripts/VNEffects/VNGlowBackdrop.cs`](Assets/Project/Scripts/VNEffects/VNGlowBackdrop.cs) — 背后柔光光环

挂在立绘上，自动在其**背后**（同父级、前一个渲染顺序）生成一个径向光晕 RawImage：
- 常态：呼吸脉动（`pulsePeriod` / `pulseStrength` 可调）
- [`Flare()`](Assets/Project/Scripts/VNEffects/VNGlowBackdrop.cs#L132)：出场瞬间光环从小到大闪耀一次再回落到呼吸状态
- 颜色 × `hdrIntensity`(默认1.6) 触发 Bloom，立绘像被一圈柔光包裹

### 6. [`Assets/Scripts/VNEffects/VNEntranceAnimator.cs`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs) — 出场/退场演出编排器
...
退场：[`PlayExitDissolve()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L494)（化作光点消散）、[`PlayExitFade()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L509)（淡出下滑）。
出场完成后调 [`StartIdleEffects()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L523) 一键开启常驻"活图"三件套（呼吸发光+悬浮+周期扫光）。
...
### 7. [`Assets/Scripts/VNEffects/VNAmbientParticles.cs`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs) — 悬浮氛围粒子
...
### 8. [`Assets/Scripts/VNEffects/VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs) — 演示驱动
...
### 9. [`Assets/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) — 一键生成演示场景
...
给任何立绘 Image 挂 [`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) + [`VNGlowBackdrop`](Assets/Project/Scripts/VNEffects/VNGlowBackdrop.cs) + [`VNEntranceAnimator`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs) 三个组件
...
场景里放几个空物体挂 [`VNAmbientParticles`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs)（选 Dust/Sparkles/Orbs 预设）即可获得常驻悬浮粒子。
...
- **构建（Build）注意**：脚本运行时用 `Shader.Find` 作后备，但正式打包请确保场景/材质资产
  ...
  程序化贴图在运行时生成，不受影响。
- **[`VNGlowBackdrop`](Assets/Project/Scripts/VNEffects/VNGlowBackdrop.cs) 在 Awake 读取图片 rect 尺寸**：立绘用固定 `sizeDelta` 没问题；若你的
...
5. 编写 [`VNProceduralTextures.cs`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs)（柔光圆/四芒星/径向光晕程序化生成）
6. 编写 [`VNImageEffectController.cs`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs)（材质实例管理 + 全部参数 API + 三种常驻循环）
7. 编写 [`VNGlowBackdrop.cs`](Assets/Project/Scripts/VNEffects/VNGlowBackdrop.cs)（背后光环：呼吸脉动 + 出场闪耀）
8. 编写 [`VNEntranceAnimator.cs`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs)（5 种出场 + 2 种退场 + 常驻效果一键开启）
9. 编写 [`VNAmbientParticles.cs`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs)（3 种氛围粒子预设 + 静态星光爆发）
10. 编写 [`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)（新输入系统按键演示 + 背景 Ken Burns）
11. 编写 [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（一键生成演示场景，自动配置贴图导入/材质/Bloom/连线）
...
### 9.1 God Rays 斜射光束 — [`VNGodRays.cs`](Assets/Project/Scripts/VNEffects/VNGodRays.cs) + 新贴图 `LightBeam`

- [`VNProceduralTextures`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) 新增 `LightBeam` 贴图（128×512：横向柔边 × 纵向上亮下渐隐），
...
### 9.2 动态暗角/聚焦渐晕 — [`VNVignetteFocus.cs`](Assets/Project/Scripts/VNEffects/VNVignetteFocus.cs)
...
- API：`FocusOn(transform)` 把角色世界坐标转视口坐标（`WorldToViewportPoint`）并把暗角
  中心补间过去、强度加深 → 玩家视线聚焦说话者；[`ClearFocus()`](Assets/Project/Scripts/VNEffects/VNVignetteFocus.cs#L81) 恢复居中基础暗角。

### 9.3 屏幕边缘情绪泛光 — [`VNEdgeGlow.cs`](Assets/Project/Scripts/VNEffects/VNEdgeGlow.cs) + 新贴图 `EdgeGlowFrame`
...
### 9.4 天气系统 — [`VNAmbientParticles`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs) 扩展 + [`VNWeatherController.cs`](Assets/Project/Scripts/VNEffects/VNWeatherController.cs)
...
- [`VNEffectsDemo`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs) 新增按键：`G` 光束开关、`V` 聚焦渐晕、`E` 情绪泛光循环、`W` 天气循环；
  提示文字同步显示当前情绪/天气状态。
- [`VNEffectsDemoSetup`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) 自动创建并连线：GodRays（背景与立绘之间）、EdgeGlow（Canvas 最后）、
...
### 10.1 场景色调预设系统 — [`VNMoodGrading.cs`](Assets/Project/Scripts/VNEffects/VNMoodGrading.cs)
...
### 10.2 情绪演出动作库 — [`VNCharacterEmotes.cs`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs)

一行代码调用的立绘小动作，全部返回 Sequence 可加入剧情编排：

| 方法 | 演出 |
|---|---|
| [`Surprise()`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs#L86) | 快速上跳 34px + 微放大，OutBounce 落地回弹 |
| [`Angry()`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs#L99) | 横向 DOShakeAnchorPos 快速抖动 + 红色 PulseEmission 发光脉冲 |
| [`Shy()`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs#L109) | 缩小到 0.97 + 下沉 7px + 粉色光晕，停顿后缓慢恢复 |
| [`Dejected()`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs#L124) | 下沉 24px + 亮度 0.72 + 饱和 0.68（**持续状态**，直到 [`Recover()`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs#L135)） |
| [`Nod()`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs#L147) | 两次快速下沉回弹（第二次幅度更小，更自然） |
| [`HeadShake()`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs#L159) | ±2.6° 小幅左右旋转摆动后归正 |

- **与悬浮飘动的冲突处理**：动作会移动 anchoredPosition，与常驻悬浮 tween 打架。
  方案：`Begin()` 时自动 [`StopFloating()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L423)（会顺带重置到基准位），动作完成后
  [`ResumeFloating()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L398) 恢复（为此给控制器加了 `IsFloating` 属性和记住上次参数的
  `ResumeFloating()`）。动作互相打断安全（每次 Begin 杀掉上一个并重置姿态）。

### 10.3 花式全屏转场库 — `VNScreenTransition.shader` + [`VNScreenTransition.cs`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs)
...
### 11.1 呼吸感立绘（Pseudo-Live2D）— 控制器新增 [`StartBreathingMotion()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L537)

- 三个正弦叠加让立绘"活着"：已有的**上下悬浮** + 新增的**横向缩放呼吸**
  （X 轴 ±1.3% 模拟胸腔起伏，Y 轴带 40% 同步微伸展）+ **极缓倾斜摆动**
  （±0.7°，周期 7s，先缓慢摆到一侧再往复，起步不跳变）。
- [`StartIdleEffects()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L523) 已自动包含，出场后立绘自动开始呼吸，零调用成本。
- 与情绪动作库联动：动作 `Begin()` 时自动暂停呼吸（重置缩放/旋转），
  结束后 [`ResumeBreathingMotion()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L530) 恢复（控制器记住上次参数）。
...
- API：`SetRimLight(颜色, 强度, 宽度, 光源角度)` / [`DORimAmount()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L348) 渐亮渐灭 /
  [`ClearRimLight()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L351)。夕阳场景橙色轮廓光（角度 40°）、月夜蓝色（140°），
  立绘与背景光照氛围立刻统一。
- 注意：采样邻域 alpha 依赖 Clamp 寻址 + FullRect 单图（本项目均满足）；
  若日后使用 SpriteAtlas 图集需关闭该效果（邻域会采到别的图）。

### 11.3 鼠标轨迹星尘 — [`VNMouseStardust.cs`](Assets/Project/Scripts/VNEffects/VNMouseStardust.cs)
...
### 11.4 热浪/空气扭曲 — [`VNHeatHaze.cs`](Assets/Project/Scripts/VNEffects/VNHeatHaze.cs) + 新粒子预设 `Mist`
...
### 12.1 说话者高亮系统 — [`VNSpeakerHighlight.cs`](Assets/Project/Scripts/VNEffects/VNSpeakerHighlight.cs) + 控制器"缩放倍率"改造
...
- **关键改造**：高亮要缩放立绘，但呼吸动作也在补间缩放，会打架。给控制器引入
  ...
  [`ResetScaleMultiplier()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L464)。
- 管理器：`SetSpeaker(fx)` —— 说话者恢复亮度 + 放大 1.03 + 在立绘之间移到最前 +
  光环 Flare 闪耀；旁听者压暗 0.6 + 降饱和 0.85 + 微缩 0.97 + 光环熄灭。[`ClearSpeaker()`](Assets/Project/Scripts/VNEffects/VNSpeakerHighlight.cs#L79) 全员复原。
...
- API：`SetWaterShimmer(强度, 颜色, 高度, 密度, 速度)` / [`DOShimmerAmount()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L375) 渐现渐隐。

### 12.3 分级屏幕震动 — [`VNScreenShake.cs`](Assets/Project/Scripts/VNEffects/VNScreenShake.cs) + SceneRoot 容器
...
### 12.4 对话框高级化 — [`VNDialogueBox.cs`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs) + [`VNTypewriterText.cs`](Assets/Project/Scripts/VNEffects/VNTypewriterText.cs) + 程序化圆角贴图

- [`VNProceduralTextures`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) 新增 SDF 圆角矩形（实心面板）与 3px 描边框两张 9-slice Sprite。
- **边缘流光**：描边框 Image 挂 [`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) 开扫光循环 —— 扫光带只点亮
...
- 对话框：半透明磨砂圆角面板 + 底部 OutBack 轻弹入场 + 骑在顶边的名牌 + 右下角 "▼"
  呼吸浮动继续箭头。API：`Say(名字, 内容)` / [`CompleteTyping()`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L651) 催促 / [`HideBox()`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L615)。
...
### 13.2 多层视差（[`VNParallax.cs`](Assets/Project/Scripts/VNEffects/VNParallax.cs)）

- 读鼠标位置归一化到 -1..1，各层 `anchoredPosition = 基准 - 偏移 × 强度`（反向移动），
  越"近"的层强度越大 → 纵深感。指数平滑（帧率无关）让跟随有"重量感"。
- 支持运行时 [`AddLayer()`](Assets/Project/Scripts/VNEffects/VNParallax.cs#L50)（将来加前景树叶/窗框装饰直接注册）。`Toggle()` 关闭时平滑回中。
- 背景本来就四边溢出 60px（Ken Burns 余量），视差 ±8px 不会露边。

### 13.3 点击涟漪（[`VNClickRipple.cs`](Assets/Project/Scripts/VNEffects/VNClickRipple.cs)）+ 新贴图 `Ring`
...
### 13.4 POV 眨眼转场 — [`VNScreenTransition`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs) 新 Mode 6
...
### 13.5 荷兰角（[`VNDutchAngle.cs`](Assets/Project/Scripts/VNEffects/VNDutchAngle.cs)）
...
### 14.2 镜头运动语言库 — [`VNCamera.cs`](Assets/Project/Scripts/VNEffects/VNCamera.cs)
...
| [`ResetCamera()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L496) | 复位 | 缩放/平移/立绘补偿全还原 |
...
### 14.3 心跳演出 — [`VNHeartbeat.cs`](Assets/Project/Scripts/VNEffects/VNHeartbeat.cs)
...
### 14.4 樱吹雪爆发 — [`VNSakuraBurst.cs`](Assets/Project/Scripts/VNEffects/VNSakuraBurst.cs)
...
### 15.1 伪景深 — Shader 微模糊 + [`VNFakeDoF.cs`](Assets/Project/Scripts/VNEffects/VNFakeDoF.cs)
...
### 15.2 立绘色调自动匹配背景 — [`VNToneMatch.cs`](Assets/Project/Scripts/VNEffects/VNToneMatch.cs)
...
### 15.3 立绘脚下阴影 — [`VNFootShadow.cs`](Assets/Project/Scripts/VNEffects/VNFootShadow.cs)
...
### 15.5 云影飘过 — [`VNCloudShadows.cs`](Assets/Project/Scripts/VNEffects/VNCloudShadows.cs)

- 3 块 950~1500px 的黑色软斑（普通透明混合 = 压暗）以不同速度缓慢横穿背景上部，
  越界回绕 + 轻微正弦纵向漂移。只挂在 LayerBack 下，**不会盖到立绘**。晴天的"活气"。

### 15.6 选项按钮演出 — [`VNChoicePanel.cs`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs)（零新特效，纯组合）
...
`[` 伪景深开关、`]` 云影开关、[`Tab`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs) 残影冲入、`退格` 选项演出；
色调匹配与脚下阴影全自动无按键。
...
- **[`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs)（ScriptableObject）**：角色 id/显示名/名牌色/**表情名→立绘映射**，
  立绘表情资产集中管理（`Create → VN → Character Definition`）。
- **[`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)**：行解析（注释/异步后缀/命令 kwargs/台词行全半角冒号），
  保留行号 → 所有报错精确到"第 N 行"。
- **[`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)**：角色运行时工厂（Image+控制器+光环+出场器+情绪+脚影全栈生成）、
  表情切换（按高度重算宽度）、背景库、说话分发（自动说话者高亮）、fx 分发、
  在场角色变化时自动刷新 ToneMatch/SpeakerHighlight 注册。
- **[`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)**：协程解释器——同步命令 `yield WaitForCompletion()`，
  `@` 异步 fire-and-forget；台词=等打字完+玩家推进（Enter/空格/点击，打字中按下=催促）。

### 16.4 场景生成器重构

- 把原 CreateDemoScene 拆出共享的 **[`BuildStageRig()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L83)**（相机/后处理/Canvas/容器层级/
...
- **[`VNFlags`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs)** 静态类：整型字典（bool=0/1），`Apply("名字+2")` 解析增减操作串，
...
**修复**：给 [`VNStage.Awake()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 加 **AutoWire 自动补线**：所有为空的引用自动
...
### 19.1 存档系统 — [`VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs)
...
### 19.2 回想 Backlog — [`VNBacklog.cs`](Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs)
...
- **[`VNToast`](Assets/Project/Scripts/VNEffects/Script/VNToast.cs)**：自建 Overlay Canvas 的轻量提示——底部气泡（"已保存"）+
  右上角常驻模式标签（AUTO ▶ / SKIP ▶▶）。
- 读档时若选项面板开着会被 `VNChoicePanel.ForceClose()`（[`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:487`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs#L487)） 强制清掉。
...
[`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 新增两个字段：

- `sizeScale`（默认 1）：该角色显示高度 = 舞台统一高度 × 此值。留白多显小→调大；近景显大→调小
- `positionOffset`：在 at:left/center/right 标准站位上的附加偏移（脚下留白多→y 负值下压）

[`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 全链路应用：登场摆位（含基准位同步）、初建尺寸、表情切换重算宽度、
...
### 21.1 音频系统 — [`VNAudio.cs`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs)（项目首次有声音）
...
- **打字音**：`typingTick` 槽位赋一个短音效 → 打字机每字自动"哒哒哒"
  （0.055s 节流 + ±6% 随机音高防机械感），[`VNTypewriterText`](Assets/Project/Scripts/VNEffects/VNTypewriterText.cs) 按整字推进触发
...
- `ApplyExpression` 升级：角色完全可见时（溶解≈1 且未淡出），换表情前复制一份
  ...
  旧表情融化成新表情。时长 `VNStage.expressionCrossfade`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:124`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L124)） 可调（0=关闭）。
...
- [`VNCamera`](Assets/Project/Scripts/VNEffects/VNCamera.cs) 新增三原语：`Cut(点,zoom)` 瞬切、`GoTo(点,zoom,秒,缓动)` 单段直达、
...
- **文本双向**：生成 camseq 文本→剪贴板；粘贴已有文本"解析载入"继续可视化调整
  （复用运行时的 [`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)，零重复解析代码）

### 23.2 关键实现点

- **数学共用**：[`VNCamera.OffsetFor`](Assets/Project/Scripts/VNEffects/VNCamera.cs) 重构出公开静态 `ComputeOffset`，
...
- [`VNCamseqPresetLibrary`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs)（`Assets/VNEffects/CamseqPresets.asset`，首次保存自动创建）：
...
**现象**：运行时报错。
**原因**：[`VNAmbientParticles.cs`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs) 的 velocityOverLifetime 模块中，X/Y 轴用了
...
**原因**：初始化顺序 bug —— 角色运行时生成于 (0,-60)，[`VNEntranceAnimator.Awake()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs)
立刻把该位置缓存为基准位；`VNStage.Show`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:326`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L326)） 随后虽移到了 at: 指定的站位，但
`PlayEntrance → PrepareHidden` 会把角色**重置回缓存的旧基准位**。
**修复**：
1. `VNEntranceAnimator` 新增 `BasePosition` 属性与 `SetBasePosition()`；
   [`VNCharacterEmotes`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs) 同样新增 `SetBasePosition()`。
2. [`VNStage.Show`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 摆位后同步调用两者的 SetBasePosition。
3. 顺带加固 [`VNFootShadow`](Assets/Project/Scripts/VNEffects/VNFootShadow.cs)：基准位改为**每帧动态读取**出场器的 BasePosition
...
| 文件 | 改动 |
|---|---|
| [`VNCameraFade.cs`](Assets/Project/Scripts/VNEffects/VNCameraFade.cs)（新增） | 截屏叠化组件：嵌套 Canvas 排序 90（对话框 40 之上、ScreenTransition 100 之下）+ 全屏 RawImage。[`CaptureCo()`](Assets/Project/Scripts/VNEffects/VNCameraFade.cs#L85) 协程等帧末用 `ScreenCapture.CaptureScreenshotIntoRenderTexture` 截屏（URP 下不能手动 `Camera.Render()`）；`FadeOut(秒)` 淡出。D3D 等平台后备缓冲上下颠倒，按 `SystemInfo.graphicsUVStartsAtTop` 用负 uvRect 翻转（Inspector 有 FlipMode 手动开关兜底） |
| [`VNCamera.cs`](Assets/Project/Scripts/VNEffects/VNCamera.cs) | `Waypoint` 加 `fade` 字段；`PlayPath` 抽出 `BuildSegment(from,to)`（编辑器预览不受影响）；新增协程版 `PlayPathCo(points, startFade, endFade)`——连续普通点仍合成一条 Sequence 保持原缓动手感，fade 点走"截屏→Cut→淡出"；[`SnapReset()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L485) 瞬间复位；`cameraFade` 引用留空时自动在 Canvas 下创建（旧场景不重建也能用） |
| [`VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) | `VNCamWaypointDef.fade`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:28`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L28)） + 路径点行识别 `xfade:`；`VNScriptCommand.KwF()`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:74`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L74)） 浮点 kwargs 助手 |
| [`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) | `PrecutFor(bgCmd)`：bg 带转场且同步执行时向后看一条命令，若是 `start:cut` 的 camseq 就把首镜头瞬切并入转场盖屏回调，并记录 `_precutDone` 让该 camseq 跳过首点；`CamseqCo` 改调 `PlayPathCo` 并解析 start/end 选项 |
| [`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) | `SetBackground` 加 `onCovered` 回调参数（转场盖屏瞬间与换图一起执行） |
| [`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) | `BuildStageRig` 创建 CameraFade 覆盖层并接线（两个演示场景共用） |
...
| 每角色开关 | `VNCharacterDef.showPortrait`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:226`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L226)）（Inspector 勾选） |
| 选图 | `VNCharacterDef.portraits`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:229`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L229)） 列表（name 对应表情名：台词行 `角色 表情: …` 自动匹配同名头像，没匹配用第一个；**列表留空 = 自动用表情立绘当头像**） |
| 缩放 | `VNCharacterDef.portraitScale`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:233`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L233)）（1 = 宽度填满窗口，调大出特写） |
| 偏移 | `VNCharacterDef.portraitOffset`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:236`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L236)）（窗口内平移，把脸挪进窗口） |
| 窗口尺寸 | `VNDialogueBox.portraitWindowSize`（[`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:46`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L46)）（Inspector） |

### 28.3 文件改动

| 文件 | 改动 |
|---|---|
| [`VNCharacterDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) | 新增「对话框头像」区块：showPortrait / portraits / portraitScale / portraitOffset + `GetPortrait(表情)`（未配头像回退立绘；showPortrait=false 返回 null） |
| [`VNDialogueBox.cs`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs) | 头像窗口（RectMask2D + Image）程序化构建；`SetPortrait(sprite, scale, offset)` / `SetPortraitEnabled(bool)`；正文 offsetMin 与名牌 x 按窗口宽度避让 |
| [`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) | `Say` 里按说话者设置头像（优先本句表情，否则用角色当前表情）；旁白/未注册角色清空头像；`SetPortraitEnabled` + `_portraitOff` 进 Capture/RestoreSnapshot |
| [`VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) | `VNSaveData.portraitOff`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:53`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L53)） 字段（旧存档缺字段默认 false = 开启，兼容） |
| [`VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) | Keywords 加 `portrait` |
| [`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) | `case "portrait"`：`stage.SetPortraitEnabled(Arg(0) != "off")` |
...
[`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 的 `sizeScale / positionOffset / portraitScale / portraitOffset` 此前只能修改
Inspector 后进入 Play，等角色登场和说出台词才能看到实际效果。不同素材的透明留白、长宽比、
人物构图差异很大，反复 Play 校准立绘高度、脚底位置和头像裁切效率很低。

### 29.2 新工具

新增 [`Assets/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs)，菜单：
...
- 「从场景读取尺寸」会读取当前场景 `VNStage.characterHeight`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:127`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L127)）、背景图和
  `VNDialogueBox.portraitWindowSize`（[`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:46`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L46)）；场景中没有这些组件时使用 880 / 230×300 默认值；
- 直接在舞台拖动立绘 = 实时修改 `positionOffset`；鼠标滚轮 = 修改 `sizeScale`；
- 同时保留精确数值输入、参数归零和当前立绘资产定位按钮。

### 29.4 头像实时预览

- 右侧按照 [`VNDialogueBox`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs) 的真实 **RectMask2D 顶边锚定裁切公式**显示头像窗口；
- 头像列表优先读取 `VNCharacterDef.portraits`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:229`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L229)）；列表为空时明确标注「回退立绘」，并使用
  `expressions` 预览，与运行时 [`GetPortrait()`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L291) 行为一致；
...
- 对话框锚点、边距、姓名条位置、正文避让、箭头位置、头像窗口与头像顶边锚定公式，均按
  [`VNDialogueBox`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs) 的运行时布局换算到编辑器预览；
...
- `sizeScale / positionOffset / showPortrait / portraitScale / portraitOffset` 全部先写入一个
  ...
  [`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs)；
...
- 修改 [`Assets/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs)；没有修改任何运行时代码、
  场景、角色资产或素材；
- `dotnet build Assembly-CSharp-Editor.csproj --no-restore` 验证通过：0 warning / 0 error。
...
| 文件 | 职责 |
|---|---|
| [`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) | **命令参数模式表**（本工具核心资产）：26 个命令每个参数的类型/候选来源/默认值。UI、生成、校验的单一数据来源；加新命令补一条，界面自动长出控件 |
| [`VNScenarioDoc.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) | 文档模型：解析（镜像运行时分词规则，含 choice `*` 行、camseq `>` 行、行尾 `@`、全半角冒号台词）→ VNRow 行列表 → 生成；label/flag 收集；校验器 |
| [`VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) | 主窗口：工具栏（Open/Save/Save As/Reload/Refresh Sources/Go to label）、三页签（Edit/Text/Issues）、ReorderableList 行编辑、按类别分组的添加菜单、撤销、外部修改检测 |

*运行时唯一改动：[`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) 加了 `CommandKeywords` 公开只读访问器（关键字单一来源）。
...
本轮围绕 [`VNScenarioEditorWindow`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) 连续完善“命令选择辨识度、图片预览选择、从任意行启动调试”三条
...
- [`VNTransition`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs) 与 `emote` 的英文枚举值也显示中文含义，但写回剧本的值仍保持英文；
...
- `bg` 从场景 `VNStage.backgrounds`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:36`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L36)） 读取 id + Sprite；主面板背景字段显示小型内联缩略图，点击
...
修复记录：初版通用回调把 `say` 的选择结果写进普通 `VNRow.values`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs:44`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs#L44)），但台词实际使用
...
Unity 自动进入 Play Mode → [`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 从该行或下一条有效命令开始。
...
- Bridge 等待 `VNScriptRunner.IsInitialized`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:95`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L95)），让默认 `playOnStart` 完成启动后再停止并接管，避免
...
- `VNAudio.ResetForDebug()`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:144`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L144)） 清除默认启动瞬间留下的 BGM/语音/循环 SE/Tween，并恢复初始音量；
- `VNStage.RestoreSnapshot`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:899`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L899)） 新增 instant 重载，支持无背景快照时清空旧背景；`VNCamera.SnapReset`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:485`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L485)）
  用于无法可靠推断的动画镜头路径；
...
| [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) | instant 快照恢复与空背景清理 |
| [`Script/VNAudio.cs`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) | 调试启动前的音频/Tween 清理和初始音量恢复 |
...
修改 [`Assets/Scripts/VNEffects/Script/VNAudio.cs`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs)：
...
在现有 [`VNScreenTransition`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs) 全屏遮罩架构中新增四种可直接用于剧本的转场：
...
- [`VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)：转场下拉菜单新增中文名称。
- [`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)：T 键循环演示包含新转场；放射类效果可从角色位置开始。
...
- `Shatter`：[`VNShatterGraphic`](Assets/Project/Scripts/VNEffects/VNShatterGraphic.cs) 生成单个 14×8 网格的 224 个独立三角碎片；中心、随机种子写入
...
新增 `VN/DirectBackgroundTransition` Shader；`VNStage.SetBackground`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1129`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1129)） 仅对上述四种 `bg` 转场走
...
- 截图复用 [`VNCameraFade`](Assets/Project/Scripts/VNEffects/VNCameraFade.cs) 的屏幕截取与 Y 翻转判断，并在截取前隐藏存读档 UI。
...
实现文件：[`VNSaveLoadPanel.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs) 负责运行时 UI、槽位刷新与确认弹窗；[`VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 负责 20 槽
元数据和缩略图读写；[`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 负责快捷键、暂停和保存上下文；`VNCameraFade.cs` 提供
...
[`VNQuickToolbar.cs`](Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs) 负责按钮布局及 Auto/Skip 状态高亮，[`VNConfigPanel.cs`](Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs) 负责运行时设置界面；
旧场景无需重建，[`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 启动时会自动挂到当前 [`VNDialogueBox`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs)。同时鼠标推进现在会检查
...
[`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 新增「眨眼」区块：
...
新增 [`VNCharacterBlink.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterBlink.cs)，由 [`VNStage.CreateCharacter()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 给每个运行时角色自动挂载：
...
[`VNCharacterVisualPreviewWindow`](Assets/Project/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs) 的草稿系统同步加入眨眼开关、闭眼图、随机间隔和闭眼时长；这些值
...
[`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 新增「说话口型」区块：
...
新增 [`VNCharacterMouth.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMouth.cs)：

- [`VNStage.CreateCharacter()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 在运行时角色下创建 `MouthOverlay` 子 Image，四边锚定铺满完整立绘；
  闭嘴时禁用该 Image，张嘴时启用，不需要计算嘴巴局部坐标。
- 嘴部 Image 与角色主 Image 共用 [`VNImageEffectController.Mat`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs)，因此溶解、明暗、高亮、闪光、
...
- `VNAudio.PlayVoice()`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:302`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L302)） 改为返回是否成功播放，并公开 `IsVoicePlaying`。
- [`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 将成功的 `voice` 命令一次性绑定到下一句 `say`；项目原有写法
...
- 新台词开始前先关闭所有旧说话者；旁白、切换说话者、角色退场、清空舞台、读档/调试重建、
  停止剧本和剧本自然结束都会调用 `VNStage.StopSpeaking()`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1101`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1101)），保证不会遗留张嘴状态。

### 38.4 角色视觉预览与验证

[`VNCharacterVisualPreviewWindow`](Assets/Project/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs) 草稿新增口型开关、张嘴图、默认表情限制与随机间隔；舞台区域可
...
- 检查 [`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)、[`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)、[`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)、[`VNFlags`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs)、[`VNSaveSystem`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs)、[`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs)、
  [`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs)、对话/回想/配置/快捷工具条，以及剧本编辑器的 Schema、文档模型和校验器。
...
> 功能。本次会话只阅读了 [`VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)、`Assets/Scenarios/` 与既有章节记录，
...
且 BGM/SE/语音全部混在 `VNAudio.library`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:45`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L45)） 一个列表里不好管理。方案（对话中比较过
...
### 40.2 [`VNAudio.cs`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) 重构
...
- [`ResetForDebug()`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L144) 清理循环 SE 时适配新字典结构，并重置全部增益。

### 40.3 剧本语法与运行时

- `bgm play 黄昏之歌 fade:2 vol:0.6` / `se 爆炸 vol:0.4` / `se 雨声 loop vol:0.5` /
  `voice v02 vol:0.8`——`vol:` 全部可省略（默认 1），旧剧本零改动。
- 存档：[`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 新增 `bgmVol = 1f`（JsonUtility 读旧档时字段缺失保持默认 1），
...
- [`VNParamSource.Audio`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) 拆为 `AudioBgm / AudioSe / AudioVoice` 三个来源；
...
2. **地图/战斗/迷你游戏 = 「暂停剧本 → 玩家交互 → 带结果返回」的外部事件**：统一成
   一个通用事件模块接口 [`VNEventModule`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs)，剧本用一条 `event` 命令 + `*` 结果行调起，
...
- 运行时：`VNEventModule`（抽象 MonoBehaviour）：`Launch(VNEventContext, Action<string> onDone)`；
  [`VNEventRegistry`](Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs)（场景组件，id → 预制体列表）实例化到独立 EventLayer
...
- **任务（P2）**：`quest start/stage/done/fail <id> [阶段]` 全部翻译成 flag 写入 +
  VNToast 通知；[`VNQuestDef`](Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs) ScriptableObject（id/标题/各阶段文案）；J 键任务日志面板
  从 flags 反查状态渲染。
- **地图（P3）**：作为第一个"正经"内置事件模块 [`VNMapModule`](Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs)：全屏地图底图 + 可点击
...
- `_eventActive` 期间 [`Update()`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs#L59) 直接 return：F5/A/S/H/隐藏 UI 全部快捷键交给模块；
  存档被现有「仅台词处可存」天然挡住。
...
- Create Script Demo Scene 现在创建 [`VNEventRegistry`](Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs) 物体 + 禁用状态的 QteTemplate 子物体，
...
- [`VNParamSource.EventId`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) 新来源：id 下拉/校验取自场景 `VNEventRegistry.modules`（[`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:25`](Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs#L25)）。
...
阶段号约定：0 未接取 / 1..n 进行中 / 100 完成 / -1 失败（[`VNQuestLog`](Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs) 常量）。
...
- Runner：Start 查找/自建 VNQuestLog；`quest` 命令分发；J 键开关日志（互斥逻辑与
  回想面板同构，打开期间不推进剧情、Esc 可关）；[`RequestQuestLog()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1676) 公开给工具条。
...
- 编辑器：[`VNParamSource.QuestId`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（扫项目 VNQuestDef 资产做下拉），quest 命令入
...
实现第四十一章规划的 P3。[`VNMapModule`](Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs) 是事件接口的第一个"正式"内置模块，
...
- **双重过滤**决定本次显示哪些地点：①`condition`（VNFlags 表达式）不满足 → 隐藏；
  ②剧本「* 结果行」没接住的地点 → 隐藏。为此 [`VNEventContext`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs) 新增 `outcomes`
  列表与 [`AcceptsOutcome()`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs#L18)（Runner 从 cmd.options 填充）——这是通用机制，
...
- **[`VNProceduralTextures.cs`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs)（改）**：
...
- **[`VNSpeedLines.cs`](Assets/Project/Scripts/VNEffects/VNSpeedLines.cs)（新）**：全屏集中线 overlay 组件。
...
- **[`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（改）**：新增 `speedLines` 字段 + AutoWire 自动查找；
  `ToggleFxNames` 加入 `"speedlines"`（存档快照/读档恢复/调试重建自动覆盖）；
  [`Fx()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1614) 新增 case：`fx speedlines on|off|burst`（burst 不记录开关状态）。
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：BuildStageRig 第 8.5 步创建 SpeedLines 物体
  （两个演示场景都有），连线 `stage.speedLines` 与 `demo.speedLines`；
  演示剧本头部语法速查补 `fx speedlines` 说明。
- **[`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)（改）**：`,` 键开关集中线、`.` 键 Burst 冲击，提示文字更新。
- **[`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：FxNames 加 `speedlines`；fx 命令 value 候选加 `burst`。
...
- **[`VNLetterbox.cs`](Assets/Project/Scripts/VNEffects/VNLetterbox.cs)（新）**：黑边组件。
...
- **[`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（改）**：
  ...
  - [`Fx()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1614) 加 case `letterbox`（`fx letterbox on|off` 同样可用，读档恢复走这里）。
  - `RestoreSnapshot`：恢复后若「回忆色调 + 黑边」同时成立则视为自动黑边。
- **[`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：
...
- **[`VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)（改）**：Keywords 加 `letterbox`。
- **[`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：FX 分类新增 letterbox 命令（on/off + height/time kwargs）；
  FxNames 加 `letterbox`。
- **[`VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)（改）**：命令中文翻译表加「letterbox → 电影黑边」。
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：BuildStageRig 第 8.6 步创建 Letterbox 物体并连线
  stage/demo；演示剧本头部语法速查补 letterbox 说明。
- **[`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)（改）**：`'`（引号）键开关黑边，提示文字更新。
...
- **[`VNShootingStars.cs`](Assets/Project/Scripts/VNEffects/VNShootingStars.cs)（新）**：夜晚偶发流星。
...
- **[`VNDriftingClouds.cs`](Assets/Project/Scripts/VNEffects/VNDriftingClouds.cs)（新）**：云本体缓移。
...
- **[`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（改）**：`shootingStars`/`driftingClouds` 字段 + AutoWire；
  `ToggleFxNames` 加 `meteor`/`skycloud`；Fx() 两个新 case。
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：BuildStageRig 第 12.5 步在 LayerBack 下创建
  DriftingClouds 与 ShootingStars（背景之上、立绘之下），连线 stage/demo；
  演示剧本：开场 `fx skycloud on`、夜晚段 `fx meteor on`，语法速查更新。
- **[`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)（改）**：`/` 键流星、`;` 键云缓移，提示更新。
- **[`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：FxNames 加 `meteor`/`skycloud`。
...
- **[`VNScreenShockwave.cs`](Assets/Project/Scripts/VNEffects/VNScreenShockwave.cs)（新）**：组件总控。
  ...
  - 画面真的在"荡"：`targets`（生成器只连背景，避免立绘脸部扭曲）的
    ...
    `OnKill` 兜底归零防中断残留；
  - 可选联动 [`VNScreenShake`](Assets/Project/Scripts/VNEffects/VNScreenShake.cs)（strength≥1.2 用 Medium，否则 Light）；
  - `PlayFrom(Transform)` 支持从受击角色位置荡开；全部 Tween `SetLink`。
- **[`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（改）**：`shockwave` 字段 + AutoWire；[`Fx()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1614) 新 case：
  `fx shockwave` 标准 / `light` 0.6 / `heavy` 1.4 倍强度；一次性演出不进 `ToggleFxNames`。
- **[`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：调试重建的 fx 汇总跳过一次性演出
  （`shockwave` 与 `speedlines burst`）——顺手修掉旧 bug：从选中行播放时
  `fx speedlines burst` 会被误还原成持续开启的速度线。
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：BuildStageRig 第 8.55 步创建 ScreenShockwave
  （targets=背景 fx；screenShake 在第 11 步创建后回填），连线 stage/demo；
  演示剧本头部语法速查补 shockwave 行。
- **[`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)（改）**：`-`（减号）键触发全屏水波，提示更新。
- **[`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：FxNames 加 `shockwave`，fx 值选项加 `light/heavy`。
...
- **[`VNRetroFilter.cs`](Assets/Project/Scripts/VNEffects/VNRetroFilter.cs)（新）**：组件总控，`VNRetroMode { None, Film, Crt }`。
...
- **[`VNMoodGrading.cs`](Assets/Project/Scripts/VNEffects/VNMoodGrading.cs)（改）**：`VNMood` 新增 **Dream（梦境）**——偏亮低对比
  柔紫粉（曝光+0.35、对比-24、品红 tint+14、紫粉 lift/gamma、轻暗角）；
  `CycleNext` 的硬编码 `% 7` 改为按枚举长度取模（修掉加枚举会漏最后一项的隐患）。
- **[`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（改）**：`retroFilter` 字段 + AutoWire；`ToggleFxNames` 加
  `filmgrain`/`crt`（存档/读档/调试重建零改动支持）；[`Fx()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1614) 两个新 case
  （互斥：开一个清另一个的状态，手动控制会接管自动滤镜）；
  `SetMood` 联动重构：Memory→黑边+胶片、Dream→CRT，`_retroAuto` 标记
  确保手动/自动互不干扰；`RestoreSnapshot` 按 mood+fx 组合恢复自动标记。
- **[`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：调试重建（从选中行播放）静默重放补齐——
  mood 命令按 `autoMoodRetroFilter` 重放自动胶片/CRT（与黑边同款逻辑）；
  fx 命令处理 filmgrain/crt 互斥与手动接管，保证重建状态与运行时一致。
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：BuildStageRig 第 8.58 步创建 RetroFilter，
  连线 stage/demo；演示剧本头部语法速查补 filmgrain/crt 两行。
- **[`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)（改）**：`=`（等号）键循环 无→胶片→CRT，提示显示当前模式。
- **[`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：FxNames 加 `filmgrain`/`crt`；
...
- **[`VNKenBurns.cs`](Assets/Project/Scripts/VNEffects/VNKenBurns.cs)（新）**：核心组件，挂在背景 Image 上。
...
- **[`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（改）**：`kenBurns` 字段 + AutoWire（找不到时**自动补挂**到背景
  ...
  [`ResetEffects()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1598) 末尾把 kenburns 重置回默认开（章节重置不该让画面死掉）。
- **[`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：调试重建快照初始种入 `kenburns`；
  `reset effects` 重放时清空 fxOn 后同样补种（与运行时 ResetEffects 一致）；
  顺带补上 reset 重放漏掉的 `autoRetro = false`（上一功能的小疏漏）。
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：BuildStageRig 第 6 步创建背景时直接
  `AddComponent<VNKenBurns>()`，连线 stage/demo；演示剧本头部语法速查补一行。
- **[`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs)（改）**：删掉 Start 里的粗糙 DOScale（亮度呼吸保留），
  改为引用/自愈补挂 VNKenBurns；`\`（反斜杠）键开关，提示更新。
- **[`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：FxNames 加 `kenburns`。
...
- **[`Script/VNFont.cs`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs)（新）**：全项目 TMP 字体统一入口（静态类），取代原先
...
- **[`Editor/VNFontAssetBuilder.cs`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs)（新）**：菜单
...
- **[`VNTypewriterText`](Assets/Project/Scripts/VNEffects/VNTypewriterText.cs) 整体重写**：`BaseMeshEffect.ModifyMesh`（TMP 不走
...
- **场景生成器 `CreateHintText`** 改建 TMP 并引用
  `VNFontAssetBuilder.EnsureFontAsset()`（[`Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs:80`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs#L80)） 持久化资产（理由见分支一）。
- **[`VNScriptRunner.LoadCommands`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)** 解析剧本后 `VNFont.Prewarm(source)`：
...
- **[`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)**：新增 `cgLibrary`（CgEntry：id/sprite/group，group 为 P2 差分组
...
- **[`VNCgUnlocks`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs)（新）**：全局解锁存储，`persistentDataPath/vn_cg_unlocks.json`，
...
- **剧本编辑器**：VNParamSource 新增 `Cg`；`cg` 命令入 Scene 分类（转场参数
  ...
  `VNStage.cgLibrary`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:49`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L49)），Refresh Sources 重建）；文档校验未注册 CG 报错
  （"off" 豁免）。
...
- **[`VNLocale`](Assets/Project/Scripts/VNEffects/Script/VNLocale.cs)（新，Script/）**：语言管理器。`Language`（中/英/日枚举，存
...
- **[`VNFont`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs) 多语言化**：日文走随包 **Noto Sans JP**（4.5MB OTF，OFL 许可，
...
- **UI 字符串全量查表**：快捷功能条/存读档面板/任务日志/回想/设置面板/QTE/地图/
  Runner Toast 的硬编码中文全部改 `VNLocale.T`（[`Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:76`](Assets/Project/Scripts/VNEffects/Script/VNLocale.cs#L76)）。**开发者可见的 [Header] 与
...
- **翻译表**：`Resources/VNLocale/Scenarios/<剧本名>.<lang>.txt`。剧本名 =
  ...
  **FNV-1a(原文)8位hex + "-" + 同文出现序号**——中文没改动时增删其他行不打乱
  ...
  运行时与编辑器工具共用同一实现。
- **[`VNScriptLocale`](Assets/Project/Scripts/VNEffects/Script/VNScriptLocale.cs)（新，Script/）**：`Apply(commands, scriptName)` 在
...
- **[`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs)**：+`displayNameEn/Ja` 与 `LocalizedDisplayName`（留空回退
  displayName）；VNStage 名牌与 GetDisplayName（Backlog 用）走译名。
  **剧本引用的角色 id 永远不翻译**（翻了会炸全部剧本与存档）。
- **[`VNQuestDef`](Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs)**：+英/日 标题/描述/阶段文案列表；`Title`/`StageText`/
  `LocalizedDescription` 按当前语言取值、缺项逐级回退中文。VNQuestLog 零改动生效
  （只把 `q.description` 换成 `q.LocalizedDescription`）。
- **[`VNMapModule.Location`](Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs)**：+英/日显示名，标记文字显示译名；逻辑（结果匹配、
...
- **根因**：[`VNScriptRunner.Update`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 推进前用 `EventSystem.IsPointerOverGameObject()`
  判断"点在 UI 上就不推进"。但本项目**整个画面都是 uGUI**（背景/立绘/对话框
  全是 Canvas 里的 Image，默认 raycastTarget=true），该判断恒为 true → 左键永拦。
- **修复**：[`IsPointerOverInteractiveUi()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L2114)——`EventSystem.RaycastAll` 命中链向上找
...
- **需求**：编辑器里给 bgm/se/voice 选了 id 后听不到声音，全凭记忆；希望参数旁有
  ▶ 小按钮直接播放试听。
- **实现**（全部在 [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)）：
...
- **实现**（全部在 [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)）：
  ...
  - 开启后每行左侧画一个 70px 小格：**当前背景缩略图**（CG 显示期间优先画 CG）+
    **左/中/右三个站位格**，有人的格子填角色专属色块（8 色调色板，颜色按
    [`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 登记顺序稳定分配，同一角色全文档同色）。
...
- **[`Script/VNStatDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs)（新）**：属性定义资产（CreateAssetMenu "VN/Stat
...
- **[`Script/VNStatsHud.cs`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs)（新）**：系统核心组件（参照 VNQuestLog 模式）。
  ...
  ④ [`EnsureInitials()`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs#L96)：定义了初始值的属性在 flag 尚不存在时写入（Start 时 +
  读档后，靠 VNFlags.Changed 触发的刷新顺带补）。语言切换销毁重建（惰性）。
- **[`Script/VNFlags.cs`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs)（改）**：新增 `Changed` 静态事件（Set/Clear 触发）——
  HUD 靠它感知一切改动来源（stat/flag 命令、choice flag: 操作、读档恢复）。
  订阅方标脏 + 下帧统一刷新（读档会连续触发多次）。
- **[`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)（改）**：关键字表加 `stat`。
- **[`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：`_statsHud` 字段（Start 找不到就自建，
  无定义资产也能工作）；命令分发 `case "stat"`；调试重建（从选中行播放）静默重放
  `case "stat"`（钳制照做不飘字）；C 键开/关属性面板（打开期间不推进剧情，
  与任务日志同款拦截）；[`RequestStatsPanel()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1696) 给功能条；右键隐藏 UI 时
  HUD 一起藏（SetInterfaceHidden 联动）。
- **[`Script/VNQuickToolbar.cs`](Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs)（改）**：新增"属性"按钮（859→936 宽）。
- **`Resources/VNLocale/ui.zh/en/ja.txt`（改）**：`toolbar.stats`、`stats.title/
  empty/toastGain/toastLose/toastSet` 六个 key ×3 语言。
- **[`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：`stat` 命令入 Flow 分类（name 用 Flag
  来源下拉，value 提供 ±1/±5/±10 候选）。
- **[`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：[`EnsureStatDefs()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L746) 生成 8 个示例
...
- **[`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)（改）**：VNChoiceOption + `condition/costOp` 字段；
  ParseChoiceOption 改为"从行尾逐个摘参数 token"（旧的 IndexOf("flag:") 写法
  没法扩展到多参数；摘 token 保持旧剧本语义不变）。
- **[`Script/VNStatsHud.cs`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs)（改）**：花费四件套 `ParseCostOp`（静态，校验共用）/
  `CanAfford` / `FormatCostLabel` / `ApplyCost`（复用 Apply 的钳制+飘字）。
- **[`VNChoicePanel.cs`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs)（改）**：新增 `Option { text, costLabel, interactable }` 与
...
- **[`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：ChoiceCo 重写选项组装——
...
- **[`Editor/VNScenarioDoc.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs)（改）**：VNChoiceOptionRow + condition/costOp；
...
- **[`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)（改）**：选项行新增「if」「$」两个小字段
  （悬停 tooltip 说明用法），与 flag/jump 下拉并排。
- **[`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：choice 命令 hint 补新语法说明。
...
- **[`Script/VNShopDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs)（新）**：商店定义资产（CreateAssetMenu "VN/Shop
  ...
  [`ItemFlagName()`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs#L72) 是道具 flag 命名的单一来源。
- **[`Script/VNShopModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs)（新）**：商店事件模块（继承 VNEventModule）。
...
- **[`Script/VNInventory.cs`](Assets/Project/Scripts/VNEffects/Script/VNInventory.cs)（新）**：I 键物品栏面板（参照 VNQuestLog 模式）。
  从 flags 反查 `道具_*>0` 的条目，文案/图标从登记的 VNShopDef 里找
  （跨商店取第一个命中；未登记道具用 id 当名字照常显示）；语言切换销毁重建。
- **[`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：`_inventory` 字段（找不到自建）；
  I 键开/关物品栏（打开期间不推进剧情）；[`RequestInventory()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1702)。
- **[`Script/VNQuickToolbar.cs`](Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs)（改）**：新增"道具"按钮（936→1013 宽）。
- **`Resources/VNLocale/ui.zh/en/ja.txt`（改）**：shop.* 十个 key +
  inventory.title/empty + toolbar.inventory ×3 语言。
- **[`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：[`EnsureShopDef()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L841) 生成示例商店
...
- **[`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)（改）**：关键字表加 `time`。
- **[`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（改）**：`ApplyTimeCommand(cmd, silent)`——
  ...
  常驻无害）；右键隐藏 UI 联动。
- **[`Script/VNCalendarHud.cs`](Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs)（新）**：右下角日历小面板（独立 Overlay Canvas 578）。
...
- **[`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（改）**：`time` 命令入 Flow 分类
  （op 下拉 pass/set、month 数字、remain/months/refill kwargs）。
- **[`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)（改）**：RaisingDemo 剧本注册进
...
- 结局文件说明普通 VNFlags 随槽位存档，而真正跨槽永久解锁应使用类似 [`VNCgUnlocks`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs) 的全局存储。

### 69.3 使用前提与验证

运行前需把七个 TextAsset 登记到场景 [`VNScriptRunner.Chapters`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)，并从 `第1章.vn.txt` 开始。
...
- [`VNEventModule`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs) 加虚属性 `RecordInBacklog`（默认 true），文档注明「纯流程控制型
  调用应返回 false」；
- [`VNScriptRunner.EventCo`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 在 `Destroy(module.gameObject)` **之前**读取该属性
  （Destroy 是帧末延迟执行，但读值要早于销毁才安全），据此决定是否 Record；
- [`VNPlanModule`](Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs) 用 `_dispatchMode` 字段区分两种 op，派发模式返回 false。
...
所有变量都读取 [`VNFlags`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs)，不存在的变量仍为 0；表达式不允许赋值，也不会改变游戏状态。

### 73.2 实现与兼容性

- 新增 [`VNExpression`](Assets/Project/Scripts/VNEffects/Script/VNExpression.cs) 递归下降解析器，运行时、可视化编辑器校验与 Flag 名收集共用同一语法真相；
- [`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 从最后一个 `jump` token 向前重组独立 `if` 表达式，因此可以保留可读空格；
...
新增 `jump 节日活动集::圣诞活动`。它先从当前 `VNScriptRunner.script`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:24`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L24)） 与 Chapters 列表中
...
- [`VNStoryAddress`](Assets/Project/Scripts/VNEffects/Script/VNStoryAddress.cs) 统一负责 `::` 地址解析和文件名规范化；`chapter` 也复用同一规范化逻辑；
...
[`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 新增 `saveVersion = 2` 和 `callStack`。每帧 JSON 保存返回章节名、返回命令索引与
...
- [`VNParameterInterpolator`](Assets/Project/Scripts/VNEffects/Script/VNParameterInterpolator.cs) 单层扫描 `${...}`，不递归展开参数值；缺失参数保持原占位符并按剧本行报错；
- Runner 每次 Dispatch 前深拷贝当前命令后插值，不修改解析缓存，因而同一公共片段可被不同参数重复调用；
- 本地化顺序为 `VNScriptLocale.Apply`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptLocale.cs:32`](Assets/Project/Scripts/VNEffects/Script/VNScriptLocale.cs#L32)） 先选译文，再对 `localizedText` 插值，译者可自由调整占位符语序；
...
根因在 `VNEffectsDemoSetup.BuildStageRig()`（[`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:83`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L83)）：
...
`cgLibrary` 是唯一不丢的——因为它靠 [`FillCgLibrary()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L1297) 扫 `Assets/CG` 目录，
**这个先例直接指出了正确解法**。

### 77.2 关键认识：要压缩的是"引用面积"而不是"数据形态"

[`VNStatDef`](Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs)/[`VNShopDef`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs)/[`VNQuestDef`](Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs)/[`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) **早就是 ScriptableObject**，
...
### 77.3 方案：[`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) + Resources 自动加载 + 目录扫描
...
[`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（角色/背景/CG）、[`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs)（三库+打字音+通道音量，必须在
`_initialXxxVolume` 快照**之前**执行）、[`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（入口剧本+章节）、
[`VNMapModule`](Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs)（底图+地点，写在 `OnLaunch` 因为模板是禁用状态 Awake 不跑）、
[`VNStatsHud`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs) / [`VNQuestLog`](Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs) / [`VNInventory`](Assets/Project/Scripts/VNEffects/Script/VNInventory.cs) / [`VNShopModule`](Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs) / [`VNPlanModule`](Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs)（定义资产列表）。

**编辑器**：新增 [`Editor/VNGameConfigTools.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs) 三个菜单——
...
**生成器** `VNEffectsDemoSetup.CreateScriptDemoScene()`（[`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:451`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L451)）：
开头 `LoadAssetAtPath<VNGameConfig>` 取配置并挂到 stage/audio/runner；
背景/角色/CG/地图在配置非空时改用配置，否则退回原 demo 逻辑（向后兼容）；
章节改为 [`ScanScenarioAssets()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L728) 扫 `Assets/Scenarios/*.vn.txt`（**修掉了老代码只登记
...
`VNScriptParser.ParseChoiceOption`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:260`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L260)） 从行尾逐个摘**空格分隔的 token**，
...
**根因**：`VNScriptParser.ParseCommand`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:303`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L303)） 对命令行每个 token 做 `key:value` 判定——
...
`VNStoryAddress.TryParse`（[`Assets/Project/Scripts/VNEffects/Script/VNStoryAddress.cs:6`](Assets/Project/Scripts/VNEffects/Script/VNStoryAddress.cs#L6)） 与 Runner 侧的限定跳转逻辑都是好的——**唯独分词器这一层没打通**，
...
值里的 `::` 原样保留传给 [`VNStoryAddress`](Assets/Project/Scripts/VNEffects/Script/VNStoryAddress.cs)。
...
| 来源 | 内容 | 为什么 |
|---|---|---|
| `VNStage.cgLibrary`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:49`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L49)）（退回 `VNGameConfig.cgLibrary`（[`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:155`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L155)）） | **目录**：一共有哪些 CG | 目录是内容配置，随资产走 |
| [`VNCgUnlocks`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs) | **解锁**：看过哪些 | 独立 JSON，与存档槽分离；读旧档/开新周目不丢 |

退回配置资产这条路径是为将来做**纯标题画面的鉴赏入口**（那时场景里没有 VNStage）预留的。

**③ 差分组 = 网格合并成一格。**
`CgEntry.group`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:46`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L46)） 非空的条目合并为一格（五十六章建 cgLibrary 时就为此预留了该字段）。
...
- **[`VNCgGallery.cs`](Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs)（新，Script/）**：程序化 UI，独立 Overlay Canvas
...
- **接线**：[`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 加 `_cgGallery` 字段 + Start 自愈创建 +
  输入链分支 + [`RequestCgGallery()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1708)；[`VNQuickToolbar`](Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs) 加 `CG` 按钮；
...
### 最重要的设计决定：复用 `VNScriptParser.Parse`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:112`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L112)）
...
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs)：分析引擎（纯逻辑，返回 `List<VNLintIssue>`）
- [`Editor/VNScenarioLinterWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinterWindow.cs)：结果窗口（按文件分组、严重度筛选、搜索、
  双击/「打开」跳到出错行，走 `AssetDatabase.OpenAsset(asset, line)`）
- 菜单 **Tools → VN Effects → 剧本检查 Lint Scenarios**，快捷键 `Ctrl+Shift+L`

素材登记状态取自 [`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) 并与**当前打开场景**的 VNStage/VNAudio/
...
- 未编译验证（Unity 占用工程），只做了括号/结构检查；上述预期输出是静态推算，
  以实际运行结果为准。
- 尚无"按规则屏蔽"机制（`VNLintIssue.code`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:13`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs#L13)） 字段已为此预留）。
...
- **新增 [`Script/VNTitleMenu.cs`](Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs)**：标题层组件，全部运行时构建
  （与存读档/设置面板同款套路）。要点：
  ...
  - 收起规则只有一条：`VNScriptRunner.ResumeAt`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1018`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1018)）（新游戏/读档/编辑器
    "从选中行播放"全走它）调 [`NotifyGameplayStarted()`](Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs#L64) → 标题层销毁
...
- **[`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)**：
  - [`Start()`](Assets/Project/Scripts/VNEffects/Editor/VNAiEditorCoroutine.cs#L42)：发现场景里有 `VNTitleMenu` 且 `showOnStart` → 跳过
    `playOnStart`，改为 `titleMenu.Open()`；否则维持旧行为（老场景零影响）。
  - [`Update()`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs#L59)：标题打开期间屏蔽全部游戏快捷键与推进（插在画廊分支之后——
    叠在标题上的面板关闭键由上方各自分支处理，天然可用）。
  - 新增 [`StartNewGame()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1037)：`VNFlags.Clear()`（[`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:36`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs#L36)） + 从入口剧本 `Play()`
    （call 栈由 Play → Prepare 清，无需额外处理）。
  - [`ResumeAt()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1018) 开头补 `_titleMenu?.NotifyGameplayStarted()`。
- **[`VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs)**：新增"标题画面"区——`gameTitle/En/Ja`（按语言取、
  缺省回退中文、全空用 "Visual Novel"）、`titleBackground`（背景 id，留空 =
  背景库第一张）、`titleBgm`（留空 = 不播）。跟随资产的既有覆盖语义，
  重建场景不丢。
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)**：`CreateScriptDemoScene` 创建 `VNTitleMenu`
...
- **新增 [`Script/VNBattleModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs)**：继承 VNEventModule，整场战斗
  封在一个类里。
...
- **[`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)**：注册表登记 `battle` 模板（与 qte/map/
  shop/plan/result 并列）。
- **[`VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs)**：BuiltinOutcomes 表补 `battle → 胜利/失败/逃跑`
...
- **新增 [`Script/VNDialogueSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs) / [`Script/VNChoiceSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNChoiceSkin.cs)**：槽位声明组件。
  ...
  0 = 排版固定不避让），基准位置绑定时采样。
- **[`VNDialogueBox.cs`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs)（核心重构）**：拆"构建"与"行为"。程序化构建装进
  ...
  打字速度跨皮肤保留。
- **[`VNChoicePanel.cs`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs)**：CreateSkinButton（克隆模板，按相对路径找文字/花费槽）
  与 CreateDefaultButton（老代码）双路径，FinishButton 共用收尾
  （CanvasGroup/特效控制器/点击/悬停）。模板根无 Image 时特效降级（fx 判空）。
- **[`VNQuickToolbar.cs`](Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs)**：SetDock(dock) 停靠支持——皮肤 toolbarAnchor >
  panel > 对话框根；旧皮肤 Destroy 是延迟的，本帧内重新挂接即可救出功能条。
- **[`VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs)**：UiSkinEntry{id,prefab} + dialogueSkins/choiceSkins
  注册列表 + FindSkin 静态查找。
- **[`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)**：SetUiSkin(kind,id,line) 统一入口（default/空 = 回默认，
  未登记 id 报错并保持现状）；CurrentDialogueSkinId/CurrentChoiceSkinId 进
  存档快照，RestoreSnapshot 最先恢复皮肤（台词/选项直接落在正确皮肤上），
  且仅在与当前不同时才切换。
- **[`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)**：Dispatch 加 `ui` 命令；RebuildStateBefore 把 ui
  记入调试重建快照。[`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 加 dialogueSkin/choiceSkin（空 = 默认，
  旧存档兼容）。
- **新增 [`Editor/VNUiSkinExporter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs)**：Tools → VN Effects → UI 皮肤 UI Skins →
...
- **[`VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)**：登记 ui 命令（Scene 分类）。
- **[`VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs)**：Registry 收集皮肤 id；新规则 bad-ui-kind（Error）
...
- `agent/system-ui-skin-core`：新增 [`VNSystemUiSkinSet`](Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs)（唯一全局主题资产）、
  [`VNSystemUiSkinBehaviour`](Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs)（校验/安全实例化基类），`VNGameConfig.systemUiSkin`（[`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:131`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L131)） 为全局入口。
- `agent/title-menu-skin`：[`VNTitleMenuSkin`](Assets/Project/Scripts/VNEffects/Script/VNTitleMenuSkin.cs)；标题、版本、开始/继续/读档/鉴赏/设置/退出、
  退出确认框全部绑定槽位。自定义标题动画目标可选；标题页打开时会正确隐藏顶部属性 HUD。
- `agent/config-panel-skin`：[`VNConfigPanelSkin`](Assets/Project/Scripts/VNEffects/Script/VNConfigPanelSkin.cs)；四条滑杆、语言、全屏、关闭和背景关闭槽位。
- `agent/quick-toolbar-skin`：[`VNQuickToolbarSkin`](Assets/Project/Scripts/VNEffects/Script/VNQuickToolbarSkin.cs) + [`VNToolbarActionSlot`](Assets/Project/Scripts/VNEffects/Script/VNToolbarActionSlot.cs)；按钮通过 action 枚举
  声明功能，可在 prefab 内任意重排、删减或改图标；Auto/Skip 激活色仍由逻辑同步。
- `agent/save-load-skin`：[`VNSaveLoadSkin`](Assets/Project/Scripts/VNEffects/Script/VNSaveLoadSkin.cs) + [`VNSaveSlotSkin`](Assets/Project/Scripts/VNEffects/Script/VNSaveSlotSkin.cs)；整个页面、动态存档卡模板和
  覆盖确认框均可编辑，缩略图/时间/末句数据仍由运行时填充。
- `agent/cg-gallery-skin`：[`VNCgGallerySkin`](Assets/Project/Scripts/VNEffects/Script/VNCgGallerySkin.cs) + [`VNCgCellSkin`](Assets/Project/Scripts/VNEffects/Script/VNCgCellSkin.cs)；网格动态模板、锁定层、计数、
  全屏 CG 查看器与前后翻页全部槽位化。
- `agent/backlog-skin`：[`VNBacklogSkin`](Assets/Project/Scripts/VNEffects/Script/VNBacklogSkin.cs) + [`VNBacklogEntrySkin`](Assets/Project/Scripts/VNEffects/Script/VNBacklogEntrySkin.cs)；纯文字动态条目模板，保留滚动定位。
- `agent/stats-ui-skin`：[`VNStatsHudSkin`](Assets/Project/Scripts/VNEffects/Script/VNStatsHudSkin.cs)/[`VNStatsHudEntrySkin`](Assets/Project/Scripts/VNEffects/Script/VNStatsHudEntrySkin.cs) 与
  [`VNStatsPanelSkin`](Assets/Project/Scripts/VNEffects/Script/VNStatsPanelSkin.cs)/[`VNStatsPanelRowSkin`](Assets/Project/Scripts/VNEffects/Script/VNStatsPanelRowSkin.cs)；顶部 HUD 和完整属性页分别配置，动态属性仍来自
  [`VNStatDef`](Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs)/flags，数值条与图标按原逻辑刷新。
- `agent/system-ui-skin-integration`：[`VNSystemUiSkinExporter`](Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs)；菜单
  **Tools → VN Effects → UI 皮肤 UI Skins → 系统主题：导出默认模板 System UI: Export Default Prefabs** 一次生成 8 个可编辑起步 prefab
  和 `VNSystemUiSkinSet_Default.asset`，并自动写入 [`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs)。重复执行会更新默认模板；
...
每个 Slider 加透明全尺寸 Image 命中层；`VNConfigPanel.BindSlider`（[`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:179`](Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs#L179)） 也会在运行时为用户自制 prefab
...
**新增**
- [`Script/VNEquipment.cs`](Assets/Project/Scripts/VNEffects/Script/VNEquipment.cs)：装备核心（纯静态）。Equip/Unequip/Use/HandleItemLost/
  RecomputeEffects；道具查表入口 `ItemResolver` 由 VNInventory 注入。
- [`Script/VNInventorySkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNInventorySkin.cs)：背包皮肤槽位声明（面板根/标题/道具列表/7 装备格/介绍区，
  含 CollectValidationErrors）。
- [`Script/VNInventoryRowSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNInventoryRowSkin.cs) / [`VNInventorySlotSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNInventorySlotSkin.cs)：道具行 / 装备格子槽位。

**修改**
- [`Script/VNShopDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs)：新增 `VNEquipSlot` 枚举（1~7 部位）；Item 加装备/使用字段：
  `equipSlot`、`statBonuses`（属性加成）、`passiveEffects`（特殊效果，含 label 英日文）、
  `useOps`（使用效果）、`consumeOnUse`；新增嵌套类 `StatOp`/`PassiveEffect`。
- [`Script/VNInventory.cs`](Assets/Project/Scripts/VNEffects/Script/VNInventory.cs)：I 键物品栏全面改造成背包界面（双栏+右键菜单+介绍区）。
  皮肤 prefab 优先（`VNSystemUiSkinUtility.Instantiate`（[`Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs:29`](Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs#L29)）），缺失/校验失败退回程序化默认 UI
  （两条路径都产出 `VNInventorySkin` 引用，下游单一代码路径）。右键用
  `IPointerClickHandler`（ClickRelay 内部类）区分左右键，Button 只负责按压视觉。
- [`Script/VNSystemUiSkinSet.cs`](Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs)：加 `inventoryPrefab` 槽位。
- [`Script/VNShopModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs)：卖出后 `VNEquipment.HandleItemLost`（[`Assets/Project/Scripts/VNEffects/Script/VNEquipment.cs:140`](Assets/Project/Scripts/VNEffects/Script/VNEquipment.cs#L140)）（卖光装备中的道具强制卸下）。
- [`Editor/VNSystemUiSkinExporter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs)：`BuildInventory/BuildInventoryRow/BuildInventorySlot`
...
**新增**
- [`Script/VNPlanSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNPlanSkin.cs)：排程面板皮肤槽位声明（面板根/标题/左行动列容器+行模板/
  右日程格容器+格模板/重置/确定按钮；label 槽可选）。
- [`Script/VNPlanActionRowSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNPlanActionRowSkin.cs)：行动行槽位（按钮/名字必需；图标、独立收益文案槽可选，
  收益槽留空则拼进 nameText 富文本小字与程序化默认一致）。
- [`Script/VNPlanSlotRowSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNPlanSlotRowSkin.cs)：日程格槽位(按钮/天数/行动名必需)；background 可选 =
  空格/已排状态改色目标，emptyColor/filledColor 两个颜色字段随模板配置。
- [`Script/VNResultPopupSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNResultPopupSkin.cs)：结算弹窗槽位（仅面板根+等级大字必需，其余全可选降级；
  冲条三槽 barRoot/barFill/percentText 齐全才播悬念演出；burstOrigin 可自定义星光原点）。

**修改**
- [`Script/VNSystemUiSkinSet.cs`](Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs)：加 `planPrefab`、`resultPopupPrefab` 槽位（第 10/11 个）。
- [`Script/VNPlanModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs)：BuildUi 拆成 BuildFromSkin（模板行克隆+槽位接线）/
...
- [`Script/VNResultPopupModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs)：接入皮肤（BuildFromSkin/BuildDefault 两路径）+
...
- [`Editor/VNSystemUiSkinExporter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs)：`BuildPlan`/`BuildPlanActionRow`/`BuildPlanSlotRow`/
...
- [`Script/VNCharacterMarks.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMarks.cs) —— 漫符组件本体。`VNMarkKind` 枚举（11 种）、
  英文正名 + 中文别名解析表、`Show/ShowInstant/FadeOut/ClearAll/ClearImmediate`、
  常驻符号序列化（`SerializeKeep`）。

**修改**

- [`VNProceduralTextures.cs`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) —— 新增漫符贴图生成区（约 260 行）：
  `MarkSprite(kind)` 懒加载缓存 + `HardMark`（4×4 超采样 + 圆核膨胀描边）+
  `SoftMark`（柔边 alpha）+ 形状基元（圆/胶囊/圆弧）+ 11 个符号的形状函数。
- [`Script/VNCharacterDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) —— 加「漫符」区：`markAnchor`（归一化偏移，默认
  `(0.2, 0.36)`）、`markScale`、`markSprites`（自定义图覆盖列表 + `MarkOverride` 类）。
- [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— `ActiveCharacter.marks`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:142`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L142)） 字段、`CreateCharacter` 挂组件、
  `Mark()` API、`CaptureSnapshot` 写常驻符号、`ShowInstant` 重载还原符号。
- [`Script/VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) —— `CharSave.marks`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:111`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L111)）（英文正名逗号串，旧存档为空即无符号）。
- [`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) —— 关键字加 `mark`。
- [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) —— `Dispatch` 的 `mark` case、`ParseMarkPos` 解析
  `pos:x,y`、`RebuildStateBefore` 的 `mark` case、`RebuildMarkState`、
  `RebuildShowState` 补上常驻符号的沿用。
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) —— `MarkNames` + `mark` 命令参数模式登记。
- [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) —— 命令中文名「漫符」+ `MarkTranslations`
  下拉中文对照 + `IsMarkParameter`。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— `bad-mark` / `bad-mark-mode` 两项检查。
...
1. **沿用「透明画布叠加层」而不是新做一层**：符号是角色 GameObject 的子物体，
   ...
   共用 [`VNImageEffectController.Mat`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs)，于是出场溶解、退场淡出、色调匹配
   全部自动生效 —— 退场路径一行代码都不用写。
...
4. **不做 `emote` 自动联动**：用户选的是「新命令」而非「挂在 emote 上」，
   ...
   将来若要联动，在 [`VNCharacterEmotes`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs) 各动作里调 `marks.Show()` 即可，
   是加法不是改法。
...
6. **只有 `keep` 进存档**：一次性符号播完即逝不是持续状态，
   ...
   与运行时 `VNStage.Show`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:326`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L326)） 不清符号的行为保持一致。
...
- [`Script/VNQuizDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs) —— 题库资产（`VN/Quiz Definition`）。`Question`（三语题干 /
  2~4 个 [`Option`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs) / `answerIndex` / 解析 / 单题 `timeLimit` / `rewardOnCorrect` /
  `penaltyOnWrong`）+ 题库级 `defaultTimeLimit`、`flagPrefix`、[`ValidQuestions()`](Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs#L128)。
- [`Script/VNQuizModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs) —— 事件模块本体。选题 → 出题 → 倒计时 → 判定 → 反馈 →
  结算的状态机（[`Phase`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs) 四态），程序化 UI（暗幕 / 面板 / 倒计时条 / 4 个选项按钮 /
...
- [`Script/VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) —— 玩法区加 `quizzes` 列表。
- [`Editor/VNGameConfigTools.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs) —— 场景导入与目录扫描都带上题库定义。
- [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) —— `QuizzesDir` 常量、注册表登记 `quiz` 模板、
  [`EnsureQuizDef()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L937) 示例题库「社团常识」（5 题，够演示随机抽 3 题）+ [`MakeQuestion()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L1003)
  辅助、Demo 剧本头部语法速查补 quiz。
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) —— `event` 命令说明补 battle / quiz 两个内置模块。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— `BuiltinOutcomes` 加 quiz 三档结果名；
...
5. **奖励复用 [`VNShopDef.StatOp`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs)**：项目里「属性 id + 增量」的条目类型已经存在
   （装备加成 / 道具使用效果都用它），再定义一个同形结构只会让 Inspector 手感不一致。
   写入统一走 `VNStatsHud.Apply()`（[`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:108`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs#L108)）（钳制 + 飘字），没有 HUD 时退回 `VNFlags.Add`（[`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:34`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs#L34)）。
...
- 新增 [`Editor/VNQuizInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNQuizInstaller.cs) —— **Tools → VN Effects → 场景装机 Install To Scene → 限时问答 Quiz Module**。
  只做三件事：在场景现有的 [`VNEventRegistry`](Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs) 下补一个禁用的 `QuizTemplate`
...
反馈区挤在一起）。查下来根因是 [`VNToast`](Assets/Project/Scripts/VNEffects/Script/VNToast.cs) 的实现：**一行裸文字**，屏幕底部上方 300px 居中，
...
- [`Script/VNStatsHud.cs`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs) —— `HudEntry` 加 `icon` / `root` / `barTween` / `rollTween`；
...
2. **两种颜色各司其职**：卡片图标底色 = `VNStatDef.color`（[`Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs:45`](Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs#L45)）（认出是哪个属性），
...
- 聊天内容如果放进 ScriptableObject 资产，就得像 [`VNQuizDef`](Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs) 那样自建三语字段，
...
- [`Script/VNSnsMessage.cs`](Assets/Project/Scripts/VNEffects/Script/VNSnsMessage.cs) —— 一条消息的可序列化数据（id / sessionId / sender /
  kind(text·voice·image·system·time) / text / assetId / unlock / read / played）。
- [`Script/VNSnsView.cs`](Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs) —— 手机聊天视图：程序化 UI（手机外框 + 顶栏头像 + 滚动聊天区 +
...
- [`VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) —— `sns` 进 Keywords；`sns reply` 复用 choice 的 `*` 子行机制；
  `sns time` / `sns system` 的自由文本从第 2 个 token 起不当 kwarg（否则 `23:47` 会被吃成键值对）。
- [`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) —— `SnsCo` / `SnsReplyCo` / `SnsSayCo` 三个协程；
  ...
  回复倒计时期间整体让位给面板。
- [`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— `sns` 字段 + AutoWire 自愈创建 + `IsSnsOpen`；
  `CaptureSnapshot` / `RestoreSnapshot` / `ClearStage` 三处接上会话状态。
- [`VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) —— `VNSaveData` 加 `snsOpen / snsPeerId / snsSessionId / snsTitle /
  snsPlayerAlias / snsMessages`（旧存档全部缺省 = 未打开）。
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) + [`VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) —— 登记 sns 命令与新分类「SNS」。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— `CheckSns` 规则组 + `late:` 标签的跳转目标解析。
- [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) —— Demo 剧本头部语法速查加 SNS 段。
...
**修改**：[`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)（单文件）
...
- `dotnet build Assembly-CSharp-Editor.csproj` **0 错误**
  （构建前 csproj 因未刷新缺 [`VNSnsView.cs`](Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs) / [`VNSnsMessage.cs`](Assets/Project/Scripts/VNEffects/Script/VNSnsMessage.cs)，临时补进去编译后已还原）。
...
- [`VNEntranceAnimator.cs`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs) —— `VNEntrancePreset` 加 `Crossfade`/`SlideIn`/`StepIn`/`WalkIn`
  （放在枚举最前，Crossfade 成为默认）；新增 `VNExitPreset`（Fade/Dissolve/RunOut/Sink）
  与 `VNSide`（Auto/Left/Right/Top/Bottom）；`BaseDuration()` 基准时长表、
  [`IsCasual()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L182) 日常向判定、`PlayEntrance(preset, side, scale)` 与 `PlayExit(...)` 新签名
  （旧签名保留为重载，Demo 场景不用改）。
- [`VNFootShadow.cs`](Assets/Project/Scripts/VNEffects/VNFootShadow.cs) —— 新增 `Impact(strength, duration)`：影子横向摊开 + 纵向压扁再缓回，
  供 `stepin` 在落地那一帧调用（LateUpdate 里乘进 localScale，不打断原有的悬浮联动）。
- [`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— [`SideFor()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L305) 方向推断；`Show` / `Hide` 加 `from`/`to` + `duration` 重载；
  `ActiveCharacter.casualEntrance`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:149`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L149)） 记录是否日常向登场。
- [`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) —— show/hide 传 `from:` / `to:` / `dur:`；
  `RebuildShowState` 把 `with:` 折算成 `casualEntrance` 一起重建。
- [`VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) —— `CharSave.casualEntrance`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:116`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L116)）（旧存档缺省 false = 保持原行为）。
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) —— show/hide 补 `from:`/`to:`/`dur:` 参数与新预设候选。
- [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) —— 三张中文名表：`EntranceTranslations`
  （如 `Crossfade（原地淡入·日常）`）、`ExitTranslations`、`SideTranslations`，
  下拉里直接看得懂每个预设做什么。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— 通用 `CheckEnum<T>` + `CheckSide`：
  预设名/方向拼错直接报错（运行时只会静默退回默认预设，最容易漏）。
- [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs)、`HowToUse.md`（show/hide 两节 + 速查卡 + 校验器表）同步。
...
2. **`casualEntrance` 进存档**：不存的话，读档摆台一律走 [`StartIdleEffects()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L523) 默认参数，
...
3. **`WalkIn` 用分轴 tween**：横向 `DOAnchorPosX` 匀速（走路不该有加减速），
   ...
   [`PrepareHidden()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L114) 也补上了 `localRotation` 重置——出场被打断时不会留下歪着头的立绘。
...
（[`VNAmbientParticles.Preset.Petals`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs)）的根因：
...
1. **白色的根因是混合模式，不是颜色配错**：`VN/Additive` 是 `Blend SrcAlpha One`，
   ...
   把 tint 调成任何粉色都没用。
2. **每片长得一模一样**：全屏共用一张柔边椭圆贴图（[`VNProceduralTextures.Petal`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs)），
   人眼几帧内就识破重复。
...
- [`VNFoliageTextures.cs`](Assets/Project/Scripts/VNEffects/VNFoliageTextures.cs) —— 叶型枚举 `VNLeafShape`（Sakura/Maple/Ginkgo/Broadleaf/Bamboo）
  + 程序化图集烘焙。图集布局 **列 = 12 翻转帧，行 = 4 形态变体**，
  RGB 存明暗（叶脉/折痕/根深尖浅/背面压暗），A 存形状，色相交给粒子 startColor。
- [`VNWeatherDef.cs`](Assets/Project/Scripts/VNEffects/VNWeatherDef.cs) —— 全部可调参数的 ScriptableObject（含五套内置预设，
  不建任何资产也能用）+ id 解析（认英文正名与中文别名）。
- [`VNFoliageSystem.cs`](Assets/Project/Scripts/VNEffects/VNFoliageSystem.cs) —— 三层景深飘落系统：图集翻转、每粒子独立相位横摆、
  全局阵风、尺寸↔速度联动、地面堆积。

**新增（编辑器）**

- [`Editor/VNWeatherPreviewWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNWeatherPreviewWindow.cs) —— Tools → VN Effects → 预览 Preview → **天气预览 Weather Preview**：
  编辑模式下播放翻转帧预览（四种变体并排、按色带着色），运行模式下滑杆实时应用到场景，
  另存为资产 + 一键登记进 VNGameConfig。

**修改**

- [`VNWeatherController.cs`](Assets/Project/Scripts/VNEffects/VNWeatherController.cs) —— 双后端：飘落类走 `VNFoliageSystem`，
  雨/雪/萤火虫仍走 `VNAmbientParticles`。统一入口 `SetWeatherId(id, …)` 三级解析
  （自定义资产 id → 内置叶型别名 → `VNWeather` 枚举名），新增 `CurrentId`、
  `Gust()`、`SetAmbient()`、[`PreviewDef()`](Assets/Project/Scripts/VNEffects/VNWeatherController.cs#L149)。
- [`VNSakuraBurst.cs`](Assets/Project/Scripts/VNEffects/VNSakuraBurst.cs) —— 改走 `VNFoliageSystem`，自动继承全部底层改进；
  另加起手阵风冲击 + Burst、中途补两记阵风、尾声风力衰减、近景层权重大幅拉高。
- `VNAmbientParticles.cs` —— `Preset.Petals` 标注弃用（仅为旧场景兼容保留）。
- [`Script/VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) —— 新增「飘落天气库」`weatherDefs`。
- [`Script/VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) —— `weather` 语义改为 id 字符串（旧存档的枚举名照常认得），
  新增四个覆盖参数字段（`weatherDensity/Speed/Size` + `weatherWindSet/Wind`）。
- [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— 天气覆盖参数由 VNStage 统一持有；
  `SetWeather(id, …)` 命令入口、`ApplyWeather()`、[`WeatherGust()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1440)；
  CG 暂停恢复由 `_cgSavedWeather`（枚举）改为 `_cgSavedWeatherId`（字符串）。
- [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) —— `weather` 命令支持 `density:/wind:/speed:/size:`；
  `RebuildStateBefore` 同步重放这四个参数。
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) —— 新增 `VNParamSource.WeatherId`，weather 条目改四参数版。
- [`Editor/VNScenarioDoc.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs)、[`VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) —— 天气候选与校验
  （`IsKnownWeatherId` 与运行时三级解析同构，中文别名也认）。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— 新增 `unknown-weather` 检查。
- [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) —— 生成 `VNParticleAlpha.mat` 并接到天气控制器与樱吹雪。
- [`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs) —— 提示文字改显示 `CurrentId`（新叶型在枚举里统统算 Petals）。
...
原有眨眼（第十九批 [`VNCharacterBlink`](Assets/Project/Scripts/VNEffects/Script/VNCharacterBlink.cs)）是**整张立绘替换**：到点把 `Image.sprite` 换成
一张完整的闭眼全身图，闭眼时间到再换回来。这要求每个角色额外准备一张与睁眼图
像素级对齐的整图，AI 生成立绘时很难保证除眼睛外一模一样，稍有偏差就"整个人抖一下"。

而说话口型 [`VNCharacterMouth`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMouth.cs) 用的是另一条路：原立绘不动，在其上方叠一张**同画布
...
- [`Script/VNCharacterBlinkOverlay.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterBlinkOverlay.cs) —— 分层眨眼组件。结构 = `VNCharacterMouth` 的
  overlay 建法（子物体 `BlinkOverlay`，anchor 全拉伸、`preserveAspect=false`、
  `raycastTarget=false`、共用 [`VNImageEffectController.Mat`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs)）+ `VNCharacterBlink` 的
  DOTween 计时序列（随机间隔 → 显示 → `blinkDuration` → 隐藏 → 重排）。
  只在默认表情工作；`blinkMode != Overlay` 时整个组件自然静默。

**修改**

- [`Script/VNCharacterDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) —— 新增枚举 `VNBlinkMode { FullSprite=0, Overlay=1 }`、
...
- [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— `ActiveCharacter` 加 `blinkOverlay` 字段；
...
- [`Editor/VNCharacterVisualPreviewWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs) —— 草稿加 `blinkMode` / `blinkOverlaySprite`
...
- [`VNLiquidPreset.cs`](Assets/Project/Scripts/VNEffects/VNLiquidPreset.cs) —— 四套内置液体预设（water / blood / ink / slime）+ `VNLiquidArgs`
  参数结构。预设覆盖空中飞行与屏幕水渍两段的全部手感参数；`VNLiquidArgs` 由剧本层、
  舞台层、存档重建三方共用，避免各写一份默认值而慢慢漂移。
- [`VNLiquidSplash.cs`](Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs) —— 舞台层喷溅。三个发射器：Body（`VN/ParticleAlpha` 拉伸公告板
  主水珠）/ Glow（`VN/Additive` + HDR 高光）/ Splinter（低速碎珠）。提供 `Burst`
  一次性爆溅、`StartSpray`/`StopSpray` 间歇喷射、`SetClickMode` 点击喷水。
- [`VNWetScreen.cs`](Assets/Project/Scripts/VNEffects/VNWetScreen.cs) —— 屏幕层水渍。对象池 + 每帧手动模拟的四段状态机：
...
- [`VNProceduralTextures.cs`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) —— 新增 5 张程序化贴图（`LiquidBlob` 头重尾轻的水珠、
  `LiquidSplinter` 碎珠、`WaterDrop` 假折射水渍、`DropSpec` HDR 高光、
  `LiquidStreak` 水痕）+ `GenerateRgba` 彩色生成器重载。
- [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— `liquidSplash`/`wetScreen` 两个引用 + AutoWire 自动接线（含
  两层互连）+ [`Liquid()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1456) 命令入口 + [`ResetLiquid()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1505) + `CaptureLiquid`/`RestoreLiquid`
  存档三处 + `ResetEffects` 联动。
- [`Script/VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) —— `VNSaveData.LiquidSave` 子结构（spray/click/wet/cover
  四个持续开关及其参数）。
- [`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) —— 关键字 `liquid`。
- [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) —— `case "liquid"` 派发 + `ParseLiquidArgs` + 调试重建
  静默重放 + `reset effects` 清空 + **点击喷水模式下屏蔽左键推进**。
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) / [`VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) —— 命令模式登记（11 个参数）
  + 中文名「液体喷溅」。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— 4 项检查：未知子命令、未知液体类型、x/y 超出屏幕
  比例范围、开关位不是 on/off。
- [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) —— 生成器创建两个组件并接线；`AssignSourceMaterial`
  拆出按字段名的 `AssignMaterialField`（`VNLiquidSplash` 要 alpha + additive 两份材质）；
  材质目录与 `EnsureMaterial` 开放给增量安装器共用。
- [`Editor/VNLiquidInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNLiquidInstaller.cs)（新增）—— 增量安装器
  ...
  Hierarchy，为了两个物体重建整个场景代价太大。照 [`VNQuizInstaller`](Assets/Project/Scripts/VNEffects/Editor/VNQuizInstaller.cs) 的思路只做加法。
- [`VNEffectsDemo.cs`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs) —— 演示按键 `` ` `` 鼠标处爆溅 / F1 间歇喷射 / F2 换液体 / F3 湿镜头。
...
推进——否则玩家会被卡死在这一句台词里。同时喷水模式会顺手让 [`VNClickRipple`](Assets/Project/Scripts/VNEffects/VNClickRipple.cs) 让位，
...
连同命中名额一起传过来（`PendingHit.angleDeg`（[`Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs:83`](Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs#L83)））；`SplatBurst` 的卫星滴则各自
...
要求全项目中文文字改用这个字体。此前 [`VNFont.cs`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs) 里中文和英文共用同一套
...
- `VNFont.cs`：把原来 zh/en 共用一套的 `ScProfile` 拆成三套独立档案——
  ...
  原来写死在 [`AssetFor()`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs#L199) 里的"日文专属 fallback 挂载"逻辑泛化成
  `Profile.fallback`（[`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:96`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs#L96)） 通用字段，解析失败时退到 fallback 档案、解析成功时
  把 fallback 字体挂进 TMP 的 `fallbackFontAssetTable` 补缺字，两种语言共用一套代码。
- [`VNFontAssetBuilder.cs`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs)：中文烘焙源指向新 ttf；新增
  [`EnsureGeneralFontAsset()`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs#L84) 烘焙英文/兜底用的 `NotoSansSC-General-Dynamic.asset`；
  [`CreateMenu()`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs#L33) 里一并调用。
- `Assets/Resources/VNFonts/NotoSansSC-Dynamic.asset`（+`.meta`）：删掉旧的
  （内容是 Noto Sans SC）后用 [`Ensure()`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs#L193) 从新字体源重新生成。
...
1. **进 Play Mode 后编辑内容全没了。** 这是本章要修的头号 bug，
   而且它跟"忘了保存"无关——[`VNScenarioEditorWindow`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) 的 `_doc` / `_path` /
...
- 命令级暂停/单步：`IsDebugPaused` / [`SetDebugPaused()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L232) / [`RequestDebugStep()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L239)，
  闸门在 [`Run()`](Assets/Project/Scripts/VNEffects/Editor/VNSoftSkinExporter.cs#L106) 主循环顶部。`Play(string)` 开局时清暂停标记，
  免得编辑器留下的暂停态把正常开局卡死。
...
- 第1章.vn.txt 有 206 行，开头一大段是教学注释，编辑器里每条注释都占满
  一行 21px，想看剧情得先滚过去一屏。
- 滚轮上滑打开回想是 [`VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 里硬编码的，有人嫌误触但关不掉。
...
只让 [`RowHeight()`](Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs#L55) 对隐藏行返回 0、`DrawRow()` 直接 return。
这样 `_doc.rows` 的索引完全不变，`SourceLineForRow` / `RowForSourceLine` /
多选 / 拖动排序 / 删除 / 舞台一览 / 校验圆点全部零改动，
也不存在过滤视图那种"拖到隐藏行之间算什么"的语义黑洞。

**只隐空行与 `#` 注释，不隐全部 Raw 行。** [`VNRowKind.Raw`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) 还兜着两种
语法残留：前面没有 choice 的孤儿 `*` 选项行、前面没有 camseq 的孤儿 `>`
路径点行（`[VNScenarioDoc.cs:113](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs#L113)/119/125` 三个入口）。那两种一旦藏起来就
...
- [`VNConfigPanel.WheelOpensBacklog`](Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs) 静态属性（PlayerPrefs `VN.Config.WheelBacklog`，
...
所以只更新导出器（`VNSystemUiSkinExporter.BuildConfig`（[`Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs:200`](Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs#L200)））没用——它只重建 `_Default`。
本章两个 prefab 都处理了：
...
- [`Editor/VNCamWaypoint.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs)
  - `VNCamWaypoint`：一行 `> ...` 的结构化视图 + 严格 `TryParse` / `Format`。
    语法与运行时 `VNScriptParser.ParseCamWaypoint`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:193`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L193)） 同构。
  - `VNCamseqText`：一整段 camseq 文本（header + 路径点行）的 `TrySplit` / `Join`，
    不经 [`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)（那边解析失败会往 Console 打 warning，编辑期不该有噪音）。
  - `VNCamseqTemplates`：11 条内置运镜模板（推近特写 / 缓慢拉远 / 推近后回位 /
    左右横摇 / 环视全景 / 三连甩镜 / 告白推镜 / 惊讶弹镜 / 叠化转特写 /
    叠化开场特写 / 转场瞬切起手）。`{char}` 占位在套用时替换成当前第一个角色 id。
- [`Editor/VNTextPromptWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNTextPromptWindow.cs)：极简单行输入弹窗（Unity 没有内置的
  「带输入框的 DisplayDialog」），「把本行存为预设」用它要名字。

**修改**

- [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)
...
- [`Editor/VNCamseqEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs)
...
- [`Editor/VNScenarioDoc.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs)：Lint 增加一条 Warning——路径点行结构化失败时点名，
  免得写错了还以为编辑器坏了。

### 技术决策与取舍

**1. 存储不变：`VNRow.camLines`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs:49`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs#L49)） 仍是唯一真相（字符串列表）**
...
**底图画的是场景里 `VNStage.backgroundImage`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:55`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L55)） 当前挂着的那张**（编辑期就是个占位图），
...
**逐行推算「执行到这一行时背景是什么、台上有谁站哪」**（[`RowStageState`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)）。
既然窗口现在知道自己绑在第几行，直接问它要就行——零新逻辑，
而且两边看到的东西永远一致。

### 文件改动清单

- [`Editor/VNCamWaypoint.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs)：新增 `VNRowStageInfo` / `VNRowStageChar` 传输类
...
[`Editor/VNCamseqEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs)（本章只动这一个文件）
...
运行时 `CreateCharacter` 会挂 [`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) / blink / mouth / marks 一大堆，
...
- [`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)：`VNCamWaypointDef` 加 `hold` 字段；`ParseCamWaypoint`
  认 `hold:秒` token（非正数告警并忽略）。
- [`VNCamera.cs`](Assets/Project/Scripts/VNEffects/VNCamera.cs)：`Waypoint` 加 `hold`；`BuildSegment` 在补间段/瞬切段之后
  `AppendInterval(hold)`；新增 `HoldCo(seconds)` 供 `PlayPathCo` 的叠化段用；
  `PlayPathCo` 在 `startFade` 与每个 `xfade` 点的 `CrossfadeTo` 之后接 hold。
- [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)：`CamseqCo` 把 `def.hold` 带进 `VNCamera.Waypoint`；
  call 参数替换的 `camPoints` 深拷贝也补上 `hold`（漏了它，带参子程序里的 hold 会丢）。

**编辑器（hold 的两套 UI）**

- [`Editor/VNCamWaypoint.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs)：`hold` 字段 + `TryParse` 认 `hold:`（非正数/重复一律返回
  false 退回纯文本）+ `Format` 在 `xfade` 之后输出。
- [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)：路径点行加 `hold` 数字框（tailW 加宽 76px）；
  黄色纯文本行的悬停语法提示补上 `[hold:秒]`。
- [`Editor/VNCamseqEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs)：`Waypoint.hold`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:215`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L215)） + 列表第二行加 hold 框
...
- `Editor/VNCamseqEditorWindow.cs`：`Guides` 位标志枚举（三分线/中心十字/安全区/
  ...
  `DrawCompositionGuides(frame, clip)` + [`DialogueBandFractions()`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs#L2210) 实测对话框 +
...
`DOTween.timeScale = skipTimeScale` 实现的（`VNScriptRunner.SetSkip`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1866`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1866)）），
...
预览的缓动分配和运行时对不上。所以 `Segment.isHold`（[`Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs:2792`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs#L2792)） 在计数和赋值两个循环里都要跳过。
...
**对话框遮挡区实测优先。** 从 `VNStage.dialogue`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:59`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L59)） 拿 RectTransform，`GetWorldCorners`
...
**快照用 [`GenerateText()`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs#L347) 字符串，不是深拷贝对象图。** 路径点 + 开场/收尾叠化设置
...
- **运行时链路**：`camseq` + 两个带 hold 的路径点 → `hold0=1.2 hold1=0.4`；
  `VNCamWaypoint.TryParse`（[`Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs:105`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs#L105)） 往返一致（`> 教室:head 1.9 0.8 ease:OutSine xfade:0.5 hold:1.2`
...
- [`VNBadmintonBallistics.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonBallistics.cs) —— 手感参数类 `VNBadmintonTuning` + 球路结构 `VNBadmintonArc`
  + 纯静态数学：三点定抛物线、飞行速度、落点抽样、精准判定、接球点反解。
  **无 MonoBehaviour / 无 UI 依赖**，可单测。
- [`VNBadmintonUi.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs) —— 三个文件共用的程序化 UI 辅助 + `VNBadmintonQuad`
  （uGUI 画不出梯形，球场透视地面靠它）。
- [`VNBadmintonCourt.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs) —— 程序化球场与 HUD（天空/广告横幅/沙地/透视球场/场地线/
  球网/记分板/得分横幅/操作提示）。不是 MonoBehaviour，由模块构造并持有。
- [`VNBadmintonActor.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonActor.cs) —— 角色表现层。六态程序化假动画、球拍绕肩画弧、扣杀残影、
  脚影与落地冲击、台词气泡。
- [`VNBadmintonDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonDef.cs) —— 对手 + 难度 + 台词 + 立绘 + 赛制 + 音效槽位 ScriptableObject。
  合并了参考实现的 `BadmintonLevelCfg` 与 `LoveBadmintonCfg` 两张表。
- [`VNBadmintonSfx.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonSfx.cs) —— 五个音效的代码合成（`AudioClip.Create`），Def 填了就覆盖。
- [`VNBadmintonModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs) —— [`VNEventModule`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs) 子类，五态状态机 + 主循环 + 计分 + 结算。

**新增编辑器**

- [`Editor/VNBadmintonInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNBadmintonInstaller.cs) —— `Tools → VN Effects → 场景装机 Install To Scene → 羽毛球对战 Badminton Module
  To Scene`，增量装机（照 [`VNQuizInstaller`](Assets/Project/Scripts/VNEffects/Editor/VNQuizInstaller.cs) 范式），不重建场景、可重复执行。
...
- [`VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) —— 加 `badmintons` 列表。
- [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) —— 加 `BadmintonDir` 常量 + 生成场景时建
  `BadmintonTemplate` 并登记 id `badminton`。
- [`Editor/VNGameConfigTools.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs) —— 从场景导入 / 扫资产两条路径都补上羽球对手库。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— 结果名白名单加 `badminton`；新增 `unknown-badminton`
...
3. **判定几何与动画分开**。[`RacketPointFor()`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonActor.cs#L111) 返回固定几何（站位 + 拍型对应的固定高度），
...
- **属性影响强度**：`powerstat: / speedstat: / jumpstat:` 指定读哪个 flag，
  ...
  照 [`VNBattleModule.patkstat`](Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs) 范式——模块不认识任何具体属性名，桥全在 flags 上。
- **战绩回写 flag**：`<前缀>_我方得分 / _对方得分 / _精准数 / _最长回合`，
  前缀默认「羽球」，剧本 `flag:` 可改。剧本可据此做「零封」「虽败犹荣」「打得漂亮」细分支。
- **多对手难度**：Def 解析三级 —— 剧本 `id:` → 剧本 `vs:`（同名资产）→ 库里只有一条时直接用。
- **立绘三级回退**：Def 的羽球专用图 → Def / `vs:` 指定角色的 [`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 默认立绘
  → 模板兜底图 → 剪影占位。
...
音量跟随 `VNAudio.seVolume`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:51`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L51)），Def 里填了真音效就逐条覆盖。
...
- [`VNPhotoStickerDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoStickerDef.cs) —— 贴纸资产（id / 三语名 / Sprite 或程序化图形 / 色调 / 初始尺寸）。
  参考实现里贴纸是边框图的附属小图，这里拆成独立资产，因为同一张贴纸要被多个边框、
  多个主题的评分表复用。
- [`VNPhotoFrameDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoFrameDef.cs) —— 边框资产。比参考多出：程序化兜底、人物开窗形状（椭圆/圆角/无）、
  前景层、水印文字、自带装饰件（可锁定）。
- [`VNPhotoThemeDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoThemeDef.cs) —— 主题 + 清单制评分表（表情/边框/贴纸三张清单，每项带分数与命中评语）
  + 限时 + 基础分 + 完美线/及格线 + 贴纸总数上限（防刷分）+ 三档总评。
  另含共用的三语文本类 `VNPhotoLine`。
- [`VNPhotoScore.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoScore.cs) —— **纯静态评分数学**，无 MonoBehaviour / UI 依赖，可单测。
  故意保持无状态、无随机：同样的装扮永远同样的分，玩家才学得会「什么样算好照片」。
- [`VNPhotoTextures.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoTextures.cs) —— 程序化贴图：5 套边框（粉格子/星空/胶片/简约白框/樱花）、
  椭圆与圆角遮罩、开窗描边环、10 种贴纸隐函数图形、相纸（九宫格）、实心圆。
- [`VNPhotoCapture.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs) —— 取景框区域截图。**「怎么拍」全关在这一个文件里**，
  以后要换成独立 Camera + RenderTexture 只改它，模块不用动。
- [`VNPhotoAlbum.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs) —— 相册全局存储（`persistentDataPath/vn_photos/` 的 PNG + index.json），
  与 20 槽存档完全分离（同 [`VNCgUnlocks`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs) 的道理）。索引损坏可从文件名恢复；
  纹理走 LRU 缓存（上限 12 张）防翻相册爆内存；容量上限 200 张。
- [`VNPhotoBoothUi.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs) —— 程序化 UI 辅助 + `VNPhotoStickerItem`
  （左键拖 / 滚轮缩放 / Shift+滚轮旋转 / 右键删，走新版 Input System）。
- [`VNPhotoSfx.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoSfx.cs) —— 五个代码合成音效（倒数滴 / 快门咔嚓 / 贴纸落位 / 计分嗒 / 结算琶音）。
  快门声是 noise 高 + 衰减快的**两下**——单簧快门就是「咔」+「嗒」。
- [`VNPhotoBoothModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs) —— [`VNEventModule`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs) 子类。五态状态机
  （Dressing → Confirm / Shooting → Result → Ending）、装扮交互、限时、
  快门序列、结算演出、flag 与属性结算。

**新增编辑器**

- [`Editor/VNPhotoBoothInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNPhotoBoothInstaller.cs) —— `Tools → VN Effects → 场景装机 Install To Scene → 拍大头照 Photo Booth Module
  Module To Scene`，增量装机（照 [`VNBadmintonInstaller`](Assets/Project/Scripts/VNEffects/Editor/VNBadmintonInstaller.cs) 范式）。
...
- [`VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) —— 新增 `photoFrames` / `photoStickers` / `photoThemes` 三个列表
  与 `photoMeCharacterId`（大头贴里「我」默认用哪个角色）。
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) —— `photo` 的结果名集合、主题/边框 id 集合（扫资产）、
  以及四项专项校验（见下）。
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) —— `event` 说明里补 `photo` 与 `badminton` 的参数用法。
...
6. **星光爆发自己画。** 事件模块三铁律之一是不碰舞台演出，所以没用
   `VNAmbientParticles.PlaySparkleBurst`（[`Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs:493`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs#L493)），改在 UI 层生成 8 个 sparkle 飞散。
...
3. **又一次被装机器的 `EditorUtility.DisplayDialog` 卡住 MCP**——
   ...
   这回做了根治：装机器拆成 `Install()`（弹框）+ [`InstallCore()`](Assets/Project/Scripts/VNEffects/Editor/VNPhotoBoothInstaller.cs#L50)（不弹框），
   **以后所有装机器都按这个形状写，脚本一律调 Core**。

### 验证方法

- 评分数学：`VNPhotoScore.Evaluate`（[`Assets/Project/Scripts/VNEffects/Script/VNPhotoScore.cs:57`](Assets/Project/Scripts/VNEffects/Script/VNPhotoScore.cs#L57)） 三个用例——满配 90 分/完美、
...
- **P5：G 键画廊的「照片」标签页 + 删除**（[`VNCgGallery`](Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs) 加标签切换、
  [`VNCgGallerySkin`](Assets/Project/Scripts/VNEffects/Script/VNCgGallerySkin.cs) 加两个可选槽位）。当前照片能拍能存，但只能去
  `persistentDataPath/vn_photos/` 目录看。
- 主角立绘：`VNGameConfig.photoMeCharacterId`（[`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:248`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L248)） 现在留空，剧本用 `me:` 临时指定；
...
6. **背景图**（第三个标签页）。新增 [`VNPhotoBackdropDef`](Assets/Project/Scripts/VNEffects/Script/VNPhotoBackdropDef.cs) + 8 种程序化样式
...
- 输入板（[`VNPhotoDoodleInput`](Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs)）只在停留于「涂鸦」页时 `raycastTarget = true`，
...
- **缩略图必须走独立缓存**：`VNPhotoAlbum.LoadTexture`（[`Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:233`](Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs#L233)） 是 12 张的 LRU，
...
| 文件 | 改动 |
|---|---|
| [`VNPhotoBoothModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs) | 全套布局常数重排；新增 `BuildHelpPanel` / `ToggleHelp`；删掉底栏说明；结算相纸放大并收起左右栏与取景框 |
| [`VNPhotoDoodle.cs`](Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs) | 画布分辨率 640×480 → 768×576 |
...
- **说明面板做成「?」圆钮 + 点击展开的卡片**，右上角。折叠时只占 54px，
  ...
  首帧量不准，与 [`VNSnsView`](Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs) 同一路数）。
...
顺带发现一个历史遗留：`VNDialogueBox.nameTagColor`（[`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:35`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L35)） 的注释写着「角色定义里的 nameColor 优先」，
但 `VNStage.Say()`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1074`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1074)） 只传了名字和文本，**从来没有把 `VNCharacterDef.nameColor`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:37`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L37)） 传给名牌**——
这次一并兑现。
...
- [`Script/VNNameplateStyle.cs`](Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs)——名牌装饰样式：纯数据 + 静态 [`ApplyTo()`](Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs#L468)，
  内置四套预设（Plain 老外观 / Bold / Plate / Outline）

**修改**

- [`Script/VNFont.cs`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs)——新增装饰字体入口 `DisplayAsset` / `DisplayAssetFor(lang)`；
  `Profile` 加 `padding` / `samplePointSize` / `atlasSize` / `isDisplay` 四个字段；
  `HandleLanguageChanged` 改成正文与装饰**分开替换**；新增 `DisplayFontChanged` 事件
- [`Script/VNCharacterDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs)——加 `overrideNameplateColors` + 渐变上色 / 下色 / 描边色三字段，
  以及 [`GetNameplateColors()`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L53)（没勾自定义时由 `nameColor` 自动推算）
- [`VNDialogueBox.cs`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs)——加 `nameplateStyle` / `autoResizeNameTag` 字段与
  [`SetNameplateStyle()`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L334) / [`SetSpeakerStyle()`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L346) API；内部 `ApplyNameplateStyle()` /
  `ResizeNameTag()` / `EnsureUnderline()` / `FindPlateImage()`；订阅 `DisplayFontChanged`
- [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)——`Say()` 里在两条分支上分别 `SetSpeakerStyle(c.def)` / `SetSpeakerStyle(null)`
...
- [`Script/VNAiKey.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiKey.cs)——API Key 三级回退读取（环境变量 → 仓库外文件 → 仓库内文件）。
  key 只在内存缓存、任何日志都不打印；`#if UNITY_EDITOR` 挡住 Build 版本读取
- [`Script/VNAiClient.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs)——Gemini `generateContent` 的协程封装，**全项目唯一碰 HTTP 的文件**。
  失败分 8 类（NoKey/Network/Auth/RateLimited/Server/Blocked/Truncated/BadResponse），
  429/5xx/网络错误自带指数退避重试
- [`Script/VNAiPersonaDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs)——人格资产（Create → VN → AI Persona）
- [`Script/VNAiConversation.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs)——纯逻辑层（无 MonoBehaviour）：system prompt 组装、
  JSON Schema 生成、历史裁剪、响应解析与钳制
- [`Script/VNAiTalkModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs)——`event aitalk` 事件模块

**新增（编辑器）**

- [`Editor/VNAiConnectionTester.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiConnectionTester.cs)——两级自检菜单（Tools → VN Effects → AI）
- [`Editor/VNAiTalkInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiTalkInstaller.cs)——增量装机（Tools → VN Effects → 场景装机 Install To Scene → AI 自由聊天 AI Talk Module）
...
- [`Script/VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs)——加 `aiPersonas` 列表 + [`FindAiPersona()`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L233)
- [`Editor/VNGameConfigTools.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs)——Sync Game Config 扫描 `VNAiPersonaDef`
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)——`event` 提示文案补 aitalk 用法
- [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs)——四条新规则 + aitalk 结果名表
...
`event` 本来就是关键字，kwargs 走通用解析，`EventCo` 已把 `cmd.kwargs` 塞进 [`VNEventContext`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs)。
...
名牌配色、打字机、口型；三个候选回复直接走 [`VNChoicePanel`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs)，飞入 / 悬停扫光 /
...
解法是按 [`VNLocale.Language`](Assets/Project/Scripts/VNEffects/Script/VNLocale.cs) 切 system prompt 让 AI 直接用目标语言回复；
...
- **跨场景记忆**（性价比最高）：每场结束让 AI 生成一句摘要写进独立 JSON
  （仿 [`VNCgUnlocks`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs) / [`VNPhotoAlbum`](Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs) 的全局存储），下次注入 `VNAiContext.memory`（[`Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs:33`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L33)） →
  「她记得你上次说过喜欢猫」。`VNAiContext.memory` 字段已经预留好
- **流式输出**（SSE）：边收边打字机，体感更快。要自定义 `DownloadHandlerScript`，
  且与结构化输出同用会更麻烦，故一期用非流式 + 「正在输入…」动画
- 中途存档：需要改成非 event 的视图层（仿 [`VNSnsView`](Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs)）+ 走 vn-save-compat 三处同步
- 每轮进回想：`EventCo` 只在结束后记一条，要逐轮进 Backlog 需经 [`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 开口子
- 道具联动、AI 当裁判点评、AI 出题喂给 [`VNQuizModule`](Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs)
- TTS 配音
...
跨场没有（`VNAiContext.memory`（[`Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs:33`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L33)） 字段留着但没接存储）。

### 文件改动清单

**新增**

- [`Script/VNAiTalkLog.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkLog.cs)——一场对话的完整记录 + 落盘。纯类不继承 MonoBehaviour
...
- [`Script/VNAiTalkModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs)——接日志：首轮存 system prompt、每轮存请求与回复、
  玩家选完回填 `pickedIndex`、`FinishWith` 落盘。新增 `logMode` 字段。
  顺带把写 flag 抽成 [`SetFlag()`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs#L501)，写 flag 的同时记一行给日志，省得两处各写一遍
...
- 结果段的五个 flag 与 [`VNFlags`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs) 实际写入一致
...
- **跨场记忆**（文档里标为最值得做的一项）：聊完让 AI 生成一句摘要写进独立 JSON，
  下次注入 [`VNAiContext.memory`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs)。字段已留好，只差存储层
...
1. `VNAiPersonaDef.Validate()`（[`Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs:243`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs#L243)） 硬写了「optionTones 必须正好 3 条」，正是用户
   Console 里那条报错的来源
...
- [`VNChoicePanel.cs`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs)——新增 `ResolveMetrics()`：按条数等比压缩条高 / 间距 / 字号，
  总高上限 `MaxTotalHeight = 430f`
- [`Script/VNAiPersonaDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs)——`optionTones` 放开到 3~6 条；新增
  `useExtendedOptions` 勾选框与 `ResolveTones(overrideCount)`；
  `Validate()` 改为查区间 + 查空值 + **查重名**；`boundaries` 默认文案加固
- [`Script/VNAiConversation.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs)——构造函数收 `optionCount`；schema 的
  `minItems/maxItems`、提示词的条数与语气清单、解析时的补齐 / 截断、
  兜底轮的条数全部改为动态；`LanguageRule(optionCount)`
- [`Script/VNAiTalkModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs)——剧本 `options:N` 覆盖；倾向 flag 只写本场实际用到的档
- [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)、`HowToUse.md`、`AiTalkGuide.md`——补 `options:` 与新章节
...
[`ResolveTones()`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs#L201) 永远返回 `optionTones` 的**前 N 条**。这样一套人格资产就能同时
...
- 顺带修掉一处措辞 bug：[`LanguageRule()`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L362) 里写死的「台词与**三个**候选回复」，
  5 条时措辞不符，改为按实际条数生成
...
- [`Script/VNAiMemory.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiMemory.cs)——跨场记忆（存档态）：条目结构、容量裁剪、
  prompt 段组装、话题清单提取、存档快照存取
- [`Script/VNAiDiary.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiDiary.cs)——日记本（全局 JSON，仿 VNCgUnlocks），上限 200 条
- [`Script/VNAiDiaryPanel.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiDiaryPanel.cs)——日记本面板（D 键），程序化 UI、角色筛选、滚动
- [`Script/VNAiTextNormalize.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTextNormalize.cs)——繁体字兜底转简体

**修改**

- [`Script/VNAiConversation.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs)——[`BuildSummaryRequest()`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L620) / [`TryParseSummary()`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L715)；
  记忆与话题清单注入提示词；全部玩家可见文本过繁转简
- [`Script/VNAiPersonaDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs)——`enableMemory` / `memoryCapacity` / `writeDiary`
- [`Script/VNAiTalkModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs)——`SummarizeCo()` 收场总结，写记忆与日记
- [`Script/VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs)——`VNSaveData.aiMemories`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:72`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L72)）
- [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)——快照存取两处接线（存档三处同步）
- [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)——D 键 + [`RequestDiary()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1683)，按需创建面板
...
**3. 收场总结必须放在 [`Done()`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs#L79) 之前**
...
1. **架构层（主因）**：[`VNMoodGrading`](Assets/Project/Scripts/VNEffects/VNMoodGrading.cs) 用 URP 全局 Volume 做调色，而场景是
...
2. **参数层**：Sunset 把 `colorFilter(1.18,0.94,0.72)`、`temperature +30`、
   ...
   **Bloom 阈值 1.0** 泛白，蓝通道压到 0.65；配上本来就偏橙的黄昏背景图就是
   "橙上加橙"。这是过曝糊白的来源。
3. **叠加层**：[`VNToneMatch`](Assets/Project/Scripts/VNEffects/VNToneMatch.cs) 换背景时按背景平均色给立绘乘 9% 染色，
...
另外 [`PrepareHidden()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L114) 原本不重置亮度，sink 退场后复用立绘会残留 0.3 亮度
（既有 bug，本次一并修掉）。合并层因此不是可选项而是必需品。

### 文件改动

**新增**
- [`VNGrade.cs`](Assets/Project/Scripts/VNEffects/VNGrade.cs) —— 调色值类型 `VNGrade`（滤镜/色相/饱和/亮度/对比度）+
...
- `VNImageEffect.shader` —— Color Grading 区段加 `_ColorFilter`（RGB 乘）与
  ...
  钳掉，否则 uGUI 混合会出脏边。
- [`VNImageEffectController.cs`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) —— 加分层调色合并层：`SetGrade(layer, grade, dur)`
...
- [`VNSpeakerHighlight.cs`](Assets/Project/Scripts/VNEffects/VNSpeakerHighlight.cs) / [`VNFakeDoF.cs`](Assets/Project/Scripts/VNEffects/VNFakeDoF.cs) → `Focus` 通道
- [`VNCharacterEmotes.cs`](Assets/Project/Scripts/VNEffects/VNCharacterEmotes.cs) / [`VNEntranceAnimator.cs`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs) → `Emote` 通道
  （并在 `PrepareHidden()` 清 Emote + Focus，**不清 Mood** —— 情绪色调是全局的，
  角色一出场就该带着当前情绪的颜色）
- [`VNWeatherController.cs`](Assets/Project/Scripts/VNEffects/VNWeatherController.cs) → `Weather` 通道
- [`VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— `AutoWire()` 里 `RegisterBackground(backgroundFx)`（老场景自愈）；
...
- [`VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) —— 演示场景没有 VNStage，直接接线 mood 的分层目标。
...
- 演示场景 [`VNEffectsDemo`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs) 需重跑 Tools → VN Effects → 演示场景 Demo Scenes → 重建特效演示场景 Create Demo Scene
  才会带上新接线。
...
- [`Editor/VNCommandSearch.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs) —— 四个共用件：
...
**修改** [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs)
...
- 抽出 [`DisplayOptionsFor()`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs#L2736)（原本内联在 `DrawParamField` 里的中英对照 if 链），
  给参数格与命令面板共用。
- 底部提示条补两行新键位说明。
...
- **`PopupString` 的同步契约不能破**，这是本次最大的坑。它被三处用着，
  而其中两处的值**不在 `VNRow.values`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs:44`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs#L44)） 里**：camseq 路径点（值在 `camLines` 文本）、
  choice 选项行（值在 [`VNChoiceOptionRow`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) 字段）。若照 `SpritePopup` 那样让弹窗回调直写
...
- **右键要自己收**：`GUI.Button` 只响应左键。但 `ConsumeRightClick` 之后**按钮照画不误**——
  ...
  `ReorderableList` 的选中/拖动也只认左键，[`Use()`](Assets/Project/Scripts/VNEffects/Script/VNEquipment.cs#L113) 掉右键不会抢它的事件。
...
- **say 行的说话者写 `VNRow.speaker`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs:38`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs#L38)） 专用字段**，不进 `values`（编辑器铁律）。
  面板里对它用了一个合成的 [`VNParamDef`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs)（id = `say.speaker`），赋值时特判分流。
- **快捷键选 Ctrl+E 不选 Ctrl+K**：Ctrl+K 是 Unity Search 的默认绑定，
  虽然窗口作用域优先，但 Shortcuts 面板里会标冲突。
- **命令候选表从 `VNScenarioSchema.Commands`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:86`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs#L86)） 现场生成**，`VNScenarioSchema.cs` 一行没改——
...
- **搜索框清空失效**（向导每换一步都要清 query，结果清不掉）：
  ...
  修法：`VNSearchListView.Reset()`（[`Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:83`](Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs#L83)） 里先 `GUIUtility.keyboardControl = 0`
...
- 匹配是子串包含，打 `sw` 命不中 `show`（要模糊子序列匹配得换打分排序），
  也没有拼音与「最近使用置顶」。当前是刻意选择，要加的话只改 `VNSearchItem.Matches`（[`Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:19`](Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs#L19)）
...
| 文件 | 职责 |
|---|---|
| [`VNAiStudioWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioWindow.cs) | 主窗口：三栏布局、会话动作、上下文组装、域重载存活 |
| [`VNAiStudioDraft.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioDraft.cs) | 草稿层：临时 SO 副本、脏标记、字段级 diff、写回/还原 |
| [`VNAiStudioSession.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioSession.cs) | 会话驱动：发请求、解析、轮次记录、重跑/分岔/重建、收场总结 |
| [`VNAiStudioMemory.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioMemory.cs) | 记忆预设存储（项目根 `AiTalkStudio/Memories/*.json`）+ 两个导入器 |
| [`VNAiStudioLog.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioLog.cs) | 把试聊会话按游戏内同格式导出到 `AiTalkLogs/Editor/` |
| [`VNAiEditorCoroutine.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiEditorCoroutine.cs) | 编辑器协程泵（从 [`VNAiConnectionTester`](Assets/Project/Scripts/VNEffects/Editor/VNAiConnectionTester.cs) 提取，两处共用） |

**修改（运行时，3 个，全是纯增量）**

| 文件 | 改了什么 |
|---|---|
| [`VNAiConversation.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs) | 加 `History` 只读、`OpeningText`、`TruncateToTurn(int)` |
| [`VNAiTalkLog.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkLog.cs) | `Save(subFolder)` 支持子目录；`Begin` 加不带 [`VNEventContext`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs) 的重载 |
| [`VNAiTalkModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs) | `BuildAffectionText` 提成 `public static(statName, value)`，实例方法转调 |
...
三个好处：将来给 [`VNAiPersonaDef`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs) 加字段窗口自动就有（不会两边脱节）；
...
**② 记忆预设完全独立于 [`VNAiMemory`](Assets/Project/Scripts/VNEffects/Script/VNAiMemory.cs)**
...
**③ 从存档导入记忆刻意不走 `VNSaveSystem.Load()`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:218`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L218)）**

那个方法会 `VNFlags.Clear()`（[`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:36`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs#L36)） 再灌入存档里的全部 flag。在编辑器里点一下「导入」
...
`VNAiConversation._history`（[`Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs:54`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L54)） 是 private List 序列化不了，但轮次记录
...
- `VNAiStudioMemory.BuildContext`（[`Assets/Project/Scripts/VNEffects/Editor/VNAiStudioMemory.cs:136`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioMemory.cs#L136)） 与 `VNAiMemory.BuildContext`（[`Assets/Project/Scripts/VNEffects/Script/VNAiMemory.cs:75`](Assets/Project/Scripts/VNEffects/Script/VNAiMemory.cs#L75)） 是两份实现，
...
| 文件 | 职责 |
|---|---|
| [`Script/VNAiPricing.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiPricing.cs) | `VNAiPricingDef` 单价表资产（Create → VN → AI Pricing）+ `VNAiPricing` 查表静态类 |
| [`Editor/VNAiCostReport.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiCostReport.cs) | 花费累计报表窗口（Tools → VN Effects → AI → 花费报表 Cost Report） |

**修改**

| 文件 | 改了什么 |
|---|---|
| [`VNAiClient.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs) | `VNAiResult` 加 `model` 字段并在 `Send` 回填；`EstimatedCostUsd` 改走 `VNAiPricing` |
| [`VNAiTalkLog.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkLog.cs) | `Session` 加 `summary*` 六个字段；新增 `RecordSummary(res)`；开销段拆出「对话 N 轮 / 收场总结」两行，平均每轮改为不含总结；单价说明改为动态、并补一句解释思考 token 口径 |
| [`VNAiTalkModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs) | `SummarizeCo` 调 `_log.RecordSummary(res)` |
| [`VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) | 加 `aiPricing` 引用 |
| [`VNAiStudioSession.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioSession.cs) | 暴露 `LastSummaryResult` |
| [`VNAiStudioLog.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioLog.cs) | `Export` 加 `summaryRes` 参数 |
| [`VNAiStudioWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioWindow.cs) | 收场改为**总结回来再写日志**；抽出 `ExportLog()` |
| [`VNAiStudioMemory.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioMemory.cs) | 导入日志时把那次请求的开销打进 Console |
...
（flash-lite / flash / pro），**零配置也能用**——同 [`VNWeatherDef`](Assets/Project/Scripts/VNEffects/VNWeatherDef.cs) 的惯例。
...
**新增供应商抽象层 [`VNAiProvider.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiProvider.cs)**
...
| 层 | 在哪 |
|---|---|
| 全局默认 | `VNGameConfig.aiProvider`（[`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:222`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L222)） / `aiModel`（**默认已设为 DeepSeek**）|
| 人格资产 | `VNAiPersonaDef.provider`（[`Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs:112`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs#L112)） / `model`，默认「跟随全局」|
| 试聊台 | 工具栏两个格子，改的是**草稿**，用于 A/B 对比 |

**[`VNAiClient`](Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs) 拆三份，但「唯一碰 HTTP 的文件」这条不变**

传输、重试、错误分类仍全在 `VNAiClient.Send`（[`Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs:160`](Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs#L160)）；各家只差「拼请求体」和「解响应」，
拆进 [`VNAiClientGemini`](Assets/Project/Scripts/VNEffects/Script/VNAiClientGemini.cs) / [`VNAiClientDeepSeek`](Assets/Project/Scripts/VNEffects/Script/VNAiClientDeepSeek.cs) 两个**纯静态、不碰网络**的类。
加第三家就是再加一个文件 + `VNAiProviders` 里加一项。

**[`VNAiKey`](Assets/Project/Scripts/VNEffects/Script/VNAiKey.cs) 改成按供应商各一套**：`DEEPSEEK_API_KEY` / `DeepSeekAiApiKey.txt` 与
Gemini 那套并存，**分开缓存**——只配了一家时另一家要能正确报「没找到 key」，
不能因为缓存了一个空值就把两家都判死。

**[`VNAiPricing`](Assets/Project/Scripts/VNEffects/Script/VNAiPricing.cs) 支持三个价 + 一个时段倍率**：未命中输入 / 命中缓存输入 / 输出，
...
- `VNAiConversation.BuildJsonFormatPrompt()`（[`Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs:392`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L392)） 把 schema 翻译成一段「输出格式」——
...
[`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) 是已存在的资产，YAML 里没有新字段。Unity 反序列化时会先跑字段初始化器
再覆盖已有字段，所以 `public VNAiProvider aiProvider = VNAiProvider.DeepSeek;`
对存量资产也生效——不用手点 Inspector 就已经是 DeepSeek。

**3. 缓存缓存缓存：`VNGameConfig.ClearCache()`（[`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:69`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L69)） 里补上两个 Invalidate**
...
- **`VNAiClient.DefaultModel`（[`Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs:154`](Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs#L154)） 从 const 变成属性**（要按当前供应商算），
  于是 [`VNAiPersonaDef`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs) 上 `[Header("模型名（留空 = " + VNAiClient.DefaultModel + "）")]`
...
- **第 2 轮起「回复正文为空」——不是偶发，是必现**（上线后当天实测到，已修）：
  ...
  **只包 `{"reply":"…"}` 3/3 成功**，且与 `thinking` 开关无关。
  修法取最省 token 的那个（[`VNAiConversation.AppendHistory`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs)，每条历史多约 12 token），
...
名牌沿用经典款紫底圆角块（装饰样式仍由 `VNDialogueBox.nameplateStyle`（[`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:38`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L38)） 全局接管）；
...
那一层交给 Lint。这跟 [`Expression`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 依赖角色参数是同一个 `dependsOn` 模式。
...
**为什么没有技术冲突**：震动作用在 `SceneRoot`（`VNScreenShake.target`（[`Assets/Project/Scripts/VNEffects/VNScreenShake.cs:96`](Assets/Project/Scripts/VNEffects/VNScreenShake.cs#L96)）），
运镜作用在它的子级 `ZoomRoot`（`VNCamera.target`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:34`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L34)））——这两层本来就是设计成可叠加的
（`VNCamera.SnapZoom()`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:140`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L140)） 早就有个 `VNScreenShake shake` 形参，在到位瞬间轻震，先例就在那儿）。
所以「一边推镜一边震」是叠加而不是打架。

**三档 → 数值只有一张表**（`VNShakeSpec`，新增在 [`VNScreenShake.cs`](Assets/Project/Scripts/VNEffects/VNScreenShake.cs)）：
...
- **控件共用一份**：`VNCamShakeUi.Draw()`（[`Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs:390`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs#L390)）（放在 [`VNCamWaypoint.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs)，与解析层同源），
  剧本编辑器的路径点行和镜头编排窗口的第二行各调一次。
  下拉 `(不震)/light/medium/heavy/自定义…`，选自定义才在右边出文本框；
  框里的值非法时**染橙**——写错了要看得见，不能让下拉悄悄把它改成「不震」。
- **`VNCamWaypoint.TryParse`（[`Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs:105`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs#L105)） 仍然严格**：`shake:20`（少了秒数）、重复的 `shake:` 都直接
...
[`VNScriptRunner.CloneWithParams()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 里 camPoints 的深拷贝是**逐字段手写**的，
加了新字段不补进去，`call` 带参数调用的子程序里 camseq 的 shake 会静默丢失
（本次已补 `shake = point.shake`）。这一处每加一个路径点字段就要跟着改，
和 `VNCamWaypoint.TryParse/Format`、[`VNCamseqEditorWindow`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs) 的 [`Waypoint`](Assets/Project/Scripts/VNEffects/VNCamera.cs) 类是同一批。
...
| [`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) | `VNCamWaypointDef.shake`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:30`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L30)） 字段 + `ParseCamWaypoint` 认 `shake:` token（非法告警并忽略） |
| `Script/VNScriptRunner.cs` | `CamseqCo` 填 `Waypoint.shake`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:216`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L216)） 并把 `stage.screenShake` 传进 `PlayPathCo`；补 `CloneWithParams` 深拷贝 |
...
| [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) | 路径点行尾加「震」控件 + 语法提示 |
| `Editor/VNCamseqEditorWindow.cs` | `Waypoint.shake` + 行 UI + 文本生成/解析 + `AddHoldSegment` 算进震动时长 + 帮助文字 |
| [`Editor/VNScenarioDoc.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) | Lint 提示语法补上 `hold:`/`shake:`（校验本身复用 TryParse，无需新代码） |
| `Script/VNScriptParser.cs`（stay） | `VNCamWaypointDef.StayToken`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:34`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L34)）/`IsStay`；stay 行数字位前移、多余数字告警 |
...
材质实例 + mood 注册，还要改 `bg` 转场逻辑，而且它和 [`VNKenBurns`](Assets/Project/Scripts/VNEffects/VNKenBurns.cs) 抢同一个 RectTransform。
...
`VNScriptDemo` 场景的背景 Image 上**原本没有 [`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs)**，
所以 `VNStage.backgroundFx`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:57`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L57)） 一直是 null，`mood` 的分层调色也没注册到背景上。
本次由 [`VNBackgroundScroll`](Assets/Project/Scripts/VNEffects/VNBackgroundScroll.cs) 的 `[RequireComponent]` 在运行时补上了 controller，
...
| [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) | `bgScroll` 引用 + 自愈补挂 + [`SetBackgroundScroll()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1195) + 换图归零偏移 + 存档读写 |
| [`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) | `bgscroll` 关键字 |
| [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) | 命令派发（含 dir/mode 认不出时的告警）+ 调试重建重放（参数留空 = 沿用，重建也要照这个语义累积）|
| [`Script/VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) | `scrollOn/scrollSpeed/scrollDir/scrollMode` 四个字段 |
| [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) | `bgscroll` 命令登记（5 个参数，剧本编辑器/Ctrl+E 面板自动出现）|
| [`Editor/VNEffectsDemoSetup.cs`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) | 场景生成器给背景挂上滚动组件并回填 VNStage |
...
[`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) 是全项目内容的总配置，一个资产里塞了 30 多个字段、十几个列表。
...
1. **`[Header]` 被写在了列表元素内部的字段上。** [`VNStage.BackgroundEntry`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) / `CgEntry` /
   [`VNAudio.AudioEntry`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) / `VNGameConfig.UiSkinEntry` 都是这个写法：
...
**② 分页 Inspector（[`VNGameConfigEditor`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs)）** —— 按功能切成
...
**③ 素材浏览器窗口（[`VNAssetBrowserWindow`](Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs)，Tools → VN Effects → 素材浏览器 Asset Browser）** ——
...
| 文件 | 改动 |
|---|---|
| [`Editor/VNAssetUi.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs)（新） | 三边共用的绘制与预览层：Sprite 缩略图（同步 UV 画法）、音频试听（AudioUtil 反射 + 多版本候选）、波形、拖拽接收、搜索匹配、Rect 切割辅助 |
| [`Editor/VNConfigEntryDrawers.cs`](Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs)（新） | 背景 / CG / 音频 / UI 皮肤四个条目的紧凑单行 drawer，含试听按钮与拖入替换 |
...
统一收进 `VNEntryDrawerBase.Overlay()`（[`Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs:121`](Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs#L121)），**只在 `Event.current.type == EventType.Repaint` 时画**，
保证叠加层绝不参与事件处理。
...
新增 [`Editor/VNTextureImportDefaults.cs`](Assets/Project/Scripts/VNEffects/Editor/VNTextureImportDefaults.cs)（`AssetPostprocessor`）：
...
新增 [`Editor/VNAssetTheme.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs)：把窗口的配色、圆角、GUIStyle 收成一处，
...
- **圆角靠程序化贴图，零美术依赖**：生成一张 32×32 的**白色**圆角贴图，
  ...
  一张贴图搞定所有尺寸与配色（与 [`VNProceduralTextures`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) 一个路子）。
...
- **★ 换浅色底之后，必须显式覆盖每一个 GUIStyle 的文字颜色。**
  ...
  这就是 `VNAssetTheme.Tint()`（[`Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs:252`](Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs#L252)） 存在的理由。
...
| [`Editor/VNAssetBrowserWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs) | 顶栏 / 侧栏 / 网格卡片 / 音频行 / 详情栏 / 拖入提示全部接主题；顶栏加 🌸 主题切换 |
...
- 素材浏览器 / VNGameConfig Inspector 写 → **[`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) 资产**
- 剧本编辑器的 bg / cg / bgm 下拉读 → **场景里的 [`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) / [`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) 组件**
  （[`VNScenarioEditorWindow.RefreshSources()`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) 里 `FindFirstObjectByType<VNStage>()`
...
（与 `VNGameConfig.ApplyList`（[`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:258`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L258)） 的规则一致）。抽了两个小工具：
[`LoadGameConfig()`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs#L574)（编辑期直接走 `AssetDatabase`，不用 `VNGameConfig.Active` 的运行时缓存，
免得受 Play Mode 进出清缓存的时机影响）和 `PickLibrary<T>(fromConfig, fromScene)`。
覆盖 backgrounds / cgLibrary / bgm / se / voice 五个库。

**② 加「改完自动刷新」**：新增 [`Editor/VNAssetLibraryEvents.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs)，一个静态 `Changed` 事件。
素材浏览器与 VNGameConfig Inspector 的**所有写入点**统一走
`Apply()` / [`ApplyAndNotify()`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs#L194)——`SerializedObject.ApplyModifiedProperties()`
...
| [`Editor/VNAssetBrowserWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs) | 所有写入点收敛到 `Apply()`，改动时广播 |
| [`Editor/VNGameConfigEditor.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs) | 所有写入点收敛到 `ApplyAndNotify()` |
...
- 手动 [`RaiseChanged()`](Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs#L26) 广播一次，订阅方重建候选无异常。
...
`VNScriptParser.Keywords`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:93`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L93)） 和 [`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 的 case 都在，
但**编辑器里搜不到**：行首命令按钮的分类菜单、打字换命令、`Ctrl+E` 命令面板
都列不出它，只能自己在文本里手打命令名（还得记住大小写是 `hideHUD`）。

### 根因

编辑器的命令候选表**从 `VNScenarioSchema.Commands`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:86`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs#L86)） 现场生成**
（[`VNCommandSearch`](Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs) 里那句「加新命令不用回来登记」说的就是这条链路），
而当初加 `hideHUD` 时只改了 Parser + Runner，**漏了 Schema 这一步**
（vn-new-command 清单第 5 步）。

顺带说明为什么它以前还能正常显示：[`VNScenarioDoc`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) 判定「这行是不是命令」用的是
`VNScriptParser.CommandKeywords`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:110`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L110)），不是 Schema，所以已有的 `hideHUD` 行会被认成
`VNRowKind.Command` 正常保留，只是 `VNScenarioSchema.Find()`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:90`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs#L90)） 返回 null、
没有中文名也没有提示 —— 表现为「能存在但列不出来」。

### 做了什么

- [`VNScenarioSchema`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) 里 `ui` 之后补一条 `Add("hideHUD", "Scene", hint)`，**零参数**
  （同 `sakura` / `return` 那种无参命令），hint 写明「只能关 + 玩家按键恢复」。
- `VNScenarioEditorWindow.CommandTranslations`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:126`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs#L126)） 补 `{ "hideHUD", "隐藏界面" }`，
...
- **分类归 `Scene` 而不是 `FX`**：它和 `ui` / `portrait` 一样是界面开关，不是画面特效，
  放 `Scene` 与 `ui` 相邻。
- 大小写保持 `hideHUD` 原样：[`VNScriptParser.Keywords`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) 是**大小写敏感**的 `HashSet`，
...
| [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) | `CommandTranslations` 补中文名「隐藏界面」 |
...
[`VNQuickToolbar`](Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs) 的根物体挂着 `overrideSorting = true` 的**嵌套 Canvas**（为了压在对话框上一层），
而带 `overrideSorting` 的子 Canvas 会**打断父级 CanvasGroup 的 alpha 传播**——
[`VNDialogueBox`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs) 把自己的 `CanvasGroup.alpha` 归零，工具栏照画不误。
（属性栏其实一直藏得掉，截图里看到它是因为点过鼠标 UI 已经恢复了。）

### 做了什么

**① 修快捷条**：`VNQuickToolbar.SetVisible(bool)` 给自己的根加一个 CanvasGroup 单独关，
由 `VNDialogueBox.SetInterfaceVisible`（[`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:664`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L664)） 调用。**只在「隐藏界面」这条路上联动**，
`Show()` / [`HideBox()`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs#L615) 的淡入淡出不动它 —— 没台词时工具栏仍在是有意的（那时也能存档）。

**② 状态从 `bool _uiHidden` 改成按部件 + 锁定**：新增 [`VNUiParts`](Assets/Project/Scripts/VNEffects/Script/VNUiParts.cs)（Flags 枚举：
...
**④ 存档三处同步**（vn-save-compat）：`VNSaveData.uiHidden`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:64`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L64)）（字符串 `"dialogue,stats"`，
...
| [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) | `_uiHiddenParts` / `_uiHideLocked`、[`SetUiHidden()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1730)、`ApplyUiHidden()`、`ApplyGameplayHudVisible()`、[`ParseHideHudArgs()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L1768)、`RestoreUiHidden()`、Update 锁定分支、存档读写、调试重建 |
| [`Script/VNSaveSystem.cs`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) | `VNSaveData.uiHidden` |
| [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) | hideHUD 三个位置参数格（同一张候选表） |
...
`VNUiPartsUtil.Parse`（[`Assets/Project/Scripts/VNEffects/Script/VNUiParts.cs:41`](Assets/Project/Scripts/VNEffects/Script/VNUiParts.cs#L41)） 的中英/大小写/中文别名与 token 往返也单独跑过。）
...
视差系数 k **写死 0.5，不做逐点覆盖**（`VNCamera.DefaultDepthRatio`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:46`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L46)），组件上可全局调）。
手册 13 章：「同一部作品内部保持一致比数值本身正确更重要」。

### 关键坑一：立绘缩放倍率必须分通道（否则说一句话尺寸就跳回去）

[`VNImageEffectController._scaleMultiplier`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) 原本是**单一 float**，而
**[`VNSpeakerHighlight`](Assets/Project/Scripts/VNEffects/VNSpeakerHighlight.cs) 每句台词都在写它**（说话者 1.03 / 旁听者 0.97）。
镜头如果写同一个字段，症状是：**推完镜头，玩家点下一句台词，说话者高亮一写倍率，
立绘尺寸当场跳回去** —— 与当年调色被六方共用踩的是同一个坑（见 `SetGrade` / [`VNGradeLayer`](Assets/Project/Scripts/VNEffects/VNGrade.cs)）。
...
**顺带修掉一个既有 bug**：`VNCamera.DollyZoom()`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:173`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L173)） 原本直接写 `DOScaleMultiplier(1/zoom)`，
...
`camcut` / `camto` 语法里没有缩放模式概念，**公开的 [`Cut()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L244) / [`GoTo()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L266) 一律 `SetMode(Both)`**，
不继承上一段 camseq 的模式（不这么做的话，`camseq mode:char` 之后紧跟一条 `camcut`，
背景不会缩放而且立绘一直是放大的，极难联想到原因）。camseq 内部叠化段要带模式，
另走私有的 `CutWithMode()`。[`DollyZoom()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L173) 则登记成 `_mode = Bg`，下一条 both 运镜自动帮它还原。

### 存档：不用动

camseq 的镜头状态**根本不进存档**（[`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 里一个 cam 字段都没有），
调试重建也明写「动画路径状态不做推断，回到默认镜头」（[`VNScriptRunner.RebuildStateBefore`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)），
走的是 [`SnapReset()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L485)。立绘的运镜倍率跟 camseq 同生命周期，在 `SnapReset()` / [`ResetCamera()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L496)
里一并还原即可，**不用走 vn-save-compat 的三处同步**。持久的只有 `camcut` / `camto`。

### 参与的立绘：全部在场角色，自动维护

`VNCamera.dollyCharacters`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:37`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L37)） 从「手填的 Dolly Zoom 补偿列表」改成「参与运镜缩放的立绘」，
由 [`VNStage.RefreshRegistries()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 在角色进出场时调 `SetCharacterTargets()` 自动维护 ——
与 `VNMoodGrading.SetCharacterTargets`（[`Assets/Project/Scripts/VNEffects/VNMoodGrading.cs:190`](Assets/Project/Scripts/VNEffects/VNMoodGrading.cs#L190)） 同一时机、同一份 `characterFx` 列表。
...
- [`CamState`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs) 增加 `charScale` 字段，**`CharScale` 属性把 0 兜底成 1** ——
...
剧本编辑器那边**零改动**就跟上了：`mode` 加进 `VNCamseqText.HeaderKeys`（[`Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs:242`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs#L242)）（排最前，
...
| 文件 | 改了什么 |
|---|---|
| [`VNCamera.cs`](Assets/Project/Scripts/VNEffects/VNCamera.cs) | `VNCamZoomMode` 枚举、`CharacterScaleFor` / `ContainerZoomFor` 两个静态公式、`depthRatio`、`SetMode()`、`ApplyCharacterZoom()` / `ResetCharacterZoom()`、`SetCharacterTargets()`、`CutWithMode()`、`PlayPath` / `PlayPathCo` 加 mode 参数、`DollyZoom` 改走运镜通道 |
| `VNImageEffectController.cs` | `_camScaleMultiplier` 通道、[`DOCamScaleMultiplier()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L485)、[`ResetCamScaleMultiplier()`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs#L467)、`ApplyScaleMultipliers()` 合并 + 共用 `_scaleTween` |
...
| [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) | `CamZoomModes` 候选表 + camseq 的 `Kw("mode")` + 命令说明 |
| [`Editor/VNCamWaypoint.cs`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs) | `HeaderKeys` 加 `mode`（排最前）、`VNCamseqText.ModeOf()`（[`Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs:246`](Assets/Project/Scripts/VNEffects/Editor/VNCamWaypoint.cs#L246)） |
| `Editor/VNCamseqEditorWindow.cs` | `_zoomMode` 窗口状态（进域重载存活组）、工具栏下拉、`CamState.charScale`（[`Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs:2728`](Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs#L2728)）、`TargetState` / 插值 / 立绘绘制 / 占位框、生成与解析文本 |
...
**新增（运行时）**
- [`VNTouchZoneDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNTouchZoneDef.cs) —— 部位区域资产。归一化坐标（与 `markAnchor` 同语义）；
  基准一套 + 按立绘/表情覆盖（同 id 覆盖、新 id 追加、未提到的继承、`replaceAll` 完全不继承）；
  命中数学 `Contains` / `Pick` 是纯静态的，编辑器与运行时共用同一份。
- [`VNInteractionDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNInteractionDef.cs) —— 一场互动的全部规则：道具（含光标动画参数）、阶段、
  部位×道具反馈池、禁忌解禁阶段、结束条件、三种结果名、flag 前缀。
- [`VNTouchScore.cs`](Assets/Project/Scripts/VNEffects/Script/VNTouchScore.cs) —— 判定数学，纯逻辑无 MonoBehaviour，可单测。
- [`VNTouchCursor.cs`](Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs) —— 道具光标：跟随、待机摆动、按住震动、速度倾斜、悬停发光。
- [`VNCharacterOverlay.cs`](Assets/Project/Scripts/VNEffects/Script/VNCharacterOverlay.cs) —— 立绘情绪叠加层（潮红/汗/泪），多层共存、强度 0~1。
- [`VNInteractionModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs) —— 事件模块本体。

**新增（编辑器）**
- [`VNInteractionInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNInteractionInstaller.cs) —— 增量装机 + 缺资产时铺一套示例。
- [`VNTouchZoneEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNTouchZoneEditorWindow.cs) —— 在立绘上拖框画部位。

**修改**
- [`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) —— 新增 `RunInlineCo(lines)`；`overlay` 的 Dispatch 与静默重放。
- [`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) —— `ActiveCharacter.overlay`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:144`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L144)） 字段、挂载、[`SetOverlay()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L463)、快照存取。
- [`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) —— `overlays` 列表。
- [`VNSaveSystem`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) —— `CharSave.overlays`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:113`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L113)）。
- [`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) / [`VNScenarioSchema`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) / [`VNScenarioEditorWindow`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) / [`VNScenarioLinter`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs)
  —— `overlay` 命令的全链路登记。
- [`VNTextureImportDefaults`](Assets/Project/Scripts/VNEffects/Editor/VNTextureImportDefaults.cs) —— Sprite 自动导入白名单加 `Art/InteractionMiniGame/`。
...
**① 坐标占位符**（[`VNInteractionModule.ExpandPlaceholders`](Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs)）
...
- `VNLiquidSplash.Burst`（[`Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs:303`](Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs#L303)） 后粒子数 212，确认液体层在本场景可用。
...
**根因**：[`VNScriptRunner.Update()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 的第一行就是
...
Enter / 空格 / 左键（排除自己的 UI），转发到 [`RequestAdvance()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L2101)。
...
| 现成机制 | 借来的部分 |
|---|---|
| [`VNCharacterMarks`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMarks.cs) | 挂在立绘下、归一化坐标、共用立绘材质实例 |
| [`VNWetScreen`](Assets/Project/Scripts/VNEffects/VNWetScreen.cs) 水渍 | 对象池 + 数量上限 + 印上→保持→淡出的生命周期 |
| [`VNCharacterOverlay`](Assets/Project/Scripts/VNEffects/Script/VNCharacterOverlay.cs) | 「淡」由颜色而非纯 alpha 主导的观感 |

### 做法

- [`VNCharacterImprints`](Assets/Project/Scripts/VNEffects/Script/VNCharacterImprints.cs)：痕迹层组件。三段生命周期（印上时放大 1.18 倍再回弹 =
  拍上去的力道 → 缓慢褪色 **红→粉** → 淡出自毁），同屏上限 20 枚、超了淘汰最旧的。
  每枚叠一点随机旋转，否则连点出来一排一模一样的印子很假。
- `VNCharacterDef.imprints`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:190`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L190)）：id + 图 + 基准尺寸 + 着色 + 默认时长 + 随机旋转范围。
...
- 新占位符 `{nx}{ny}`（光标处的立绘归一化坐标）、`{znx}{zny}`（部位中心）、
  ...
  不是一回事，所以是两组占位符。
- `VNInteractionFeedback.trigger`（[`Assets/Project/Scripts/VNEffects/Script/VNInteractionDef.cs:55`](Assets/Project/Scripts/VNEffects/Script/VNInteractionDef.cs#L55)）（点击/拖动/都行）：掌印必须选「只在点击时」，
  否则拖一下会连着印出一串。

**「按道具分 + 按部位分」零新字段**：[`VNInteractionZoneRule`](Assets/Project/Scripts/VNEffects/Script/VNInteractionDef.cs) 本来就有 `zoneId` 和
...
### 例外：小改动的「直接 main」快速通道

不是所有改动都值得开分支走 PR。以下几类可以**不开分支、直接在 `main` 上改**：
纯文档（`.md`）／typo、注释、玩家可见文案与本地化词条微调／单文件几行的小 bug 修复与参数调值／
技能文件（`.claude/skills/`）与素材登记这类配置性改动。

三条约束：

1. **走哪条路由用户拍板**——AI 不自己判断大小，**每个任务动手前先问一句**
   「走 `feature` 分支 + PR，还是当小改动直接改 main？」（用户已在需求里说明的不用再问）。
2. **快速通道仍是两段式**：改完报告 → 用户说「推」→ 才 `git add` + commit + `git push origin main`。
   push 到 main 不可撤回，绝不自作主张推；也不许 `--force` 或 rebase 已推送的历史。
3. **边界模糊就退回分支**：改着改着发现要动第三个文件、碰存档格式或架构，立刻停下来问用户
   要不要改走分支路线，别闷头在 main 上做大改。功能 PR 里的第 ③ 阶段文档不走这条。
...
- **小改动**：旧约定是「每个改动都开分支」→ 现在纯文档/typo/单文件小修/技能与素材登记可直推 main，
  但走哪条路每次都先问用户，且推之前仍要用户点头
...
- `.claude/skills/vn-new-feature/SKILL.md`：整篇重写——第 0 步「先问走哪条路」+ 路线 A 四阶段清单
  + 路线 B 快速通道，补 `gh pr create` 的用法与坑
- `.claude/skills/vn-doc-update/SKILL.md`：「何时用我」改为「只在用户明确说更新文档时」，
  新增「写在哪、怎么提交」小节，并区分 A 挂在功能 PR 上的文档（留在功能分支）与
  B 独立的纯文档改动（走快速通道直接改 main）
...
2. 做完发现 `enter = Transition`（复用全屏转场）时，**画面会先变成一片黑再出现过场图**。
   查下来这不是 bug 而是 `VNScreenTransition.Play()`（[`Assets/Project/Scripts/VNEffects/VNScreenTransition.cs:285`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs#L285)） 的设计本身，于是顺带把「不经过纯色的
...
| 文件 | 一句话 |
|---|---|
| [`Script/VNInterludeDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNInterludeDef.cs) | 过场资产：三语标题/副标题、语音池、专属图池、loading 时长、进出方式、暗幕与配色；[`PickVoice()`](Assets/Project/Scripts/VNEffects/Script/VNInterludeDef.cs#L128) / [`PickImage()`](Assets/Project/Scripts/VNEffects/Script/VNInterludeDef.cs#L134) 负责随机取，图池为空时退到全局池 |
| [`Script/VNInterludeScreen.cs`](Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs) | 过场层：cover 铺图 + 暗幕 + 标题（装饰字体）+ 右下 spinner；自己持一份贴图模式材质做图案进出 |
...
| [`VNScreenTransition.cs`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs) | 新增 [`CreatePatternMaterial()`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs#L143) / [`ConfigurePattern()`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs#L160) / [`SupportsPatternBackground()`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs#L187) / [`PlayBackgroundPattern()`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs#L197) |
| [`VNProceduralTextures.cs`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) | 加 `LoadingRing` / `LoadingRingSprite`（锐边细环，spinner 用） |
| [`Script/VNStage.cs`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) | `interlude` 字段 + AutoWire 自愈 + [`ClearStage()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L983) 收起；`SwapStageImage()` 加 `viaBlack` 与图案直接过渡分支；`SetBackground` / `ShowCg` / `HideCg` 透传 |
| [`Script/VNScriptParser.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) | 关键字 `interlude` |
| [`Script/VNScriptRunner.cs`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) | `interlude` 的 Dispatch case + [`FindInterlude()`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L966)；`bg` / `cg` 透传 `via:` |
| [`Script/VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) | 「过场库」`interludes` + 「全局转场图池」`interludeImages` |
| [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) | 新参数来源 `InterludeId`；登记 `interlude`；`bg` / `cg` 加 `via` |
| [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) | 中文名「过场」+ 过场 id 下拉候选 |
| [`Editor/VNScenarioDoc.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) | `InterludeId` 的编辑期校验 |
| [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) | `unknown-interlude` / `interlude-no-id` 两条检查 |
| [`Editor/VNGameConfigEditor.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs) | 两个新字段登记进「舞台」页 |
| [`Editor/VNGameConfigTools.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs) | 目录扫描自动登记 `VNInterludeDef` |
...
[`PlayBackground()`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs#L242) 专用几何 shader（这四种做得更漂亮，保留）；其余图案 →
...
**不进存档。** 过场播完什么都不留，所以既不进 [`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs)，也不需要在
`RebuildStateBefore` 里静默重放。但 `ClearStage()` 里必须 [`HideImmediate()`](Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs#L136)——
...
## 一三一、擦雾小游戏 `event wipefog`：把雾擦开看清 CG（2026-09-01，分支 `feature/fog-wipe-minigame`）

### 需求

整屏起雾盖住 CG，玩家用鼠标（光标变成雨刷）把雾擦开看清画面。
动手前做了三轮问答定案，十一项决策里有四项决定了整个架构：

| 决策 | 结论 | 影响 |
|---|---|---|
| 玩法形态 | 清晰度达标 + **回雾对抗** | 结束条件 = 达标 or 时限到 |
| 角色是否实时反应 | 在场，但只是**被动 CG** | **不接部位框/表情/立绘**，模块完全不碰舞台 |
| 投入规模 | 正式可复用模块 | 做资产 + DSL + 装机器 + Lint |
| 手感取向 | **轻松，擦到就掉，无速度惩罚** | 不做速度反比曲线 |

### ★ 一个写在最前面的设计风险

「回雾对抗」+「擦到就掉」+「面积就是挑战」这三条放一起，难度的**唯一**来源就退化成
「鼠标划过的面积 vs 回雾速度」，也就是纯手速体力活。后果只有两种，取决于参数：

- 回雾调慢 → 玩家疯狂甩鼠标几秒通关，对抗形同虚设
- 回雾调快 → 变成必须一直高速甩鼠标的累人活，且依然没有技巧、只有体力

这不是设计错误，是取向选择（要的就是操作上轻松）。但它把全部成败压到了参数上，
所以下面那张数学表必须算准，且**编辑器调参窗口是刚性需求而不是加分项**。
...
1920×1080 = 2,073,600 px²，重叠浪费按 1.5 倍估（玩家画不出完美不重叠的覆盖路径）：

| 笔刷直径 | 拖速 | 每秒覆盖 | 占全屏 | 含浪费实际擦满 |
|---|---|---|---|---|
| 120 px | 800 px/s | 96,000 | 4.6 % | 32~38 s |
| **180 px** | **800 px/s** | **144,000** | **6.9 %** | **20~25 s** |
| 240 px | 1200 px/s | 288,000 | 13.9 % | 10~13 s |

再减回雾速率 R 得净推进：

| R | 净推进 | 擦到「完美」90% | 60 秒时限下的体感 |
|---|---|---|---|
| 2.0 %/s | +2.6 %/s | 35 s | 毫无对抗 |
| **3.0 %/s** | **+1.6 %/s** | **55 s** | **紧张但拿得到** ← 出厂默认 |
| 3.5 %/s | +1.1 %/s | 82 s | 时限内根本过不了 |

这个估算是**保守**的：边缘侵蚀只能吃掉已经擦净的像素，玩家没擦到的地方没得吃，
所以实际回雾比设定值少、真实进度更快些。用它定起点，最终手感一定要试玩微调。

### 文件改动

**新增 —— 运行时（`Assets/Project/Scripts/VNEffects/Script/`）**

| 文件 | 一句话 |
|---|---|
| [`VNFogMask.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs) | 掩码层：擦除 Stamp / 线段补点 / 边缘权重图 / 回雾衰减 / 清晰度求和。**纯逻辑无 MonoBehaviour**，编辑器调参窗口跑的是同一份代码 |
| [`VNFogScore.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs) | 阶段推进（只升不降）+ 三档判定（按峰值）。**纯静态可单测**，沿用 [`VNTouchScore`](Assets/Project/Scripts/VNEffects/Script/VNTouchScore.cs) / [`VNPhotoScore`](Assets/Project/Scripts/VNEffects/Script/VNPhotoScore.cs) 的分层 |
| [`VNFogWipeDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogWipeDef.cs) | 定义资产：雾外观九参 / 笔刷 / 回雾五参 / 规则 / 分阶段台词（三语 + 语音 + 属性奖励）/ 音效覆盖 |
| [`VNFogWipeModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs) | 事件模块：自绘底图与雾层、输入轮询、HUD、台词条、倒计时、ESC 确认、结算、flag 回写 |
| [`VNFogSfx.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogSfx.cs) | 四个代码合成音效；擦拭循环音的音量与音调跟鼠标速度联动 |
| [`VNFogTextures.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogTextures.cs) | 四种道具光标的程序化 SDF 贴图（雨刷/手掌/抹布/手指），RGB 存明暗、A 存形状 |

**新增 —— 编辑器（`Assets/Project/Scripts/VNEffects/Editor/`）**

| 文件 | 一句话 |
|---|---|
| [`VNFogWipeInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNFogWipeInstaller.cs) | 场景装机（Tools → VN Effects → 场景装机 → **擦雾 Fog Wipe**）：补禁用的 FogWipeTemplate（必须带 RectTransform）+ 造示例资产 + 登记进 VNGameConfig，可重复执行 |
| [`VNFogTuneWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNFogTuneWindow.cs) | 调参窗口（Tools → VN Effects → 预览 → **擦雾调参 Fog Wipe Tuning**）：上半**算出**预计通关秒数并给手感评语，下半是可拖鼠标试擦的掩码预览，回雾照参数实时跑 |

**新增 —— shader 与内容**

`Art/Shaders/VNFogWipe.shader`、`Assets/Scenarios/FogWipeDemo.vn.txt`、
`Assets/VNEffects/FogWipes/浴室镜面.asset`

**修改**

| 文件 | 改了什么 |
|---|---|
| [`Script/VNGameConfig.cs`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) | 「擦雾库」`fogWipes` |
| [`Editor/VNGameConfigEditor.cs`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs) | `fogWipes` 登记进「玩法」页 |
| [`Editor/VNScenarioSchema.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) | `wipefog` 的 EventVariants 一行（9 个参数）+ event 帮助文本 |
| [`Editor/VNScenarioLinter.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) | 结果名表 + `fogWipeIds` 扫描 + 三条检查 |
| `Resources/VNLocale/ui.zh/en/ja.txt` | 7 个 key（`fogwipe.*`） |

### 技术决策与取舍

**掩码只有 384×216，不是全屏。** 雾本身就是模糊的，掩码不需要高分辨率——双线性放大
5 倍看不出来，换来的是每帧全图遍历只有 8 万次运算、上传只有 81KB、清晰度统计顺手在
同一次遍历里求和。全屏 1920×1080 的话这三项都要差 25 倍。
一开始定的是 256×144，实测擦痕边缘能看出方块阶梯（一个掩码像素 = 7.5 屏幕像素，
刚好落在肉眼分辨得出的尺度上），384 压到 5 像素就看不出来了，再往上加是浪费。
**改分辨率务必保持 16:9**，否则 `384/1920 == 216/1080` 这个等式一破，笔刷会被拉成椭圆。

**内部是 `float[]` 而不是直接操作 `byte[]`。** 回雾是每帧减一个极小的量
（3%/秒 ÷ 60fps ≈ 0.0005），byte 精度下会被整数截断完全吃掉——症状是「雾根本不回来」。
多一份 147KB 内存换数值正确。

**★ Stamp：一笔之内取 max，笔与笔之间才累加。** 这一条同时解决两个问题：

- 不能直接 `mask += a*strength`：沿线段补点时同一个像素会被七八个 stamp 连着盖到，
  羽化带一路被累加填满、边缘退化成硬边，放大后就是方块阶梯，
  shader 的边缘噪声也失去了发挥空间（没有过渡带可打碎）。
- 也不能单纯取 max：那样 `wipeStrength` 就成了永久天花板，
  「反复擦同一处会越来越透」再也不会发生。

正解是用 `_strokeMark[]` / `_strokeBase[]` 记住「这一笔碰到该像素之前的值」，
本笔上限 = 那个值 + `a*strength`。于是 `wipeStrength` 的语义变成诚实的
「一笔能推进多少」：填 1 就是一次划过全清（默认，对应「轻松」取向），
填 0.5 就是要来回擦两遍才透。

**边缘侵蚀的速率语义。** 资产里填的「%/秒」是**整屏已擦净时**的速率。
实现上预算一张「到边缘距离」权重图（只算一次，之后每帧一次乘法），
再按权重图的平均值反算系数，保证填多少就掉多少。但侵蚀只能吃掉已经有值的像素，
玩家只擦了中间一条时边缘没得吃，实测掉的会明显少于设定值——这正是「边缘侵蚀」
该有的行为，调参窗口的预估因此偏保守。

**雾 = 底图的模糊提亮版，不是一层纯白半透明。** 剪影能透出来，玩家一眼看得见
「那边有个人影」，自然知道往哪擦，顺带让画面比纯白廉价感好一个档次。
shader 里做 9-tap 模糊（抄 `VNImageEffect` 的实现），噪声用内置 `fbm` 现算免噪声贴图。

**★ 边界必须用噪声扰动。** 掩码直接当 alpha 用会得到光滑的圆形边界，非常假。
噪声把边界打碎成水汽的形状，这一条对真实感的贡献超过任何其他单项
（`edgeNoise` 调到 0 就退回假的橡皮擦柔边）。

**层级：事件层 60 在对话框 40 之上，雾必然盖住对话框。** 与其让底下露出半截对话框，
不如**模块自己铺一份清晰 CG 打底**，画面完全自洽；过程中的台词也因此改用自绘的台词条
（贴着画面下缘，像镜面上的旁白）。同样因为角色是被动 CG，本模块**不像
[`VNInteractionModule`](Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs) / [`VNAiTalkModule`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs) 那样需要破模块三铁律**，全部绘制都在自己的
UI 子树内，也因此不进存档、无需走 vn-save-compat。

**★ 剧本不能在 event 之前先 `cg`。** 雾要到 `OnLaunch` 才铺得出来，剧本先 `cg` 的话，
从 cg 的转场开始到事件真正启动为止，清晰画面一直摆在玩家眼前——谜底在开始擦之前
就已经揭晓，整个玩法的前提就没了（中间再夹一句台词，还得等玩家点一下，暴露得更久）。
正确写法是用 `cg:` 参数交给模块自己铺底图，进事件的第一帧就是盖满雾的状态；
要让画面在事件后留下继续演，就在「* 结果行」跳到的分支里再写 `cg`
（Runner 的 `Destroy` 是帧末延迟销毁，与下一条命令同一帧交接，不会闪）。
Lint 的 `wipefog-cg-before-event` 盯着这件事，且只在「先 cg 的那张 == 要擦的那张」
时才报——先 cg 出别的图再擦另一张是正常写法。

**结算用历史峰值，不用结束瞬间的值。** 时限到的那一帧刚好被雾吞掉一点就把玩家从
「完美」打到「普通」，纯属运气而非操作。为了不让玩家困惑，HUD 的进度条上同时画出
**峰值刻度线**——显示什么、结算什么，两者必须对得上。进度条上还有一道
「普通档门槛」的白线，玩家才知道擦到哪算够。

**阶段只升不降。** 清晰度在回雾对抗下会反复穿越阈值，允许回退的话玩家一停手就在
阈值边界反复横跳，同一句台词播个没完（`VNTouchScore` 的同款教训）。

**CG 解锁要在模块里补一次。** `VNCgUnlocks.Unlock`（[`Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs:48`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs#L48)） 本来是 `VNStage.ShowCg`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1234`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1234)） 顺手做的，
推荐写法下剧本不再提前 `cg`，如果结果分支也没写（比如失败分支），这张 CG 就永远不会
解锁进画廊——玩家明明看过却是灰的。现在模块在结算时补一次，且只在擦到 20% 以上时算
（一进来就 ESC 放弃不该算看过）。

**擦拭循环音是唯一的速度反馈。** 本作刻意不做速度惩罚，于是「擦得快不快」在画面上
是没有反馈的。循环音的音量与音调跟着鼠标速度走，少了它擦拭会变成一件安静而无感的
体力活。用一段 1 秒的滤波白噪改 pitch，而不是切多段素材——切段会在切换点爆音。

**光标不用 `Cursor.SetCursor`。** 沿用 [`VNTouchCursor`](Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs)（硬件光标做不了摆动/按住震动/
速度倾斜/悬停发光）。代价是 `Dispose()` 必须在 **Finish / CancelForDebug / OnDestroy /
OnDisable 四处**都调，漏一条玩家的系统鼠标指针就永久消失。

### 修复记录（都是实机截图与冒烟测试抓出来的）

| # | 症状 | 根因与修法 |
|---|---|---|
| 1 | 擦痕边缘有肉眼可见的方块阶梯 | `Stamp` 沿线段累加把羽化带填满 → 改 per-stroke 缓冲；掩码 256×144 → 384×216 |
| 2 | `wipeStrength` 名不副实：一次划过中心就到 1.0 | 同一个 per-stroke 缓冲一并修正语义；默认值改 1.0 对应「轻松」取向 |
| 3 | 不擦也能看清七八成，玩法动机被削弱 | 截图实测后把 `fogMix` 0.55 → 0.76、`blurAmount` 0.006 → 0.013 |
| 4 | 按典型手速永远拿不到完美档（估算 66s > 时限 60s） | 笔刷 170 → 180px，落到完美≈55s |
| 5 | 关一次调参窗口泄漏一张贴图 | `VNFogMask.Destroy()`（[`Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs:111`](Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs#L111)） 用了 `Object.Destroy`，编辑模式下只打一条 error 什么都不销毁 → 按 `Application.isPlaying` 分路 |
| 6 | 进游戏那一瞬间 CG 完全没有雾，谜底提前揭晓 | 示例剧本在 event 之前写了 `cg` → 改用 `cg:` 参数 + 加 Lint 检查（见上） |

### 剧本语法

```
: （什么都看不清……擦一擦吧。）
event wipefog id:浴室镜面 cg:CG_test time:60 target:65 perfect:90 stat:感性 rate:0.08 flag:擦雾1
* 完美 -> 完美
* 普通 -> 普通
* 失败 -> 失败

label 完美
cg CG_test          ← 事件结束后再摆上舞台，画面才留得住继续演
```

参数：`id:` 定义资产 / `cg:` 要擦的图（★ 别在 event 前先写 `cg`）/ `time:` 时限 /
`target:` 普通门槛% / `perfect:` 完美门槛% / `vs:` 台词条显示名 /
`stat:` `rate:` 属性换算 / `flag:` 成绩前缀 / `tutorial:` 基类通用。

回写 flag：`<前缀>_清晰度`（0~100）、`<前缀>_用时`（秒）、
`<前缀>_档位`（**整数** 2 完美 / 1 普通 / 0 失败——[`VNFlags`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs) 是整型字典，同 [`VNPhotoBoothModule`](Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs)）。

### Lint 新增三条

| 代号 | 抓什么 |
|---|---|
| `unknown-fogwipe` | `id:` 拼错 → 事件直接返回空结果，整段被静默跳过 |
| `wipefog-no-failure-branch` | 没接住「* 失败」→ 玩家没擦到门槛时剧情静默继续 |
| `wipefog-cg-before-event` | event 之前先 `cg` 出同一张图 → 谜底提前揭晓 |

### 验证方法

- **装机**：Tools → VN Effects → 场景装机 Install To Scene → 擦雾 Fog Wipe
- **试玩**：Scenario Editor 打开 `FogWipeDemo.vn.txt` → 选中首行 → ▶ 从选中行播放
- **调参**：Tools → VN Effects → 预览 Preview → 擦雾调参 Fog Wipe Tuning
  （改一个数立刻看出手感差别，不用进 Play Mode）
- 冒烟测试覆盖：擦除↑ / 侵蚀↓ / 雾团↓、掩码格式 R8 384×216、阶段只升不降、
...
一律无效**——球照飞、倒计时照跑。[`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 里那个 `Time.timeScale = 0` 只对
...
| 文件 | 一句话 |
|---|---|
| [`Script/VNPause.cs`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs) | 句柄式全局暂停 `VNPause` + 受它影响的时间源 `VNTime.Delta`（[`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:144`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs#L144)） / `VNTime.Time`（[`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:148`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs#L148)） |
| [`Script/VNTutorialDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNTutorialDef.cs) | 教程资产：步骤列表（锚点/兜底矩形、洞形状、羽化、圆角、三语标题正文、配图、卡片位置、音效）+ 暗幕浓度、HDR 描边色、`once` / `allowSkip` |
| [`Script/VNTutorialSeen.cs`](Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs) | 「看过了」的全局 JSON（`vn_tutorial_seen.json`）+ 「显示教程提示」总开关（PlayerPrefs） |
| [`Script/VNTutorialAnchors.cs`](Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs) | 锚点注册表 `id → RectTransform` + 给 prefab 控件用的 `VNTutorialAnchor` 组件 |
| [`Script/VNTutorialMask.cs`](Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs) | 暗幕挖洞组件：世界四角换算、每帧跟随、描边呼吸 |
| [`Script/VNTutorialPlayer.cs`](Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs) | 播放器：覆盖层、卡片（皮肤优先/程序化兜底）、步进、暂停与光标接管 |
| [`Script/VNTutorialSkin.cs`](Assets/Project/Scripts/VNEffects/Script/VNTutorialSkin.cs) | 卡片皮肤槽位（只 `panelRoot` + `bodyText` 必需） |
| `Art/Shaders/VNTutorialMask.shader` | 圆角矩形/椭圆挖洞 + 羽化 + HDR 描边，一次最多 4 个洞 |
| [`Editor/VNTutorialSamples.cs`](Assets/Project/Scripts/VNEffects/Editor/VNTutorialSamples.cs) | 菜单：导出羽毛球示例教程并自动登记 |

**修改**

| 文件 | 改了什么 |
|---|---|
| [`VNBadmintonModule`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs) / [`VNQteModule`](Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs) / [`VNQuizModule`](Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs) / [`VNPhotoBoothModule`](Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs) / [`VNInteractionModule`](Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs) / [`VNAiTalkModule`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs) / [`VNBattleModule`](Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs) / [`VNShopModule`](Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs) / [`VNResultPopupModule`](Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs) / [`VNSnsView`](Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs) | [`Update()`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs#L59) 首行加 `if (VNPause.IsPaused) return;`（**一律在 ReadInput 之前**），`Time.unscaledDeltaTime` → `VNTime.Delta`、`Time.unscaledTime` → `VNTime.Time` |
| [`VNEventModule`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs) | 加 `tutorialId` 字段；`Launch` 在 `OnLaunch` **之后**调 `VNTutorialPlayer.PlayAuto`（[`Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:136`](Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs#L136)） |
| [`VNBadmintonCourt`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs) / `VNBadmintonModule` | 暴露记分板/操作提示/球网；登记 6 个教程锚点并在销毁时反注册 |
| [`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs) / `VNScriptRunner` | `tutorial` 关键字与分派；`FindTutorial`；`Update` 首行加 `VNPause.IsPaused` 屏蔽全局快捷键；`CleanupActiveEvent` 收教程并 `ReleaseAll` 兜底 |
| [`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) / [`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) / [`VNSystemUiSkinSet`](Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs) | 教程库、`tutorial` 字段与自愈创建、`tutorialPrefab` 槽位 |
| [`VNConfigPanel`](Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs) / [`VNConfigPanelSkin`](Assets/Project/Scripts/VNEffects/Script/VNConfigPanelSkin.cs) / [`VNSystemUiSkinExporter`](Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs) | 「显示教程提示」开关 + 「重置教程记录」按钮；面板加两行；新增**只重导设置面板**的菜单 |
| `Editor/VNScenarioSchema` / [`VNScenarioDoc`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) / [`VNScenarioEditorWindow`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) / [`VNScenarioLinter`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs) | `tutorial` 命令登记、`TutorialId` 参数源与下拉候选、`tutorial-no-id` / `unknown-tutorial` 两条检查 |
| `Editor/VNGameConfigEditor` / [`VNGameConfigTools`](Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs) | 教程库落在「舞台」页；目录扫描自动登记 |
...
裸的 Push/Pop 计数漏掉任何一条，症状是**游戏永久卡死**——比 [`VNTouchCursor`](Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs) 漏
`Dispose()` 导致光标消失还严重。所以句柄绑一个宿主 `Object`，宿主没了句柄自动失效，
`IsPaused` 每次读都会顺手清掉这类死句柄；`VNScriptRunner.CleanupActiveEvent` 再加一层
[`ReleaseAll()`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs#L99) 兜底。
...
[`VNInterludeScreen`](Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs)：Screen Space - Overlay 永远压在 Screen Space - Camera 之上，
...
小游戏的 UI 全是代码程序化生成的（`VNBadmintonCourt` / [`VNPhotoBoothUi`](Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs) /
...
- `VNTutorialMask.shader` 只能在 Unity 里验证编译；万一报错表现是暗幕退化成整屏压暗
  （写了兜底，不会白屏）
- 出正式包前给场景里 `VNTutorialPlayer.maskMaterial`（[`Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:55`](Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs#L55)） 指一份材质资产：
  只被 `Shader.Find` 引用的 shader 会被打包剥掉（同 [`VNScreenShockwave.sourceMaterial`](Assets/Project/Scripts/VNEffects/VNScreenShockwave.cs) 的老问题）
...
根因：[`VNScenarioSchema`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) 里 `event` 只登记了一个位置参数 `id`。因为 `event` 是通用入口，
...
| [`Editor/VNScenarioDoc.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs) | 解析/生成/校验三处都按模块 id 取变体；换模块时保住上一个模块写过的参数 |
| [`Editor/VNScenarioEditorWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) | `event` 行两行布局；`AssetId` 的下拉候选与缓存；`RefreshSources` 清缓存 |
...
- **必须缓存**：`OptionsFor` 每帧都被 OnGUI 调，直接 `AssetDatabase.FindAssets`
  ...
  [`VNAssetLibraryEvents`](Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs) 触发）。
...
`id:` 指的那份 [`VNBadmintonDef`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonDef.cs)；`vs:` 在 `ResolveOpponentBody` 里只是「资产没配立绘时
...
`Ctrl+E` 命令面板（[`VNCommandSearch`](Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs)）仍用基础定义，插入 `event` 时只会问模块 id
