...
- 层在**角色资产** `VNCharacterDef.overlays`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:160`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L160)） 里登记（层 id + 透明图 + maxAlpha）。
...
- 痕迹在**角色资产** `VNCharacterDef.imprints`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:190`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L190)） 里登记：id + 图 + 基准尺寸 +
...
- `depth` 的视差系数固定 0.5（组件 `VNCamera.depthRatio`（[`Assets/Project/Scripts/VNEffects/VNCamera.cs:40`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L40)） 可全局调）。
  数值本身不重要，**全篇一致才重要**
...
> ⚠️ **老场景要先装一次**：`liquid` 命令依赖场景里的 [`VNWetScreen`](Assets/Project/Scripts/VNEffects/VNWetScreen.cs) + [`VNLiquidSplash`](Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs)
...
- **三通道独立音频库**：声音在 **VNGameConfig** 的 **Bgm/Se/Voice Library**
  ...
  （填在场景 [`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) 上也能跑，但重建场景就没了——见第八章）
...
| 参数 | 说明 |
|---|---|
| 第 1 个参数 | 过场 id（[`VNInterludeDef`](Assets/Project/Scripts/VNEffects/Script/VNInterludeDef.cs) 资产的 id，须登记进 VNGameConfig 的「过场库」） |
...
2. 打开 [`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) →「舞台」页 →「过场库」拖进去；顺便往「全局转场图池」拖几张图
3. 剧本里写 `interlude <id>`
...
建完登记进 [`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) 的「教程库」）：
...
| `id:` | 互动定义资产 [`VNInteractionDef`](Assets/Project/Scripts/VNEffects/Script/VNInteractionDef.cs) 的 id |
...
**两份资产**
- [`VNTouchZoneDef`](Assets/Project/Scripts/VNEffects/Script/VNTouchZoneDef.cs)（部位在哪）：一个角色一份。用
...
| 参数 | 说明 |
|---|---|
| `vs:` | 对手角色 id（`VNCharacterDef.id`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:25`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L25)）），用于名字与立绘回退 |
...
| 产物 | 存哪 | 效果 |
|---|---|---|
| **记忆**（摘要 + 话题清单 + 关键事实） | 存档（`VNSaveData.aiMemories`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:72`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L72)）） | 下次见面她记得上次聊过什么，并**主动避开已聊过的话题**；读旧档会一起回退 |
...
> 场景组件（`VNStage.backgrounds`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:36`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L36)）、`VNAudio.bgmLibrary`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:37`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L37)） 等）依然存在且可用，
...
打开 [`VNGameConfig.asset`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs) 后，顶部是页签，**一次只显示一组，不用再滚到底**：
...
生成器会**扫描该目录自动登记**，不需要手动拖进 `VNStage.characters`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:21`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L21)）。
...
> `bg1`、`bg2`… 塞进 [`VNStage.backgrounds`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)（demo 行为，重建就重排，别依赖它）。
...
- **新开素材目录**要走这套的话，去 `VNTextureImportDefaults.Roots`（[`Assets/Project/Scripts/VNEffects/Editor/VNTextureImportDefaults.cs:37`](Assets/Project/Scripts/VNEffects/Editor/VNTextureImportDefaults.cs#L37)） 补一行。
...
> 它**复用运行时的 [`VNScriptParser`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)**，不是另写一套分词——
