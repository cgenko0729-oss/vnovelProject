...
3. **event 是唯一玩法插槽**。任何"暂停剧本→玩家交互→带结果返回"的玩法
   都实现 [`VNEventModule`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs)，剧本契约（id + kwargs + 结果名）永不改变。
...
1. **解析**：`VNScriptRunner.Play()`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:191`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs#L191)） 把整个文件交给 `VNScriptParser.Parse()`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:112`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L112)），
   得到 `List<VNScriptCommand>`。这一行首 token 不是关键字 → 走 `ParseSay()`，
   拆出 `speaker=亚里沙, expression=微笑, text=今天天气真好。`
2. **执行**：主循环 [`Run()`](Assets/Project/Scripts/VNEffects/Editor/VNSoftSkinExporter.cs#L106) 协程逐条取命令进 `Dispatch()` → `case "say"` →
   `SayCo()` 协程。
3. **上台**：`SayCo` 调 `VNStage.Say()`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1074`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1074)）——舞台查 [`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 找到亚里沙、
   按"微笑"切表情（交叉溶解）、通知 [`VNSpeakerHighlight`](Assets/Project/Scripts/VNEffects/VNSpeakerHighlight.cs) 高亮说话者、
   把名牌颜色/头像/文本交给 [`VNDialogueBox`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs)。
4. **打字**：`VNDialogueBox` 内部的 [`VNTypewriterText`](Assets/Project/Scripts/VNEffects/VNTypewriterText.cs) 逐字上浮显示，每个字
   调一次 `VNAudio.TypeTick()`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:370`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L370)） 打字音；如果之前有 `voice` 命令绑定，
   [`VNCharacterMouth`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMouth.cs) 同步开合口型、[`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) 压低 BGM。
5. **等待**：`SayCo` 设 `_waitingAtSay = true`（此刻才允许 F5 存档），等玩家
   点击/Enter/Auto 计时；`VNBacklog.Record()`（[`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:48`](Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs#L48)） 已把这句记进回想。
6. **推进**：玩家点击 → `_advance = true` → 主循环取下一条命令。

存档时：`VNStage.CaptureSnapshot()`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:840`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L840)） 收集背景/天气/色调/BGM/角色站位 +
`VNFlags.All`（[`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:23`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs#L23)） + 当前台词索引 → [`VNSaveSystem`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 写 JSON + PNG 缩略图。
...
- **关键成员**：`Keywords`（27 个关键字集合，编辑器通过 `CommandKeywords`
  共享同一来源）、[`VNScriptCommand`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs)（keyword/args/kwargs/options/camPoints/line）。
...
| 区块 | 内容 |
|---|---|
| 播放控制 | `Play/Stop/ResumeAt/SwitchChapter`；主循环 [`Run()`](Assets/Project/Scripts/VNEffects/Editor/VNSoftSkinExporter.cs#L106) 协程逐条 `Dispatch` |
| Dispatch | 巨型 switch：每个关键字 → 一段执行代码或专用协程（SayCo/ChoiceCo/EventCo/CamseqCo…） |
| 输入 | [`Update()`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs#L59)：推进/催促打字、H 回想、J 任务、F5/F9 存读档、A 自动、S 快进、右键+U 隐藏 UI。**新版 Input System**（`Keyboard.current`），禁止旧 `Input.` |
...
**维护注意**：给舞台加新的"有开关状态"的效果时，记得 ①[`Fx()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1614) 路由
...
- 剧本 `cg <id> [transition:] [chars:keep] [fx:keep]` / `cg off`；素材在
  `Assets/CG/`（文件名 = id），生成器灌入 `VNStage.cgLibrary`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:49`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L49)）（CgEntry.group
  为 P2 差分组预留字段）。
...
- **职责**：角色定义 ScriptableObject（`Assets/VNEffects/Characters/*.asset`）。
  ...
  口型配置（张嘴图/间隔/仅默认表情）。
- **枚举 [`VNBlinkMode`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs)** 也定义在本文件：`FullSprite=0`（整张替换，缺省，
  保证旧资产行为不变）/ `Overlay=1`（眼部叠加层）。便捷属性 `ActiveBlinkSprite`
  按当前方式返回实际用到的那张图。
- **扩展**：给角色加新的"每角色可配"能力时字段加在这里，然后在
  [`VNCharacterVisualPreviewWindow`](Assets/Project/Scripts/VNEffects/Editor/VNCharacterVisualPreviewWindow.cs) 里补预览 UI（保持"确认后写入资产"流程）。

### VNCharacterBlink.cs / VNCharacterBlinkOverlay.cs / VNCharacterMouth.cs（`Script/`）

- **职责**：自动眨眼两种实现 / 说话口型（台词+语音期间在叠加子 Image 上随机开合张嘴图）。
- **眨眼两条路径互斥，由 `VNCharacterDef.blinkMode`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:107`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L107)） 二选一**，两个组件都会挂在角色上，
  各自在 `CanBlink` 里检查 `blinkMode` 后决定是否工作：
  - [`VNCharacterBlink`](Assets/Project/Scripts/VNEffects/Script/VNCharacterBlink.cs)（`FullSprite`）：默认表情下随机间隔把 `Image.sprite`
    换成完整闭眼立绘再换回。素材要整张像素级对齐。
  - [`VNCharacterBlinkOverlay`](Assets/Project/Scripts/VNEffects/Script/VNCharacterBlinkOverlay.cs)（`Overlay`）：底图不动，开关一张只有眼部有像素的
    透明闭眼图，做法与口型层完全一致。素材只要画布尺寸/Pivot 一致即可。
  - 间隔与闭眼时长字段（`blinkIntervalMin/Max`、`blinkDuration`）两者共用。
- **共同设计**：整张透明画布叠加、与主体共享 [`VNImageEffectController.Mat`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs)
  材质（溶解/调色同步）、DOTween 随机间隔驱动。
- **接线**：[`VNStage.CreateCharacter()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 挂三个组件；`ApplyExpression()` 必须对
...
- **定位**：`VNCharacterDef.markAnchor`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:204`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L204)） 归一化偏移（`(0,0)` = 立绘中心），
...
- **状态归属**：`keep` 符号 → [`VNSaveData.CharSave.marks`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs)（英文正名逗号串）；
...
- **VNTypewriterText**：逐字上浮打字机（**TMP 版**）。`LateUpdate` 里
  ...
  每字回调 `VNAudio.TypeTick()`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:370`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L370)）；`IsTyping/Complete()` 供"催促"用。
...
- **VNFontAssetBuilder**：菜单 Tools → VN Effects → 字体 Fonts → 生成 TMP 字体资产 Create TMP Font Asset，
  ...
  文字必须用它**（[`EnsureFontAsset()`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs#L80)），运行时临时资产存进场景会变 Missing。
- 扩展：换字体 = 替换 Resources/VNFonts 下的 OTF + 重新生成预烘焙资产；
  加第二字体（如标题花体）= 照 VNFont 模式再写一个静态入口，别混进 VNFont。
- **多语言**（五十七章）：[`VNFont.Asset`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs) 按 [`VNLocale.Language`](Assets/Project/Scripts/VNEffects/Script/VNLocale.cs) 返回字体——
...
- **维护注意**：[`ResetForDebug()`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L144) 只给编辑器中间行调试用；
...
[`VNEventContext`](Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs) 给模块的信息：`eventId / stage / kwargs（Kw/KwF/KwI 读取）/
outcomes（剧本 * 结果行的结果名，AcceptsOutcome() 判断）/ line`。

**模块三大铁律**（违反会破坏存档/调试/快进）：
1. 只操作自己的 UI 子树和 VNFlags，**不直接改舞台演出**
2. 计时走 **`VNTime.Delta`（[`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:144`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs#L144)） / `VNTime.Time`（[`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:148`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs#L148)）**、Tween 加 `SetUpdate(true)`（不受快进影响）
3. 所有 Tween `SetLink`（模块随时可能被销毁）

> **铁律 2 在一三二章改过一次**：原本写的是直接用 `Time.unscaledDeltaTime`。
> 那样做躲开了快进，但也躲开了**暂停**——`Time.timeScale = 0` 对模块一律无效，
> 教程弹出来球还在飞。现在统一走 [`VNTime.Delta`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs)（暂停时为 0，且带 0.05s 单帧上限），
> 并且每个模块 [`Update()`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs#L59) 第一行必须是 `if (VNPause.IsPaused) return;`
> ——**要在 `ReadInput()` 之前**，晚了「冻结」期间照样能挥拍起跳。
> 新写模块请照抄这两条。

> **铁律 1 的唯一破例：[`VNAiTalkModule`](Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs)（一〇六章）。** AI 控制立绘表情就是改舞台，
...
- 地点配置在模板 Inspector（[`Location`](Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs)：名字=结果名/归一化坐标/
  VNFlags 显示条件/可选图标）。
...
- **题目全在资产**：[`VNQuizDef.Question`](Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs)（三语题干 / 2~4 个选项 / `answerIndex`（0 起）/
  解析 / 单题 `timeLimit` / `rewardOnCorrect` / `penaltyOnWrong`）。
  [`ValidQuestions()`](Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs#L128) 过滤填一半的题——坏题只是不出，不会让事件卡住。
...
- 属性联动：逐题的 StatOp 复用 [`VNShopDef.StatOp`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs)，统一走 `VNStatsHud.Apply`（[`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:108`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs#L108)）
  （钳制+飘字），没有 HUD 时退回 `VNFlags.Add`（[`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:34`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs#L34)）。模块同样不认识任何具体属性名。
- 输入双通道：选项按钮 + 数字键 1~4（`TryAnswer` 会挡掉本题不存在的序号）。
- 示例题库由生成器 [`EnsureQuizDef()`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs#L937) 造在 `Assets/VNEffects/Quizzes/`；
  示例剧本 `Assets/Scenarios/QuizDemo.vn.txt`（随机抽题/指定题号/成绩 flag 细分）。
- **扩展**：想打乱选项顺序，在 `ShowQuestion()` 里洗一次显示索引映射、
  `Answer()` 换算回原始下标即可，是加法不是改法。
- [`Editor/VNQuizInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNQuizInstaller.cs)（Tools → VN Effects → 场景装机 Install To Scene → 限时问答 Quiz Module）：
...
| 文件 | 层 | 允许依赖 |
|---|---|---|
| [`VNBadmintonBallistics.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonBallistics.cs) | 纯数学 | 只有 `UnityEngine` 的 `Mathf/Vector2`。**无 MonoBehaviour、无 UI**，可单测 |
| [`VNBadmintonUi.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs) | UI 原语 | uGUI。含 `VNBadmintonQuad`（uGUI 画不出梯形） |
| [`VNBadmintonCourt.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs) | 场景与 HUD | UI 原语。不是 MonoBehaviour，由模块 new 出来持有 |
| [`VNBadmintonActor.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonActor.cs) | 角色表现 | UI 原语。**换真动画只改这一个文件** |
| [`VNBadmintonModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs) | 玩法 | 以上全部 + [`VNFlags`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs) / [`VNBadmintonDef`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonDef.cs) |
...
- 养成联动照 [`VNBattleModule.patkstat`](Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs) 范式：`powerstat/speedstat/jumpstat` 指定读哪个
  flag，`ApplyTuning()` 在 `CopyFrom(def.tuning)` **之后**叠加加成——
  顺序反了会被编辑器每帧重读冲掉。
- 音效 [`VNBadmintonSfx`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonSfx.cs) 全部 `AudioClip.Create` 合成（指数包络 × (正弦 + 低通白噪)），
  Def 填了真音效逐条覆盖，音量跟随 `VNAudio.seVolume`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:51`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L51)）。
- `debugAutoPlayer` 开关：让玩家也交给 AI 自动对拉，验证回合逻辑用。
- [`Editor/VNBadmintonInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNBadmintonInstaller.cs) 与 `VNQuizInstaller` 同款增量装机。
...
| 文件 | 职责 |
|---|---|
| [`VNAiKey.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiKey.cs) | Key 三级回退读取（环境变量 → 仓库外 → 仓库内），只在内存缓存、永不打印 |
| [`VNAiClient.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs) | **全项目唯一碰 HTTP 的文件**。换模型 / 换供应商 / 改走自建中转只动它 |
| [`VNAiPricing.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiPricing.cs) | **全项目唯一算钱的文件**（一一三章）。单价按模型查表、可用资产覆盖。`VNAiResult.model`（[`Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs:121`](Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs#L121)） 由 `Send` 回填——没有它就无从按模型计价 |
| [`VNAiPersonaDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs) | 人格资产：性格、边界、白名单、模型参数、兜底台词 |
| [`VNAiConversation.cs`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs) | **纯逻辑（无 MonoBehaviour，可单测）**：提示词组装 / schema 生成 / 历史裁剪 / 解析钳制 |
...
- [`Editor/VNAiConnectionTester.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiConnectionTester.cs)：两级自检（网络 / 逻辑层），**不进 Play Mode**。
  前者过、后者挂 = 问题在提示词或人格配置。
- [`Editor/VNAiTalkInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNAiTalkInstaller.cs)：与 `VNQuizInstaller` 同款增量装机，
...
- **扩展位**：`VNAiContext.memory`（[`Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs:33`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs#L33)） 字段已预留，接跨场景记忆时把往期摘要塞进去即可，
  提示词层不用改（存储仿 [`VNCgUnlocks`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs) / [`VNPhotoAlbum`](Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs) 的全局 JSON）。
...
| 文件 | 职责 |
|---|---|
| [`VNFogMask.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs) | 掩码缓冲：擦除 Stamp / 线段补点 / 边缘权重图 / 回雾衰减 / 清晰度求和 |
| [`VNFogScore.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs) | 阶段推进（只升不降）+ 三档判定（按历史峰值）。**纯静态可单测** |
| [`VNFogWipeDef.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogWipeDef.cs) | 资产：雾外观九参 / 笔刷 / 回雾五参 / 三档门槛 / 分阶段台词 / 音效覆盖 |
| [`VNFogWipeModule.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs) | 表现层：自绘底图与雾层、输入轮询、HUD、台词条、倒计时、ESC、结算 |
| [`VNFogSfx.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogSfx.cs) | 四个代码合成音效 |
| [`VNFogTextures.cs`](Assets/Project/Scripts/VNEffects/Script/VNFogTextures.cs) | 四种道具光标的程序化 SDF 贴图 |
...
- **事件层 60 盖住对话框是必然的**（雾要铺满整屏）。解法不是让雾避开对话框，而是
  **模块自己铺一份清晰 CG 打底**，画面完全自洽；台词改用自绘的台词条。
  这一条与 [`VNInteractionModule`](Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs)「不铺全屏暗幕」的解法正好相反——那个玩法全程要角色说话
...
- **难度是算得出来的**：`每秒擦除面积 ≈ 笔刷直径 × 鼠标速度`，减去回雾速率即净推进。
  [`Editor/VNFogTuneWindow.cs`](Assets/Project/Scripts/VNEffects/Editor/VNFogTuneWindow.cs) 就是把这个公式做成窗口，**不进 Play Mode** 直接给出预计
  通关秒数与手感评语。本玩法刻意不做速度惩罚，难度唯一来源就是「笔刷面积 vs 回雾速度」，
  手感全压在参数上，所以这个窗口是**刚性需求而不是加分项**。
- [`Editor/VNFogWipeInstaller.cs`](Assets/Project/Scripts/VNEffects/Editor/VNFogWipeInstaller.cs)：与 `VNQuizInstaller` 同款增量装机。
- **易漏点**：`VNFogMask.Destroy()`（[`Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs:111`](Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs#L111)） 必须按 `Application.isPlaying` 分路——
  这个类编辑器也在用，`Object.Destroy` 在编辑模式下只打一条 error 什么都不销毁，
  症状是每关一次调参窗口泄漏一张贴图。同理 `VNTouchCursor.Dispose()`（[`Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:217`](Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs#L217)） 四处都要调。
- **CG 解锁**：`VNCgUnlocks.Unlock`（[`Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs:48`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs#L48)） 本来是 `VNStage.ShowCg`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1234`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1234)） 顺手做的，推荐写法下剧本不再
...
- 命令入口 [`VNScriptRunner.SnsCo`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs)（open/close/voice/image/typing/read/time/system/reply），
...
- [`VNQuestDef`](Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs) 资产只管文案（标题/描述/各阶段目标）；**没建资产的任务
  照常运作**（id 当标题）。
- [`VNQuestLog`](Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs)：执行 `quest start|stage|done|fail <id> [阶段]`（写 flag +
...
- **纯静态类，状态全在 flags**：`装备_<道具id>`=部位编号（1~7 见 `VNEquipSlot`）、
  ...
  `装备效果_<效果id>`=特殊效果合计（穿脱后 [`RecomputeEffects()`](Assets/Project/Scripts/VNEffects/Script/VNEquipment.cs#L147) 整体重算）。
...
| 脚本 | 职责 | 要点 |
|---|---|---|
| VNSaveSystem.cs | 存档读写（静态类） | 20 槽 JSON + PNG 缩略图存 `persistentDataPath`；[`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 是唯一存档结构——**加字段要给默认值**（旧档兼容靠字段初始化器） |
...
| VNBacklog.cs | H 键回想 | 独立 Overlay Canvas 600；[`Record()`](Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs#L48) 由 SayCo/ChoiceCo/EventCo 调 |
...
| VNInventory.cs | I 键背包 | 左道具列表+右 7 装备格+介绍区；右键菜单走 [`ClickRelay`](Assets/Project/Scripts/VNEffects/Script/VNInventory.cs)（IPointerClickHandler 区分左右键，Button 只管按压视觉）；皮肤 prefab（`VNSystemUiSkinSet.inventoryPrefab`（[`Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs:28`](Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs#L28)） + VNInventorySkin/RowSkin/SlotSkin 槽位）优先，缺失退回程序化 UI，**两条路径都产出 VNInventorySkin 引用**，下游单一代码路径；装备逻辑全部委托 VNEquipment |
...
| VNPause.cs / VNTime.cs（同一文件） | 全局玩法暂停 + 时间源 | 句柄式：`Acquire(owner, reason)` 绑宿主对象，宿主销毁句柄自动失效，`IsPaused` 每次读顺手清死句柄；外加 [`VNScriptRunner.CleanupActiveEvent`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 里的 [`ReleaseAll()`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs#L99) 兜底。**释放路径有五条**（正常结束/ESC/CancelForDebug/被 Destroy/换场景），漏一条 = 游戏永久卡死，所以不能用裸 Push/Pop 计数 |
...
- **VNImageEffectController**：单张图的特效总控。为图片创建**独立材质实例**
  ...
  **扩展特效参数时**：shader（VNImageEffect.shader）加参数 → 这里加包装
  ...
  - **分层调色合并层**（`SetGrade` / `DOGradeField` / `ClearGrade` / `GetGrade`）：
    ...
    症状是「说一句话立绘颜色就跳回去」。现在每个来源占一个 [`VNGradeLayer`](Assets/Project/Scripts/VNEffects/VNGrade.cs)
...
- **VNEntranceAnimator**：出场预设×10 + 退场×4 + `StartIdleEffects`
  ...
  - 日常向 `Crossfade`(默认)/`SlideIn`/`StepIn`/`WalkIn` —— 无粒子无光环，
    [`IsCasual()`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs#L182) 判定，登场后**不开周期扫光**（差别进存档，见 `CharSave.casualEntrance`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:116`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L116)））。
  - 华丽向 `DissolveGlow`/`FadeSlideUp`/`ScaleBounce`/`ShineReveal`/`FlashBloom`/
    `AfterimageDash`；退场 `Fade`(默认)/`Dissolve`/`RunOut`/`Sink`。
  - [`VNSide`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs) 方向由 VNStage.SideFor 按站位推断（剧本 `from:` / `to:` 可覆盖）；
...
- **VNAmbientParticles**：粒子预设×8（尘埃/星光/光斑/花瓣/雨+溅落/雪/萤火虫/雾）
  ...
  ⚠ [`Preset.Petals`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs) 已弃用（加法混合 + 单一贴图 + 全局噪声，就是「白色纸片同步飘」的老实现），
...
- **VNWeatherDef**：飘落天气的全部参数（ScriptableObject）。五套内置预设走
  [`CreateBuiltin()`](Assets/Project/Scripts/VNEffects/VNWeatherDef.cs#L160)，**不建任何资产也能用**；自定义资产登记进 `VNGameConfig.weatherDefs`（[`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:160`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs#L160)）。
  新增参数字段时记得同步 `CopyFrom()` 与 [`EnsureLayers()`](Assets/Project/Scripts/VNEffects/VNWeatherDef.cs#L109)。
...
- **VNWeatherController**：天气总控，两套后端并存 ——
  ...
  [`VNWeather`](Assets/Project/Scripts/VNEffects/VNWeatherController.cs) 枚举名，所以旧存档里的 `"Petals"`/`"Rain"` 不需要迁移。
...
- **VNMoodGrading**：八种情绪色调，**分层调色**（不是全屏后处理）。
  ...
  一起染橙。现在色彩逐层写进各自的 [`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) 材质实例
  （背景 1.0 / 中景 0.8 / 立绘 0.3，UI 不在目标列表所以完全不受影响），
  Volume 只留 `FilmGrain + Vignette`（不改色相，压四角反而有电影感），
  仍是 A/B 双 Volume 交叉过渡。立绘目标由 [`VNStage.RefreshRegistries()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)
...
- **VNCamZoomMode**（枚举，在 [`VNCamera.cs`](Assets/Project/Scripts/VNEffects/VNCamera.cs)）：camseq 的缩放模式 both/depth/bg/char
  ...
  **`both` 下 `ApplyCharacterZoom` 是空转**（否则每个路径点都给每个立绘起补间，
  ...
  公开的 [`Cut()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L244) / [`GoTo()`](Assets/Project/Scripts/VNEffects/VNCamera.cs#L266)（= camcut / camto）一律 `SetMode(Both)` 不继承上一段模式；
...
- **VNLiquidSplash / VNWetScreen / VNLiquidPreset**：液体喷溅（`liquid` 命令，九十四章）。
  **一个效果两层，缺一层就不成立**：
  - [`VNLiquidSplash`](Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs)（场外世界空间粒子，sortingOrder 28）= 空中飞的水珠。三个发射器
  ...
  - [`VNWetScreen`](Assets/Project/Scripts/VNEffects/VNWetScreen.cs)（Canvas 下 overlay，默认 30 让开对话框 40）= 溅在镜头玻璃上的水渍。
    **不是粒子**：每滴要挂住、按各自节奏下滑、拖一条渐淡的痕、慢慢干，
    是逐个体的四段状态机，ParticleSystem 的曲线模型表达不了 → uGUI 对象池 + 手动模拟。
  - 假折射（C1）：不采样背景，玻璃感全烘进 [`VNProceduralTextures.WaterDrop`](Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs) 的 RGB
...
| VNFogTuneWindow.cs | Tools → VN Effects → 预览 Preview → **擦雾调参 Fog Wipe Tuning** | 擦雾调参（一三一章）：上半按 `每秒擦除面积 ≈ 笔刷直径 × 鼠标速度` **算出**预计通关秒数并给手感评语，下半是可拖鼠标试擦的掩码预览、回雾照参数实时跑。跑的是运行时同一个 [`VNFogMask`](Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs)，看到的行为和游戏里一致。本玩法难度唯一来源是「笔刷面积 vs 回雾速度」，手感全压在参数上，**这个窗口是刚性需求不是加分项** |
...
| VNAiStudioDraft.cs | （草稿层） | 人格资产的内存副本。**用临时 SO 而不是自建字段**：`SerializedObject` 迭代画＝零 UI 代码就有全部字段，加新字段自动跟上，[`VNAiConversation`](Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs) 也直接能吃。写回逐属性 `CopyFromSerializedProperty`（**不能用 `CopySerialized`**，它连 `m_Name` 一起抄成「xxx(Clone)」） |
| VNAiStudioSession.cs | （会话层） | 发请求 / 解析 / 轮次记录 / 重跑 / 分岔。域重载后靠轮次记录 `BuildRequest`+`RecordReply` **重建历史**（那两个方法只是组装与追加，不发请求） |
| VNAiStudioMemory.cs | （记忆层） | 可命名记忆预设 + 从日志 / 从存档两个导入器。**完全独立于运行时 [`VNAiMemory`](Assets/Project/Scripts/VNEffects/Script/VNAiMemory.cs)**（那是存档态）；读存档**自己读 JSON，绝不调 `VNSaveSystem.Load()`（[`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:218`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs#L218)）**——那个会 `VNFlags.Clear()`（[`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:36`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs#L36)） |
...
| VNConfigEntryDrawers.cs | （PropertyDrawer） | 背景 / CG / 音频 / UI 皮肤四个条目的紧凑单行绘制。**挂在类型上**，所以 [`VNGameConfig`](Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs)、[`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs)、[`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) 三处的同名列表一起受益 |
...
（`VNEntryDrawerBase.Overlay()`（[`Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs:121`](Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs#L121)） 就是干这个的）。
...
（[`VNScenarioEditorWindow`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs) 与 [`VNAiStudioWindow`](Assets/Project/Scripts/VNEffects/Editor/VNAiStudioWindow.cs) 都遵循这条）。
...
1. `VNScriptParser.Keywords`（[`Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:93`](Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs#L93)） 加关键字
2. [`VNScriptRunner.Dispatch`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 加 `case`（要等待→写 `XxxCo` 协程返回；瞬发→返回 null）
3. 命令会改舞台可见状态？→ `RebuildStateBefore()` 加静默重放 case
4. 有开关状态要进存档？→ [`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 加字段（带默认值）+ [`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 快照存取
5. [`VNScenarioSchema`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs) 登记参数模式（编辑器 UI 自动生成）
6. `VNScenarioEditorWindow.CommandTranslations`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:126`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs#L126)） 加中文名；新参数来源则
   `VNParamSource` + `OptionsFor` + `VNScenarioDoc.Validate`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs:532`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs#L532)） 三处补
...
3. 场景里（或生成器里）建禁用模板物体挂上组件，`VNEventRegistry.modules`（[`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:25`](Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs#L25)）
   登记 id
...
1. 音频文件拖进工程 → 场景选中 [`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) → 对应通道库（bgm/se/voice）加条目
...
1. （可选但推荐）Project 右键 → Create → VN → Quest Definition，填 id/标题/
   各阶段文案，拖进场景 `VNQuestLog.quests`（[`Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:23`](Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs#L23)）
...
2. 要接 `fx` 命令：VNStage 加引用 + [`Fx()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L1614) 路由 + `_fxStates` + 快照三件套
3. [`VNEffectsDemoSetup`](Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs) 生成器里创建/连线 → 重建演示场景
...
- 运行中看变量：目前看 Console 或存档 JSON；（Flags 监视面板在
  第三十九章待办清单里）
- 音频/状态残留：调试重建自动调 `VNAudio.ResetForDebug()`（[`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:144`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs#L144)），别手动调
...
3. 文字全部用 TextMeshPro，字体取 [`VNFont.Asset`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs)（编辑期存场景的文字用
   `VNFontAssetBuilder.EnsureFontAsset()`（[`Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs:80`](Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs#L80)））；禁止 legacy Text / LegacyRuntime.ttf
...
- **`Time.timeScale = 0` 冻不住任何玩法模块**：模块按三铁律②用不受缩放的时间，
  所以要暂停必须走 [`VNPause`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs) + `VNTime.Delta`（[`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:144`](Assets/Project/Scripts/VNEffects/Script/VNPause.cs#L144)），并且模块 [`Update()`](Assets/Project/Scripts/VNEffects/Script/VNFogScore.cs#L59) 首行早退
...
- 编辑器 `OnGUI` 里**不要直接 `AssetDatabase.FindAssets`**：它每帧都会跑，
  下拉候选一律缓存（`VNScenarioEditorWindow._assetIdCache`（[`Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:2780`](Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs#L2780)） 就是为此，`RefreshSources` 里清）
- kwargs 值不能含空格；`if` 条件串不能含空格
- 粒子 velocityOverLifetime 三轴曲线模式必须一致
- 运行时创建带 Awake 配置的组件：先 SetActive(false) 挂好赋值再激活
- [`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 加字段必须给初始化器默认值（JsonUtility 旧档兼容）
...
- **纯逻辑类被编辑器复用时，`Object.Destroy` 要按 `Application.isPlaying` 分路**
  ...
  **什么都不销毁**。[`VNFogMask`](Assets/Project/Scripts/VNEffects/Script/VNFogMask.cs) 踩过——症状是每关一次调参窗口就泄漏一张贴图。
