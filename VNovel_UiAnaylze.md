...
| ③ | **事件模块 UI** | uGUI，EventLayer 嵌套 Canvas | `VNEventRegistry` 建的 EventLayer | 模块 `OnLaunch` 里 100% 程序化生成 | 羽毛球、大头贴、擦雾、问答、商店、战斗… |
| ④ | **Editor 工具 UI** | IMGUI（`OnGUI` / `EditorWindow`） | 不在场景里 | `EditorWindow.GetWindow` | 剧本编辑器、素材浏览器、镜头编排、擦雾调参 |

### 1.2 为什么要分成四套

**① 与 ② 的分界线是「吃不吃后处理」**。
主 Canvas 是 `RenderMode.ScreenSpaceCamera`（`BuildStageRig`，`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:130`），
`planeDistance = 10`（同一方法内，`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:132`），
所以它**在相机的渲染路径里**，会吃到 URP 的 Bloom / Vignette。
这是整个专案「发光 = HDR 颜色(>1) + Bloom」这条硬约定的物理基础。

而系统面板（存读档、设置…）用的是 `RenderMode.ScreenSpaceOverlay`
（`VNSaveLoadPanel.Build`，`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:122`；
`VNConfigPanel.Build`，`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:95`）。
...
1. **想被 Bloom 点亮的 UI，必须挂主 Canvas 下。**
   ...
   （类注释见 `VNTutorialPlayer`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:25`）。
   过场层 `VNInterludeScreen` 出于同样理由（`Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:25`）。
2. **想盖住全屏转场的 UI，做不到。**
   全屏转场 `VNScreenTransition.sortingOrder` 默认 100（`Assets/Project/Scripts/VNEffects/VNScreenTransition.cs:50`）
...
生命周期完全由 `VNScriptRunner.EventCo`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2909`）掌控。
...
1. **零 prefab 起步、程序化优先。** 几乎每个面板都能在「一个 prefab 都没有」的情况下跑起来，
   贴图由 `VNProceduralTextures`（`Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:10`）现场生成。
   美术资产是**可选的增强**，不是前提。
2. **皮肤 = 槽位声明，不是继承。** 想换外观就做一个 prefab，在根上挂一个「槽位声明组件」
   （`VNDialogueSkin` / `VNSystemUiSkinBehaviour` 的子类），代码只认槽位引用，
   prefab 里的其余装饰节点代码一概不管。
3. **状态不在 UI 里。** 属性、道具、任务、装备的真值全在 `VNFlags`
   （`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:15`）这个整型字典里；
   UI 只是这份字典的一个视图，靠 `VNFlags.Changed` 事件（`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:21`）标脏重画。
...
主 Canvas 本身 `sortingOrder = 0`（`BuildStageRig`，`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:133`），
其下的容器层级是：
...
这四层容器由 `BuildStageRig` 依次创建（`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:142`
起连续几行 `CreateStretchRect`）。背景 Image 四边溢出 60px（`BuildStageRig`，
`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:157`），给 Ken Burns 与视差留余量。
...
| 排序 | 组件 | 出处 |
|---|---|---|
| 10~12 | 氛围粒子（尘埃/星光/光斑） | `VNAmbientParticles.sortingOrder`（Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs:47） |
| 20 | 情绪泛光 EdgeGlow | `VNEdgeGlow.sortingOrder`（Assets/Project/Scripts/VNEffects/VNEdgeGlow.cs:29） |
| 25 | 漫画速度线 | `VNSpeedLines.sortingOrder`（Assets/Project/Scripts/VNEffects/VNSpeedLines.cs:21） |
| 26 | 全屏情绪水波 | `VNScreenShockwave.sortingOrder`（Assets/Project/Scripts/VNEffects/VNScreenShockwave.cs:25） |
| 28 | 液体喷溅（舞台层） | `VNLiquidSplash.sortingOrder`（Assets/Project/Scripts/VNEffects/VNLiquidSplash.cs:34） |
| 30 | 镜头水渍 | `VNWetScreen.sortingOrder`（Assets/Project/Scripts/VNEffects/VNWetScreen.cs:44） |
| 30 | 鼠标星尘拖尾 | `VNMouseStardust.sortingOrder`（Assets/Project/Scripts/VNEffects/VNMouseStardust.cs:25） |
| 31 | 点击涟漪 | `VNClickRipple.sortingOrder`（Assets/Project/Scripts/VNEffects/VNClickRipple.cs:27） |
| 34 | 胶片 / CRT 复古滤镜 | `VNRetroFilter.sortingOrder`（Assets/Project/Scripts/VNEffects/VNRetroFilter.cs:32） |
| 35 | 电影黑边 | `VNLetterbox.sortingOrder`（Assets/Project/Scripts/VNEffects/VNLetterbox.cs:18） |
| **40** | **对话框** | `VNDialogueBox.sortingOrder`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:28） |
| **41** | **快捷功能条**（对话框 +1） | `VNQuickToolbar.ConfigureCanvas`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:67） |
| **45** | **选项面板** | `VNChoicePanel.sortingOrder`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:29） |
| **60** | **事件层 EventLayer** | `VNEventRegistry.LayerSortingOrder`（Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:27） |
| **70** | **偷拍取景 HUD** | `VNSecretPhotoUi.SortingOrder`（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoUi.cs:27） |
| **90** | **过场章节卡** | `VNInterludeScreen.sortingOrder`（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:40） |
| 90 | 存档缩略图抓拍用的淡入淡出层 | `VNCameraFade.sortingOrder`（Assets/Project/Scripts/VNEffects/VNCameraFade.cs:27） |
| **92** | **教程层** | `VNTutorialPlayer.sortingOrder`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:47） |
| **100** | **全屏转场（天花板）** | `VNScreenTransition.sortingOrder`（Assets/Project/Scripts/VNEffects/VNScreenTransition.cs:50） |

**独立 Overlay Canvas**（不与上表同一个排序空间，整体都压在主 Canvas 之上）：

| 排序 | 面板 | 出处 |
|---|---|---|
| 500 | 标题菜单 | `VNTitleMenu.Build`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:188） |
| 578 | 日历 HUD | `VNCalendarHud.Build`（Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:93） |
| 580 | 属性 HUD + 属性面板 | `VNStatsHud.EnsureCanvas`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:240） |
| 600 | 回想 Backlog | `VNBacklog.Build`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:90） |
| 600 | 任务日志 | `VNQuestLog.Build`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:250） |
| 900 | 存读档 | `VNSaveLoadPanel.Build`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:123） |
| 950 | 设置面板 | `VNConfigPanel.Build`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:96） |
| **999** | **Toast 提示卡片（最高）** | `VNToast.EnsureCanvas`（Assets/Project/Scripts/VNEffects/Script/VNToast.cs:165） |

`VNQuestLog.Build` 里的注释直接写明「与回想同层：同一时刻只会开一个」
（`Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:250`）——
...
修法记在 `VNQuickToolbar.SetVisible` 的注释里
（`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:143`）：
工具栏自己再挂一个 CanvasGroup（`VNQuickToolbar.SetVisible`，
`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:146`），
由 `VNDialogueBox.SetInterfaceVisible`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:664`）
显式调用它（`VNDialogueBox.SetInterfaceVisible`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:678`）。
...
- `VNAiTalkModule`：「EventLayer 排序 60，选项面板在 45——**在本模块下方**。所以本模块自绘的
  任何东西都必须 raycastTarget=false，否则会把选项的点击全吃掉。唯一的例外是 ESC 确认框」
  （`Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:34`）。
- `VNInteractionModule`：「1) 不铺全屏暗幕 —— 会盖住对话框，台词就看不见了；
  2) 自绘的一切 raycastTarget = false，只有道具按钮和结束钮例外」
  （`Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:26`）。
- `VNFogWipeModule`：「擦拭输入走 Update 轮询 Mouse.current，不依赖 EventSystem，
  所以自绘的一切 raycastTarget = false —— 只有 ESC 确认框的两个按钮例外」
  （`Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:52`）。
- `VNSecretPhotoUi`：「除三个按钮外一律 raycastTarget=false」
  （`Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoUi.cs:22`）。

在构建工具里这条约定被写死成默认值：`VNBadmintonUi.CreateImage` 里
`image.raycastTarget = false`（`Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:61`）、
`CreateText` 里 `text.raycastTarget = false`（`Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:103`）、
`VNDialogueBox.CreateChildImage` 里 `img.raycastTarget = false`
（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:557`）、
`VNToast.CreateImage` 里 `image.raycastTarget = false`
（`Assets/Project/Scripts/VNEffects/Script/VNToast.cs:282`）。
...
- **持有者**：`VNScriptRunner`。它有一整排字段——
  `_backlog`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:58`）、
...
- **组件的创建时机**：`VNScriptRunner.Start`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:122`）
  ...
  （`VNScriptRunner.Start`，`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:128`）。
...
- **UI 实体的创建**：**不在 Start**，而在第一次 `Open()` 时。
  例如 `VNBacklog.Open`（`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:61`）先调
  `Build()`（`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:81`），
  `Build` 开头一句 `if (_panel != null) return;`
  （`VNBacklog.Build`，`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:83`）
  ——**这就是惰性构建的全部实现**。
- **关闭**：只是 `_panel.SetActive(false)`
  （`VNBacklog.Close`，`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:75`），**不销毁**。
  下次打开直接复用，只重建列表内容（`VNBacklog.RebuildList`，
  `Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:124`）。
- **销毁**：几乎不销毁。唯一会销毁整个画布的情形是**语言切换**。

#### 语言切换 = 销毁重建：全专案重复了八次的模式

`VNBacklog.OnLanguageChanged`（`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:36`）
...
| 面板 | 方法与出处 |
|---|---|
| 回想 | `VNBacklog.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:36） |
| 存读档 | `VNSaveLoadPanel.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:44） |
| 属性 HUD | `VNStatsHud.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:72） |
| 背包 | `VNInventory.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNInventory.cs:52） |
| CG 画廊 | `VNCgGallery.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs:66） |
| 日历 HUD | `VNCalendarHud.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:39） |
| 任务日志 | `VNQuestLog.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:113） |
| 标题菜单 | `VNTitleMenu.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:40） |
| 快捷功能条 | `VNQuickToolbar.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:29） |
| 设置面板 | `VNConfigPanel.RebuildPanel`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:282） |

`VNConfigPanel` 那份稍有不同：它是从 `SetLanguage`
（`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:275`）直接调 `RebuildPanel`，
而且会记住「重建前开着没有」再决定重新打开
（`VNConfigPanel.RebuildPanel`，`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:284`）。
...
> （`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:47` 起连续多行），
> 就是这个原因。

#### 模型 B：随台词共存（对话框、选项面板）

- **持有者**：`VNStage`——`dialogue` 字段（`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:59`）、
  `choicePanel` 字段（`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:107`）。
  它们是**场景里预先存在的物体**，由场景生成器连线。
- **构建时机**：`Awake` 就建——`VNDialogueBox.Awake`
  （`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:104`）调 `Build`
  （`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:109`）；
  `VNChoicePanel.Awake`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:71`）
  调 `Build`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:76`）。
  两者都有幂等守卫（`if (_built) return;`）。
- **开关**：不销毁、不 SetActive，靠 `CanvasGroup.alpha` + DOTween——
  `VNDialogueBox.Show`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:595`）、
  `VNDialogueBox.HideBox`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:615`）。
- **内容销毁**：选项按钮是**每次 Show 都新建、选完就销毁**的——
  `VNChoicePanel.Show`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:183`）开头先
  `ClearEntries()`；`HideAll` 的完成回调里再 `ClearEntries()`
  （`VNChoicePanel.HideAll`，`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:475`）。

#### 模型 C：用完即毁（事件模块）

- **模板持有者**：`VNEventRegistry.modules` 列表
  （`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:25`）。
  模板是**场景里禁用状态的物体**或 prefab（`VNEventRegistry.Entry.template`，
  `Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:21`）。
- **实例化**：`VNEventRegistry.Create`（`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:42`）
  →`Instantiate(entry.template, EnsureLayer(canvas))`
  （`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:59`），
  然后把 RectTransform 拉伸铺满（同方法 `:63` 起四行）并 `SetActive(true)`
  （`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:68`）。
- **销毁**：`VNScriptRunner.EventCo` 在模块回调 `Done` 之后 `Destroy(module.gameObject)`
  （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2956`）。
- **异常路径销毁**：剧本被 Stop / 调试中断时走 `CleanupActiveEvent`
  （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1086`），
  先 `CancelForDebug()` 再 Destroy（`VNScriptRunner.CleanupActiveEvent`，
  `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1096`）。

**这里有一个很聪明的细节**：`EventLayer` 只创建一次并缓存
（`VNEventRegistry.EnsureLayer` 开头的 `if (_layer != null) return _layer;`，
`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:74`），
...
1. **`VNScriptRunner` 是唯一的中枢**。所有系统面板的开关都要经过它的 `Request*` 方法：
   `RequestSavePanel`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1678`）、
   ...
   `Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:104`）。
2. **`VNStage` 是舞台层的持有者，但它不管系统面板**。这条分界很干净：
   `VNStage` 只知道对话框、选项、转场、过场、教程、SNS——都是「演出的一部分」；
   存读档、设置、画廊这类「元游戏 UI」它完全不知道。
3. **`VNToast` 是唯一的 static 全局 UI**（`Assets/Project/Scripts/VNEffects/Script/VNToast.cs:22`），
   而且是 `DontDestroyOnLoad`（`VNToast.EnsureCanvas`，
   `Assets/Project/Scripts/VNEffects/Script/VNToast.cs:162`）。
...
```
> 出处：`VNBacklog.Build`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:83）
...
```
> 出处：`VNDialogueBox.Build`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:111）
...
（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:142`）：
...
```
> 出处：`VNDialogueBox.BuildDefaultSkin`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:142）

上方注释解释得很清楚（`VNDialogueBox.BuildDefaultSkin`，
`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:138` 起）：
...
`VNDialogueBox` 的类注释把这条设计讲得很明白
（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:16` 起）：
...
- `VNDialogueBox.Build`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:109`）：
  建嵌套 Canvas（`:117` 起）设 `overrideSorting`、建 CanvasGroup、
  然后 `BuildDefaultSkin()` + `Bind(_defaultRoot.GetComponent<VNDialogueSkin>())`
  （`VNDialogueBox.Build`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:127`）。
- `VNDialogueBox.BuildDefaultSkin`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:136`）：
  ...
  `skin.arrow`（`:221`）。
- `VNDialogueBox.ApplySkin`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:236`）：
  传 null 就把 `_defaultRoot` 打开、还原根矩形、Bind 回默认
  （`VNDialogueBox.ApplySkin`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:249` 起）；
  传 prefab 就把 `_defaultRoot` 关掉、把根拉伸铺满画布、实例化 prefab、Bind
  （同方法 `:255` 起）。
- `VNDialogueBox.Bind`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:272`）：
  逐个抄槽位引用到私有字段，并在这里**按需补组件**——
  打字机没有就 `AddComponent<VNTypewriterText>()`
  （`VNDialogueBox.Bind`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:302`）、
  流光框没有特效控制器就 `AddComponent<VNImageEffectController>()`
  （同方法 `:312`）。

> **这个设计好在哪**：
> 1. **零素材可运行**——新人 clone 下来直接进 Play 就有对话框。
> 2. **美术不需要懂代码**——只要在 prefab 根上挂 `VNDialogueSkin` 把几个引用拖进去，
>    prefab 里其他装饰节点（渐变底图、角花、多层描边）代码完全不管
>    （`VNDialogueSkin` 类注释里的「授权自由」，`Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:12`）。
> 3. **行为逻辑只写一遍**——打字机、名牌显隐、头像避让、箭头呼吸都只认 `_typer`、`_nameTag` 这些字段。

### 4.3 槽位声明组件长什么样

`VNDialogueSkin`（`Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:24`）就是一堆 public 字段：

| 槽位 | 类型 | 行号 | 留空的后果 |
|---|---|---|---|
| `panel` | RectTransform | `Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:27` | 出入场动画作用于整个皮肤层 |
...
`Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:14`）。
...
```
> 出处：`VNDialogueBox.Say`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:645）

`VNChoiceSkin`（`Assets/Project/Scripts/VNEffects/Script/VNChoiceSkin.cs:22`）则相反——
...
```
> 出处：`VNChoicePanel.ApplySkin`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:131）
...
（`VNDialogueBox.StretchRootToCanvas`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:526`），
切回默认时再还原（`VNDialogueBox.RestoreRootRect`，
`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:535`）。
原始值在第一次 Build 时采样保存（`VNDialogueBox.SaveRootRect`，
`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:515`）。

`VNDialogueSkin` 的注释把这条写成了给美术看的说明：
「皮肤实例会被拉伸铺满整个画布（1920×1080 参考分辨率），panel 想放屏幕哪儿就锚在哪儿」
（`Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:16`）。
...
```
> 出处：`VNChoicePanel.ApplySkin`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:143）
...
// 错落飞入：右侧 90px 外滑入
```
> 出处：`VNChoicePanel.PlayEntrance`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:206）

没有 LayoutGroup → 以模板锚点为首项向下堆叠
（`VNChoicePanel.CreateSkinButton`，`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:232`）。
...
_costPath = skin.buttonCost != null
    ...
    : null;
```
> 出处：`VNChoicePanel.ApplySkin`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:144）

`PathBetween`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:151`）沿 parent 链往上拼名字，
`FindByPath`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:160`）在克隆体上按同路径 `Find`。
路径为空串表示槽位就是模板根本身（`VNChoicePanel.FindByPath`，
`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:163`）。

> **注意这与第十一章教程锚点的做法正好相反**。
> 这里能用路径寻址，是因为路径是**从 prefab 结构现场算出来的**，
> 美术改了层级路径也跟着变，不会失效。
> 而教程锚点不能用路径，是因为那些 UI 是代码程序化生成的，路径写死在教程资产里，
> 改一次布局就静默失效（`VNTutorialAnchors` 类注释，
> `Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:9`）。
>
...
- `VNStage.CurrentDialogueSkinId`（`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:753`）
- `VNStage.CurrentChoiceSkinId`（`:755`）
- `VNStage.CurrentNameplateStyleId`（`:757`）

写入存档：`VNStage.CaptureSnapshot` 里三行
（`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:869` 起）。
对应的存档字段是 `VNSaveData.dialogueSkin`（`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:58`）、
`choiceSkin`（`:59`）、`nameplateStyle`（`:60`）。

读档恢复：`VNStage.RestoreSnapshot`（`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:905`）
里**皮肤是最先恢复的**，注释写明理由：「UI 皮肤最先恢复：随后重放的台词/选项直接落在正确皮肤上」
（`VNStage.RestoreSnapshot`，`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:909`）。
而且只在与当前不同时才切换，避免无谓重建（同方法 `:911`）。

剧本入口是 `ui` 命令，Runner 转发给 `VNStage.SetUiSkin`：
```csharp
case "ui":
    ...
    return null;
```
> 出处：`VNScriptRunner.Dispatch`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2401）

`VNStage.SetUiSkin`（`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:764`）
按 `kind` 分三路，从 `VNGameConfig.FindSkin`
（`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:134`）查 prefab，
查不到就报错并**保持现状**（`VNStage.SetUiSkin`，
`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:781` 起）。
...
`VNSystemUiSkinSet`（`Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs:11`）
就是十二个 GameObject 字段：

| 分组 | 字段 | 行号 |
|---|---|---|
| 整页 / 弹窗 | `titleMenuPrefab` | `Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs:14` |
...
这个资产挂在 `VNGameConfig.systemUiSkin`
（`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:131`）上，全局唯一。
...
```
> 出处：`VNSystemUiSkinUtility.Instantiate`（Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs:29）
...
```
> 出处：`VNSystemUiSkinUtility.Prefab`（Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs:56）
...
```
> 出处：`VNBacklog.Build`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:95）
...
```
> 出处：`VNSystemUiSkinBehaviour`（Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs:7）
...
```
> 出处：`VNBacklogSkin.CollectValidationErrors`（Assets/Project/Scripts/VNEffects/Script/VNBacklogSkin.cs:19）

**「模板的模板」这一层也有校验**：`entryTemplate` 是 `VNBacklogEntrySkin`
（`Assets/Project/Scripts/VNEffects/Script/VNBacklogEntrySkin.cs:7`），
它不是 `VNSystemUiSkinBehaviour` 子类（不需要被 Instantiate 校验），
所以只提供一个 `IsValid` 属性（`Assets/Project/Scripts/VNEffects/Script/VNBacklogEntrySkin.cs:12`），
由父级槽位在 `CollectValidationErrors` 里手动检查。
同一模式见 `VNSaveSlotSkin.IsValid`（`Assets/Project/Scripts/VNEffects/Script/VNSaveSlotSkin.cs:21`）、
`VNStatsHudEntrySkin.IsValid`（`Assets/Project/Scripts/VNEffects/Script/VNStatsHudEntrySkin.cs:14`）、
`VNStatsPanelRowSkin.IsValid`（`Assets/Project/Scripts/VNEffects/Script/VNStatsPanelRowSkin.cs:14`）。
...
```
> 出处：`VNQuickToolbar.Build`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:48）

同样抛异常的还有：
`VNConfigPanel.Build`（`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:106`）、
`VNSaveLoadPanel.Build`（`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:133`）、
`VNBacklog.Build`（`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:99`）、
`VNTitleMenu.Build`（`Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:198`）、
`VNStatsHud.BuildHud`（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:255`）、
`VNResultPopupModule.OnLaunch`（`Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs:84`）。
...
```
> 出处：`VNStatsHud.BuildPanel`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:466）
...
```
> 出处：`VNConfigPanel.BindCustomSkin`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:164）

注释解释了理由：「槽位可留空：老的皮肤 prefab 没有这个按钮，缺了就只是设置面板里没这一项，
开关本身照常生效（默认开），不影响面板其它部分」
（`VNConfigPanel.BindCustomSkin`，`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:162`）。
...
>   而且导出模板菜单（`VNSystemUiSkinExporter.ExportAll`，
>   `Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs:23`）一键就能生成一整套，
...
**系统菜单主题**：`VNSystemUiSkinExporter.ExportAll`
（`Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs:23`）
一次性生成 11 个 prefab 并写进 `VNSystemUiSkinSet`
（同方法 `:36` 起连续 11 行），然后自动登记到 `VNGameConfig.systemUiSkin`
（同方法 `:52`）。资产目录写死在 `Dir` 常量
（`Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs:13`）。

还有一个「只重导两个面板」的入口 `ExportEventPanels`
（`Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs:68`），
避免把手改过的 prefab 一起覆盖掉——这是很实用的工程细节。

**对话框/选项皮肤**：`VNUiSkinExporter.ExportAll`
（`Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs:30`）
...
```
> 出处：`VNUiSkinExporter.ExportAll`（Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs:35）

`BakeSprite`（`Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs:61`）里有一个
很容易踩的坑：程序化贴图调过 `Apply(false, true)` 释放了 CPU 拷贝，**不可读**，
所以必须先经 RenderTexture 走一遍 GPU 再 `ReadPixels` 拿回可读副本
（`VNUiSkinExporter.BakeSprite`，`Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs:64` 的注释）。
...
| 部件 | 做法 | 行号 |
|---|---|---|
| 半透明磨砂面板 | `RoundedRectSprite` + `Image.Type.Sliced` | `Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:156` |
...
（`VNDialogueBox.BuildDefaultSkin`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:189`）。
...
见 `VNPhotoBoothModule.BuildViewFinder`，`Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:532`。）
...
```
> 出处：`VNDialogueBox.Bind`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:284）
...
```
> 出处：`VNDialogueBox.ApplyPortrait`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:717）
...
`VNStage.Say`（`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1077`）里分两条路：
角色已注册（`:1081`）走完整流程；未注册或旁白（`:1093`）则清高亮、无头像、名牌配色回默认
（`VNStage.Say`，`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:1095` 起）。

`VNDialogueBox.Say`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:628`）本身很短：
```csharp
Build();
if (!_shown) Show();
...
_typer.Play(content);
```
> 出处：`VNDialogueBox.Say`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:628）

`_lastSpeaker` / `_lastContent` 存下来是为了**切皮肤时能把当前这句重现**
（`VNDialogueBox.RestoreVisualState`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:484`）：
...
```
> 出处：`VNDialogueBox.RestoreVisualState`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:489）
...
> （`VNTypewriterText.Complete`，`Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:53`）
> 就是让下一次 `LateUpdate` 把网格刷成完全显示。

### 6.3 打字机：TMP 顶点动画

`VNTypewriterText`（`Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:14`）
只有 124 行，但把 TMP 逐字动画的正确做法完整演示了一遍。

**核心思路**：不改 `text` 字符串，只改**顶点**。
`Play` 一次性把整段文字赋给 TMP（`VNTypewriterText.Play`，
`Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:44`），
...
```
> 出处：`VNTypewriterText.LateUpdate`（Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:80）

**五个必须知道的点**：

1. **`ForceMeshUpdate()` 要在读 textInfo 之前调**
   （`VNTypewriterText.LateUpdate`，`Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:75`）。
   否则 `characterInfo` 可能是上一帧的、顶点索引对不上。
2. **富文本标签不占字数**。类注释写明「characterInfo 已剔除控制符，优于旧版按 quad 计数」
   （`VNTypewriterText` 类注释，`Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:10`）。
...
4. **动画结束要停止每帧重建网格**：
   ...
   ```
   > 出处：`VNTypewriterText.LateUpdate`（Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:121）
...
5. **空行的死锁防护**：
   ...
   ```
   > 出处：`VNTypewriterText.LateUpdate`（Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:107）
...
```
> 出处：`VNTypewriterText.Update`（Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:62）

`VNAudio.TypeTick`（`Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:370`）里再做一层节流与
随机音高：
```csharp
...
```
> 出处：`VNAudio.TypeTick`（Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:374）
...
```
> 出处：`VNScriptRunner.NormalSayCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2625）
...
`VNNameplateStyle`（`Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:53`）
是一个**纯数据 + 静态 Apply** 的类（不持有场景引用，可单独测试）。
十个内置预设的 id 在 `VNNameplateStyleId`（`Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:7`）。

#### 三个硬约束（类注释里明确列出，`Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:43` 起）
...
if (mat == null) return;
```
> 出处：`VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:520）
...
这条与 `VNFont` 的装饰字体设计直接相关，`VNFont` 里的注释算出了公式
（`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:71` 起）：
...
（`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:69`）、
`DisplayAtlasPadding = 22`（`:79`），
正文则是 `SamplePointSize = 64`（`:58`）、`AtlasPadding = 6`（`:59`）。

**但 padding 也不能无脑放大**——`VNFont` 的注释记了一次实测失败
（`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:62` 起）：
...
```
> 出处：`VNNameplateStyle.UnderlayUse`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:56）
...
```
> 出处：`VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:542）

而且改 underlay 之前必须 `EnableKeyword`：
```csharp
mat.EnableKeyword(UnderlayKeyword);
```
> 出处：`VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:539）
...
```
> 出处：`VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:497）

Neon 样式（`VNNameplateStyleId.Neon`，`Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:29`）
...
```
> 出处：`VNNameplateStyle.ApplyBevel`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:559）

注释里那句「直接 SetFloat 不会报错但也不会有效果，静默失效比报错更难查」
（`Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:557`）是本专案的一贯态度。
而且警告只发一次（`_bevelWarned` 静态标志，
`Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:595`），
因为「每句台词都会重新上妆，不然刷屏」（同处注释）。

#### 三层字系列的立身之本

`VNNameplateStyleId` 的注释里有一句设计洞察
（`Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:18` 起）：
...
```
> 出处：`VNFont.DisplayFontChanged`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:321）

`VNFont.HandleLanguageChanged`（`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:277`）
在替换完场景里所有 TMP 文字的字体之后触发它
（`VNFont.HandleLanguageChanged`，`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:314`）。
...
```
> 出处：`VNDialogueBox.HookLocaleEvents`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:470）

并在 `OnDestroy` 退订（`VNDialogueBox.OnDestroy`，
`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:757`）。
...
```
> 出处：`VNFont.HandleLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:281）

注释说明理由：「把名牌的 Heavy 字体也换成正文字体，粗描边样式会当场垮掉
（这正是加装饰字体后最容易踩的坑）」
（`VNFont.HandleLanguageChanged`，`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:279`）。
...
```
> 出处：`VNDialogueBox.ResizeNameTag`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:424）

注释解释为什么不动自定义皮肤：「美术 prefab 的名牌尺寸是照着背景图排的，擅自改会破相」
（`VNDialogueBox.ResizeNameTag`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:421`）。

`GetPreferredValues` 是 TMP 的**同步**测量方法——当场就能算出来，
不需要等一次布局。这一点在 `VNToast.BuildCard` 里也被利用了：
```csharp
...
```
> 出处：`VNToast.BuildCard`（Assets/Project/Scripts/VNEffects/Script/VNToast.cs:245）
...
```
> 出处：`VNDialogueBox.EnsureUnderline`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:437）

注释：「名牌下的横线装饰按需补建（皮肤 prefab 里没有也能用）」
（`VNDialogueBox.EnsureUnderline`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:436`）。

而 `FindPlateImage`（`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:454`）
...
```
> 出处：`VNDialogueBox.FindPlateImage`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:458）
...
`VNChoicePanel` 类注释开门见山（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:10` 起）：
「零新特效，纯组合现有件」——
选项错落飞入、悬停扫光 + 微放大、选中闪光确认 + 其余噪声溶解。

三段的实现分别在：
- 飞入：`VNChoicePanel.PlayEntrance`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:201`），
  延迟 `index * 0.09f` 做错落。
- 悬停：`VNChoicePanel.FinishButton` 里的 `EventTrigger`
  （`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:402` 起），
  `PointerEnter` 播扫光 + `DOScale(1.045f)`，`PointerExit` 缩回。
- 选中：`VNChoicePanel.Choose`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:431`），
...
```
> 出处：`VNChoicePanel.Choose`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:456）
...
```
> 出处：`VNChoicePanel.MaxTotalHeight`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:283）
...
```
> 出处：`VNChoicePanel.ResolveMetrics`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:290）

`MinFontSize = 20`（`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:306`）
...
>    `Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:286`）——
...
```
> 出处：`VNChoicePanel.CreateDefaultButton`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:330）
> 同样一行也在 `VNChoicePanel.CreateSkinButton`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:263）
...
```
> 出处：`VNChoicePanel.FinishButton`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:421）
...
`VNScriptRunner.ChoiceCo`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2838`）
在调用面板之前做了三件事：
...
```
> 出处：`VNScriptRunner.ChoiceCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2851）
...
```
> 出处：`VNScriptRunner.ChoiceCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2862）
...
```
> 出处：`VNScriptRunner.ChoiceCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2885）
...
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2896` 起）：
```csharp
...
```
> 出处：`VNScriptRunner.ChoiceCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2896）
...
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2876`）——
翻译不会破坏分支逻辑。
...
```
> 出处：`VNScriptRunner.EnsureQuickToolbar`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1659）
...
```
> 出处：`VNQuickToolbarSkin.Slots`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbarSkin.cs:17）

每个按钮挂一个 `VNToolbarActionSlot`（`Assets/Project/Scripts/VNEffects/Script/VNToolbarActionSlot.cs:8`），
上面填一个 `VNToolbarAction` 枚举值（`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbarSkin.cs:6`）。
绑定时按 action 分派：
```csharp
...
```
> 出处：`VNQuickToolbar.BindCustomSlots`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:72）

> **「按钮顺序和数量完全由子级 ActionSlot 决定」**
> （`VNQuickToolbarSkin` 类注释，`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbarSkin.cs:12`）
> 这是一个很干净的「数据驱动 UI」范例：
...
```
> 出处：`VNQuickToolbar.Update`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:160）

`SetActiveState` 只改一个 Graphic 的颜色（`VNToolbarActionSlot.SetActiveState`，
`Assets/Project/Scripts/VNEffects/Script/VNToolbarActionSlot.cs:17`），开销可忽略。
...
```
> 出处：`VNDialogueBox.DockToolbar`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:505）

`VNQuickToolbar.SetDock`（`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:127`）
只是换个 parent（`VNQuickToolbar.AttachRoot`，
`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:153`）。
...
六个 key 全在类顶部（`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:13` 起）：
...
```
> 出处：`VNScriptRunner.Start`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:164）

`VNConfigPanel.Initialize`（`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:42`）
调 `ApplySavedSettings`（`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:66`），
...
```
> 出处：`VNConfigPanel.WheelOpensBacklog`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:36）

Runner 每帧读它（`VNScriptRunner.Update`，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2097`）。
...
```
> 出处：`VNConfigPanel.BindSlider`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:179）
...
```
> 出处：`VNConfigPanel.BindSlider`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:193）
...
```
> 出处：`VNScriptRunner.RequestSavePanel`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1678）
...
```
> 出处：`VNSaveLoadPanel.PrepareForSaveCapture`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:62）

**`_open = true` 但 `_panel` 关着**——这是为了让 Runner 的 `Update` 立刻进入
「存读档面板打开」的分支（`VNScriptRunner.Update`，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2015`），
抓图这几帧里玩家按键不会推进剧情。
...
```
> 出处：`VNScriptRunner.CaptureSaveThumbnailCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1895）

`CancelSaveCapture`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1924`）
...
```
> 出处：`VNSaveLoadPanel._loadedThumbnails`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:14）

- 建卡时读入并登记：`VNSaveLoadPanel.CreateSlotCard`
  （`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:205`）。
- 重建列表时先全清：`VNSaveLoadPanel.RebuildSlots`
  （`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:194`）调 `ClearLoadedThumbnails`。
- 关闭时清：`VNSaveLoadPanel.Close`
  （`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:106`）。
- 组件销毁时清：`VNSaveLoadPanel.OnDestroy`
  （`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:290`）。
...
```
> 出处：`VNSaveLoadPanel.ReplacePendingThumbnail`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:276）

存完之后 Runner 也会销毁它：
```csharp
if (thumbnail != null) Destroy(thumbnail); // PNG 已落盘，纹理即可释放
```
> 出处：`VNScriptRunner.SaveTo` 附近（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1640）
...
```
> 出处：`VNSaveLoadPanel.CreateCustomSlotCard`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:216）
...
```
> 出处：`VNSaveLoadPanel.ShowConfirm`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:260）

> 同一 Canvas 内的绘制顺序 = Hierarchy 顺序。
> 想让一个已存在的节点浮到最上，`SetAsLastSibling()` 是最省事的做法，
> 不需要额外的 Canvas / sortingOrder。
> 大头贴的贴纸拿起时也用了这招（`VNPhotoStickerItem.OnPointerDown`，
> `Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs:61`）。
...
```
> 出处：`VNBacklog.Entry`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:14）
...
```
> 出处：`VNBacklog.Record`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:48）

上限 `maxEntries = 200`（`Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:12`），
超了从头删。
...
- 普通台词：`VNScriptRunner.NormalSayCo`
  （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2623`）
- 选项：`VNScriptRunner.ChoiceCo`
  （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2897`）
- 事件结果：`VNScriptRunner.EventCo`
  （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2962`）
- 剧本外插话（偷拍被发现）：`VNScriptRunner.SayOutOfScript`
  （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1837`）

**SNS 台词不进回想**——`SnsSayCo`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2655`）
里没有 `_backlog?.Record`，注释说明理由：「不进回想（聊天窗本身就是历史记录）」
（`VNScriptRunner.SnsSayCo` 上方注释，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2653`）。

**事件是否进回想由模块自己决定**：
```csharp
public virtual bool RecordInBacklog => true;
```
> 出处：`VNEventModule.RecordInBacklog`（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:91）

注释解释：「纯流程控制型调用（如 event plan op:next 逐格派发，一周会调 7 次）应返回 false，
否则回想里全是无意义的重复条目」
（`Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:87`）。
Runner 在**销毁模块之前**读它：
```csharp
bool recordInBacklog = module.RecordInBacklog; // 销毁前读取
Destroy(module.gameObject);
```
> 出处：`VNScriptRunner.EventCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2955）
...
```
> 出处：`VNBacklog.Open`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:66）
...
> **对比**：`VNQuestLog.Open`（`Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:222`）
...
**它同时是三个角色**（类注释，`Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:8` 起）：
...
```
> 出处：`VNQuestLog.MarkDirty`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:73）
> 与 `VNQuestLog.Update`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:80）
...
```
> 出处：`VNQuestLog.Awake`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:61）

> **这个模式是本专案 UI 数据绑定的标准答案**，`VNStatsHud`
> （`VNStatsHud.MarkDirty`，`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:70`）
> 与 `VNCalendarHud`（`VNCalendarHud.MarkDirty`，
> `Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:37`）用的完全一样。
>
> **为什么不在事件里直接刷新**：`VNFlags.Changed` 在读档时会**连续触发几百次**
> （逐个 Set 回去）。直接刷新等于重建几百次 UI。
> `VNFlags.Changed` 的文档注释就写着这条：「读档时会连续触发多次，
> 订阅方应做"标脏 + 下帧统一刷新"而不是立即重建」
> （`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:20`）。
...
```
> 出处：`VNQuestLog.EnsureConfigured`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:103）
...
```
> 出处：`VNQuestLog.UpdateBadge`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:90）

`_badgeCount` 初始 -1（`Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:37`），
保证第一次一定会写一遍（即使是 0）。
...
```
> 出处：`VNQuestLog.Build`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:303）
...
> 本专案的 `Stretch` 辅助函数（例 `VNDialogueBox.Stretch`，
> `Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:576`）是用 `offsetMin/offsetMax` 清零，
> 效果等价。
...
Canvas（`VNStatsHud.EnsureCanvas`，`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:232`）。

**属性变动的四重反馈**（`VNStatsHud.RefreshHud`，
`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:305`）：

1. **数值滚动**（`VNStatsHud.RollValue`，`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:369`）：
   ```csharp
   ...
   ```
   > 出处：`VNStatsHud.RollValue`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:377）

   注释：「20 → 23 逐格跳，比瞬间换字更容易被注意到」
   （`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:368`）。
   差 1 时直接赋值不滚（`VNStatsHud.RollValue`，
   `Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:372`）——滚一格没有意义。

2. **进度条补间**：`DOVirtual.Float` 驱动 `rect.anchorMax.x`
   （`VNStatsHud.RefreshHud`，`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:331`）。
...
3. **数字弹跳 + 染色**：
   ...
   ```
   > 出处：`VNStatsHud.RefreshHud`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:345）

4. **图标弹跳 + `+N` 上飘**（`VNStatsHud.SpawnFloatingDelta`，
   `Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:386`）。
...
```
> 出处：`VNStatsHud.SpawnFloatingDelta`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:381）
...
```
> 出处：`VNStatsHud.Apply`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:154）

注释解释了双色的分工：「图标+主题色认属性，左侧竖条认涨跌」
（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:153`）。

> **两个视觉通道分别承载两个信息维度**，是很扎实的信息设计。
> 玩家不需要读文字就能知道「哪个属性动了」（图标+色）与「涨还是跌」（竖条色）。

**`cost:` 判定三件套**（都是纯静态或近乎纯函数，可单测）：
- `VNStatsHud.ParseCostOp`（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:170`）——静态解析
- `VNStatsHud.CanAfford`（`:185`）——付不付得起
- `VNStatsHud.FormatCostLabel`（`:194`）——右侧小字文案
- `VNStatsHud.ApplyCost`（`:206`）——真扣
...
```
> 出处：`VNStatsHud.CanAfford`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:189）
...
```
> 出处：`VNCalendarHud.Refresh`（Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:63）

**「月份」flag 不存在 = 不是养成模式 = 整个面板隐藏**
（类注释，`Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:10`）。
...
> 这也是为什么 `VNFlags.All` 要暴露成 `IReadOnlyDictionary`
> （`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:29`）而不只是 `Get(key)`。

**排序号 578 的注释**：
```csharp
_canvas.sortingOrder = 578; // 与属性 HUD(580) 同档，低于各面板(600)
```
> 出处：`VNCalendarHud.Build`（Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:93）

**位置避开对话框右下角**：
```csharp
rect.anchoredPosition = new Vector2(-24f, 88f); // 避开对话框右下角
```
> 出处：`VNCalendarHud.Build`（Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:104）
...
```
> 出处：`VNInventory.Awake`（Assets/Project/Scripts/VNEffects/Script/VNInventory.cs:34）
...
```
> 出处：`VNInventory.OnDestroy`（Assets/Project/Scripts/VNEffects/Script/VNInventory.cs:47）
...
> （`Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:40`）——
...
```
> 出处：`VNInventory.FindItem`（Assets/Project/Scripts/VNEffects/Script/VNInventory.cs:64）

> **「道具的定义住在商店资产里」是一个有意思的取舍**。
> 优点：不用再建一个道具库资产，商品与道具天然同源。
> 缺点：不卖的道具（任务奖励、捡到的）没地方登记，只能显示 id。
> 类注释承认了这一点：「未登记的道具用 id 照常显示」
> （`Assets/Project/Scripts/VNEffects/Script/VNInventory.cs:13`）。
...
Page _page = Page.Cg;
```
> 出处：`VNCgGallery.Page`（Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs:41）
...
```
> 出处：`VNCgGallery.PhotoItem`（Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs:45）
...
```
> 出处：`VNCgGallery.Open`（Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs:97）
...
```
> 出处：`VNCgGallery.Close`（Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs:116）

**全屏浏览是一个独立的子状态**：
```csharp
public bool IsViewerOpen => _viewer != null && _viewer.activeSelf;
```
> 出处：`VNCgGallery.IsViewerOpen`（Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs:60）
...
```
> 出处：`VNScriptRunner.Start`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:172）

而 `ResumeAt` 会顺手收起标题层：
```csharp
_titleMenu?.NotifyGameplayStarted(); // 任何入口开始播放都顺手收起标题层
```
> 出处：`VNScriptRunner.ResumeAt`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1054`）

> **这个「任何入口都收标题」的兜底很关键**。
> 标题菜单可以被三个东西绕过：编辑器从选中行播放、读档、`StartNewGame`。
> 全部走 `Play` → `ResumeAt`（`VNScriptRunner.ResumeAt`，
> `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1042`），
...
```
> 出处：`VNTitleMenu.ApplyTitleStage`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:82）

配对的还原在 `VNTitleMenu.RestoreSceneUi`
（`Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:111`）。

注意 `_stageApplied` 只守住「背景与 BGM」这一段——那是一次性的；
而隐藏对话框/HUD 是每次 Open 都要做的。这个分界拆得很准。

> **可以改进的一点**：`GameObject.Find("HintText")`
> （`VNTitleMenu.ApplyTitleStage`，`Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:107`）
> 是按名字找物体，正是 `VNTutorialAnchors` 类注释里批评的那种做法
> （`Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:9`）。
> HintText 是演示场景的提示文字，改名或删掉就静默失效（好在后果只是提示文字没藏起来）。
> 更一致的做法是让 `VNStage` 持有它，或者干脆把它也纳入 `VNUiParts`。
...
```
> 出处：`VNTitleMenu.FindLatestSlot`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:144）

从 slot 0 开始扫是因为 0 号是快存槽（`VNScriptRunner.QuickSaveSlot`，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1599`），
「继续」应该包含快存。
...
```
> 出处：`VNTitleMenu.PlayEntrance`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:276）

因为读档/设置面板会把 `Time.timeScale` 归零
（`VNScriptRunner.PauseForSaveLoadMenu`，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1909`），
标题层的动画不该跟着停。
...
**它与回想的分工写在类注释里**
（`Assets/Project/Scripts/VNEffects/Script/VNAiDiaryPanel.cs:16` 起）：
...
```
> 出处：`VNScriptRunner.RequestDiary`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1716）
...
```
> 出处：`VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2110）

对比其它面板都是 `_backlog?.Open()`、`_questLog?.Open()` 这样直接调
（`VNScriptRunner.Update`，`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2100`、`:2106`）。
...
这一章讲整个专案最集中、也最容易被忽略的一段代码：`VNScriptRunner.Update`
（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1974`）。
...
| 顺序 | 条件 | 行为 | 出处 |
|---|---|---|---|
| 1 | `_eventActive` | 直接 return，输入全归模块 | `VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1980） |
| 2 | `VNPause.IsPaused` | 直接 return，教程讲解中 | `:1985` |
...
```
> 出处：`VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1982）
...
```
> 出处：`VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1987）
...
```
> 出处：`VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1995）
...
```
> 出处：`VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2089`）
...
2. **CG 画廊有两层**（网格 / 全屏浏览），Esc 的含义不同：
   ...
   ```
   > 出处：`VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2067）
...
if (!pressed) return;
```
> 出处：`VNScriptRunner.Update`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2150）
...
```
> 出处：`VNScriptRunner.IsPointerOverInteractiveUi`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2202）

结果列表是静态复用的（`VNScriptRunner._pointerRaycastResults`，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2172`），避免每帧 GC。
...
```
> 出处：`VNScriptRunner.RequestAdvance`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2189）

配套暴露一个只读状态给模块判断要不要转发：
```csharp
public bool IsWaitingAtSay => _waitingAtSay;
```
> 出处：`VNScriptRunner.IsWaitingAtSay`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2200）
...
```
> 出处：`VNScriptRunner._waitingAtSay`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:83）
...
_advance = false;
```
> 出处：`VNScriptRunner.NormalSayCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2633）

三个观察：

1. **Auto 的等待时间按字数加权**：`autoDelay + sayText.Length * 0.045f`
   （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2636`）。
   长句子多等一会儿，符合阅读速度。
2. **回想面板开着时 Auto / Skip 暂停**：`if (_backlog == null || !_backlog.IsOpen)`
   （`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2639`）——
   玩家在翻回想，剧情不该自己往前跑。
3. **Skip 也要等 0.07 秒**（`:2641`）——完全不等的话一帧过一句，
   玩家看到的是纯粹的闪屏，且 DOTween 的演出会全部被截断。

`_waitingAtSay` 的消费者有三处：
- `RequestSavePanel`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1680`）——不是台词处就拒绝并 Toast
...
```
> 出处：`VNScriptRunner.SetAuto`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1942）
> 与 `VNScriptRunner.SetSkip`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1950）
...
```
> 出处：`VNScriptRunner.UpdateModeLabel`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1960）
...
```
> 出处：`VNScriptRunner.OnDestroy`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1963）

> **凡是改了全局状态（`Time.timeScale`、`DOTween.timeScale`、`Cursor.visible`）
> 的组件，`OnDestroy` 必须还原**。
> 这条在本专案里出现了三次：这里、`VNTouchCursor.Dispose`
> （`Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:217`）、
> `VNTutorialPlayer.Close`（`Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:236`）。
...
```
> 出处：`VNScriptRunner.PauseForSaveLoadMenu`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1904）

面板关闭时回调 Runner：`VNSaveLoadPanel.Close` 里的 `_runner?.OnSaveLoadPanelClosed()`
（`Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:108`）、
`VNConfigPanel.Close` 里的 `_runner?.OnConfigPanelClosed()`
（`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:63`）。
...
```
> 出处：`VNUiParts`（Assets/Project/Scripts/VNEffects/Script/VNUiParts.cs:13）

**为什么对话框与工具条绑成一项**（注释在 `VNUiParts` 上方，
`Assets/Project/Scripts/VNEffects/Script/VNUiParts.cs:9`）：
...
```
> 出处：`VNUiPartsUtil.Names`（Assets/Project/Scripts/VNEffects/Script/VNUiParts.cs:29）

三个函数：`Parse`（`:41`）、`ToToken`（`:48`）、`FromToken`（`:59`）。
`ToToken` 的注释：「存名字不存位，方便肉眼查存档」
（`Assets/Project/Scripts/VNEffects/Script/VNUiParts.cs:47`）。
...
```
> 出处：`VNScriptRunner.ApplyUiHidden`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1782）
...
```
> 出处：`VNScriptRunner.ApplyGameplayHudVisible`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1845）
...
```
> 出处：`VNScriptRunner.RestoreUiHidden`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1876）
...
```
> 出处：`VNSaveData.uiHidden`（Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:64）
...
这一段的因果链很值得完整读一遍（`VNPause` 类注释，
`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:9` 起）：
...
```
> 出处：`VNPause.Handle`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:28）

理由写在类注释里（`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:15` 起）：
...
```
> 出处：`VNPause.IsPaused`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:44）
...
```
> 出处：`VNPause.Prune`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:107）
...
**三重保险**：
1. 正常 `Release(ref handle)`（`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:85`）
2. 宿主销毁 → `Prune` 自动清
3. `ReleaseAll()` 兜底（`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:99`），
...
```
> 出处：`VNScriptRunner.CleanupActiveEvent`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1092）
...
#endif
```
> 出处：`VNPause.ResetStatics`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:117）
...
>
> 本专案在两个地方做了这件事：`VNPause.ResetStatics`
> 与 `VNTutorialAnchors.ResetStatics`
> （`Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:73`）。
> **`VNToast` 的 `_canvas` / `_cards` 没有做**——
...
```
> 出处：`VNTime`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:136）
...
```
> 出处：`VNTime.Install`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:151）

类注释解释理由（`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:133`）：
「计时由一个隐藏驱动物体每帧推进，**与读取顺序无关**——
用「读的时候才累加」的懒惰实现会在没人读的帧上丢时间。」

> **`MaxStep = 0.05f` 这个保护是从羽毛球模块提炼出来的**
> （注释在 `VNTime` 类上方，`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:130`）：
...
```
> 出处：`VNQteModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs:53）
...
void Update()
{
    ...
    ReadInput();
```
> 出处：`VNBadmintonModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:410）

同样的守卫在：
`VNShopModule.Update`（`Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs:68`，
注释「教程讲解中：ESC 不该把商店关掉」）、
`VNQuestBoardModule.Update`（`Assets/Project/Scripts/VNEffects/Script/VNQuestBoardModule.cs:55`，
注释「三铁律②：必须在读输入之前」）、
`VNResultPopupModule.Update`（`Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs:90`，
...
```
> 出处：`VNTutorialMask.Sync`（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:167）
...
```
> 出处：`VNTutorialPlayer.WaitStepCo`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:355）
...
```
> 出处：`VNTutorialPlayer` 类注释（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:15）
...
```
> 出处：`VNScriptRunner.Dispatch`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2452）
...
```
> 出处：`VNEventModule.Launch`（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:62）
...
```
> 出处：`VNTutorialPlayer.PlayAuto`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:147）
...
```
> 出处：`VNTutorialAnchors`（Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:24`）
...
```
> 出处：`VNBadmintonModule.RegisterTutorialAnchors`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:376）

id 是 const 字符串（`VNBadmintonModule.AnchorScore`，
`Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:399` 起六行），
反注册在 `UnregisterTutorialAnchors`（`:389`），
由 `OnDestroy` 调用（`VNBadmintonModule.OnDestroy`，
`Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:363`）。
...
```
> 出处：`VNTutorialAnchors.Get`（Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:48）
...
```
> 出处：`VNTutorialAnchors.Unregister`（Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:36）
...
```
> 出处：`VNTutorialAnchor`（Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:82）
...
```
> 出处：`VNTutorialMask` 类注释（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:11）
...
```
> 出处：`VNTutorialMask.TryResolve`（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:183）

`_corners` 是静态复用数组（`VNTutorialMask._corners`，
`Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:220`），避免每帧分配。
...
```
> 出处：`VNTutorialMask` 类注释（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:16）

在 `LateUpdate` 里（`VNTutorialMask.LateUpdate`，
`Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:138`）——
...
```
> 出处：`VNTutorialMask.Sync`（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:158）

最多 4 个洞（`VNTutorialMask.MaxHoles`，
`Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:27`），
洞数据打包成 `Vector4(中心x, 中心y, 半宽, 半高)`
（`VNTutorialMask.TryResolve` 返回值，
`Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:216`）。
...
```
> 出处：`VNTutorialMask.Awake`（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:79）
...
```
> 出处：`VNTutorialPlayer.maskMaterial`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:52）
...
```
> 出处：`VNTutorialPlayer.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:177）

理由在类注释里（`Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:30`）：
...
yield return _group.DOFade(0f, FadeOut).SetUpdate(true).SetLink(gameObject)
                   .WaitForCompletion();
```
> 出处：`VNTutorialPlayer.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:202）
...
```
> 出处：`VNTutorialPlayer.MinStepTime`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:44）
...
```
> 出处：`VNTutorialPlayer.WaitStepCo`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:356）

同一个 0.22 秒也在 `ApplyStep` 的卡片弹入动画里
（`VNTutorialPlayer.ApplyStep`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:316`）——
...
```
> 出处：`VNTutorialPlayer.Close`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:231）
...
```
> 出处：`VNScriptRunner.CleanupActiveEvent`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1088）
...
```
> 出处：`VNTutorialPlayer.PlaceCard`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:322）

`TryGetFirstHoleUv`（`Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:223`）
...
```
> 出处：`VNTutorialPlayer.BuildProceduralCard`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:499）

而暗幕**要**吃射线：
```csharp
_image.raycastTarget = true; // 只读演示：洞外洞内一律吃掉点击
```
> 出处：`VNTutorialMask.Awake`（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:70）
...
```
> 出处：`VNTutorialSeen` 类注释（Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:11）

实现照抄 `VNCgUnlocks`（`Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs:14`），
两者的结构几乎一模一样：
- 一个 `[System.Serializable] class SaveShape { public List<string> ids; }`
  （`VNTutorialSeen.SaveShape`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:24`；
  `VNCgUnlocks.SaveShape`，`Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs:17`）
- 一个 `HashSet<string>` 内存缓存 + `EnsureLoaded()` 惰性读盘
  （`VNTutorialSeen.EnsureLoaded`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:45`；
  `VNCgUnlocks.EnsureLoaded`，`Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs:27`）
- 有新增才写盘：`if (!_seen.Add(id)) return;`
  （`VNTutorialSeen.Mark`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:78`）
- 写盘前排序，「文件内容稳定，便于人工查看」
  （`VNTutorialSeen.Flush`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:96`）
- 读失败按「全未看/全未解锁」处理并 LogError
  （`VNTutorialSeen.EnsureLoaded`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:61`）
...
```
> 出处：`VNTutorialSeen.Enabled`（Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:39）
...
```
> 出处：`VNTutorialPlayer.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:208）

三个数据归属的对照表（这张表值得记住）：

| 数据 | 存哪 | 语义 | 出处 |
|---|---|---|---|
| 剧情状态（好感、道具、任务） | `VNFlags` → 存档 | 跟着存档回退 | `VNFlags`（Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:15） |
| CG 解锁 | 独立 JSON | 全局永久，新周目不丢 | `VNCgUnlocks`（Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs:14） |
| 教程看过 | 独立 JSON | 同上 | `VNTutorialSeen`（Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:19） |
| 音量/语言/文字速度 | PlayerPrefs | 玩家偏好，与存档无关 | `VNConfigPanel.BgmKey` 等（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:13） |
| AI 跨场记忆 | 存档 | 剧情状态，读旧档她不该记得未来 | `VNSaveData.aiMemories`（Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:72） |
| AI 日记本 | 独立 JSON | 玩家收藏品，同 CG | `VNAiDiaryPanel` 类注释（Assets/Project/Scripts/VNEffects/Script/VNAiDiaryPanel.cs:12） |
...
```
> 出处：`VNEventModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:43）

进：`VNEventContext`（`Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:8`）
```csharp
...
```
> 出处：`VNEventContext`（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:10）
...
```
> 出处：`VNEventModule.Done`（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:79）
...
```
> 出处：`VNEventContext.AcceptsOutcome`（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:18）

地图模块用它来「只开放本次剧情接住的地点」
（`VNMapModule` 类注释，`Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs:15`）。
...
写在 `VNEventModule` 的类注释里（`Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:47` 起）：
...
| 模块 | 破例内容 | 边界收紧为 | 出处 |
|---|---|---|---|
| `VNAiTalkModule` | AI 控制立绘表情 | 只碰表情与对话框内容，三条退出路径都还原 | 类注释（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:24） |
| `VNInteractionModule` | 直接摸舞台上的立绘 | 只碰表情与叠加层 + 临时关鼠标视差，三条路径还原 | 类注释（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:20） |
| `VNSecretPhotoMode` | 直接操控 ZoomRoot 镜头 | 只碰镜头容器、UI 可见性、Ken Burns 开关，四条路径还原 | 类注释（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoMode.cs:12） |
...
> 出处：`VNAiTalkModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:24）
...
```
> 出处：`VNFogWipeModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:43）
...
```
> 出处：`VNEventRegistry.EnsureLayer`（Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:72）

**60 这个数字的理由**写在类注释里
（`Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:9`）：
...
同一件事在 `VNChoicePanel.Build` 里也做了：
```csharp
// 嵌套 Canvas 需要自己的 Raycaster 才能接收点击
if (gameObject.GetComponent<GraphicRaycaster>() == null)
    gameObject.AddComponent<GraphicRaycaster>();
```
> 出处：`VNChoicePanel.Build`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:91）
...
```
> 出处：`VNScriptRunner.EventCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2924）

从立绘层往上找到根 Canvas——不硬编码物体名，是正确做法。
同一手法在 `VNPhotoBoothModule.OnLaunch`
（`Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:196`）
与 `VNSecretPhotoMode.Initialize`
（`Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoMode.cs:68`）里也用了。
...
_eventActive = false;
```
> 出处：`VNScriptRunner.EventCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2928）
...
```
> 出处：`VNScriptRunner.EventCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2963）
...
> `Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:22`
> 「必须接住否则玩家卡死」）。
...
```
> 出处：`VNScriptRunner.RunInlineCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:315）
...
```
> 出处：`VNScriptRunner.InlineBlockedKeywords`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:304）
...
```
> 出处：`VNScriptRunner.RunInlineCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:337）
...
```
> 出处：`VNScriptRunner.SayOutOfScript`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1828）
...
```
> 出处：`VNPhotoBoothModule.ApplyConfig`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:215）

同样的模式在 `VNShopModule.OnLaunch`
（`Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs:43`）、
`VNMapModule.OnLaunch`（`Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs:75`）、
`VNQuestLog.Awake`（`Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:55`）、
`VNStatsHud.Awake`（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:52`）、
`VNInventory.Awake`（`Assets/Project/Scripts/VNEffects/Script/VNInventory.cs:38`）里。
...
```
> 出处：`VNGameConfig.ApplyList`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:265）

**为什么需要这一层**（`VNGameConfig` 类注释，
`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:9`）：
场景生成器会 `NewScene(EmptyScene)` 从零重造场景，
所有挂在场景组件上的引用每次重建全部清空。
把数据搬进 `Assets/Resources/VNGameConfig.asset`
（`VNGameConfig.AssetPath`，`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:39`），
场景怎么重建都不受影响。

> **这是所有「场景可重建」项目必须解决的问题**。
> 本专案的解法很干净：ScriptableObject 放 Resources、运行时 `Resources.Load` 直接取
> （`VNGameConfig.Active`，`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:48`），
> 场景里一个引用字段都不需要。
> 组件上的 `config` 字段只是可选覆盖（`VNScriptRunner.config`，
> `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:105`）。
...
```
> 出处：`VNQteModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs:104）
...
```
> 出处：`VNQteModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs:60）
...
DOVirtual.DelayedCall(0.8f, () => Done(success ? "success" : "fail"), true)
         .SetLink(gameObject);
```
> 出处：`VNQteModule.Finish`（Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs:88）
...
public class Location
{
    ...
    public Sprite icon;
```
> 出处：`VNMapModule.Location`（Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs:26）
...
```
> 出处：`VNMapModule.Location.DisplayName`（Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs:42）

> **这是本地化设计里最重要的一条规则**：
> **标识符永不翻译，显示名才翻译**。
> 本专案在多处贯彻：结果名固定中文（`VNBattleModule` 类注释，
> 「结果名固定中文：胜利 / 失败 / 逃跑（事件结果名是逻辑标识符，永不翻译）」，
> `Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs:24`）、
> 选项匹配按索引（`VNScriptRunner.ChoiceCo`，
> `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2876`）。
...
```
> 出处：`VNBattleModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs:253）
...
```
> 出处：`VNBattleModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs:20）
...
（`VNBattleModule` 类注释，`Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs:25`）。
...
_panel.DOScale(1f, 0.28f).SetEase(Ease.OutBack)
      .SetUpdate(true).SetLink(gameObject);
```
> 出处：`VNShopModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs:147）
...
```
> 出处：`VNShopModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs:11）
...
```
> 出处：`VNShopModule.OnLaunch`（Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs:49）

同样的「只有一套时可省 id」在问答模块的文档里也有
（`VNQuizModule` 类注释，`Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:22`）。
...
```
> 出处：`VNQuizModule.Phase`（Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:53）
...
```
> 出处：`VNQuizModule.UrgentSeconds`（Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:44）

同一个概念在大头贴（`VNPhotoBoothModule.UrgentSeconds`，
`Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:118`，值 3）
与擦雾（`VNFogWipeModule.UrgentSeconds`，
`Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:65`，值 5）里各有一份。
...
```
> 出处：`VNQuizModule.CorrectFlagSuffix`（Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:40）

**结果延迟 1.4 秒**（比 QTE 的 0.8 长，因为要看最终成绩）：
```csharp
DOVirtual.DelayedCall(1.4f, () => Done(outcome), true).SetLink(gameObject);
```
> 出处：`VNQuizModule` 结算段（Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:358）
...
```
> 出处：`VNPlanModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs:11）

**派发模式不进回想**（`_dispatchMode` 字段，
`Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs:59`，
...
_panel.DOScale(1f, 0.28f).SetEase(Ease.OutBack)
      .SetUpdate(true).SetLink(gameObject);
```
> 出处：`VNPlanModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs:234）

> **这是「双路径构建」的正确写法**：
> 两条路各自负责「把 `_panel` 指向正确的对象」，
> 之后的公共处理（弹入动画、刷新数据）写一遍。
> 对比 `VNStatsHud.BuildPanel`（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:460`）
...
```
> 出处：`VNPlanModule.SlotFlagPrefix`（Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs:35）
> 与 `VNPlanModule.SlotFlagName`（Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs:41`）
...
```
> 出处：`VNResultPopupModule.StyleOf`（Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs:27）
...
```
> 出处：`VNResultPopupModule.OnLaunch`（Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs:67）
...
```
> 出处：`VNResultPopupModule.inputDelay`（Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs:13）

`_shownAt = float.MaxValue`（`VNResultPopupModule.OnLaunch`，
`Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs:77`）
...
/// 悬念演出，否则直接揭晓大字。
```
> 出处：`VNResultPopupSkin` 类注释（Assets/Project/Scripts/VNEffects/Script/VNResultPopupSkin.cs:9）
...
```
> 出处：`VNQuestBoardModule.OnLaunch`（Assets/Project/Scripts/VNEffects/Script/VNQuestBoardModule.cs:39）
...
```
> 出处：`VNQuestBoardInstaller.Install`（Assets/Project/Scripts/VNEffects/Editor/VNQuestBoardInstaller.cs:58）
...
```
> 出处：`VNQuestBoardModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNQuestBoardModule.cs:15）

> **「显示条件」与「执行条件」必须是同一个函数**，
> 否则一定会出现「按钮亮着但点了没反应」。这是 UI 设计的铁律。
> 本专案在选项 `cost:` 上也是这么做的：
> `CanAfford`（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:185`）
...
| 层 | 文件 | 职责 | 依赖 |
|---|---|---|---|
| 纯数学 | `VNBadmintonBallistics` | 三点定抛物线、落点抽样、精准判定 | 无 MonoBehaviour，可单测 |
| 视觉 | `VNBadmintonCourt`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs:21） | 程序化球场与 HUD | 不是 MonoBehaviour |
| 表现 | `VNBadmintonActor` | 角色六态假动画 | 换真动画只改这个文件 |
| UI 辅助 | `VNBadmintonUi`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:44） | CreateImage / CreateText / CreateQuad / CreateLine | 静态类 |
| 逻辑 | `VNBadmintonModule`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:28） | 状态机、AI、判定 | 持有上面全部 |
...
```
> 出处：`VNBadmintonCourt` 类注释（Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs:10）
...
> 但这个模块的参数都在 `VNBadmintonTuning`（`VNBadmintonModule.tuning`，
> `Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:34`）与 Def 资产里，所以不需要。
...
```
> 出处：`VNBadmintonCourt` 类注释（Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs:12）
...
```
> 出处：`VNBadmintonUi.AnchorBottomCenter`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:118）

布局常量全在类顶部（`VNBadmintonCourt.SkyBottomY` 起，
`Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs:24` 到 `:34`），
注释写明「由参考截图逐像素量出并折算到 1920×1080」
（`Assets/Project/Scripts/VNEffects/Script/VNBadmintonCourt.cs:23`）。
...
```
> 出处：`VNBadmintonQuad`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:17）
...
```
> 出处：`VNBadmintonQuad` 类注释（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:12）
...
```
> 出处：`VNBadmintonUi.CreateQuad`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:65）
...
```
> 出处：`VNBadmintonUi.CreateLine`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:76）
...
```
> 出处：`VNBadmintonModule.OpenConfirm`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:495）

冻结整局的方式与 `VNPause` 一致（在 ReadInput 之前 return）：
```csharp
if (_confirmOpen) { TickConfirm(); return; }
```
> 出处：`VNBadmintonModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:428）

`TickConfirm` 只读键盘（Y/Enter = 是，N/Esc = 否，
`VNBadmintonModule.TickConfirm`，`Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:556`），
按钮的鼠标点击走 uGUI 的 Button（`VNBadmintonModule.MakeConfirmButton`，
`Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:541`）。
...
#endif
```
> 出处：`VNBadmintonModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:420）
...
**布局尺寸全部常数化在文件头**（`VNPhotoBoothModule.ViewW` 起，
`Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:98`）：

| 常量 | 值 | 含义 | 行号 |
|---|---|---|---|
| `ViewW` / `ViewH` | 1040 / 780 | 取景框（4:3，照片就是这块） | `Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:98` |
...
```
> 出处：`VNPhotoBoothModule.PanelW`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:105`）
...
```
> 出处：`VNPhotoBoothModule.BuildViewFinder`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:535）

完整的取景框层序（`VNPhotoBoothModule.BuildViewFinder`，
`Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:521` 起）：
```
ViewFinder
├── FrameBack        边框的背层
...
`Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:558`）——
...
```
> 出处：`VNPhotoBoothModule.BuildViewFinder`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:548）
...
```
> 出处：`VNPhotoBoothModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:350）
...
```
> 出处：`VNPhotoBoothModule._helpRoot`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:421）
...
```
> 出处：`VNPhotoStickerItem`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs:17）
...
```
> 出处：`VNPhotoStickerItem` 类注释（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs:13）
...
```
> 出处：`VNPhotoStickerItem.Update`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs:92）
...
```
> 出处：`VNPhotoStickerItem.OnDrag`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs:66）
...
```
> 出处：`VNPhotoStickerItem.OnPointerClick`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs:79）
...
```
> 出处：`VNPhotoDoodle` 类注释（Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs:53）

尺寸常量：`Width = 768` / `Height = 576`
（`Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs:70`），
...
```
> 出处：`VNPhotoDoodle.Snapshot`（Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs:91）
...
```
> 出处：`VNPhotoDoodleInput` 类注释（Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs:9）
...
```
> 出处：`VNPhotoDoodleInput.ToUv`（Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs:36）
...
```
> 出处：`VNPhotoCapture` 类注释（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:11）
...
```
> 出处：`VNPhotoCapture.Capture`（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:38）
...
```
> 出处：`VNPhotoCapture.Capture`（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:47）
...
```
> 出处：`VNPhotoCapture.Capture`（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:70）
...
```
> 出处：`VNPhotoCapture.Crop`（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:104）
...
```
> 出处：`VNPhotoCapture.MaxSide`（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:22）
...
```
> 出处：`VNFogWipeModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:23）
...
```
> 出处：`VNFogWipeModule` 类注释（Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:47）

`_lineBar` 就是那条自绘台词条（`VNFogWipeModule._lineBar`，
`Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:127`），
尺寸 1240×132 贴在画面下缘（`VNFogWipeModule.BuildHud`，
`Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:770`）。
...
```
> 出处：`VNFogWipeModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:619）

shader 参数 id 全部缓存成 static readonly int
（`VNFogWipeModule.IdMaskTex` 起，
`Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:76` 到 `:86`）——
...
```
> 出处：`VNFogWipeModule` 结算段（Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:406`
> 与 `:457`）
...
```
> 出处：`VNFogWipeModule.BuildHud`（Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:745）
...
（`Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:455` 与 `:588`，
...
```
> 出处：`VNFogTuneWindow` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNFogTuneWindow.cs:9）
...
```
> 出处：`VNInteractionModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:754）

HUD 缩到左下角 420×108（`Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:767`），
道具栏走右侧竖排：
```csharp
/// <summary>
...
```
> 出处：`VNInteractionModule.BuildItemBar`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:814）
...
```
> 出处：`VNInteractionModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:802）
...
```
> 出处：`VNInteractionModule.BuildZoneOverlay`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:968）
...
```
> 出处：`VNInteractionModule.RefreshZoneMarkers`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:1000）
...
/// 补间和逐帧写位置混在一起只会互相覆盖。
```
> 出处：`VNTouchCursor` 类注释（Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:9）

**「跟随」与「动画」分两层**是这段里最重要的设计
（`_root` 跟随、`_icon` 承载动画，
`VNTouchCursor._root` 与 `_icon`，
`Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:25`）。
...
```
> 出处：`VNTouchCursor.hoverGlowColor`（Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:44）

```csharp
_glowMat.SetColor(IdTintColor, c);   // HDR 走材质，顶点色会被钳到 1
```
> 出处：`VNTouchCursor.SetGlowAlpha`（Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:195）
...
```
> 出处：`VNTouchCursor.SetGlowAlpha`（Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:197）
...
```
> 出处：`VNTouchCursor.Dispose`（Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:217）

模块侧还有两处（`VNInteractionModule` 的 `Finish` 路径
`Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:584`、
`CancelForDebug` `:660`、`OnDestroy` `:671`）。

**注意 `Dispose` 里也销毁了材质实例**——
`new Material(shader)` 创建的实例不会被 GC 自动回收，必须显式 Destroy。
同一件事在 `VNTutorialMask.OnDestroy`
（`Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:96`）里也做了。
...
```
> 出处：`VNAiTalkModule._hintRoot` 等（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:82）
...
```
> 出处：`VNAiTalkModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:538）

**唯一吃射线的是确认框暗幕**：
```csharp
dim.GetComponent<Image>().raycastTarget = true;   // 这一层就是要独占输入
```
> 出处：`VNAiTalkModule` 确认框构建段（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:595）

**七阶段状态机**（最多的一个）：
```csharp
enum Phase { Booting, Thinking, Speaking, Pausing, Choosing, Confirming, Ending }
```
> 出处：`VNAiTalkModule.Phase`（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:61）
...
```
> 出处：`VNAiTalkModule.optionDelay`（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:50）

> **这种「给眼睛一个缓冲」的小停顿在本专案里出现了很多次**：
> 选项回调延迟 0.8 秒（`VNChoicePanel.Choose`）、
> 结算弹窗输入延迟 0.4 秒（`VNResultPopupModule.inputDelay`）、
> 教程每步最短 0.22 秒（`VNTutorialPlayer.MinStepTime`）、
> 这里的 0.4 秒。
...
```
> 出处：`VNAiTalkModule.OutcomeUp`（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:42）
...
```
> 出处：`VNSecretPhotoMode` 类注释（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoMode.cs:10）
...
/// 在事件层（60）之上、教程（92）之下。
```
> 出处：`VNSecretPhotoUi` 类注释（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoUi.cs:17）
...
```
> 出处：`VNSecretPhotoUi.IconClicked`（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoUi.cs:34）
...
```
> 出处：`VNSecretPhotoMode.Initialize`（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoMode.cs:80）
...
```
> 出处：`VNSecretPhotoUi.Build`（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoUi.cs:96）
...
```
> 出处：`VNSecretPhotoUi.OnDestroy`（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoUi.cs:105）
...
```
> 出处：`VNSecretPhotoUi.Build`（Assets/Project/Scripts/VNEffects/Script/VNSecretPhotoUi.cs:87）
...
```
> 出处：`VNScriptRunner.IsSecretPhotoIconAllowed`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1799）
...
```
> 出处：`VNSnsView` 类注释（Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:13）
...
```
> 出处：`VNScriptRunner.SayCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2608）

> **「换呈现层而不换机制」是一个非常划算的设计**。
> 如果把 SNS 做成 event 模块，就要重新实现存档点、分支、翻译、Auto/Skip——
> 而且「聊天中途不能存档」会是一个明显的体验缺陷。
> 现在只多了一个 `SnsSayCo`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2655`），
> 二十行。
>
...
```
> 出处：`VNSnsView` 类注释（Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:18）
...
```
> 出处：`VNSnsView.BuildTextRow`（Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:566）
...
```
> 出处：`VNSnsView.Layout`（Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:862）
...
> （`VNToast.BuildCard`，`Assets/Project/Scripts/VNEffects/Script/VNToast.cs:219`），
> 因为那里只需要「宽度跟着文字走」，不需要知道确切数值。

**布局常量全在类顶部**（`VNSnsView.PhoneW` 起，
`Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:32` 到 `:42`），
与大头贴、羽毛球同一习惯。
...
```
> 出处：`VNSnsView.CaptureSnapshot`（Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:500）
> 与 `VNSnsView.RestoreSnapshot`（Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:512）

由 `VNStage` 转发（`VNStage.CaptureSnapshot`，
`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:882`；
`VNStage.RestoreSnapshot`，`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:978`）。
读档时按消息列表重建整屏气泡。
...
```
> 出处：`VNSnsView.IsBlockingInput`（Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:81）

Runner 据此 return（`VNScriptRunner.Update`，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1993`）。
...
```
> 出处：`VNInterludeScreen` 类注释（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:13）

关键在于**图案参数仍走同一张表**：
```csharp
VNScreenTransition.ConfigurePattern(_patternMat, def.transition, ...);
```
> 出处：`VNInterludeScreen.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:91）
...
```
> 出处：`VNInterludeScreen._hudGroup`（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:51）

```csharp
_hudGroup.DOFade(1f, hudFade).SetDelay(hudDelay).SetLink(gameObject);
```
> 出处：`VNInterludeScreen.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:118）
...
```
> 出处：`VNInterludeScreen.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:80）

**不进存档，但 ClearStage 必须收起**：
```csharp
if (interlude != null) interlude.HideImmediate(); // 过场层同理，不能留在屏幕上
```
> 出处：`VNStage.ClearStage`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:990）

> **「不进存档」不等于「不用清理」**。
> 过场是一次性演出，播完什么都不留——所以不进 `VNSaveData`；
...
if (_skip) return null;
```
> 出处：`VNScriptRunner.Dispatch` 的 interlude 分支（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2445）

教程也是同样处理（`VNScriptRunner.Dispatch` 的 tutorial 分支，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2458`）。
...
```
> 出处：`VNFlags`（Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:15）
...
```
> 出处：`VNFlags.Set`（Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:34）

**两个事件的分工**（注释在 `VNFlags.KeyChanged` 上方，
`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:23`）：
...
| 订阅者 | 事件 | 做什么 | 出处 |
|---|---|---|---|
| `VNStatsHud` | `Changed` | 标脏，下帧刷 HUD 与面板 | `VNStatsHud.Awake`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:55） |
| `VNCalendarHud` | `Changed` | 标脏，下帧刷月份 | `VNCalendarHud.Awake`（Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:27） |
| `VNQuestLog` | `Changed` | 标脏，下帧跑任务引擎求值 | `VNQuestLog.Awake`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:62） |
| `VNTracker` | `KeyChanged` | 派生 @最高/@累计/@次数 | （见 CLAUDE.md 组件表；本次未逐行核对） |
...
```
> 出处：`VNStatsHud.MarkDirty`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:70）
> 与 `VNStatsHud.Update`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:296）
...
```
> 出处：`VNFlags.All`（Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:29）

`VNCalendarHud.Refresh` 用它判断「有没有这个 key」
（`Assets/Project/Scripts/VNEffects/Script/VNCalendarHud.cs:65`），
`VNStatsHud.EnsureInitials` 用它避免覆盖已有值
（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:101`）。
...
| UI | 调用 | 目标 |
|---|---|---|
| 选项 `cost:` | `_statsHud?.ApplyCost(opt.costOp, opt.line)` | `VNScriptRunner.ChoiceCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2898） |
| 选项 `flag:` | `VNFlags.Apply(opt.flagOp)` | 同上（`:2899`） |
| 快捷条按钮 | `_runner?.RequestSavePanel()` 等 | `VNQuickToolbar.Execute`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:104） |
| 存档卡点击 | `_runner?.SaveTo(slot, _pendingThumbnail)` | `VNSaveLoadPanel.SaveSlot`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:251） |
| 设置滑条 | `_stage?.vnAudio?.SetVolume("bgm", value)` | `VNConfigPanel.BindCustomSkin`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:136） |
| 标题「开始」 | `_runner.StartNewGame()` | `VNTitleMenu.OnStartClicked`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:133） |

`VNFlags.Apply` 是一个小型 DSL 解析器：
```csharp
...
```
> 出处：`VNFlags.Apply`（Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:50）

**「从第 2 个字符起找」这个细节**：如果从 0 开始找，`-血量+1` 这种名字开头带负号的会被误判。
同一个技巧在 `VNStatsHud.ParseCostOp`
（`Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:176`）里重复了一次。
...
| `VNTutorialMask.LateUpdate` | 目标的世界四角 | 目标可能在动，事件表达不了 |
| `VNScenarioEditorWindow.OnInspectorUpdate` | `runner.CurrentLine` | 「不用给运行时加事件」（注释） |
...
```
> 出处：`VNScenarioEditorWindow.OnInspectorUpdate`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:311）
...
`VNGameConfig`（`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:34`）
...
```
> 出处：`VNGameConfig.Active`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:48）
...
```
> 出处：`VNGameConfig.ClearCache`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:69）
...
`VNSaveData`（`Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:9`）
里的 UI 相关字段：

| 字段 | 内容 | 行号 |
|---|---|---|
| `dialogueSkin` | 对话框皮肤 id（空 = 默认） | `Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:58` |
...
```
> 出处：`VNSaveData.bgmVol`（Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:52）
...
```
> 出处：`VNSaveData.aiMemories`（Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:66）
...
```
> 出处：`VNSaveData.LiquidSave.sprayDirSet`（Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:87）

> **`JsonUtility` 不能序列化 NaN / Infinity** 是一个实际会咬人的限制。
> 解法就是这里的做法：另加一个 bool 表示「这个值有没有被设置」。
> 同一模式在天气（`weatherWindSet`，
> `Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:42`）里用过一次。
...
```
> 出处：`VNProceduralTextures`（Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:10）
...
```
> 出处：`VNProceduralTextures.RoundedRectSprite`（Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:284）
...
```
> 出处：`VNProceduralTextures.RoundedBoxDist`（Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:275）

```csharp
float d = RoundedBoxDist(dx * size, dy * size, size * 0.5f - 1f, size * 0.5f - 1f, 16f);
return Mathf.Clamp01(0.5f - d); // 1px 抗锯齿
```
> 出处：`VNProceduralTextures.RoundedRectSprite`（Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:293）
...
```
> 出处：`VNProceduralTextures.RoundedFrameSprite`（Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:318）
...
| 名称 | 用途 | 行号 |
|---|---|---|
| `SoftCircle` | 尘埃/光斑粒子、属性图标兜底 | `Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:21` |
| `Sparkle` | 四芒星光 | `:36` |
| `RadialGlow` | 径向光晕 | `:57` |
| `LightBeam` | 光束 | `:75` |
...
```
> 出处：`VNFont` 类注释（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:18）
...
```
> 出处：`VNFont.Profile`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:90）

三个语言档案：`ZhProfile`（`Assets/Project/Scripts/VNEffects/Script/VNFont.cs:121`）、
...
```
> 出处：`VNFont` 类注释（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:15）
...
```
> 出处：`VNFont.Prewarm`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:263）
...
const string CommonUiChars =
    ...
    "，。、；：？！…—·「」『』（）《》【】“”‘’▼▲◀▶★☆♪％×";
```
> 出处：`VNFont.CommonUiChars`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:84）

```csharp
PrewarmAsset(asset, CommonUiChars);
```
> 出处：`VNFont` 字体解析尾部（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:255）
...
```
> 出处：`VNPhotoAlbum._textureCache`（Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:52）
...
```
> 出处：`VNPhotoAlbum.LoadTexture`（Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:233）
...
```
> 出处：`VNPhotoAlbum.LoadThumbnail`（Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:271）
...
small.Apply(false);
```
> 出处：`VNPhotoAlbum.LoadThumbnail`（Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:305）

`+ 0.5f` 是像素中心采样（避免半像素偏移）。
同样的手法在 `VNPhotoCapture.Downscale`
（`Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:144`）里也用了。
...
```
> 出处：`VNPhotoAlbum.LoadThumbnail`（Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:325）
...
VNSecretAlbum.ClearCache();
```
> 出处：`VNCgGallery.Close`（Assets/Project/Scripts/VNEffects/Script/VNCgGallery.cs:117）
...
| 场景 | 谁创建 | 谁销毁 | 出处 |
|---|---|---|---|
| 存档缩略图（读盘展示） | `VNSaveLoadPanel.CreateSlotCard` | `ClearLoadedThumbnails`（4 个出口） | Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:283 |
| 存档缩略图（待存的那张） | `CaptureSaveThumbnailCo` | `ReplacePendingThumbnail` + `SaveTo` 存完 | Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:276 |
| 相册全尺寸 | `VNPhotoAlbum.LoadTexture` | LRU 驱逐 + `ClearCache` | Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:344 |
| 相册缩略图 | `VNPhotoAlbum.LoadThumbnail` | `ClearCache` | 同上 |
| 抓屏原图 | `VNPhotoCapture.Capture` | `finally` 里立即 Destroy | Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:72 |
| 拍照成品 | `VNPhotoCapture.Crop` | 调用方（模块的 `_shotTexture`） | Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:183 |
| 涂鸦画布 | `VNPhotoDoodle.Build` | 模块销毁时 | Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs:83 |
| 程序化贴图 | `VNProceduralTextures` | 不销毁（`HideFlags.DontSave`，域重载清） | Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:300 |

**Material 同理**：
| 材质 | 创建 | 销毁 |
|---|---|---|
| 教程暗幕 | `VNTutorialMask.Awake`（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:75） | `OnDestroy`（`:96`） |
| 光标发光 | `VNTouchCursor.Initialize`（Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:63） | `Dispose`（`:224`） |
| 过场图案 | `VNInterludeScreen._patternMat`（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:58） | （随组件销毁；未逐行核对） |
| TMP 名牌材质实例 | `text.fontMaterial` 首次访问（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:520） | TMP 自己管 |

> **规律**：`new Material(shader)` 与 `new Texture2D(...)` 出来的东西都要手动 Destroy。
> 本专案对每一处都做了配对，而且都加了 `hideFlags = HideFlags.DontSave`
> （例 `VNTouchCursor.Initialize`，`Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:63`）
> 作为第二道保险——即使漏了 Destroy，域重载时也会被清掉（编辑器里）。

### 15.5 Prefab 的加载与释放

系统面板的皮肤 prefab **不走 Resources、不走 Addressables**，
而是通过 `VNGameConfig.systemUiSkin` 的**直接引用**
（`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:131`）。

这意味着：`VNGameConfig` 一被 `Resources.Load` 出来，
...
对话框/选项皮肤同理，走 `VNGameConfig.dialogueSkins`
（`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:126`）的直接引用。

**实例的释放**：
- 对话框皮肤实例：切皮肤时 `Destroy(_customRoot)`
  （`VNDialogueBox.ApplySkin`，`Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:245`）。
- 选项皮肤实例：同上（`VNChoicePanel.ApplySkin`，
  `Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:112`）。
...
| 名称 | 用途 | 入口 | 出处 |
|---|---|---|---|
| `VNProceduralTextures` | 程序化贴图/Sprite | `RoundedRectSprite` 等属性 | Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:10 |
| `VNFont` | TMP 字体统一入口 | `Asset` / `DisplayAsset` / `Prewarm` | Assets/Project/Scripts/VNEffects/Script/VNFont.cs:31 |
| `VNLocale` | UI 字符串本地化 | `T(key)` / `T(key, args)` / `LanguageChanged` | Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:26 |
| `VNToast` | 提示卡片 / 模式标签 / 角标 | `Show` / `SetMode` / `SetBadge` | Assets/Project/Scripts/VNEffects/Script/VNToast.cs:22 |
| `VNFlags` | 游戏状态字典 | `Get` / `Set` / `Apply` / `Evaluate` / `Changed` | Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:15 |
| `VNPause` | 全局玩法暂停 | `Acquire` / `Release` / `IsPaused` | Assets/Project/Scripts/VNEffects/Script/VNPause.cs:25 |
| `VNTime` | 受暂停影响的时间源 | `Delta` / `Time` | Assets/Project/Scripts/VNEffects/Script/VNPause.cs:136 |
| `VNTutorialAnchors` | 教程高亮目标注册表 | `Register` / `Unregister` / `Get` | Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:24 |
| `VNCgUnlocks` | CG 解锁全局记录 | `Unlock` / `IsUnlocked` / `All` | Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs:14 |
| `VNTutorialSeen` | 教程已看全局记录 | `Has` / `Mark` / `ResetAll` / `Enabled` | Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs:19 |
| `VNSystemUiSkinUtility` | 皮肤 prefab 安全实例化 | `Prefab` / `Instantiate<T>` | Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs:27 |
| `VNGameConfig` | 全局内容配置 | `Active` / `ApplyList` / `FindSkin` | Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:34 |
...
| 名称 | 服务对象 | 提供 | 出处 |
|---|---|---|---|
| `VNBadmintonUi` | 羽毛球三个文件 | `CreateNode` / `CreateImage` / `CreateQuad` / `CreateLine` / `CreateText` / `Stretch` / `AnchorBottomCenter` | Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:44 |
| `VNPhotoBoothUi` | 大头贴六个文件 | `CreateImage` / `CreateText` / `CreateNode` / `Center` / `Stretch` + 配色常量 | Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs（贴纸组件在同档 `:17`） |
| 各模块私有的 `CreateImage/CreateText/Stretch` | 单个模块 | 同名同签名，各写一份 | 例 `VNQteModule.BuildUi` 附近、`VNShopModule.CreateButton`（Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs:383） |
...
```
> 出处：`VNBadmintonUi` 类注释（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:41）
...
| 组件 | 必填槽位 | 出处 |
|---|---|---|
| `VNDialogueSkin` | 无（全可选） | Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:24 |
| `VNChoiceSkin` | container / buttonTemplate / buttonLabel | Assets/Project/Scripts/VNEffects/Script/VNChoiceSkin.cs:22 |

**系统菜单（全局主题，不进存档）**——全部继承 `VNSystemUiSkinBehaviour`：

| 组件 | 出处 |
|---|---|
| `VNTitleMenuSkin` | Assets/Project/Scripts/VNEffects/Script/VNTitleMenuSkin.cs:9 |
| `VNConfigPanelSkin` | Assets/Project/Scripts/VNEffects/Script/VNConfigPanelSkin.cs:9 |
| `VNSaveLoadSkin` | Assets/Project/Scripts/VNEffects/Script/VNSaveLoadSkin.cs:9 |
| `VNBacklogSkin` | Assets/Project/Scripts/VNEffects/Script/VNBacklogSkin.cs:9 |
| `VNQuickToolbarSkin` | Assets/Project/Scripts/VNEffects/Script/VNQuickToolbarSkin.cs:13 |
| `VNStatsHudSkin` | Assets/Project/Scripts/VNEffects/Script/VNStatsHudSkin.cs:6 |
| `VNStatsPanelSkin` | Assets/Project/Scripts/VNEffects/Script/VNStatsPanelSkin.cs:8 |
| `VNInventorySkin` | Assets/Project/Scripts/VNEffects/Script/VNInventorySkin.cs（未逐行核对行号） |
| `VNCgGallerySkin` | Assets/Project/Scripts/VNEffects/Script/VNCgGallerySkin.cs（同上） |
| `VNPlanSkin` | Assets/Project/Scripts/VNEffects/Script/VNPlanSkin.cs（同上） |
| `VNResultPopupSkin` | Assets/Project/Scripts/VNEffects/Script/VNResultPopupSkin.cs:16 |
| `VNTutorialSkin` | Assets/Project/Scripts/VNEffects/Script/VNTutorialSkin.cs:18 |

**行/卡模板（不继承基类，只提供 `IsValid`）**：

| 组件 | 出处 |
|---|---|
| `VNSaveSlotSkin` | Assets/Project/Scripts/VNEffects/Script/VNSaveSlotSkin.cs:8 |
| `VNBacklogEntrySkin` | Assets/Project/Scripts/VNEffects/Script/VNBacklogEntrySkin.cs:7 |
| `VNStatsHudEntrySkin` | Assets/Project/Scripts/VNEffects/Script/VNStatsHudEntrySkin.cs:7 |
| `VNStatsPanelRowSkin` | Assets/Project/Scripts/VNEffects/Script/VNStatsPanelRowSkin.cs:7 |
| `VNToolbarActionSlot` | Assets/Project/Scripts/VNEffects/Script/VNToolbarActionSlot.cs:8 |
| `VNCgCellSkin` / `VNInventoryRowSkin` / `VNInventorySlotSkin` / `VNPlanSlotRowSkin` / `VNPlanActionRowSkin` | 同目录同名档案 |

### 16.4 通用组件

| 组件 | 用途 | 出处 |
|---|---|---|
| `VNTypewriterText` | TMP 逐字打字机 | Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:14 |
| `VNNameplateStyle` | 名牌样式（纯数据 + 静态 Apply） | Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:53 |
| `VNTouchCursor` | 自绘跟随光标（可换图/摆动/发光） | Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:21 |
| `VNBadmintonQuad` | 四点自由四边形（画梯形） | Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:17 |
| `VNPhotoStickerItem` | 可拖/缩/转/删的贴纸 | Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothUi.cs:17 |
| `VNPhotoDoodleInput` | 指针 → 画布 uv 的输入板 | Assets/Project/Scripts/VNEffects/Script/VNPhotoDoodle.cs:12 |
| `VNTutorialAnchor` | prefab 上的教程锚点自动登记 | Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:82 |
| `VNTutorialMask` | 暗幕挖洞（最多 4 洞） | Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:25 |
| `VNPhotoCapture` | 取景框截图（静态） | Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:19 |
...
| 机制 | 长什么样 | 调用 | 出处 |
|---|---|---|---|
| **Toast 中性卡** | 左上角滑入卡片，1.6 秒后滑出 | `VNToast.Show("已保存")` | Assets/Project/Scripts/VNEffects/Script/VNToast.cs:53 |
| **Toast 带图标色条** | 图标 + 主题色 + 左侧涨跌竖条 | `VNToast.Show(msg, icon, iconColor, accent, hold)` | Assets/Project/Scripts/VNEffects/Script/VNToast.cs:62 |
| **模式标签** | 右上角常驻 "AUTO ▶" / "SKIP ▶▶" | `VNToast.SetMode(label)` | Assets/Project/Scripts/VNEffects/Script/VNToast.cs:85 |
| **角标** | 模式标签下一行，任务可领取数 | `VNToast.SetBadge(label)` | Assets/Project/Scripts/VNEffects/Script/VNToast.cs:92 |
| **属性飘字** | HUD 条目上方 "+3" 上飘淡出 | `VNStatsHud.SpawnFloatingDelta`（内部） | Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:386 |

**Toast 的堆叠机制**（`VNToast` 类注释，
`Assets/Project/Scripts/VNEffects/Script/VNToast.cs:15`）：
...
```
> 出处：`VNToast.MaxCards`（Assets/Project/Scripts/VNEffects/Script/VNToast.cs:24）
...
Relayout(false);
```
> 出处：`VNToast.Show`（Assets/Project/Scripts/VNEffects/Script/VNToast.cs:65）
...
```
> 出处：`VNToast.Relayout`（Assets/Project/Scripts/VNEffects/Script/VNToast.cs:135）
...
```
> 出处：`VNToast` 类注释（Assets/Project/Scripts/VNEffects/Script/VNToast.cs:21）

### 17.2 强调类（把视线拉过去）

| 机制 | 实现 | 出处 |
|---|---|---|
| **数值滚动** | `DOVirtual.Int(from, to, 0.45f, ...)` | `VNStatsHud.RollValue`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:377） |
| **数字弹跳 + 染色** | `DOScale(1.35→1, OutBack)` + `DOColor` | `VNStatsHud.RefreshHud`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:347） |
| **图标弹跳** | `DOPunchScale(0.35, 0.42s, 9, 0.7)` | `VNStatsHud.RefreshHud`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:357） |
| **面板弹跳（连打）** | `DOPunchScale(0.04, 0.15s, 8, 0.6)` | `VNQteModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs:68） |
| **面板弹入** | `localScale 0.92 → DOScale(1, 0.28s, OutBack)` | `VNShopModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNShopModule.cs:151）、`VNPlanModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs:249）、`VNInteractionModule.BuildUi`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:810） |
| **确认框弹入** | `localScale 0.8 → DOScale(1, 0.2s, OutBack)` | `VNBadmintonModule.OpenConfirm`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:526） |
| **卡片翻页弹入** | 下沉 18px 回位 + 淡入 | `VNTutorialPlayer.ApplyStep`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:308） |
| **对话框出现** | 下方 70px 滑入 + 淡入，`OutBack 1.2` | `VNDialogueBox.Show`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:608） |
| **选项错落飞入** | 右侧 90px 滑入，延迟 `index * 0.09` | `VNChoicePanel.PlayEntrance`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:214） |
| **标题上浮** | 36px 下方回位，1.1 秒 OutCubic | `VNTitleMenu.PlayEntrance`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:281） |
...
| 机制 | 实现 | 出处 |
|---|---|---|
| **继续箭头呼吸** | `DOAnchorPosY(-7, 0.55s, InOutSine, Yoyo, -1)` | `VNDialogueBox.ShowArrow`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:740） |
| **悬停扫光 + 微放大** | `fx.PlayShine(0.5f)` + `DOScale(1.045)` | `VNChoicePanel.FinishButton`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:409） |
| **选中闪光 + 落选溶解** | `DOFlash` + `DODissolve(0, 0.45s)` | `VNChoicePanel.Choose`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:443） |
| **对话框流光边框** | `VNImageEffectController.StartShineLoop(2.2, 1.3)` | `VNDialogueBox.Bind`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:314） |
| **工具条激活态** | 换一个 Graphic 的颜色 | `VNToolbarActionSlot.SetActiveState`（Assets/Project/Scripts/VNEffects/Script/VNToolbarActionSlot.cs:17） |
| **页签选中态** | 两个 targetGraphic 换色 | `VNSaveLoadPanel.SetMode`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:179） |
| **置灰不可选** | 底色 ×0.55、文字 alpha 0.45、Button.interactable=false | `VNChoicePanel.CreateDefaultButton`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:326） |
| **教程洞口描边呼吸** | shader 参数 × `sin(unscaledTime*3.4)` | `VNTutorialMask.Sync`（Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:168） |
| **倒计时最后冲刺** | 变红 + 脉动 + 轻抖 | `VNQuizModule.UrgentSeconds`（Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:44） |

### 17.4 听觉反馈

| 机制 | 实现 | 出处 |
|---|---|---|
| **打字音（带节流 + 随机音高）** | `VNAudio.TypeTick()` | Assets/Project/Scripts/VNEffects/Script/VNAudio.cs:370 |
| **QTE 连打音** | `_audio.PlaySe("se1")` | `VNQteModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs:65） |
| **教程步骤音** | `audio?.PlaySe(step.se)` | `VNTutorialPlayer.PlayStepSe`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:344） |
| **擦拭循环音变 pitch** | 一段 1 秒滤波白噪改 pitch | （见 CLAUDE.md 的 `VNFogSfx` 说明；本次未逐行核对） |

### 17.5 确认与防误触

| 机制 | 实现 | 出处 |
|---|---|---|
| **覆盖存档二次确认** | `ShowConfirm(msg, onYes)` + `SetAsLastSibling` | `VNSaveLoadPanel.ShowConfirm`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:255） |
| **退出游戏二次确认** | prefab 里的 `quitConfirmRoot` | `VNTitleMenu.BindCustomSkin`（Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:236） |
| **认输二次确认** | 模块内自建暗幕 + 两个按钮 | `VNBadmintonModule.OpenConfirm`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:495） |
| **AI 聊天 ESC 确认** | 唯一吃射线的自绘层 | `VNAiTalkModule` 确认框段（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:595） |
| **结算弹窗输入延迟** | `inputDelay = 0.4f`，`_shownAt` 初始 MaxValue | `VNResultPopupModule.inputDelay`（Assets/Project/Scripts/VNEffects/Script/VNResultPopupModule.cs:13） |
| **教程每步最短停留** | `MinStepTime = 0.22f` | `VNTutorialPlayer.MinStepTime`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:44） |
| **选中后延迟 0.8 秒才回调** | `DOVirtual.DelayedCall(0.8f, ...)` | `VNChoicePanel.Choose`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:456） |
| **淡出跨两帧再解除暂停** | `WaitForCompletion()` | `VNTutorialPlayer.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:206） |
...
```
> 模板来源：`VNBacklog`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:9）
> 与 `VNQuestLog`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:18）

**2. 在 Runner 里注册**（三处）
- 加字段（照 `VNScriptRunner._questLog`，
  `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:62` 的位置）
- 在 `Start` 里「先找、找不到就 new」（照 `VNScriptRunner.Start`，
  `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:132` 那一段）
- 加 `RequestAchievements()`（照 `VNScriptRunner.RequestQuestLog`，
  `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1709`，
  记得 `if (_eventActive) return;`）

**3. 在 Update 的模态链里插两段**
- 「面板打开时只响应 K / Esc」的分支（照
  `VNScriptRunner.Update` 的任务日志分支，
  `Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2032`）
- 「K 键打开」（照同方法 `:2104`）
- **位置要选对**：放在 CG 画廊分支之后、标题菜单之前

**4. 补 `IsSecretPhotoIconAllowed`**
在 `VNScriptRunner.IsSecretPhotoIconAllowed`
（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1799`）里加一行
`if (_achievements != null && _achievements.IsOpen) return false;`
——**这一步最容易漏**。

**5. 可选：加进快捷功能条**
- `VNToolbarAction` 枚举加一项（`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbarSkin.cs:6`）
- `VNQuickToolbar.LabelFor` 加一个 case（`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:83`）
- `VNQuickToolbar.Execute` 加一个 case（`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:104`）
- 在工具条 prefab 里复制一个按钮、改枚举值

**6. 可选：加皮肤支持**
- 建 `VNAchievementSkin : VNSystemUiSkinBehaviour`（照
  `VNBacklogSkin`，`Assets/Project/Scripts/VNEffects/Script/VNBacklogSkin.cs:9`）
- `VNSystemUiSkinSet` 加字段（`Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs:11`）
- `VNSystemUiSkinExporter.ExportAll` 加一个 `BuildAchievement()`（
  `Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs:23`）

**7. 本地化**
所有玩家可见文字走 `VNLocale.T("achievement.title")`，
翻译写进 `Assets/Resources/VNLocale/ui.zh.txt` 等三份表
（格式见 `VNLocale.ParseTable`，`Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:124`）。
...
```
> 骨架来源：`VNQteModule`（Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs:19）

**2. 遵守三铁律**
- 只碰自己的 UI 子树与 `VNFlags`；要破例就在类注释里写明边界与还原路径
- 计时 `VNTime.Delta`，Tween 全部 `SetUpdate(true)`
- Tween 全部 `SetLink(gameObject)`

**3. 决定射线策略**
- 独占全屏 → 铺一层吃射线的暗幕
- 要让对话框继续工作 → **不铺暗幕**，自绘一律 `raycastTarget = false`
  （照 `VNInteractionModule.BuildUi`，
  `Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:761`）

**4. 登记到注册表**
写一个 Installer（照 `VNQuestBoardInstaller.Install`，
`Assets/Project/Scripts/VNEffects/Editor/VNQuestBoardInstaller.cs:23`）：
- 找场景里的 `VNEventRegistry`，找不到就弹对话框提示
- 建一个**禁用的、带 RectTransform 的** 模板物体挂到注册表下
- 加进 `registry.modules`
- 顺便把工程里的定义资产扫进 `VNGameConfig`
- 结尾 `VNAssetLibraryEvents.RaiseChanged()`
  （`Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs:26`）

**5. 教程锚点（可选）**
`VNTutorialAnchors.Register(id, rect)` + `OnDestroy` 里 `Unregister`
（照 `VNBadmintonModule.RegisterTutorialAnchors`，
`Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:376`）。

**6. 剧本编辑器 Schema**
在 `VNScenarioSchema` 的 `EventVariants` 表里补一行，把 `ctx.Kw(...)` 用到的参数登记上
（见 CLAUDE.md 与第十九章 19.3）。

**7. 结果契约与 Lint**
- 结果名用 const 字符串（照 `VNAiTalkModule.OutcomeUp`，
  `Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs:42`）
...
- 对话框/选项：Tools → VN Effects → UI 皮肤 UI Skins → 导出皮肤模板
  （`VNUiSkinExporter.ExportAll`，`Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs:30`）
- 系统菜单：Tools → VN Effects → UI 皮肤 UI Skins → 系统主题：导出默认模板
  （`VNSystemUiSkinExporter.ExportAll`，`Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs:23`）

**2. 复制 prefab 改**
- 随便加装饰节点，代码只认槽位
- 记住：对话框皮肤会被拉满整个画布，`panel` 想放哪就锚哪
  （`VNDialogueSkin` 类注释，`Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:16`）
- 选项模板要保持**禁用**、根上要有 Image
  （`VNChoiceSkin` 类注释，`Assets/Project/Scripts/VNEffects/Script/VNChoiceSkin.cs:17`）

**3. 登记**
- 对话框/选项：`VNGameConfig.dialogueSkins` / `choiceSkins` 里加 id + prefab
  （`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:126`）
- 系统菜单：把新的 `VNSystemUiSkinSet` 资产挂到 `VNGameConfig.systemUiSkin`
  （`Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:131`）

**4. 剧本切换**
`ui dialogue <id>` / `ui choice <id>` / `ui name <样式>`
（`VNScriptRunner.Dispatch` 的 ui 分支，
`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2401`）。

**5. 验证**
如果必填槽位缺失，运行时会报中文错误并指出缺哪个
（`VNSystemUiSkinUtility.Instantiate`，
`Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinBehaviour.cs:42`）。
...
```
> 出处：`VNScenarioEditorWindow` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:22）
...
```
> 出处：`VNScenarioEditorWindow._docText`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:36）
...
```
> 出处：`VNScenarioEditorWindow.OnBeforeSerialize`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:279）
...
```
> 出处：`VNScenarioEditorWindow.RestoreAfterDomainReload`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:264）
...
>    `Dictionary` 都不行）。
>
> **同一模式在 `VNAiStudioWindow` 里也用了**
> （类注释，`Assets/Project/Scripts/VNEffects/Editor/VNAiStudioWindow.cs:18`），
> 而且注释里同样强调「★ 加新窗口状态时必须同时改 OnBeforeSerialize 和 OnEnable」
> （`Assets/Project/Scripts/VNEffects/Editor/VNAiStudioWindow.cs:23`）。
...
```
> 出处：`VNScenarioEditorWindow.BuildList`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:331）
...
```
> 出处：`VNScenarioEditorWindow.RowHeight`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1592）
...
```
> 出处：`VNScenarioEditorWindow.IsRowCollapsed`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1625）
...
```
> 出处：`VNScenarioEditorWindow.IsHiddenRow`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1618）
...
```
> 出处：`VNScenarioEditorWindow._pendingInsertAt`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:68）
...
```
> 出处：`VNScenarioEditorWindow._pendingNewRow`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:74）
...
```
> 出处：`VNScenarioEditorWindow._popupResults`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:79）

> **这段揭示了一个设计约束**：
> 有些值存在 `VNRow.values` 字典里（回调可以直接写），
...
```
> 出处：`VNScenarioEditorWindow.PlayFromSourceLine`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1458）
...
```
> 出处：`VNScenarioEditorWindow.AutoSaveBeforePlay`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1504）
...
```
> 出处：`VNScenarioEditorWindow.HotReplay`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1511）
...
```
> 出处：`VNPlayFromLineBridge`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:3686）
...
```
> 出处：`VNPlayFromLineBridge.OnPlayModeStateChanged`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:3712）
...
```
> 出处：`VNPlayFromLineBridge.TryStartRunner`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:3727）
...
```
> 出处：`VNScenarioEditorWindow.OnInspectorUpdate`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:312）
...
```
> 出处：`VNScenarioEditorWindow.OnEnable`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:240）
> 与 `VNScenarioEditorWindow.OnDisable`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:298）

`VNAssetLibraryEvents` 的类注释把风险说得很清楚：
```csharp
/// 【订阅方必须退订】
...
```
> 出处：`VNAssetLibraryEvents` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs:14）
...
```
> 出处：`VNAssetLibraryEvents.RaiseChanged`（Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs:25）
...
```
> 出处：`VNScenarioEditorWindow.OnFocus`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:292）
...
```
> 出处：`VNScenarioEditorWindow.EventParamLine`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1606）
...
**两个复用件**：`VNSearchItem`（一条候选，
`Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:11`）
...
/// 先 Use() 掉才轮得到我们。
```
> 出处：`VNSearchListView` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:35）
...
```
> 出处：`VNSearchListView.Draw`（Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:113）
...
```
> 出处：`VNSearchListView.Reset`（Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:83）
...
```
> 出处：`VNSearchListView` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:39）
...
/// 音频同理：波形 + 一键试听，不靠名字猜。
```
> 出处：`VNAssetBrowserWindow` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs:11）
...
```
> 出处：同上（`Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs:18`）
...
```
> 出处：`VNAssetBrowserWindow.Cats`（Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs:51）
...
GUI.EndScrollView();
```
> 出处：`VNAssetBrowserWindow` 网格绘制段（Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs:397）
...
```
> 出处：`VNAssetBrowserWindow` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs:22）
...
```
> 出处：`VNAssetUi` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:17）
...
```
> 出处：`VNAssetUi.DrawSpriteThumb`（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:74）
...
```
> 出处：`VNAssetUi.ProbeAudioUtil`（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:166）
...
```
> 出处：`VNAssetUi.CanPreviewAudio`（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:193）
...
static readonly Dictionary<UnityEngine.Object, double> _previewDeadline =
    ...
```
> 出处：`VNAssetUi._previewDeadline`（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:302）
...
```
> 出处：`VNGameConfigEditor` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs:15）
...
```
> 出处：`VNGameConfigEditor.CollectOrphans`（Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs:70）
...
```
> 出处：`VNEntryDrawerBase` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs:8）
...
```
> 出处：`VNGameConfig.UiSkinEntry` 上方注释（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:112）
...
```
> 出处：`VNEntryDrawerBase.OnGUI`（Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs:50）
...
> 本专案三条都做到了（`VNEntryDrawerBase.GetPropertyHeight`，
> `Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs:30`）。
...
```
> 出处：`VNAssetTheme` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs:9）
...
```
> 出处：同上（`Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs:15`）
...
```
> 出处：`VNAssetTheme.Tint`（Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs:252）
...
```
> 出处：同上（`Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs:20`）
...
```
> 出处：`VNAssetTheme.Enabled`（Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs:56）
...
```
> 出处：`VNFogTuneWindow.AssumedDragSpeed`（Assets/Project/Scripts/VNEffects/Editor/VNFogTuneWindow.cs:26）

**关键是「用运行时同一份代码」**：
```csharp
readonly VNFogMask _mask = new VNFogMask();
```
> 出处：`VNFogTuneWindow._mask`（Assets/Project/Scripts/VNEffects/Editor/VNFogTuneWindow.cs:37）

> **判断「要不要写调参窗口」的标准**：
> - 参数直接决定手感 + 参数之间有非线性交互 + 无法一次调对 → **要写**
> - 参数只影响外观、改了立刻能看到 → 不用写，Inspector 就够
> - 参数在 Play Mode 里能实时改并看到效果 → 用 `#if UNITY_EDITOR` 的实时重读
>   （羽毛球的做法，`VNBadmintonModule.Update`，
>   `Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:423`）
>
> **而「预览用运行时同一份代码」是不可妥协的**——
> 另写一套近似的预览，调出来的参数进游戏就不对，那这个窗口就是负资产。
> 本专案在三处贯彻了这条：擦雾掩码、震动等级表（`VNShakeSpec`）、
> 镜头缩放公式（`VNCamera.CharacterScaleFor` / `ContainerZoomFor`）。
...
```
> 出处：`VNTouchZoneEditorWindow` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNTouchZoneEditorWindow.cs:8）
...
```
> 出处：`VNTouchZoneEditorWindow._undo`（Assets/Project/Scripts/VNEffects/Editor/VNTouchZoneEditorWindow.cs:42）
...
```
> 出处：`VNTouchZoneEditorWindow.ZoneList`（Assets/Project/Scripts/VNEffects/Editor/VNTouchZoneEditorWindow.cs:53）
...
```
> 出处：`VNScenarioEditorWindow._undoStack`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:120）
...
```
> 出处：`VNCamseqEditorWindow.Waypoint`（Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs:27）
...
static readonly GUIContent ZoomModeLabel = new GUIContent("缩放模式",
    ...
    ...);
```
> 出处：`VNCamseqEditorWindow.ZoomModeLabel`（Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs:57）
...
```
> 出处：`VNCamseqEditorWindow.ZoomModeNames`（Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs:52）
...
```
> 出处：`VNTextPromptWindow.Prompt`（Assets/Project/Scripts/VNEffects/Editor/VNTextPromptWindow.cs:20）
...
八个 Installer（`VNQuizInstaller`、`VNLiquidInstaller`、`VNPhotoBoothInstaller`、
`VNInteractionInstaller`、`VNBadmintonInstaller`、`VNFogWipeInstaller`、
`VNQuestBoardInstaller`、`VNSecretPhotoInstaller`、`VNAiTalkInstaller`）遵循同一套模式：
...
/// 重复执行安全：已经装过就只刷新任务列表。
```
> 出处：`VNQuestBoardInstaller` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNQuestBoardInstaller.cs:9）
...
```
> 出处：`VNQuestBoardInstaller.Install`（Assets/Project/Scripts/VNEffects/Editor/VNQuestBoardInstaller.cs:25`）
...
```
> 出处：`VNQuestBoardInstaller.Install`（Assets/Project/Scripts/VNEffects/Editor/VNQuestBoardInstaller.cs:56）
...
**「增量装机」vs「重建场景」**：
`VNEffectsDemoSetup.CreateScriptDemoScene`
（`Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:451`）是**重建**——
...
| # | 症状 | 原因 | 修法 | 出处 |
|---|---|---|---|---|
| 1 | 「对话框藏了，一排圆按钮还浮在半空」 | `overrideSorting` 的子 Canvas 打断父 CanvasGroup 的 alpha 传播 | 子 Canvas 自己挂 CanvasGroup，由父级显式通知 | `VNQuickToolbar.SetVisible`（Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:134） |
| 2 | 嵌套 Canvas 上的按钮点不到 | `overrideSorting` 的 Canvas 需要自己的 GraphicRaycaster | `AddComponent<GraphicRaycaster>()` | `VNChoicePanel.Build`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:91）、`VNEventRegistry.EnsureLayer`（Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:87） |
| 3 | 「任务标题左边缺字」 | 拉伸锚点下 `sizeDelta` 默认 (100,100) = 比父级宽 100px，被 RectMask2D 裁掉 | 显式 `sizeDelta = Vector2.zero` | `VNQuestLog.Build`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:307） |
| 4 | 描边环被开窗裁掉 | `Mask` 会裁掉所有子物体 | 描边环放在 Mask **外面** | `VNPhotoBoothModule.BuildViewFinder`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:558） |
| 5 | 属性飘字被 HUD 皮肤裁掉一半 | 挂在带 Mask 的条目下 | 挂 Canvas 根 + 坐标换算 | `VNStatsHud.SpawnFloatingDelta`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:381） |
| 6 | 「人后贴纸永远点不到」 | 拖动板 `raycastTarget=true` 压在贴纸之上吃掉射线 | 拖动板压到最底层 | `VNPhotoBoothModule.BuildViewFinder`（Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:535） |
| 7 | 「点了个买不起的选项，剧情居然往下走了」 | 置灰选项不吃射线，点击穿透到背景 | 置灰项仍 `raycastTarget=true`，只禁 Button | `VNChoicePanel.CreateDefaultButton`（Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:330） |
| 8 | 「点 Slider 轨道没反应」 | Slider 根上没有 Graphic 就收不到轨道点击 | 补一个 `Color.clear` 的 Image | `VNConfigPanel.BindSlider`（Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:182） |
| 9 | 「一切状态都对但就是不画」 | 自定义 `Graphic` 子类没有 CanvasRenderer；`RequireComponent` 不走继承链 | 子类上再写一遍 `[RequireComponent]`，并在创建时显式 `AddComponent<CanvasRenderer>()` | `VNBadmintonQuad` 类注释（Assets/Project/Scripts/VNEffects/Script/VNBadmintonUi.cs:12） |
| 10 | 全屏点击被拦掉 | `IsPointerOverGameObject()` 对任何 raycastTarget 都为 true，而本项目全屏皆 UI | `RaycastAll` + 按 `Selectable` 过滤 | `VNScriptRunner.IsPointerOverInteractiveUi`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2202） |
| 11 | 面板被 Overlay 画布压住、转场盖不住 | Screen Space - Overlay 永远压在 Screen Space - Camera 之上 | 需要被转场盖住 / 吃 Bloom 的层挂主 Canvas | `VNTutorialPlayer` 类注释（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:25）、`VNInterludeScreen` 类注释（Assets/Project/Scripts/VNEffects/Script/VNInterludeScreen.cs:25） |
| 12 | 滚动列表打开时位置不对 | `SetActive(true)` 后 content 尺寸还没算 | `Canvas.ForceUpdateCanvases()` 再设 `verticalNormalizedPosition` | `VNBacklog.Open`（Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs:68）、`VNQuestLog.Open`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:230） |
| 13 | 确认框被别的东西盖住 | 同 Canvas 内绘制顺序 = Hierarchy 顺序 | `transform.SetAsLastSibling()` | `VNSaveLoadPanel.ShowConfirm`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:261） |
| 14 | 空 RawImage 显示成纯白且忽略 color | `RawImage.texture == null` 时的默认行为 | 用 `Texture2D.whiteTexture` 当底再染色 | `VNSaveLoadPanel.CreateCustomSlotCard`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:216） |

### 20.2 TextMeshPro 层

| # | 症状 | 原因 | 修法 | 出处 |
|---|---|---|---|---|
| 15 | 改一个名牌样式，全游戏文字一起变 | 改的是 `fontSharedMaterial`（字体资产自带的） | 一律走 `text.fontMaterial`（自动 new 实例） | `VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:520） |
| 16 | underlay 参数设了没反应 | 没有 `EnableKeyword("UNDERLAY_ON")` | 改 underlay 前先 EnableKeyword | `VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:539） |
| 17 | 描边推到某个值就不再变粗 | padding 是描边厚度的天花板（`outlineWidth ×(padding+1)×(字号/采样点)`） | 装饰字体单开一套大 padding 资产 | `VNFont.DisplayAtlasPadding`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:71） |
| 18 | padding 调大反而更糊 | padding 占采样点比例过大会挤掉字形分辨率（64pt+padding24=37% 实测更差） | 采样点与 padding 等比例，保持 ~18% | `VNFont.DisplaySamplePointSize`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:62） |
| 19 | 切语言后名牌变回光板白字 | 换 font 会让 TMP 丢掉材质实例 | 订阅 `VNFont.DisplayFontChanged` 重新上样式 | `VNDialogueBox.HookLocaleEvents`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:466） |
| 20 | 切语言把名牌 Heavy 字体也换成正文字体，粗描边垮掉 | 正文与装饰字体没分开管理 | 两个 HashSet 分别替换 | `VNFont.HandleLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:279） |
| 21 | Heavy 字重叠伪粗后描边糊了 | TMP 的 Bold 是 SDF 膨胀 | 用装饰字体时 `fontStyle = Normal` | `VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:486） |
| 22 | HDR 发光写了没用 | uGUI 顶点色被钳到 1，Bloom 阈值也是 1.0 | 走材质 `_FaceColor`；代价是与顶点渐变二选一 | `VNNameplateStyle.ApplyTo`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:497） |
| 23 | 金/银浮雕没效果也不报错 | Mobile 版 TMP shader 没有 Bevel 属性 | `HasProperty` 挡掉并**只警告一次** | `VNNameplateStyle.ApplyBevel`（Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs:559） |
| 24 | 刚创建的文字 `preferredWidth` 是 0 | 该属性要等一次布局 | 用 `GetPreferredValues(text)`（同步） | `VNToast.BuildCard`（Assets/Project/Scripts/VNEffects/Script/VNToast.cs:245）、`VNDialogueBox.ResizeNameTag`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:430） |
| 25 | 打字机顶点索引对不上 | 没先 `ForceMeshUpdate()` | 读 textInfo 前先 ForceMeshUpdate | `VNTypewriterText.LateUpdate`（Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:75） |
| 26 | 空行台词卡死剧本 | 打字机永远等不到「所有字显现」 | `visibleIndex == 0` 视作立即播完 | `VNTypewriterText.LateUpdate`（Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:107） |
| 27 | 台词播完后仍每帧重建网格 | 没有停止 `_animating` | `if (!_playing && !anyPartial) _animating = false;` | `VNTypewriterText.LateUpdate`（Assets/Project/Scripts/VNEffects/VNTypewriterText.cs:121） |
| 28 | `lineSpacing` 设 1.25 结果行距几乎没变 | TMP 的 lineSpacing 单位是字号百分比 | 要 1.25 倍就写 25 | `VNDialogueBox.BuildDefaultSkin`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:189） |

### 20.3 生命周期 / 状态层

| # | 症状 | 原因 | 修法 | 出处 |
|---|---|---|---|---|
| 29 | 进 Play 后对话框有两层，底下那层是死的 | 编辑期误触发 `Build()` 让生成物存进了场景 | 按固定名字 `Find` 捡回来复用 | `VNDialogueBox.BuildDefaultSkin`（Assets/Project/Scripts/VNEffects/VNDialogueBox.cs:138） |
| 30 | 语言切换后面板再也建不出来 | 销毁 Canvas 但没清缓存字段，Unity 伪 null 让 `if (_panel != null) return;` 提早返回 | 所有缓存字段都要置 null | `VNSaveLoadPanel.OnLanguageChanged`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:44） |
| 31 | 「游戏永久卡死」 | `VNPause` 句柄漏了 Release | 句柄绑宿主自动失效 + 七层保险 | `VNPause` 类注释（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:15） |
| 32 | 一进 Play 就是暂停状态 | 关闭域重载后静态字段不清 | `[RuntimeInitializeOnLoadMethod(SubsystemRegistration)]` 重置 | `VNPause.ResetStatics`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:113） |
| 33 | 「玩家的鼠标指针永久消失」 | `Cursor.visible = false` 后漏了还原 | `Dispose` 在四处调（Finish/Cancel/OnDestroy/OnDisable） | `VNTouchCursor.Dispose`（Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs:213） |
| 34 | 互动结束后部位框留在角色脸上 | 框挂在立绘下面，不在模块子树里 | 模块销毁时显式删 | `VNInteractionModule.BuildZoneOverlay`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs:970） |
| 35 | 事件结束后剧本跳到莫名其妙的地方 | 模块用 `RunInlineCo` 播了控制流命令 | 黑名单挡掉 jump/choice/call/event… | `VNScriptRunner.InlineBlockedKeywords`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:304） |
| 36 | 偷拍插话后玩家点击不推进 | 嵌套 SayCo 把外层 `_waitingAtSay` 清掉了 | 用 `SayOutOfScript` 绕过状态机 | `VNScriptRunner.SayOutOfScript`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1828） |
| 37 | 教程结束那一下 ESC 被羽毛球当成认输 | `wasPressedThisFrame` 同帧对所有读取者为 true | 淡出跨两帧再解除暂停 | `VNTutorialPlayer.PlayCo`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:203） |
| 38 | 教程冻结了但还能挥拍 | `VNPause` 检查放在 ReadInput 之后 | **必须在 ReadInput 之前 return** | `VNBadmintonModule.Update`（Assets/Project/Scripts/VNEffects/Script/VNBadmintonModule.cs:412） |
| 39 | `Time.timeScale = 0` 对小游戏无效 | 模块用 unscaled 计时（为了躲 Skip 加速） | `VNPause` + `VNTime.Delta` | `VNPause` 类注释（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:9） |
| 40 | 切窗口回来球瞬移过整个球场 | 巨大的 `unscaledDeltaTime` | `VNTime.MaxStep = 0.05f` 钳制 | `VNTime.MaxStep`（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:139） |
| 41 | 模块结果偶尔跳错分支 | `Done` 被调用两次 | `_finished` 守卫 | `VNEventModule.Done`（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:79） |
| 42 | 事件结束后 hideHUD keep 的段落被弹回 UI | 瞬态隐藏写进了持久状态 | 瞬态用独立 bool，可见性 = 合成函数 | `VNScriptRunner.ApplyUiHidden`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1784） |
| 43 | 读档后界面莫名其妙全没了 | 非锁定隐藏被存进了存档 | 只存 `hideHUD keep` 的锁定隐藏 | `VNSaveData.uiHidden`（Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:61） |
| 44 | 读档时 UI 重建几百次 | 直接在 `VNFlags.Changed` 里刷新 | 标脏 + 下帧一次 | `VNFlags.Changed`（Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:20） |
| 45 | 改了 VNGameConfig 但「没反应」 | 下游还各自缓存了从它读出来的值 | `ClearCache` 连带清下游 | `VNGameConfig.ClearCache`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:69） |
| 46 | 静态委托退订误伤了后来注册的实例 | 直接置 null | `ReferenceEquals(delegate.Target, this)` 才置 null | `VNInventory.OnDestroy`（Assets/Project/Scripts/VNEffects/Script/VNInventory.cs:47） |
| 47 | 两个实例先后登记同一 id，先销毁的抹掉后来者 | 反注册不检查当前登记的是不是自己 | `Unregister(id, rect)` 带 rect 校验 | `VNTutorialAnchors.Unregister`（Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:36） |

### 20.4 资源层

| # | 症状 | 原因 | 修法 | 出处 |
|---|---|---|---|---|
| 48 | 相册网格里的图变白块 | LRU 容量 12 < 同屏 30 张，正在用的被驱逐 | 缩略图单独一份不驱逐的小图缓存 | `VNPhotoAlbum.LoadThumbnail`（Assets/Project/Scripts/VNEffects/Script/VNPhotoAlbum.cs:271） |
| 49 | 打开相册几分钟后内存爆掉 | `Texture2D` 不受 GC 管理 | 四个出口都 Destroy + `ClearCache` | `VNSaveLoadPanel.ClearLoadedThumbnails`（Assets/Project/Scripts/VNEffects/Script/VNSaveLoadPanel.cs:283） |
| 50 | 抓图裁到左下角四分之一 | 高 DPI 屏上抓图尺寸 ≠ `Screen.width` | 按比例换算 | `VNPhotoCapture.Crop`（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:104） |
| 51 | 抓到的还是旧画面 | 关掉 UI 之后没等 Canvas 重画 | `yield return null;` + `WaitForEndOfFrame` | `VNPhotoCapture.Capture`（Assets/Project/Scripts/VNEffects/Script/VNPhotoCapture.cs:47） |
| 52 | 程序化贴图 `EncodeToPNG` 返回空 | `Apply(false, true)` 释放了 CPU 拷贝 | 经 RenderTexture + ReadPixels 取回可读副本 | `VNUiSkinExporter.BakeSprite`（Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs:64） |
| 53 | 打包后 shader 找不到 | 只被 `Shader.Find` 引用的 shader 会被剥掉 | 提供一个 Material 资产字段 | `VNTutorialPlayer.maskMaterial`（Assets/Project/Scripts/VNEffects/Script/VNTutorialPlayer.cs:52） |
| 54 | 9-slice 拉伸不对 | `Sprite.Create` 没传 border，或 meshType 是 Tight | 传 `new Vector4(22,22,22,22)` + `SpriteMeshType.FullRect` | `VNProceduralTextures.RoundedRectSprite`（Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:296） |
| 55 | 生成的贴图被当成场景资源保存 | 没设 hideFlags | `hideFlags = HideFlags.DontSave` | `VNProceduralTextures.RoundedRectSprite`（Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs:300） |
| 56 | `JsonUtility` 存 NaN 读回来是垃圾 | JsonUtility 不支持 NaN/Infinity | 另加一个 bool 表示「有没有设置」 | `VNSaveData.LiquidSave.sprayDirSet`（Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:87） |
| 57 | 静态图集调 `TryAddCharacters` 无效 | 只有动态图集支持按需添加 | 先判 `atlasPopulationMode == Dynamic` | `VNFont.PrewarmAsset`（Assets/Project/Scripts/VNEffects/Script/VNFont.cs:268） |

### 20.5 IMGUI / 编辑器层

| # | 症状 | 原因 | 修法 | 出处 |
|---|---|---|---|---|
| 58 | `ArgumentException: Getting control N's position...` | 在 KeyDown 里改了列表长度 | 记 pending，下一个 Layout 再执行 | `VNScenarioEditorWindow._pendingInsertAt`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:68） |
| 59 | 代码里改文本框的值不生效 | 控件持有 keyboardControl 时用它自己的 TextEditor 缓冲 | `GUIUtility.keyboardControl = 0` 再改 | `VNSearchListView.Reset`（Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:78） |
| 60 | 搜索框里按 ↑↓ 变成移光标、Enter 结束编辑 | TextField 先吃掉了按键 | 键盘处理排在 TextField **之前** + `e.Use()` | `VNSearchListView.Draw`（Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:113） |
| 61 | 进 Play Mode 后编辑器窗口内容全没了 | 域重载重建窗口，普通字段清空 | `[SerializeField]` + `ISerializationCallbackReceiver` | `VNScenarioEditorWindow` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:22） |
| 62 | `OnBeforeSerialize` 里调 Unity API 出问题 | 它跑在序列化线程语境下 | 只做纯 C# 运算，还原挪到 `OnEnable` | `VNScenarioEditorWindow.OnBeforeSerialize`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:281） |
| 63 | `DateTime` 存不进 `[SerializeField]` | Unity 序列化不支持 | 存 `long ticks` | `VNScenarioEditorWindow._fileTimeTicks`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:38） |
| 64 | 静态事件攥着已销毁窗口的引用 | 订阅了没退订 | `OnEnable` 订 / `OnDisable` 退 | `VNAssetLibraryEvents` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs:14） |
| 65 | 素材库广播每帧触发 | `ApplyModifiedProperties()` 每帧都调 | 只在返回 true 时 `RaiseChanged()` | `VNAssetLibraryEvents.RaiseChanged`（Assets/Project/Scripts/VNEffects/Editor/VNAssetLibraryEvents.cs:25） |
| 66 | 列表里几十张缩略图闪一片空白 | `AssetPreview.GetAssetPreview` 是异步的 | Sprite 走 `DrawTextureWithTexCoords` 同步画 | `VNAssetUi` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:17） |
| 67 | 某些资产的预览图永远等不到 | 有些资产就是没有预览 | 自己给 3 秒 deadline，到点放弃 | `VNAssetUi._previewDeadline`（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:302） |
| 68 | `IsLoadingAssetPreview(int)` / `GetInstanceID()` 报弃用错误 | Unity 6.5 起是 error 级弃用 | 不问 Unity，自己维护 deadline | 同上（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:302） |
| 69 | `sprite.textureRect` 抛异常 | 打包进图集且 tight 模式时取不到 | `try/catch` 退回 `sprite.rect` | `VNAssetUi.DrawSpriteThumb`（Assets/Project/Scripts/VNEffects/Editor/VNAssetUi.cs:75） |
| 70 | 列表里每个条目都重画一遍 Header，占 6~7 行 | `[Header]` 是 DecoratorDrawer，列表元素每项都画 | 挂 `CustomPropertyDrawer` 接管绘制，说明改 `[Tooltip]` | `VNEntryDrawerBase` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs:9） |
| 71 | 自定义 Drawer 里控件错位 | `indentLevel` 又给 rect 加了缩进 | 手算 rect 时 `indentLevel = 0` | `VNEntryDrawerBase.OnGUI`（Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs:51） |
| 72 | 换浅色底后字消失 | Dark 主题下 EditorStyles 文字是浅色的 | 覆盖八个状态的 textColor | `VNAssetTheme.Tint`（Assets/Project/Scripts/VNEffects/Editor/VNAssetTheme.cs:252） |
| 73 | 加了新字段但 Inspector 里看不见 | 分页 Inspector 没登记这个字段 | 自动收集孤儿字段落到「其他」页 | `VNGameConfigEditor.CollectOrphans`（Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs:70） |
| 74 | 隐藏空行时把语法错误行也藏了 | 按 `kind == Raw` 一刀切 | 只藏空行与 `#`，孤儿 `*` / `>` 必须显形 | `VNScenarioEditorWindow.IsHiddenRow`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1613） |
| 75 | 装机时模板没有 RectTransform，位置全乱 | `new GameObject("X")` 默认挂普通 Transform | `new GameObject(name, typeof(RectTransform))` | `VNQuestBoardInstaller.Install`（Assets/Project/Scripts/VNEffects/Editor/VNQuestBoardInstaller.cs:58） |
| 76 | 编辑器改资产不进 Undo、不保存 | 直接写字段 | 走 `SerializedObject` | `VNAssetBrowserWindow` 类注释（Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs:22） |

### 20.6 本地化层

| # | 症状 | 原因 | 修法 | 出处 |
|---|---|---|---|---|
| 77 | 切语言后分支走错 | 翻译参与了逻辑匹配 | 标识符永不翻译；选项按索引匹配 | `VNMapModule.Location.DisplayName`（Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs:41）、`VNScriptRunner.ChoiceCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2876） |
| 78 | 缺 key 时显示空白 | 没有回退链 | 当前语言 → 中文 → key 本身 | `VNLocale.T`（Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:76） |
| 79 | 占位符与参数不匹配时抛异常 | `string.Format` 抛 `FormatException` | try/catch 返回原始 format + 警告 | `VNLocale.T`（Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:94） |
| 80 | 语言切换时字体没换、名牌样式垮 | `LanguageChanged` 在换字体之前触发 | setter 里先 `VNFont.HandleLanguageChanged()` 再触发事件 | `VNLocale.Language`（Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:53） |
...
**现状**：`VNScriptRunner.IsSecretPhotoIconAllowed`
（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:1799`）
逐个列举了 9 个面板的 `IsOpen`。
...
**现状**：`VNQuickToolbar.Build`（`Assets/Project/Scripts/VNEffects/Script/VNQuickToolbar.cs:51`）、
`VNConfigPanel.Build`（`:106`）、`VNSaveLoadPanel.Build`（`:133`）、
`VNBacklog.Build`（`:99`）、`VNTitleMenu.Build`（`:198`）、
`VNStatsHud.BuildHud`（`:255`）、`VNResultPopupModule.OnLaunch`（`:84`）
...
```
`BuildEmergencyPanel` 可以做成 `VNSystemUiSkinUtility` 里的一个共用静态方法
（一个暗幕 + 一段红字 + 一个关闭按钮，二十行）。

**优先级：高**。玩家不该因为配置缺失而卡在一个按键上。

#### 债务 3：`VNToast` 的静态状态没有域重载重置

**现状**：`VNToast` 的 `_canvas` / `_stack` / `_cards`
（`Assets/Project/Scripts/VNEffects/Script/VNToast.cs:34`）是静态字段，
...
**建议**：照 `VNPause.ResetStatics`
（`Assets/Project/Scripts/VNEffects/Script/VNPause.cs:117`）补一段：
```csharp
#if UNITY_EDITOR
[RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
static void ResetStatics()
...
同样值得检查的静态状态：`VNProceduralTextures` 的贴图缓存
（有 `HideFlags.DontSave` 会被自动清，但静态字段仍指向已销毁对象——
不过每个 getter 都判空重建，所以是安全的）、
`VNFont` 的 `_cache`（同理，判空重建）、
`VNLocale` 的 `_tables`（纯数据，无 Unity 对象，安全）。
...
**现状**：见 8.11 节。D 键走 `RequestDiary()`（按需创建），
...
值分别是 3 / 3 / 5（`VNQuizModule.UrgentSeconds`，
`Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:44`；
`VNPhotoBoothModule.UrgentSeconds`，
`Assets/Project/Scripts/VNEffects/Script/VNPhotoBoothModule.cs:118`；
`VNFogWipeModule.UrgentSeconds`，
`Assets/Project/Scripts/VNEffects/Script/VNFogWipeModule.cs:65`）。
...
`VNScriptRunner.Update`（`Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2097`）
每帧调一次 `VNConfigPanel.WheelOpensBacklog`
（`Assets/Project/Scripts/VNEffects/Script/VNConfigPanel.cs:36`），
它内部是 `PlayerPrefs.GetInt`。

**建议**：缓存到一个静态 bool，setter 时同步更新。
**优先级：低**（Windows 上 PlayerPrefs 是内存缓存，成本极小）。

#### 观察 9：`VNPhotoStickerItem.Update` 每个贴纸一个
...
见 15.5 节。`VNGameConfig.systemUiSkin` 的直接引用会把 12 个 prefab 一起加载。
...
`VNTitleMenu.ApplyTitleStage`（`Assets/Project/Scripts/VNEffects/Script/VNTitleMenu.cs:107`）
按名字找物体，正是 `VNTutorialAnchors` 类注释里批评的做法。

**建议**：让 `VNStage` 持有它，或者把它纳入 `VNUiParts`。
**优先级：低**（后果只是提示文字没藏起来）。

#### 观察 12：`VNCamseqEditorWindow` 的下拉数组与运行时枚举靠注释同步

`ZoomModeNames`（`Assets/Project/Scripts/VNEffects/Editor/VNCamseqEditorWindow.cs:52`）
...
2. **`MaxTotalHeight` 写成 const 而不是 public 字段**
   （`VNChoicePanel.MaxTotalHeight`，`Assets/Project/Scripts/VNEffects/VNChoicePanel.cs:283`）——
   它是几何结论不是手感参数，做成序列化字段会被场景里的旧值盖掉。

3. **手工布局 SNS 而不用 LayoutGroup**
   （`VNSnsView` 类注释，`Assets/Project/Scripts/VNEffects/Script/VNSnsView.cs:18`）——
   聊天列表需要精确高度，LayoutGroup 的重建时序不可控。

4. **搜索只做子串包含不做模糊匹配**
   （`VNSearchListView` 类注释，`Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs:42`）——
   行为可预期比「聪明」重要。

5. **教程暗幕吃掉洞内点击（只读演示）**
   （`VNTutorialMask.Awake`，`Assets/Project/Scripts/VNEffects/Script/VNTutorialMask.cs:70`）——
   这是明确的功能边界。要做交互式教程时再改，不要提前设计。

6. **三次「破铁律」都保留**——每次都写明了边界与还原路径，
   而且注释互相引用。这是好的工程记录，不是坏味道。

7. **`VNPause` 的七层保险**——后果是「游戏永久卡死」，保险层数应该多。
