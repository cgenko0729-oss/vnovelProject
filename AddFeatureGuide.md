...
| 总配置 | `Assets/Resources/VNGameConfig.asset`（路径写死在 `VNGameConfig.AssetPath`，Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:39） |

注意资产目录被切成 `Assets/Art/VNEffects/` 与 `Assets/VNEffects/` 两半，这是历史整理留下的。
**新资产放哪边都能用** —— 所有扫描都走全工程的 `AssetDatabase.FindAssets` 类型查询，
不限目录（`FindAll<T>`，Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs:223）。
但为了别人找得到，请跟同类资产放一起。
...
| 你想加的 | 在本专案里它其实是 | 章节 |
|---|---|---|
| 一个新角色（立绘、表情、名牌） | `VNCharacterDef` 资产 | §3 |
| 一张新背景 / CG / 一段 BGM 或 SE | 素材文件 + `VNGameConfig` 库条目 | §4 |
| 剧本里想写的一句新指令（`bgscroll`、`imprint` 这种） | DSL 关键字 | §5 |
| 一个新小游戏 / QTE / 战斗 / 任何「暂停剧本→玩家操作→带结果回来」 | `VNEventModule` 子类 | §6 |
| 一类新的可配置数据（题库、对手、主题、预设…） | `ScriptableObject` 定义资产 | §7 |
| 一个新的养成数值（体力、魅力、金钱…） | `VNStatDef` 资产 + `VNFlags` | §8 |
| 一件新道具 / 装备 / 一家新商店 | `VNShopDef.Item` | §9 |
| 一条新任务线 | `VNQuestDef` 资产 + `quest` 命令 | §10 |
| 一套新的对话框外观 / 系统菜单主题 | `VNDialogueSkin` / `VNSystemUiSkinSet` prefab | §11 |
| 一个新的画面特效（发光、粒子、全屏运动） | `VNEffects` 组件 + `fx` 开关 | §12 |
| 一个新章节 / 一段新剧情（≈ 别的游戏的「关卡」） | `.vn.txt` 剧本文件 + `label` | §13 |
| 一篇新手引导 | `VNTutorialDef` 资产 | §14 |
| 一个新的 AI 聊天人格 | `VNAiPersonaDef` 资产 | §15 |
| 一种新天气 / 一张新过场章节卡 | `VNWeatherDef` / `VNInterludeDef` 资产 | §16 |

**「技能」怎么办？** 本专案没有技能系统。最接近的两个落点是：
装备的被动效果（`VNShopDef.PassiveEffect`，Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs:45，
合计写进 flag「装备效果_<id>」，由剧本 `if` 判断生效），
以及战斗模块里从 flag 读属性的攻防值（`VNBattleModule` 的 `patkstat` / `phpstat` / `pdefstat`）。
...
有人格有台词的对手走 `VNBadmintonDef` 那种「一个对手一个资产」的模式（§7）。
...
Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:450），内部会
`NewScene(EmptyScene)` 把当前场景整个丢掉重造。任何挂在场景组件上的人工绑定都会消失。

所以有了 `VNGameConfig`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:34），
...
实现是 `VNGameConfig.ApplyList`（同档:258）：
...
`VNStage` 在 `Awake` 里调 `ApplyGameConfig()`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:170）
把角色／背景／CG 三个库覆盖进来；事件模块自己在 `OnLaunch` 里做同样的事，
例如问答模块（Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:85）：
...
范本是 `VNStatDef.DisplayName`（Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs:68）：
...
同一个模式在 `VNCharacterDef.LocalizedDisplayName`（Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:74）、
`VNQuestDef.Title`（Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs:44）、
`VNShopDef.Item.DisplayName`（Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs:128）、
`VNQuizDef.Option.Display`（Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs:28）里各写了一遍。
**新资产照抄这个三段式回退**（当前语言 → 中文 → id），不要自己发明。

### 2.3 运行时状态一律进 VNFlags，不要自己造字段

`VNFlags` 是一个整型字典（`VNFlags`，Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:15），
...
| 系统 | flag 名 | 出处 |
|---|---|---|
| 养成属性 | 直接用属性 id | `VNStatDef.id`（Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs:32） |
| 道具持有数 | `道具_<商品id>` | `VNShopDef.ItemFlagName`（Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs:72） |
| 任务阶段 | `任务_<任务id>` | `VNQuestLog.FlagName`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:55） |
| 装备中 | `装备_<道具id>` = 部位编号 | `VNEquipSlot`（Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs:7） |
| 装备实际加成 | `装备实增_<…>` | `VNEquipment` |
| 装备被动效果合计 | `装备效果_<效果id>` | `VNShopDef.PassiveEffect`（Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs:45） |
| 事件数值结果 | `事件结果` | `VNScriptRunner.EventCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2821） |
| 各模块成绩 | `<前缀>正确数` / `<前缀>_我方得分` … | 各模块的 `flag:` 参数 |

`VNFlags.Changed` 事件（Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:21）在任何写入后触发，
...
| ④ | 存档 + 调试重建 | Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs、`VNStage.CaptureSnapshot`／`RestoreSnapshot`、`VNScriptRunner.RebuildStateBefore` | 读档后状态丢失；「从选中行播放」画面不对 |
...
`VNScriptParser.Parse`（Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:112）
...
一个「角色」= 一个 `VNCharacterDef` 资产（Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:22）
...
（`VNTextureImportDefaults.Roots`，Assets/Project/Scripts/VNEffects/Editor/VNTextureImportDefaults.cs:37），
...
`Create → VN → Character Definition`（`CreateAssetMenu`，Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:21），
...
（`VNGameConfigTools.RescanAssetFolders`，Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs:178）。
它会全工程扫 `VNCharacterDef` 覆盖 `config.characters`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:148）。
...
Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:834 / :846）。

### 3.3 这一类不需要做的事

- **不用改剧本编辑器 Schema** —— 角色下拉走 `VNParamSource.Character`
  （枚举 `VNParamSource`，Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:7），
  扫 `VNCharacterDef` 资产自动出现；表情下拉走 `VNParamSource.Expression`，
  用 `dependsOn`（`VNParamDef.dependsOn`，同档:39）指向同行的角色参数。
- **不用改存档** —— 在场角色由 `VNStage.CaptureSnapshot`
  （Assets/Project/Scripts/VNEffects/Script/VNStage.cs:840）统一存进 `data.characters`。
...
`RescanAssetFolders`（Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs:178）
明确划了线，注解就写在函式里：
...
| 素材 | 能否自动扫 | 怎么登记 |
|---|---|---|
| 背景 | 否 | 手工填 `VNGameConfig.backgrounds`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:152） |
...
| 章节剧本 | 是 | Rescan 扫 `Assets/Scenarios/*.vn.txt`（`ScanChapters`，Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs:233） |

> ### ⚠️ CG 扫描的一个真实地雷
>
> `ScanCg`（Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs:245）扫的是写死的常数
...
> 更麻烦的是 `VNEffectsDemoSetup` 里有一句
> `EnsureFolder("Assets/CG")`（Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:1299），
> 会**主动建这个目录**。
>
...
`Tools → VN Effects → 素材浏览器 Asset Browser`（`VNAssetBrowserWindow`，
...
（注解在 Assets/Project/Scripts/VNEffects/Editor/VNAssetBrowserWindow.cs:896：
「`Assets/CG` 与 `Assets/Art/Images/CG` 并存过，写死路径必然过时」）。

登记完之后 `VNAssetLibraryEvents.RaiseChanged()` 会广播「素材库改了」，
...
2. Inspector 是九页分页的（`VNGameConfigEditor`，
   Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs）：
   剧本｜标题｜UI皮肤｜舞台｜音频｜玩法｜AI｜大头贴｜全部
3. 背景条目是 `VNStage.BackgroundEntry`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:29），
   只有 `id` + `sprite` 两栏
4. 批量拖入会自动用文件名填 id
5. 存档（Ctrl+S）。Inspector 改完会自动 `VNGameConfig.ClearCache()`
   （Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:69）——
...
CG 条目 `VNStage.CgEntry`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:39）有个 `group` 栏位：
**同 group 的 CG 在 G 键鉴赏画廊里合并成一格翻差分**，留空 = 独立一格。
解锁记录走 `VNCgUnlocks` 独立 JSON（与存档槽分离 —— 读旧档／开新周目都不该丢）。

反过来，`interludeImages`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:168）
里的过场图**刻意不进画廊**，注解写得很直白：「它们是演出素材，不是收集品」（同档:167）。

### 4.5 验证

Lint 会检查 bg / cg / bgm / se / voice 的 id 是否登记
（`CheckAssets`，Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:502），
...
`VNScriptRunner.Run`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2133）`yield return` 它，
行尾 `@` 的异步语义也由主循环处理。

注意 `default` 分支（同档:2510）只在**关键字已进白名单**时才可能到达，
所以它捕捉不到步骤 1 漏掉的情况。

取参数一律用 `VNScriptCommand` 上的辅助方法：`Arg(i, def)`（同档:63）、
`ArgF(i, def)`（:65）、`Kw(key, def)`（:71）、`KwF(key, def)`（:74）。

#### 步骤 4：实现落到 VNStage 或组件

演出类命令的实现一律挂在 `VNStage`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs）上，
...
- `VNParamSource`（同档:7）决定编辑器画什么控件、下拉候选从哪来
...
```
  ...
  （`VNParamDef.assetType` / `assetIdField` 上方，同档:51-53）：
  「字段名各资产不统一（quizId / badmintonId / themeId / id …），所以显式写死，
  不用反射猜 —— 猜错的表现是下拉里一片空白，很难联想到原因。」

- 候选只是提示、认不出不该报错的参数，包一层 `Soft`（`VNParamDef.softRef`，同档:48；
  辅助函式 `Soft`，同档:339）
...
4. `F5` 从选中行播放（`VNScriptRunner.PlayFromSourceLine`，
   Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:245），看演出对不对
...
一个事件模块 = 一个 `VNEventModule` 子类
（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:52）。

生命周期（注解在同档:43-45）：Runner 从 `VNEventRegistry` 实例化到事件层 → `Launch` →
...
注解原文在 Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:47-50：
...
- `VNAiTalkModule`（Assets/Project/Scripts/VNEffects/Script/VNAiTalkModule.cs）：
  直接驱动舞台立绘换表情。原因是自绘立绘要把眨眼／口型／色调匹配／出场动画全部重接一遍。
  边界收紧为「只碰表情和对话框内容」，且**正常结束／ESC／`CancelForDebug` 三条路径都还原原表情**。
- `VNInteractionModule`（Assets/Project/Scripts/VNEffects/Script/VNInteractionModule.cs）：同上，
  边界为「只碰表情与叠加层，三条退出路径都还原」。

**铁律②的落地是 `VNTime.Delta`**（Assets/Project/Scripts/VNEffects/Script/VNPause.cs:144），
...
它同时做了两件事：受 `VNPause` 冻结（教程弹窗），以及单帧上限 `MaxStep`（防瞬移，
收编自羽毛球）。

**每个模块的 Update 第一行必须是暂停早退，而且必须在读输入之前**
（范例 `VNQuizModule.Update`，Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:201）：
...
另有 Runner 的 `ReleaseAll()` 兜底（:99）。**释放路径有五条**
...
`ctx.outcomes`（Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:13）。
模块可以据此**只开放本次剧情接住的分支**（`AcceptsOutcome`，同档:18）。

`EventCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2821）的收尾逻辑：
...
> （Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:64）专门校验拼写。
> **加新模块务必往这张表补一行。**
...
参照实作：`VNQuizModule`（Assets/Project/Scripts/VNEffects/Script/VNQuizModule.cs:34），
...
- **找不到资产要 `Done("")` 早退**。不 Done 的话剧本永远卡在
  ...
  （Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2861）。
- **纯流程控制型模块要覆写 `RecordInBacklog => false`**
  （Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:91）。
...
- **有藏系统光标 / 有 `VNPause` 句柄 / 有挂在立绘下的可视化物件时，
  ...
  漏一条的后果分别是：玩家鼠标指针永久消失（`VNTouchCursor`，
  Assets/Project/Scripts/VNEffects/Script/VNTouchCursor.cs）、游戏永久卡死（`VNPause`）、
  部位框留在角色脸上（`VNInteractionModule`）。

#### 步骤 2：射线层级的坑

事件层排序是 **60**（`VNEventRegistry.LayerSortingOrder`，
Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:27），
...
照抄 `VNQuizInstaller`（Assets/Project/Scripts/VNEffects/Editor/VNQuizInstaller.cs:21），
它只做三件事（注解在同档:15-19）：
...
5. **最后一定要 `VNGameConfig.ClearCache()`**（同档:99），否则 Inspector 改完不生效
...
注解把设计理由写得很清楚
（Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:97-101）：
...
> ```csharp
> // Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:662
...
模块 id 的下拉候选来自 `VNParamSource.EventId`，扫场景 `VNEventRegistry.modules`
（`VNEventRegistry.Ids`，Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:32）
...
③`reg.yourIds` 要先在 `Registry` 类（同档:100）加一个 `HashSet<string>`，
...
（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:613，
注解：「事件结果无法推断，不重放」）。
...
调 `VNScriptRunner.RunInlineCo`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:291）。
...
> 注解原文在 Assets/Project/Scripts/VNEffects/Script/VNStage.cs:24-27：
...
> （范例：`VNGameConfig.UiSkinEntry`，
> Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:116）。
...
`VNGameConfigEditor`（Assets/Project/Scripts/VNEffects/Editor/VNGameConfigEditor.cs）
...
（`VNConfigEntryDrawers`，Assets/Project/Scripts/VNEffects/Editor/VNConfigEntryDrawers.cs）。
...
`VNStatDef`（Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs:21）**只管钳制范围与显示规则**，
数值本体全部存 `VNFlags`，flag 名 = 属性 id（注解在同档:16-17）。
...
| `initialValue` | :53 | 进游戏／读档后 flag 不存在时自动写入（`VNStatsHud.EnsureInitials`，Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:96） |
...
（`VNGameConfig.stats`，Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:206）
...
`stat` 的分派在 Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2482：
...
`VNStatsHud.Apply`（Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs:108）
...
> （Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:326）的 switch 里
> 也加一条 silent 重放。**

**⑥ 不需要做的事**：`stat` 命令与 Schema 已存在
（`Add("stat", ...)`，Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:617）；
...
道具**不是独立资产**，是 `VNShopDef.Item`
（Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs:75）—— 一家商店的商品清单里的一项。
...
2. `items` 列表（同档:166）加一项，填（行号都在 Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs）：
   ...
   - `condition`（:104）—— 上架条件，`VNFlags` 表达式（如 `好感度>=2`），留空 = 总是上架
3. Rescan → 进 `config.shops`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:207）

当前持有数直接读 `Item.Owned`（同档:150）。
...
`* 离开` 是 shop 唯一的结果名
（`BuiltinOutcomes["shop"]`，Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:67）。

`shopId`（同档:154）是 `event shop id:` 引用的 id；
`currencyStat`（同档:163）默认 `金钱`，指向一个 `VNStatDef` 的 id。
Schema 端 shop 的变体只有一个 `KwAsset("id", "商店", "VNShopDef", "shopId")`
（Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:163 附近的 `["shop"]` 条目）。
...
`VNQuestDef`（Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs:14）**只管显示文案**，
进行状态全部存 flag `任务_<id>`
（`VNQuestLog.FlagName`，Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:55）。
...
| 100 | 完成 | `VNQuestLog.StageDone`（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:18） |
| -1 | 失败 | `VNQuestLog.StageFailed`（同档:19） |

查询走 `VNQuestLog.StageOf`（同档:58）。
**没有定义资产的任务也能正常运作**（注解在 Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs:11），
...
4. Rescan → 进 `config.quests`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:209）
5. 剧本：
...
`quest` 的分派在 Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2498，落到
`VNQuestLog.Apply(op, id, stage, silent, line)`
（Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs:72）。
`quest start <id> 2` 可以直接从阶段 2 开始（同档:86）。

6. Schema 已存在（`Add("quest", ...)`，
   Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:688），
   id 下拉走 `VNParamSource.QuestId` 扫资产，自动出现
7. J 键看任务日志

**调试重建**里 quest 也是静默重放
（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:584）。
...
| | 对话框／选项皮肤 | 系统菜单主题 |
|---|---|---|
| 载体 | prefab 根挂 `VNDialogueSkin` / `VNChoiceSkin` | 一个 `VNSystemUiSkinSet` 资产（Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs:11） |
| 登记 | `VNGameConfig.dialogueSkins` / `choiceSkins`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:126 / :128） | `VNGameConfig.systemUiSkin`（同档:131），**全局唯一** |
...
（`VNUiSkinExporter`，Assets/Project/Scripts/VNEffects/Editor/VNUiSkinExporter.cs）——
烘焙贴图 + 生成默认／顶部／右列样例并自动登记。
另有 `导出无框渐变皮肤（白·粉·黑）Export Soft Gradient Skins`
（`VNSoftSkinExporter`，Assets/Project/Scripts/VNEffects/Editor/VNSoftSkinExporter.cs）。

**② 改 prefab**

`VNDialogueSkin`（Assets/Project/Scripts/VNEffects/Script/VNDialogueSkin.cs:24）
...
| `nameText` | :32 | 不显示名字 |
| `bodyText` | :35 | **必需** —— 运行时自动挂 `VNTypewriterText` 打字机 |
...
`VNGameConfig` 的「UI皮肤」页，`dialogueSkins` 加一条 `UiSkinEntry`
（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:116）：
...
分派在 `VNStage` 的 `case "dialogue"`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:768）
与 `case "choice"`（同档:787）。找不到会告警：
「…没有在 `VNGameConfig.dialogueSkins` 登记（或 prefab 缺 `VNDialogueSkin`）」（同档:779）。

**⑤ 存档**

皮肤 id 进存档（`VNSaveData.dialogueSkin`，
Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:58；`choiceSkin`，:59），
`RestoreSnapshot` **最先恢复 UI 皮肤**
（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:906），
注解理由：「随后重放的台词/选项直接落在正确皮肤上」（同档:904）。
**这一段已经写好了，你加新皮肤不用改程式码** —— 存的是 id 字串。

**⑥ Lint**

`Registry.dialogueSkins` / `choiceSkins`
（Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:100 的 `Registry` 类内）
已经在收集，`ui` 命令的 id 会被校验。

### 11.2 名牌样式（第三条线，最轻）

`ui name <样式|default>`，十套内置预设（`VNNameplateStyle`，
Assets/Project/Scripts/VNEffects/Script/VNNameplateStyle.cs），**不用登记任何东西**。
分派在 `VNStage` 的 `case "name"`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:806）——
注解特别指出「这里的 id 不是 VNGameConfig 里登记的 prefab」（同档:809）。
状态进存档（`VNSaveData.nameplateStyle`，
Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:60）。
...
安全实例化的基类是 `VNSystemUiSkinBehaviour`
...
1. `Tools → VN Effects → UI 皮肤 UI Skins → 系统主题：导出默认模板 System UI: Export Default Prefabs`
   （`VNSystemUiSkinExporter`，Assets/Project/Scripts/VNEffects/Editor/VNSystemUiSkinExporter.cs）
2. 改 prefab（每种面板有自己的槽位组件：`VNPlanSkin` / `VNResultPopupSkin` /
   `VNInventorySkin` / `VNTutorialSkin` / `VNSaveLoadSkin` / `VNBacklogSkin` /
   `VNStatsHudSkin` / `VNCgGallerySkin` / `VNQuickToolbarSkin` / `VNConfigPanelSkin` /
   `VNTitleMenuSkin`，全在 Assets/Project/Scripts/VNEffects/Script/ 下）
3. 建一个 `VNSystemUiSkinSet` 资产（`Create → VN → System UI Skin Set`，同档:10），
   把 prefab 拖进槽位
4. 填进 `VNGameConfig.systemUiSkin`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:131）
...
| 贴图全程序化生成（`VNProceduralTextures`，Assets/Project/Scripts/VNEffects/VNProceduralTextures.cs） | 零美术依赖 |
...
Assets/Project/Scripts/VNEffects/Script/VNStage.cs:111），
并在 `AutoWire()` 里补自动查找。
...
再到 `Fx()`（同档:1614）的 switch 里加一个 case：
...
（Assets/Project/Scripts/VNEffects/Editor/VNEffectsDemoSetup.cs:345）。
...
想让某层躲开情绪色调，正解是把它移出 `VNMoodGrading`
...
Parser 在 `ParseCommand`（Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:317）
专门为它做了「第一个冒号后紧跟另一个冒号」的判据，
并留了反例保护（`title:第1章::序` 仍按 key:value 处理，值里保留 `::` 原样传给 `VNStoryAddress`）。
...
台词行三种写法（`ParseSay`，Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:340）：
`角色 表情: 内容`（表情可省）／`旁白: 内容`（说话者不是注册角色时只显示名字）／
`: 内容`（冒号开头 = 无名牌旁白）／无冒号整行 = 无名牌旁白。

3. **Rescan** → 自动进 `config.chapters`
   （`ScanChapters`，Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs:233，
   只收 `Assets/Scenarios` 下 `.vn.txt` 结尾的）
   —— **`jump 文件::标签` / `chapter` 的目标必须登记在 `chapters` 里**
   （注解在 Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:88）
4. `Ctrl+Shift+L` Lint
...
- `F5` 从选中行播放（`VNScriptRunner.PlayFromSourceLine`，
  Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:245）——
...
`VNTutorialDef`（Assets/Project/Scripts/VNEffects/Script/VNTutorialDef.cs:103）
+ 一组 `VNTutorialStep`（同档:27），三语文案。`id` 在同档:106。

播放三种触发：剧本 `tutorial <id> [force:on]`；
事件行写 `event <模块> tutorial:<id>`；
模块模板的 `tutorialId` 栏位（`VNEventModule.tutorialId`，
Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:56）首次自动播。
...
另有一个挂在物件上的便捷组件 `VNTutorialAnchor`（同档:82）。
...
1. `Create → VN → Tutorial Def`（Assets/Project/Scripts/VNEffects/Script/VNTutorialDef.cs:102），
   存 `Assets/VNEffects/Tutorials/`
2. 填 `id`（同档:106）+ 各步骤的三语文案 + 高亮锚点 id
3. 目标 UI 那边 `VNTutorialAnchors.Register`
4. Rescan → 进 `config.tutorials`
   （Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:172）
5. 剧本 `tutorial <id>`，或模块模板 `tutorialId`，或剧本行 `event xxx tutorial:<id>`
6. Schema 已有：`tutorial` 命令在
   Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:421，
   `event` 的通用 `tutorial:` 参数在同档:664（`VNParamSource.TutorialId`）
7. 样例：`Tools → VN Effects → 教程 Tutorials → 导出羽毛球示例教程 Export Badminton Sample`
   （`VNTutorialSamples`，Assets/Project/Scripts/VNEffects/Editor/VNTutorialSamples.cs）

### 14.4 三个不能漏的时序细节

- **暂停走 `VNPause` 不是 `timeScale`**（理由见 §6.2）。
  教程必须在 `OnLaunch` **之后**播，注解在
  Assets/Project/Scripts/VNEffects/Script/VNEventModule.cs:68-70：
...
走 `VNTutorialSeen`（Assets/Project/Scripts/VNEffects/Script/VNTutorialSeen.cs）全局 JSON，
语义同 CG 解锁：读旧档、开新周目都不该重看。ESC 跳过也算看过。
设置面板有「显示教程提示」开关可关可重置。

### 14.6 卡片皮肤

`VNTutorialSkin`（Assets/Project/Scripts/VNEffects/Script/VNTutorialSkin.cs）
...
`VNAiPersonaDef`（Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs:26）
与 `VNCharacterDef` 是分开的两个资产 ——
...
2. 填 `id`（同档:29）+ 绑定的角色 id + 身份 / 说话方式 / 关系 / 边界 等提示词段落
   + 表情／漫符白名单（留空 = 取角色资产全部）
3. 供应商选「跟随全局」即跟着
   `VNGameConfig.aiProvider`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:222）换；
...
5. **不进 Play Mode 调参**：`Tools → VN Effects → AI → AI 试聊台 AI Talk Studio`
   （`VNAiStudioWindow`，Assets/Project/Scripts/VNEffects/Editor/VNAiStudioWindow.cs；
...
- **`* 失败` 必须接住**，否则玩家断网／没配 key／被内容安全拦下时会静默跳过。
  ...
  （Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:1106），
...
组装逻辑在 `VNAiConversation`（Assets/Project/Scripts/VNEffects/Script/VNAiConversation.cs，
纯逻辑层，无 MonoBehaviour，可单测）。

**永远不信任模型输出**：好感强制 Clamp（Gemini schema 不支持 minimum/maximum，
不钳实测会给 +5）、表情越界降级、选项不足补齐。繁体字有确定性代码兜底
（`VNAiTextNormalize`，Assets/Project/Scripts/VNEffects/Script/VNAiTextNormalize.cs）——
提示词管「大部分时候对」，兜底管「永远不出错」。

### 15.5 换供应商 / 换模型时

- 供应商能力差异（谁支持硬 schema、安全阈值）、默认模型、key 名全登记在 `VNAiProviders`
  （Assets/Project/Scripts/VNEffects/Script/VNAiProvider.cs）
- **全项目唯一碰 HTTP 的文件是 `VNAiClient`**
  （Assets/Project/Scripts/VNEffects/Script/VNAiClient.cs）；
  各家只差「拼请求体」和「解响应」，拆在 `VNAiClientGemini` / `VNAiClientDeepSeek`（纯静态）
- **换模型必须同步单价表** `VNAiPricingDef`
  （Assets/Project/Scripts/VNEffects/Script/VNAiPricing.cs:56），
  填进 `VNGameConfig.aiPricing`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:230）
  —— 注解直说：「换了模型却不改这里的话，日志里的成本数字会静默偏低」（同档:229）
- `VNGameConfig.ClearCache()`（同档:69）会一并 `VNAiProviders.Invalidate()`
  与 `VNAiPricing.Invalidate()`（同档:76-77）—— 这就是为什么改配置后必须清缓存
...
1. `Create → VN → Weather Def`（`CreateAssetMenu`，
   Assets/Project/Scripts/VNEffects/VNWeatherDef.cs:21）
2. 填 `id`（同档:48）—— **可中文，会覆盖同名内置预设**
3. Rescan → 进 `config.weatherDefs`
   （Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:160）
4. 调参：`Tools → VN Effects → 预览 Preview → 天气预览 Weather Preview`
   （`VNWeatherPreviewWindow`，Assets/Project/Scripts/VNEffects/Editor/VNWeatherPreviewWindow.cs）
5. 剧本：`weather <id> [density:] [wind:] [speed:] [size:]`

`weather` 的 id 是**三级解析**（`VNWeatherController.SetWeatherId`，
Assets/Project/Scripts/VNEffects/VNWeatherController.cs）：
自定义资产 → 内置叶型别名（含中文）→ `VNWeather` 枚举。
留空 `weatherDefs` 也能用 —— 内置五套预设走 `petals` / `maple` / `ginkgo` / `leaves` / `bamboo`
（注解在 Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:157-159）。

Schema 用 `VNParamSource.WeatherId`
（Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:7 的枚举；
命令定义在同档:382），候选 = 内置叶型 + 资产 + 雨雪萤火虫枚举，自动出现。
天气状态进存档（`VNSaveData.weather` 与四个覆盖参数，
Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs:36-44），
`CaptureSnapshot` 已经处理（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:843-848）。

### 16.2 新过场章节卡

1. `Create → VN → Interlude Def`（Assets/Project/Scripts/VNEffects/Script/VNInterludeDef.cs:37），
   存 `Assets/Art/VNEffects/Interlude/`
2. 填 `id`（同档:41）+ 三语标题 + 语音池 + 图池 + 进出方式
3. **图池留空 = 从 `VNGameConfig.interludeImages` 全局池随机抽**
   （Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:168）
4. Rescan → 进 `config.interludes`（同档:164）
5. 剧本：`interlude <id> [time:秒]`
   （Schema 在 Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:413）

三个设计点：

- **过场图不进 CG 画廊**（注解在 Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:167）
  —— 它们是演出素材不是收集品
- 过场层排序 **90**（事件层 60 之上、全屏转场 100 之下），
  也因此**必须挂主 Canvas 下**
- **不进存档，但 `ClearStage()` 必须 `HideImmediate()`**
...
| 内容 | 通道 | 出处 |
|---|---|---|
| UI 字符串（按钮、提示、系统文案） | `VNLocale.T("key")` 查表，表在 `Resources/VNLocale/ui.<code>.txt` | `VNLocale.T`，Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:76 |
| 剧本台词与选项 | 旁路表，按剧本文件独立成表 | `VNScriptLocale.TableFolder`，Assets/Project/Scripts/VNEffects/Script/VNScriptLocale.cs:25 |
...
（注解在 Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:22）。
查表回退链：当前语言 → 中文（源语言）→ key 本身，保证任何情况下不显示空白（同档:23）。

**② 剧本只写中文。** 译文抽取走
`Tools → VN Effects → 本地化 Localization → 抽取剧本译文 Extract Script Translations`
（`VNLocalizationTools.ExtractAll`，
Assets/Project/Scripts/VNEffects/Editor/VNLocalizationTools.cs:36），
校验走 `校验剧本译文 Validate Script Translations`（`ValidateAll`，同档:66）。

译文 key 由原文算出（`VNScriptLocale.NextKey`，
Assets/Project/Scripts/VNEffects/Script/VNScriptLocale.cs:106；
`Hash`，同档:115），所以**改了中文原文，译文就对不上了** —— 改完要重跑 Extract。

套用发生在 `VNScriptLocale.Apply`（同档:32），把译文写进
`VNScriptCommand.localizedText`（Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:55）
与 `VNChoiceOption.localizedText`（同档:18）。
注意注解写明：**event 结果行是逻辑标识符（结果匹配 / 去过_xx flag），不参与翻译**（同档:17）。

**③ 新资产照抄三段式回退**（§2.2）。

**④ 加新语言**：`VNLanguage` 枚举
（Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:8）——
注解明确写着「枚举顺序即 PlayerPrefs 存储值，**勿改动已有项顺序**」，只能往后加。
还要同步 `Codes` 数组（同档:29）与 `DisplayName`（同档:65，「永远以该语言本身书写」）。

### 17.3 字体

文字全用 TextMeshPro + `VNFont.Asset`
（Assets/Project/Scripts/VNEffects/Script/VNFont.cs）统一入口，**禁止 legacy Text**。
编辑期存场景的 TMP 文字必须用 `VNFontAssetBuilder.EnsureFontAsset()`
（Assets/Project/Scripts/VNEffects/Editor/VNFontAssetBuilder.cs）持久化资产。

语言切换时**正文与装饰字体分开替换**（装饰字体 = `VNFont.DisplayAsset`，名牌等大字专用）。
换 font 会丢材质实例，所以有 `DisplayFontChanged` 事件通知重新上样式。

`VNLocale.Language` 的 setter（同档:39）里，
`VNFont.HandleLanguageChanged()`（同档:53）在 `LanguageChanged?.Invoke()`（同档:54）
...
| 全局收集品 | 否（独立 JSON） | CG 解锁（`VNCgUnlocks`）、大头贴相册（`VNPhotoAlbum`）、AI 日记（`VNAiDiary`）、教程已读（`VNTutorialSeen`） |
| 剧情状态 | 是 | 全部 flag、AI 跨场记忆（`VNAiMemory`，`CaptureSnapshot` 在 Assets/Project/Scripts/VNEffects/Script/VNStage.cs:879 被调） |
...
**② `VNStage.CaptureSnapshot` / `RestoreSnapshot`**
...
**③ `VNScriptRunner.RebuildStateBefore`**
...
**文本是唯一真相**：`.vn.txt ↔ VNScenarioDoc.rows`（`VNScenarioDoc`，
Assets/Project/Scripts/VNEffects/Editor/VNScenarioDoc.cs:95）。
...
登记进 `VNScenarioSchema` 之后，**不用改窗口程式码**就自动有：

- 行首命令按钮的分类菜单（`VNCommandDef.category`，
  Assets/Project/Scripts/VNEffects/Editor/VNScenarioSchema.cs:61）与右键打字搜索
- 每个参数一个控件，类型由 `VNParamSource` 决定
- 底部 `+` 加行下拉、`Ctrl+E` 命令面板（`VNCommandSearch`，
  Assets/Project/Scripts/VNEffects/Editor/VNCommandSearch.cs；候选表从 Schema 现场生成）
- tooltip（`VNCommandDef.hint`，同档:62）
...
- **行号换算**：`SourceLineForRow`
  （Assets/Project/Scripts/VNEffects/Editor/VNScenarioEditorWindow.cs:1560）
...
- **路径点行禁用 `CharacterPopup` / `SpritePopup`** —— 那套是异步回调、会把值写进
  `VNRow.values`，和 `camLines` 是两条路径，必须用同步的 `PopupString` / `EditorGUI.Popup`
...
- **素材候选按覆盖语义取数**：`VNGameConfig` 里填了就用它的，留空才回退场景组件
  （`RefreshSources()` 里的 `PickLibrary`）—— 以前只读场景组件，导致在配置资产里
  新登记的素材在下拉里根本搜不到

### 19.4 Lint 的严重度与命名

`VNLintIssue`（Assets/Project/Scripts/VNEffects/Editor/VNScenarioLinter.cs:10）
...
| 10 | 事件模块：四条退出路径 | 正常结束 / ESC / `CancelForDebug` / 换场景 —— 光标、`VNPause` 句柄、挂在立绘下的物件都要还原 |

### 常见症状 → 原因速查

| 症状 | 最可能的原因 |
|---|---|
| 命令行被当台词念出来 | 忘了加进 `Keywords`（Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:93） |
| 什么都没发生，也没报错 | id 拼错（走 Lint）；或事件结果名对不上（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2886 只有一条 Warning） |
| 参数值被切成 key:value | 值里含冒号，`ParseCommand`（Assets/Project/Scripts/VNEffects/Script/VNScriptParser.cs:303）需要加豁免 |
| 重建场景后功能消失 | 没登记进 `VNGameConfig` |
| Inspector 改了没反应 | 没 `VNGameConfig.ClearCache()`（Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:69） |
| 读档后状态丢了 | `VNSaveData` / `CaptureSnapshot` / `RestoreSnapshot` 三处没同步 |
| 从选中行播放画面不对 | `RebuildStateBefore`（Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:326）的 switch 少一条 |
...
| 事件里点不到选项 | 模块自绘 UI 没设 `raycastTarget = false`（事件层 60 压在选项面板 45 上，`VNEventRegistry.LayerSortingOrder`，Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs:27） |
| 剧本卡在事件里不动 | 模块某条路径没调 `Done`（Runner 在 Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs:2861 无限等） |
| 模块运行时抛 InvalidCastException | 模板物体没带 `RectTransform`（Assets/Project/Scripts/VNEffects/Editor/VNQuizInstaller.cs:63-65） |
| 游戏永久卡死 | `VNPause` 句柄没释放（五条路径漏了一条，`VNPause.Release`，Assets/Project/Scripts/VNEffects/Script/VNPause.cs:85） |
| 玩家鼠标指针永久消失 | `VNTouchCursor.Dispose()` 没在四处都调 |
| 教程讲解时球还在飞 | 用了 `Time.timeScale = 0`；应该是 `VNPause.IsPaused` 早退 + `VNTime.Delta` |
| 教程高亮挖到空气上 | 用了名字/路径寻址，没用 `VNTutorialAnchors.Register`（Assets/Project/Scripts/VNEffects/Script/VNTutorialAnchors.cs:30） |
| 列表条目文字叠印、输入框点不进去 | 内嵌类里用了 `[Header]`，应该用 `[Tooltip]`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:24-27） |
| 新图拖进来不能填 Sprite 栏 | 目录不在 `VNTextureImportDefaults.Roots`（Assets/Project/Scripts/VNEffects/Editor/VNTextureImportDefaults.cs:37） |
| 手工维护的 CG 库突然空了 | `Assets/CG` 目录被建出来 + 跑了 Rescan（见 §4.1 与 附录 D） |
| 英/日切换后某处仍是中文 | 硬编码字串，应走 `VNLocale.T`（Assets/Project/Scripts/VNEffects/Script/VNLocale.cs:76） |
| 改了中文原文，译文全失效 | 译文 key 由原文算出（`VNScriptLocale.Hash`，Assets/Project/Scripts/VNEffects/Script/VNScriptLocale.cs:115），要重跑 Extract |
...
全部 `[CreateAssetMenu]` 一览，以及它在 `VNGameConfig` 里对应的库栏位
（库栏位行号都在 Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs）。

| Create 菜单 | 类 | 档案:行号 | VNGameConfig 库栏位 |
|---|---|---|---|
| VN/Game Config | `VNGameConfig` | Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:33 | （本体） |
| VN/Character Definition | `VNCharacterDef` | Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:21 | `characters`:148 |
| VN/Stat Definition | `VNStatDef` | Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs:20 | `stats`:206 |
| VN/Shop Definition | `VNShopDef` | Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs:27 | `shops`:207 |
| VN/Plan Definition | `VNPlanDef` | Assets/Project/Scripts/VNEffects/Script/VNPlanDef.cs:13 | `plans`:208 |
| VN/Quest Definition | `VNQuestDef` | Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs:13 | `quests`:209 |
| VN/Quiz Definition | `VNQuizDef` | Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs:15 | `quizzes`:210 |
| VN/Badminton Definition | `VNBadmintonDef` | Assets/Project/Scripts/VNEffects/Script/VNBadmintonDef.cs:19 | `badmintons`:211 |
| VN/Fog Wipe Definition | `VNFogWipeDef` | Assets/Project/Scripts/VNEffects/Script/VNFogWipeDef.cs:34 | `fogWipes`:214 |
| VN/Interaction Definition | `VNInteractionDef` | Assets/Project/Scripts/VNEffects/Script/VNInteractionDef.cs:259 | （模块模板持有） |
| VN/Touch Zone Definition | `VNTouchZoneDef` | Assets/Project/Scripts/VNEffects/Script/VNTouchZoneDef.cs:94 | （互动资产引用） |
| VN/Tutorial Def | `VNTutorialDef` | Assets/Project/Scripts/VNEffects/Script/VNTutorialDef.cs:102 | `tutorials`:172 |
| VN/Interlude Def | `VNInterludeDef` | Assets/Project/Scripts/VNEffects/Script/VNInterludeDef.cs:37 | `interludes`:164 |
| VN/Weather Def | `VNWeatherDef` | Assets/Project/Scripts/VNEffects/VNWeatherDef.cs:21 | `weatherDefs`:160 |
| VN/AI Persona | `VNAiPersonaDef` | Assets/Project/Scripts/VNEffects/Script/VNAiPersonaDef.cs:25 | `aiPersonas`:218 |
| VN/AI Pricing | `VNAiPricingDef` | Assets/Project/Scripts/VNEffects/Script/VNAiPricing.cs:56 | `aiPricing`:230 |
| VN/System UI Skin Set | `VNSystemUiSkinSet` | Assets/Project/Scripts/VNEffects/Script/VNSystemUiSkinSet.cs:10 | `systemUiSkin`:131 |
| VN/Photo Frame | `VNPhotoFrameDef` | Assets/Project/Scripts/VNEffects/Script/VNPhotoFrameDef.cs:38 | `photoFrames`:242 |
| VN/Photo Sticker | `VNPhotoStickerDef` | Assets/Project/Scripts/VNEffects/Script/VNPhotoStickerDef.cs:30 | `photoStickers`:243 |
| VN/Photo Backdrop | `VNPhotoBackdropDef` | Assets/Project/Scripts/VNEffects/Script/VNPhotoBackdropDef.cs:31 | `photoBackdrops`:244 |
| VN/Photo Theme | `VNPhotoThemeDef` | Assets/Project/Scripts/VNEffects/Script/VNPhotoThemeDef.cs:48 | `photoThemes`:245 |

其余不是 `CreateAssetMenu` 但也算「一类内容」的：
UI 皮肤是 prefab + `VNGameConfig.UiSkinEntry`（同档:116），
背景／CG 是 `VNStage.BackgroundEntry`（Assets/Project/Scripts/VNEffects/Script/VNStage.cs:29）
与 `VNStage.CgEntry`（同档:39），
音频是 `VNAudio.AudioEntry`（Assets/Project/Scripts/VNEffects/Script/VNAudio.cs），
地图地点是 `VNMapModule.Location`
（Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs，登记在 `mapLocations`，
Assets/Project/Scripts/VNEffects/Script/VNGameConfig.cs:203）。
...
| 菜单 | 出处 |
|---|---|
| `游戏配置 Game Config → 创建或选中配置资产 Create or Select` | Assets/Project/Scripts/VNEffects/Editor/VNGameConfigTools.cs:31 |
...
（Assets/Project/Scripts/VNEffects/Editor/VNLocalizationTools.cs:35）／
...
### ② `VNGameConfigTools.CgDir` 指向一个不存在的目录，且是一颗定时炸弹
