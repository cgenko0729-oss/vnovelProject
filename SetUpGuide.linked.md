...
（[`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) 自动管理这套实例化）。
...
> 这份 profile 是"基础层"。[`VNMoodGrading`](Assets/Project/Scripts/VNEffects/VNMoodGrading.cs)（情绪调色）运行时会另建两个全局
> Volume 以更高 priority 叠在上面做交叉过渡，不会动这份资产。
> [`VNVignetteFocus`](Assets/Project/Scripts/VNEffects/VNVignetteFocus.cs) 则是直接补间这份 profile 里的 Vignette 参数做"聚焦渐晕"。
...
之后（见 6.9）[`VNVignetteFocus`](Assets/Project/Scripts/VNEffects/VNVignetteFocus.cs) 也挂在这个物体上并引用此 Volume。
...
> [`VNStage.AutoWire()`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 用 `GameObject.Find` 按名字查找（旧场景自愈用），
> 手动搭建时保持原名可以少连两根引用；改名则必须手动把
> `VNStage.characterLayer`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:53`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L53)） / `backgroundImage` 拖好。
...
1. **[`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs)**：单图特效总控。`sourceMaterial` 拖
   `VNImageEffect.mat`。背景的溶解换图、色调匹配、热浪扭曲、水面波光、
   全屏水波的扭曲脉冲都要通过它；
2. **[`VNKenBurns`](Assets/Project/Scripts/VNEffects/VNKenBurns.cs)**：背景 Ken Burns 漂移（默认参数即可：缩放 1.0~1.06、
...
在 `LayerMid` 下创建全拉伸空物体 `GodRays`，挂 [`VNGodRays`](Assets/Project/Scripts/VNEffects/VNGodRays.cs)，
...
| 物体名 | 组件 | 说明 |
|---|---|---|
| CloudShadows | [`VNCloudShadows`](Assets/Project/Scripts/VNEffects/VNCloudShadows.cs) | 地面云影缓移（fx clouds） |
| DriftingClouds | [`VNDriftingClouds`](Assets/Project/Scripts/VNEffects/VNDriftingClouds.cs) | 天上的云本体缓移（fx skycloud）。与云影独立开关：白天有影无云、夜晚有云无影都是合理演出 |
| ShootingStars | [`VNShootingStars`](Assets/Project/Scripts/VNEffects/VNShootingStars.cs)（sourceMaterial = VNAdditive.mat） | 夜晚偶发流星（fx meteor） |

### 4.4 立绘（手动摆 vs 剧本生成）

**剧本场景不需要手动摆立绘**——[`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 会在 `show 角色` 命令时运行时生成完整
...
4. 依次挂：`VNImageEffectController`（VNImageEffect.mat）→
   [`VNGlowBackdrop`](Assets/Project/Scripts/VNEffects/VNGlowBackdrop.cs)（VNAdditive.mat）→ [`VNEntranceAnimator`](Assets/Project/Scripts/VNEffects/VNEntranceAnimator.cs) → [`VNFootShadow`](Assets/Project/Scripts/VNEffects/VNFootShadow.cs)。
   顺序无所谓，但材质引用别拖错。
...
| 物体名 | 挂载组件 | 需要设置的字段 |
|---|---|---|
| EdgeGlow | [`VNEdgeGlow`](Assets/Project/Scripts/VNEffects/VNEdgeGlow.cs) | sourceMaterial = VNAdditive.mat |
| SpeedLines | [`VNSpeedLines`](Assets/Project/Scripts/VNEffects/VNSpeedLines.cs) | sourceMaterial = VNAdditive.mat |
| ScreenShockwave | [`VNScreenShockwave`](Assets/Project/Scripts/VNEffects/VNScreenShockwave.cs) | targets = [背景的 VNImageEffectController]（水波的扭曲脉冲只作用背景，不扭立绘的脸）；screenShake = 第 6 章的 VNScreenShake（受击联动轻震动） |
| RetroFilter | [`VNRetroFilter`](Assets/Project/Scripts/VNEffects/VNRetroFilter.cs) | 无需设置（默认参数即可） |
| Letterbox | [`VNLetterbox`](Assets/Project/Scripts/VNEffects/VNLetterbox.cs) | 无需设置 |
| ScreenTransition | [`VNScreenTransition`](Assets/Project/Scripts/VNEffects/VNScreenTransition.cs) | sourceMaterial = VNScreenTransition.mat |
| ChoicePanel | [`VNChoicePanel`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs) | 无需设置 |
| CameraFade | [`VNCameraFade`](Assets/Project/Scripts/VNEffects/VNCameraFade.cs) | 无需设置（camseq 的 fade 用；之后拖给 VNCamera.cameraFade） |
...
| 物体名 | 组件 | 必须连的引用 | 为什么 |
|---|---|---|---|
| ScreenShake | [`VNScreenShake`](Assets/Project/Scripts/VNEffects/VNScreenShake.cs) | target = **SceneRoot** | 只震最外层，见 0.2 |
| Parallax | [`VNParallax`](Assets/Project/Scripts/VNEffects/VNParallax.cs) | layers 加三条：LayerBack/8、LayerMid/13、LayerFront/19 | 前景动得多才有纵深 |
| DutchAngle | [`VNDutchAngle`](Assets/Project/Scripts/VNEffects/VNDutchAngle.cs) | target = **TiltRoot** | 独占旋转层 |
| VNCamera | [`VNCamera`](Assets/Project/Scripts/VNEffects/VNCamera.cs) | target = **ZoomRoot**；cameraFade = 第 5 章的 CameraFade | 独占运镜层 |
| Heartbeat | [`VNHeartbeat`](Assets/Project/Scripts/VNEffects/VNHeartbeat.cs) | target = **SceneRoot**；edgeGlow = EdgeGlow | 心跳缩放 + 红色泛光联动 |
| WeatherController | [`VNWeatherController`](Assets/Project/Scripts/VNEffects/VNWeatherController.cs) | additiveMaterial = VNAdditive.mat；moodTargets = [背景 fx]（有常驻立绘可一并加入） | 天气切换时给画面做调色联动 |
| MoodGrading | [`VNMoodGrading`](Assets/Project/Scripts/VNEffects/VNMoodGrading.cs) | 无 | 运行时自建双 Volume 做情绪调色交叉过渡 |
| FakeDoF | [`VNFakeDoF`](Assets/Project/Scripts/VNEffects/VNFakeDoF.cs) | backgroundFx = 背景 fx；backLayer = LayerBack | UI 没有深度缓冲做不了真景深，伪景深=模糊背景+微缩放背景层 |
| ToneMatch | [`VNToneMatch`](Assets/Project/Scripts/VNEffects/VNToneMatch.cs) | characters 可空（剧本场景运行时自动登记） | 立绘色调匹配背景 |
| SakuraBurst | [`VNSakuraBurst`](Assets/Project/Scripts/VNEffects/VNSakuraBurst.cs) | additiveMaterial = VNAdditive.mat；heartbeat = Heartbeat | 樱吹雪告白组合技 |
| SpeakerHighlight | [`VNSpeakerHighlight`](Assets/Project/Scripts/VNEffects/VNSpeakerHighlight.cs) | characters 可空（运行时登记） | 说话者提亮、非说话者压暗 |
| HeatHaze | [`VNHeatHaze`](Assets/Project/Scripts/VNEffects/VNHeatHaze.cs) | targets = [背景 fx]；additiveMaterial = VNAdditive.mat | 热浪扭曲+蒸汽雾 |

另外把 **[`VNVignetteFocus`](Assets/Project/Scripts/VNEffects/VNVignetteFocus.cs)** 挂到第 2 章的 `Global Volume` 物体上，
...
| 物体名 | 组件 | 设置 |
|---|---|---|
| Ambient_Dust | [`VNAmbientParticles`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs) | preset=Dust，tint=(1,0.97,0.88)，sortingOrder=11，sourceMaterial=VNAdditive.mat |
...
| ClickRipple | [`VNClickRipple`](Assets/Project/Scripts/VNEffects/VNClickRipple.cs) | sourceMaterial=VNAdditive.mat（点击涟漪+星光） |
| MouseStardust | [`VNMouseStardust`](Assets/Project/Scripts/VNEffects/VNMouseStardust.cs) | sourceMaterial=VNAdditive.mat（鼠标拖尾） |
...
> 挂组件赋值再激活"的模式（见 `VNAmbientParticles.Create`（[`Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs:60`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs#L60)））——手动搭建不用管，
> 但自己写新粒子组件时要遵守。
...
挂 [`VNDialogueBox`](Assets/Project/Scripts/VNEffects/VNDialogueBox.cs)。面板、流光边框、名牌、推进箭头、打字机文本、快捷功能条
（Save/Load/Auto/Skip/Log/任务/Config/隐藏UI）全部由它 Awake 自动构建。

**字体说明**（2026-07-18 起）：文本全部用 **TextMeshPro（TextMeshProUGUI）**。
运行时创建的文字由代码自动取 [`VNFont.Asset`](Assets/Project/Scripts/VNEffects/Script/VNFont.cs)（随包 Noto Sans SC 动态 SDF，
...
创建空物体 [`VNStage`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs) 挂 `VNStage`，这是**剧本命令 → 特效 API 的落地层**，
引用最多的组件。逐项连线：
...
创建空物体 [`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) 挂 `VNAudio`，拖给 `VNStage.vnAudio`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:109`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L109)）。
三个独立音频库按需填：
...
- 空物体 [`VNScriptRunner`](Assets/Project/Scripts/VNEffects/Script/VNScriptRunner.cs) 挂 `VNScriptRunner`：`stage` = VNStage、
  `script` = 你的 .vn.txt（TextAsset）、`playOnStart` 勾上；
- 空物体 [`VNBacklog`](Assets/Project/Scripts/VNEffects/Script/VNBacklog.cs) 挂 `VNBacklog`（H 键/滚轮回想历史）。
...
1. 空物体 [`VNEventRegistry`](Assets/Project/Scripts/VNEffects/Script/VNEventRegistry.cs) 挂 `VNEventRegistry`，拖给 `VNStage.eventRegistry`（[`Assets/Project/Scripts/VNEffects/Script/VNStage.cs:111`](Assets/Project/Scripts/VNEffects/Script/VNStage.cs#L111)）；
2. 其下建子物体 `QteTemplate`（RectTransform）挂 [`VNQteModule`](Assets/Project/Scripts/VNEffects/Script/VNQteModule.cs)，
   **SetActive 关掉**——模板必须禁用，运行时 Instantiate 副本才激活。
   **为什么**：带 Awake 初始化的模板若在场景里活着，会在启动时抢跑构建 UI；
3. 同理建 `MapTemplate` 挂 [`VNMapModule`](Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs)，填 `mapSprite`（地图底图）和
...
剧本用到 `quest` 命令时：`Assets/VNEffects/Quests/` 下创建 [`VNQuestDef`](Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs) 资产
（id/标题/描述/阶段文案），空物体 [`VNQuestLog`](Assets/Project/Scripts/VNEffects/Script/VNQuestLog.cs) 挂 `VNQuestLog` 并把资产拖进
...
- 创建空物体 [`VNEffectsDemo`](Assets/Project/Scripts/VNEffects/VNEffectsDemo.cs) 挂 `VNEffectsDemo` 脚本，把所有组件引用逐个拖上
