...
**落地要点**：
- [`VNStatDef`](Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs) 加 `GetFinal(name)`；加成来源存在不进存档的运行时列表，读档后由 [`VNEquipment`](Assets/Project/Scripts/VNEffects/Script/VNEquipment.cs) 重建
...
**映射本项目**：把 [`VNExpression`](Assets/Project/Scripts/VNEffects/Script/VNExpression.cs) 抽成**可被资产字段引用的类型**（`[SerializeField] string condition` + Inspector 抽屉），让 [`VNShopDef`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs) / [`VNQuestDef`](Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs) / 事件池共用一个求值器，Lint 也能一次检查所有条件串。
...
**映射本项目**：[`VNBattleModule`](Assets/Project/Scripts/VNEffects/Script/VNBattleModule.cs) 现在 `patkstat/phpstat/pdefstat` 从 flag **线性**读。加 `AnimationCurve` 字段即可获得「前期成长快、后期边际递减」的正常养成手感。
...
| option | 作用 | 本项目 |
|---|---|---|
| `lipSync` | 唇形同步 | ✅ [`VNCharacterMouth`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMouth.cs) |
...
已读记录是**跨周目**的（独立档案，与存档槽分离 —— 同 [`VNCgUnlocks`](Assets/Project/Scripts/VNEffects/Script/VNCgUnlocks.cs) 模式）。
...
**映射本项目**：[`VNSaveData`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 加 `string snapshotJson` 字段（走 `vn-save-compat` 三处同步）。
...
**本项目已有**：漫符（[`VNCharacterMarks`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMarks.cs) 更全）、表情、站位、震屏、擦除转场、CG Still、BGM/SE/语音。
...
演出几乎全现成：[`VNToast`](Assets/Project/Scripts/VNEffects/Script/VNToast.cs) 列加成条目、[`VNStatsHud`](Assets/Project/Scripts/VNEffects/Script/VNStatsHud.cs) 数字滚动、`fx flash`/`shockwave` 揭晓、`mood` 成败色调。只需新增一个转盘 UI。「命运点」= 一个新 [`VNStatDef`](Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs)。
...
| [`Option`](Assets/Project/Scripts/VNEffects/VNChoicePanel.cs)（对话选项） | 胜 → `OptionCfg.effect` + `talkId`；败 → `effect2` + `talkId2` |
| `Evt`（事件） | `ShowEvent(typeId)` |
| [`Talk`](Assets/Project/Scripts/VNEffects/Script/VNBadmintonDef.cs)（对话） | 胜 → `TalkCfg.effect` + `nextTalk` |
...
**做 `VNTween` 系列组件**（基类管 curve / duration / delay / loop / playOnEnable / **unscaled**），把已有写法包一层。**尤其 `TweenMaterialFloat`** —— [`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) 管着九种效果的材质属性（溶解量、扫光位置、发光强度、HSV），组件化 = shader 能力从「写代码」下放到「拖组件」。
...
**映射本项目**：背包、商店、任务日志、画廊、SNS 都是「玩家可能错过新内容」的地方。与已有的 `VNFlags.Changed`（[`Assets/Project/Scripts/VNEffects/Script/VNFlags.cs:21`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs#L21)） 事件天然契合。
...
**`HtmlTxtUtil` 直接抄进 [`VNToast`](Assets/Project/Scripts/VNEffects/Script/VNToast.cs) / 属性面板** —— 「+3 体力」的绿色、图标、数字格式化应该有唯一实现。
...
**映射本项目**：[`VNAudio`](Assets/Project/Scripts/VNEffects/Script/VNAudio.cs) 有三通道 + 基准音量，加这三个 —— 语音播放时自动 duck BGM、BGM 位置记忆、转场静音窗口。成本极低，听感提升明显。
...
本项目硬约定是「每张图独立材质实例」（[`VNImageEffectController`](Assets/Project/Scripts/VNEffects/VNImageEffectController.cs) 管理）—— 立绘多了之后是实打实的 GC 与 SetPass 压力。材质池 + `MaterialPropertyBlock` 是标准解法。

### 37. uGUI 内的粒子遮罩 💎💎 🔧中
> 出处：WitchSpring3 用了 `UiParticles`

本项目粒子（[`VNAmbientParticles`](Assets/Project/Scripts/VNEffects/VNAmbientParticles.cs) / [`VNFoliageSystem`](Assets/Project/Scripts/VNEffects/VNFoliageSystem.cs)）是场外 ParticleSystem + sortingOrder，**无法被 UI 的 Mask 裁剪、无法跟着 ScrollRect 滚动、无法插在两层 UI 中间**。
...
本项目 [`VNSpeakerHighlight`](Assets/Project/Scripts/VNEffects/VNSpeakerHighlight.cs) 现在大概是压暗非说话者 —— 改成「说话者轮廓光缓慢脉动」会更高级，且不损失非说话者可读性。
...
**映射本项目**：现在好感度大概是一个 `flag 好感_小雪`。建议至少加：关系阶层（独立 flag，只有剧情事件能推进）、本月互动次数（配合 `time` 重置）、送礼偏好表（挂在 [`VNShopDef.Item`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs) 上）、角色在场表（[`VNPlanDef`](Assets/Project/Scripts/VNEffects/Script/VNPlanDef.cs) 或日历系统承载）。
...
**映射本项目**：[`VNMapModule`](Assets/Project/Scripts/VNEffects/Script/VNMapModule.cs) + [`VNPlanModule`](Assets/Project/Scripts/VNEffects/Script/VNPlanModule.cs) + `quest` 接起来就需要这个调度器。`_alertDialogs`（走错地点的提示）尤其值得抄 —— VN 里最常见的玩家卡关点。
...
**映射本项目**：flag 是字符串 key（比数字 id 安全），但 [`VNStatDef`](Assets/Project/Scripts/VNEffects/Script/VNStatDef.cs) / [`VNQuestDef`](Assets/Project/Scripts/VNEffects/Script/VNQuestDef.cs) / [`VNShopDef`](Assets/Project/Scripts/VNEffects/Script/VNShopDef.cs) / [`VNWeatherDef`](Assets/Project/Scripts/VNEffects/VNWeatherDef.cs) / [`VNQuizDef`](Assets/Project/Scripts/VNEffects/Script/VNQuizDef.cs) / [`VNPlanDef`](Assets/Project/Scripts/VNEffects/Script/VNPlanDef.cs) 的 id 字段，以及装备部位编号（`装备_<道具id>=部位编号`）同样适用。**建议浓缩进 `vn-save-compat` 技能。**
...
**映射本项目**：[`VNSaveData.RestoreSnapshot`](Assets/Project/Scripts/VNEffects/Script/VNSaveSystem.cs) 加一个 `MigrateLegacy()` 阶段。
...
| 反射索引器 `this[string fieldName]` | 每次 `GetField()`，性能与类型安全双输。本项目 [`VNFlags`](Assets/Project/Scripts/VNEffects/Script/VNFlags.cs) 字典方案更好 |
