...
| 场景 | `Assets/Project/Scenes/` 3 个（`SampleScene` / `VNEffectsDemo` / `VNScriptDemo`） |
...
- **演出层不知道剧本存在**：`VNCamera` / `VNMoodGrading` / `VNAmbientParticles` 等只暴露
  `Show()/Hide()/SetXxx()/DOXxx()`，可以脱离剧本单独在演示场景里跑。
- **逻辑层通过 `VNStage` 这一个门面访问演出层**——`VNScriptRunner.Dispatch()` 几乎每个 case
  都是 `stage.XXX(...)`，从不直接摸 `VNGodRays` 之类的组件。
- **工具层可以反向读逻辑层**（Lint 复用 `VNScriptParser.Parse`，编辑器复用 `VNCamera.ContainerZoomFor`），
...
1. **文本 → 演出**：`.vn.txt` → `VNScriptParser` → `VNScriptCommand` → `VNScriptRunner.Dispatch()` → `VNStage.XXX()` → 特效组件。
2. **玩法 → 状态**：一切玩法结果最终都落到 `VNFlags`（整数字典）。属性、任务、装备、日程、
   道具、事件成绩**没有一个有自己的存储**，全都是 flag 的命名约定（第 6 章有完整命名空间表）。
3. **状态 → 存档**：`VNSaveData` = 剧本指针（`commandIndex` + `chapter` + 调用栈）+ 全部 flag + 舞台快照。

---

## 4. 剧本引擎：Parser → Runner → Stage

### 4.1 `VNScriptParser`（386 行，纯静态）
...
| `VNCamWaypointDef` | `point` / `zoom` / `duration` / `ease` / `fade`（xfade）/ `hold` / `shake` / `line`；常量 `StayToken = "stay"` + `IsStay()` |
...
反例保护：`title:第1章::序` 的第一个冒号后是「第」，仍按 kwarg 处理，`::` 原样传给 `VNStoryAddress`。

**`ParseCamWaypoint()` 的 `stay` 数字位前移**：普通路径点第 1 个数字是 zoom、第 2 个是时长；
...
### 4.2 `VNScriptRunner`（3097 行，全专案心脏）
...
`Start()` 里的**「自愈」模式**贯穿全专案：每个附属管理器都是
...
| `if` | 从**最后一个** `jump` token 往前重组条件串 → `VNFlags.Evaluate()` → `JumpTo()` | 瞬发 |
...
| `hideHUD` | `ParseHideHudArgs()` → `SetUiHidden(parts, hide, lock)` | 瞬发 |
...
两组互斥关系写在 `Fx()` 里：`filmgrain` ⇄ `crt` 互斥，且手动操作会把 `_retroAuto` 置 false
（接管 mood 自动滤镜）；`letterbox` 手动操作把 `_letterboxAuto` 置 false。

`ResetEffects()`（`reset effects`）把全部 toggle 关掉、天气清空、mood 回 Neutral、液体重置，
...
`VNStatsHud.MarkDirty()` / `VNCalendarHud.MarkDirty()` 就是这个模式（`_dirty` 标记 + `Update()` 里刷）。

### 6.2 `VNExpression`（278 行，纯静态递归下降解析器）
...
| 方法 | 用途 |
|---|---|
| `TryEvaluate(expr, valueResolver, out bool, out error)` | 运行时求值（`VNFlags.Evaluate` 调它） |
...
| 前缀 / 名字 | 常量定义处 | 写入者 | 语义 |
|---|---|---|---|
| `任务_<id>` | `VNQuestLog.FlagPrefix` | `VNQuestLog.Apply()` | 任务阶段；`StageDone = 100`、`StageFailed = -1` |
| `道具_<id>` | `VNShopDef.ItemFlagPrefix` | `VNShopModule` | 持有数量 |
| `装备_<道具id>` | `VNEquipment.EquipFlagPrefix` | `VNEquipment.Equip()` | 部位编号（`VNEquipSlot`） |
| `装备实增_<slot>_<statId>` | `VNEquipment.AppliedFlagPrefix` | `VNEquipment` | **实际生效的增量**（卸下时按此扣回，防钳制不对称） |
| `装备效果_<效果id>` | `VNEquipment.EffectFlagPrefix` | `RecomputeEffects()` | 特殊效果合计（生效逻辑由剧本 `if` 判断） |
| `日程_<N>` | `VNPlanModule.SlotFlagPrefix` | `VNPlanModule` | 第 N 格排的行动编号 |
...
| `<前缀>正确数` / `<前缀>总数` | `VNQuizModule.CorrectFlagSuffix/TotalFlagSuffix` | `VNQuizModule` | 答题成绩 |
| `<前缀>_我方得分/_对方得分/_精准数/_最长回合` | — | `VNBadmintonModule.WriteFlags()` | 羽球战绩 |
| `<前缀>_清晰度/_用时/_档位` | `VNFogWipeModule.*FlagSuffix` | `VNFogWipeModule.WriteFlags()` | 擦雾成绩 |
| `<前缀>_分数/_档位/_次数` | — | `VNPhotoBoothModule` | 大头贴成绩 |
| 属性 id（`金钱`/`行动力`/`好感度_结衣`…） | `VNStatDef.id` | `VNStatsHud.Apply()` | **属性值就是同名 flag**，`VNStatDef` 只提供钳制 / 样式 / 等级 |
| 任意剧本自定义 | — | `flag` 命令 / `choice` 的 `flag:` | 剧情开关 |

> 这个设计的代价：**flag 名冲突没有编译期保护**。Lint 里的 `CheckLoopGuards` / `FlagNames()`
...
| mood Memory → 自动黑边 / 胶片 | 用 `autoLetterbox` / `autoRetro` 布尔重放 `VNStage.SetMood` 的自动联动逻辑 |
...
`silent = true` 时跳过 `VNToast` 飘字与音效，只改状态。
...
2. `VNStage.CaptureSnapshot()` + `RestoreSnapshot()` 成对；
3. `VNScriptRunner.RebuildStateBefore()` 加对应 case。

**刻意不进存档的状态**（各有明确理由）：

| 状态 | 为什么不存 |
|---|---|
| 镜头（`VNCamera`） | 调试重建走 `SnapReset()`；镜头位置玩家看不出「读档后该不该保留」 |
| 立绘痕迹（`VNCharacterImprints`） | 它会自己褪色消失，也就没有「读档后该不该还在」的问题 |
| 过场章节卡（`VNInterludeScreen`） | 演出中不可存档；但 `ClearStage()` 必须 `HideImmediate()` |
...
`Launch()` 里教程**必须在 `OnLaunch` 之后**播——要高亮记分板，记分板得先存在。
讲解期间 `VNPause` 冻住全局，模块 `Update()` 第一行早退，所以模块虽然「开着」但一帧都不会跑：
...
| # | 铁律 | 为什么 |
|---|---|---|
| ① | **只操作自己的 UI 子树与 `VNFlags`**，不直接改舞台演出 | 背景 / 立绘交给事件前后的剧本行；否则存档与调试重建对不上 |
...
- `VNAiTalkModule` 破铁律 ①：直接驱动舞台立绘换表情。理由是自绘立绘要把眨眼 / 口型 / 色调匹配 /
  出场动画全部重接一遍。边界收紧为「只碰表情和对话框内容」，且**正常结束 / ESC / `CancelForDebug`
  三条路径都还原**（`RestoreStage()`）。
- `VNInteractionModule` 同样破例：只碰表情与叠加层，三条退出路径都还原（`RestoreExpression()`）。

**铁律 ② 的现代写法**：`VNTime.Delta`（含 `MaxStep = 0.05f` 单帧上限，收编自羽毛球的防瞬移），
配合 `if (VNPause.IsPaused) return;` 作为 `Update()` 首行（**必须在 `ReadInput` 之前**，晚了照样能挥拍）。

**射线坑**：EventLayer 排序 60 在 `VNChoicePanel` 45 之上，所以模块自绘的一切默认
...
| 模块 | 剧本写法 | 结果名 | 核心机制 |
|---|---|---|---|
| `VNQteModule` | `event qte [time:] [target:] [title:]` | `success` / `fail` | 连打条；`Update()` 里数点击 |
| `VNMapModule` | `event map` | 地点名 | 归一化坐标标记 + `condition` 显隐 + `AcceptsOutcome` 过滤 + `去过_xx` flag |
| `VNBattleModule` | `event battle enemy: ehp: eatk: phpstat: patkstat: pdefstat: escape: pname:` | 胜利 / 失败 / 逃跑 | 回合制；`StatOrKw()` 让属性从 flag 读 = **养成联动** |
| `VNShopModule` | `event shop id:` | — | 买卖走 `currencyStat` + `道具_<id>` flag；买 / 卖两页签 |
| `VNPlanModule` | `event plan id: slots: pool: title:` / `event plan op:next` | — | 排格写 `日程_<N>`；`op:next` 派发到 `当前行动`；`RecordInBacklog => !_dispatchMode` |
| `VNResultPopupModule` | `event result grade:fail\|normal\|good\|great [title:] [sub:] [se:]` | — | 冲条 0→100 悬念 → 四档大字 + 星光爆发 |
| `VNQuizModule` | `event quiz id: count: time: pass: pick: flag:` | 全对 / 及格 / 失败 | 三语题库；超时按答错；最后 3 秒红色脉动 |
| `VNBadmintonModule` | `event badminton vs: id: target: first: mode: powerstat:/speedstat:/jumpstat: flag:` | 胜利 / 失败 / 结束 | 弹道数学（见 8.4） |
| `VNPhotoBoothModule` | `event photo vs: me: theme: mode: frame: bg: time: stat:/rate: flag:` | 完美/普通/失败 **或** 完成 | 四标签页装扮 + 取景框截图 + 清单制评分 |
| `VNFogWipeModule` | `event wipefog id: cg: time: target: perfect: stat:/rate: flag:` | 完美 / 普通 / 失败 | 掩码擦除 + 回雾（见 8.5） |
...
**弹道核心**（`VNBadmintonBallistics`，全静态）：
...
按 `VNBadmintonDef.powerPerStat` 等系数（上限 `statCap`）折算成 `_bonusPower` / `_bonusSpeed` / `_bonusJump`，
...
到 `VNTutorialAnchors`，`OnDestroy` 里 `UnregisterTutorialAnchors()`。
...
配套：`VNFogSfx`（四个代码合成音效——**擦拭循环音是唯一的速度反馈**，用一段 1 秒滤波白噪改 pitch
而非切多段素材，切段会爆音）、`VNFogTextures`（四种道具光标的程序化 SDF 贴图）、
`VNFogTuneWindow`（调参窗口，见 17.1）。
...
**`Contains` / `Pick` 是纯静态的**——所以**编辑器画框工具（`VNTouchZoneEditorWindow`）与运行时用同一份命中判定**，
画布上点得中的地方游戏里就摸得到。

**阶段只升不降**（`StageFor` 的 `current` 参数）：允许回退的话玩家一停手，表情就会在阈值边界反复横跳。

**`VNTouchCursor`（247 行）不用 `Cursor.SetCursor`**——硬件光标做不了「摆动 / 按住震动 / 速度倾斜 / 悬停发光」
这四件事，而它们正是手感来源（`VNCursorIdleAnim` / `VNCursorPressAnim` 两个枚举 + 8 个动画参数）。
...
| `VNPhotoScore` | **纯静态评分数学**：`Evaluate(VNPhotoDressing, VNPhotoThemeDef) : Result`（清单制加分 + 命中评语）；结果名常量 `OutcomePerfect/Normal/Fail/Free` |
| `VNPhotoTextures` | 程序化边框 / 遮罩 / 10 种贴纸 / 相纸 |
| `VNPhotoCapture` | **取景框截图**：`Capture(RectTransform, Canvas, ...)` + `ScreenRectOf()` + `Crop()` + `Downscale()`（`MaxSide = 1600`）——「怎么拍」全在这一个文件，换 RenderTexture 只改它 |
| `VNPhotoAlbum` | 全局相册（PNG + `index.json`，LRU 纹理缓存 `TextureCacheSize = 12`、缩略图 `ThumbnailWidth = 192`、`Capacity = 200`） |
| `VNPhotoDoodle` | 涂鸦：**两张 768×576 位图画布**（普通笔走 Alpha、荧光笔走 `VN/Additive`），线段插值补点，撤销 5 步 |
...
| 方法 | 作用 |
|---|---|
| `Apply(name, valueToken, silent, line)` | 解析 `+5` / `-3` / `500`（也支持 `stat 金钱+5` 挤在一个 token 里）→ `Clamp` → `VNFlags.Set` → Toast 卡片 |
| `EnsureInitials()` | 首次进游戏按 `initialValue` 种入 |
...
**演出双轨**：HUD 就地（数字滚动 + 条补间 + 图标弹跳 + `+N` 上飘）+ 左上角 `VNToast` 卡片
（图标 + 主题色认属性、左侧竖条认涨跌）。

### 9.2 `VNEquipment`（纯静态，194 行）
...
`VNCalendarHud` 订阅 `VNFlags.Changed` 标脏 → `Update()` 里 `Refresh()`。
...
| 文件 | 职责 | 关键点 |
|---|---|---|
| `VNAiProvider` / `VNAiProviders` | 供应商能力登记表 | `SupportsResponseSchema()` / `SupportsSafetySettings()` / `EnvVarFor()` / `KeyFileFor()` / `DefaultModelFor()` / `TryFromModelName()`；全局默认在 `VNGameConfig.aiProvider`（当前 **DeepSeek `deepseek-v4-flash`**） |
| `VNAiKey` | Key 三级回退 | 环境变量 → 仓库外 → 仓库内；**按供应商各一套**分开缓存；`#if UNITY_EDITOR \|\| VN_AI_ALLOW_LOCAL_KEY` 挡住 Build 版本读取；永不打印 |
| `VNAiClient`（347 行） | **全专案唯一碰 HTTP 的文件** | `Send(req, onDone) : IEnumerator`；失败分 8 类 `VNAiFailure`；429 / 5xx / 网络错误指数退避（`maxRetries = 2`）；协程封装而非 async/await（与 `EventCo` 的轮询等待天然契合） |
| `VNAiClientGemini` / `VNAiClientDeepSeek` | 只差「拼请求体」和「解响应」 | 纯静态，不碰网络 |
| `VNAiConversation`（798 行） | **纯逻辑层，无 MonoBehaviour** | prompt 组装、schema 生成、历史裁剪、响应解析与钳制 |
| `VNAiPersonaDef` | 人格资产 | 独立于 `VNCharacterDef`（一个角色可有多套人格共用立绘）；`MinOptions = 3` / `MaxOptions = 6` |
| `VNAiMemory` / `VNAiDiary` / `VNAiDiaryPanel` | 跨场记忆 / 日记本 / D 键面板 | **存储语义刻意相反**，见 10.4 |
| `VNAiPricing` / `VNAiPricingDef` | **算钱的唯一入口** | 见 10.5 |
| `VNAiTextNormalize` | 繁转简兜底 | 提示词管「大部分时候对」，兜底管「永远不出错」；作用于台词 / 选项 / 日记 / 摘要 / 话题 |
| `VNAiTalkLog`（431 行） | 对话日志 | `VNAiLogMode`（Off / EditorOnly / Always）→ `AiTalkLogs/*.json` |
...
- **DeepSeek**：没有 `json_schema`（只有 `json_object`，格式改由提示词约束 + 解析层兜底）；
  **历史里 assistant 的消息也必须是 JSON**（是纯文本时模型会退化成吐一串空白 → 第 2 轮起必挂，
  故 `VNAiConversation.WrapReplyAsJson()` 存在）；system 是 messages 第一条；助手角色叫 `assistant`；
...
|---|---|---|
| 存哪 | `VNSaveData.aiMemories`（存档槽内） | `persistentDataPath/vn_ai_diary.json`（全局） |
...
思考 token 按输出价计费。资产走 `Create → VN → AI Pricing` 登记进 `VNGameConfig.aiPricing`，
不建资产就用内置默认表。
...
### 11.1 `VNGameConfig` 存在的理由

场景生成器 `VNEffectsDemoSetup` 内部会 `EditorSceneManager.NewScene(EmptyScene)`，
...
`VNGameConfigTools.HookPlayModeCacheClear()` 在每次进出 Play Mode 时调它。
...
- 不放：**能靠「扫目录」自动得到的**——角色 / 属性 / 商店 / 日程 / 任务定义资产、章节剧本、CG
  （由 `VNGameConfigTools.RescanAssetFolders()` 自动登记）。
...
`VNGameConfigEditor` 把它画成九页分页 Inspector：**剧本｜标题｜UI 皮肤｜舞台｜音频｜玩法｜AI｜大头贴｜全部**
...
| 资产 | 被谁读 | 关键字段 |
|---|---|---|
| `VNCharacterDef` | `VNStage` | `id` / 三语 `displayName` / `nameColor` / `overrideNameplateColors` + 三色 / `expressions` / 眨眼三参 + `blinkMode`（`VNBlinkMode`）/ 口型 / `overlays` / `imprints` / `markSprites` + `markAnchor` + `markScale` / `sizeScale` / `positionOffset` / `rotationZOffset` / `portraits` + `portraitScale/Offset`；方法 `GetSprite(expr)` / `ActiveBlinkSprite` / `DefaultSprite` / `IsDefaultExpression()` / `GetPortrait()` / `GetNameplateColors()` |
| `VNStatDef` | `VNStatsHud` | 见 9.1 |
| `VNShopDef` | `VNShopModule` / `VNInventory` / `VNEquipment` | `shopId` / 三语 `shopName` / `currencyStat`（默认「金钱」）/ `items`（`Item` 含 `StatOp` 列表、`PassiveEffect` 列表、装备槽 `VNEquipSlot`）；常量 `ItemFlagPrefix = "道具_"` |
| `VNQuestDef` | `VNQuestLog` | `id` / `title` / `description` / `stages`（各三语）；`StageText(stage)` |
| `VNPlanDef` | `VNPlanModule` | `planId` / 三语 `title` / `actions`（`ActionDef` 带编号、条件、消耗）；`FindByNumber(n)` |
| `VNQuizDef` | `VNQuizModule` | `quizId` / `defaultTimeLimit` / `flagPrefix` / `questions`（`MaxOptions = 4`，各三语，每题奖惩）；`ValidQuestions()` 过滤非法题（题干空 / 选项 < 2 / 答案序号越界） |
| `VNBadmintonDef` | `VNBadmintonModule` | 三语对手名 + 立绘 + `tuning` + `powerPerStat`/`speedPerStat`/`jumpPerStat` + `statCap` + 五个音效 + `TalkSet` 台词池；`TalkEnabled` |
| `VNFogWipeDef` | `VNFogWipeModule` | 雾外观九参（`fogColor`/`fogMix`/`fogDensity`/`blurAmount`/`brightness`/`edgeNoise`/`noiseScale`/`grain`/`falloff`）+ 笔刷三参 + `cursorKind`（`VNWiperKind`）+ 回雾五参 + 三档门槛 + `stages` 分阶段台词 + 四音效；`SortedStages()` |
| `VNInteractionDef` | `VNInteractionModule` | `VNInteractionItem`（道具 + 光标动画 8 参）、`VNInteractionStage`、`VNInteractionZoneRule`、`VNInteractionFeedback`（含 `trigger`（`VNFeedbackTrigger`）/`cooldown`/`weight`/`blocking`/`scriptLines`/`voicePool`） |
| `VNTouchZoneDef` | 同上 + 编辑器 | 见 8.6 |
| `VNAiPersonaDef` | `VNAiTalkModule` | 见 10.2 |
| `VNTutorialDef` | `VNTutorialPlayer` | `steps`（`anchor` / `area` / `shape`（`VNTutorialHole`）/ `padding` / `corner` / `feather` / 三语标题正文 / `image` / `card`（`VNTutorialCardSpot`）/ `se`）+ `edgeColor` / `edgeWidth` / `allowSkip` / `once` |
| `VNInterludeDef` | `VNInterludeScreen` | 三语标题副标题 + `voices` 池 + `images` 池 + `loadingDuration = 1.5f` + `enter`（`VNInterludeEnter`）/ `transition`；`PickVoice()` / `PickImage()` |
| `VNWeatherDef` | `VNWeatherController` / `VNFoliageSystem` | 五套内置预设，不建资产也能用 |
| `VNPhotoFrameDef` / `StickerDef` / `BackdropDef` / `ThemeDef` | `VNPhotoBoothModule` | 主题是**清单制评分表**（`expressionRules` / `frameRules` / `backdropRules` / `stickerRules` 四张加分清单 + `perfectComment` / `normalComment` / `failComment`）；`baseScore = 20` / `perfectLine = 70` / `passLine = 40` |
| `VNLiquidPreset` | `VNLiquidSplash` / `VNWetScreen` | water / blood / ink / slime 四套（黏度 = 重力 + 拉伸 + 下滑速度 + 干涸时间四参数合谋） |
| `VNSystemUiSkinSet` | 全部系统面板 | 唯一全局主题 prefab 集 |
| `VNAiPricingDef` | `VNAiPricing` | 每百万 token 单价表 + 高峰时段 |
...
| 40 | `VNDialogueBox` | |
| 45 | `VNChoicePanel` | |
| 60 | `VNEventRegistry` EventLayer | **在选项面板之上** → 模块自绘元素默认 `raycastTarget = false` |
| 90 | `VNInterludeScreen` | 必须挂主 Canvas（Overlay 画布会永远压在 Screen Space - Camera 之上） |
| 92 | `VNTutorialPlayer` | 同上；且洞口 HDR 描边靠 Bloom，Overlay 吃不到 |
| 100 | `VNScreenTransition` | |
| 500 | `VNTitleMenu` | 同场景覆盖层 Canvas |

### 12.2 ★ 分层调色：`VNGrade` / `VNGradeLayer`
...
立绘目标由 `VNStage.RefreshRegistries()` 在角色进出场时自动维护。
...
`Memory` 自动上电影黑边 + 胶片滤镜、`Dream` 自动上 CRT（由 `VNStage.autoMemoryLetterbox` /
...
**两个静态公式是「编辑器预览与运行时同一份」的落点**——`VNCamseqEditorWindow` 画取景框时调它们，
所以窗口里看到的构图与游戏里一致。
...
三级震动的「等级 → 数值」唯一一张表在 `VNShakeSpec`（`TryParse` 同时被 Parser、运行时与编辑器预览共用）。
...
`CreatePatternMaterial()` / `ConfigurePattern(mat, type, aspect, ...)` 是静态共用的材质配置入口
...
**飘落天气走 `VNFoliageSystem`**（三层景深 + 图集翻转 + **每粒子独立相位横摆** + 自动阵风
+ 尺寸↔速度伪透视 + 地面堆积），雨 / 雪 / 萤火虫走 `VNAmbientParticles`；
`VNWeatherController.SetWeatherId` 三级解析 id（自定义资产 → 内置叶型别名含中文 → `VNWeather` 枚举）。
...
| `VNRetroFilter` | 胶片颗粒 / CRT |
| `VNShockwave` | 全屏水波 |
| `VNTutorialMask` | 教程挖洞（圆角矩形 / 椭圆 + 羽化 + 最多 4 洞） |
...
`VNSystemUiSkinSet` + 安全实例化基类 `VNSystemUiSkinBehaviour`，覆盖：

| 面板 | 槽位组件 |
|---|---|
| 标题 | `VNTitleMenuSkin` |
| 设置 | `VNConfigPanelSkin` |
| CG 画廊 | `VNCgGallerySkin` + `VNCgCellSkin` |
| 回想 | `VNBacklogSkin` + `VNBacklogEntrySkin` |
| 快捷条 | `VNQuickToolbarSkin` + `VNToolbarActionSlot` |
| 存读档 | `VNSaveLoadSkin` + `VNSaveSlotSkin` |
| 顶栏属性 HUD | `VNStatsHudSkin` + `VNStatsHudEntrySkin` |
| 完整属性页 | `VNStatsPanelSkin` + `VNStatsPanelRowSkin` |
| 背包 | `VNInventorySkin` + `VNInventoryRowSkin` + `VNInventorySlotSkin` |
| 排程 | `VNPlanSkin` + `VNPlanActionRowSkin` + `VNPlanSlotRowSkin` |
| 结算 | `VNResultPopupSkin` |
| 教程卡 | `VNTutorialSkin`（只 `panelRoot` + `bodyText` 必需；**暗幕不走皮肤，它是功能件**） |

**降级规则统一**：单项缺失或槽位无效时**只退回该项的程序化 UI**，不会整个面板崩掉。
（`VNResultPopupModule` 是唯一例外——它 `throw new InvalidOperationException` 要求 prefab 必须存在。）

### 13.3 `VNNameplateStyle` 的三条硬约定
...
配色每角色一套（`VNCharacterDef` 没勾 `overrideNameplateColors` 就由 `nameColor` 自动推算渐变，存量资产零改动）。

### 13.4 UI 隐藏 `VNUiParts` / `hideHUD`
...
- **锁定隐藏**（`hideHUD keep`）：玩家点击只推进台词，不会把界面弹回来；
  ...
  **只有这种才写进 `VNSaveData.uiHidden`**（`VNUiPartsUtil.ToToken(parts)`）。

### 13.5 SNS 手机聊天 `VNSnsView`（1102 行）
...
抽取 / 校验走 `VNLocalizationTools.ExtractAll()` / `ValidateAll()`（增量，已译保留）。
...
- `VNFont.DisplayAsset` — **装饰字体**（思源黑体 Black，名牌等少量大字专用，正文别用）。
...
| **② 全局 JSON** | `vn_cg_unlocks.json`（`VNCgUnlocks`）<br/>`vn_ai_diary.json`（`VNAiDiary`）<br/>`vn_tutorial_seen.json`（`VNTutorialSeen`）<br/>`vn_photos/`（`VNPhotoAlbum`） | CG 解锁、日记、教程看过记录、大头贴相册 | **不回退** | 玩家的**收藏品 / 已知信息**；开新周目也不该重看教程 |
...
> 「读旧档她不该记得未来」→ 存档槽（`VNAiMemory`）；
> 「读旧档不该让玩家丢东西」→ 全局 JSON（`VNAiDiary` / `VNCgUnlocks` / `VNPhotoAlbum` / `VNTutorialSeen`）。

**同一个 AI 聊天会话，一次总结产出的两份数据分别落进 ① 和 ②**，这是理解本专案存储哲学的最佳例子。

**一个刻意的反例**：编辑器试聊台的 `VNAiStudioMemory` **完全独立于运行时 `VNAiMemory`**——
编辑器往运行时记忆里写等于造出「读旧档她却记得未来」的幽灵状态。
从游戏存档槽导入记忆时，它**自己读 JSON，绝不调 `VNSaveSystem.Load()`**——
那个会 `VNFlags.Clear()` 冲掉工程 flag。

---

## 16. 输入优先级栈与全局暂停

### 16.1 `VNScriptRunner.Update()` 的优先级栈（顺序即优先级）
...
**`RequestAdvance()` 的存在理由**：`Update()` 第一行就是 `if (_eventActive) return;`，
于是模块内部用 `RunInlineCo()` 播的阻塞台词会死等 `_advance`，玩家点破屏幕也过不去。
...
**漏一条就是游戏永久卡死**——所以除了句柄绑宿主，Runner 还有 `ReleaseAll()` 兜底。

### 16.3 教程 `VNTutorialPlayer` 的三个细节
...
「看过了」记录走 `VNTutorialSeen`（**全局 JSON**）：`Has(id)` / `Mark(id)` / `ResetAll()` / `Enabled`
...
**`VNScenarioSchema`（698 行）** 声明每条命令长什么样：
...
| 能力 | 实现 |
|---|---|
| 素材候选按覆盖语义取数 | `PickLibrary(fromConfig, fromScene)` — `VNGameConfig` 填了就用它的，留空才回退场景组件。**以前只读场景组件，导致在配置资产里新登记的素材在下拉里根本搜不到** |
| 素材登记后自动刷新 | `VNAssetLibraryEvents.RaiseChanged()`（**只在 `ApplyModifiedProperties()` 返回 true 时发**——OnGUI 每帧都 Apply，无条件广播 = 每帧重建全部候选；订阅方 `OnDisable` 必须退订，否则静态事件会攥着已销毁窗口的引用） |
...
| 打字搜索 | `VNCommandSearch`：行首命令按钮**右键** = 打字换命令（左键仍是分类菜单）；`Ctrl+E` 向导式命令面板（选命令 → 逐个问位置参数 → 可选参数菜单循环 → Enter 插入 / Shift+Enter 插上方 / Tab 跳过 / Esc 取消）；**匹配只做子串包含**（中英皆可，不做模糊 / 拼音） |
...
### 17.3 镜头编排 `VNCamseqEditorWindow`（3289 行）

- **双向绑定**一行 `camseq`：改窗口 → 回写 `VNRow.camLines`；改剧本行 → 窗口跟随（可开「跟随选中」）。
...
- 辅助线逐项勾选（`enum Guides`，存 EditorPrefs `VNCamseq.Guides`）：三分线 / 中心十字 / 安全区 / 对话框遮挡区。
  **整图模式画在选中路径点的取景框内**（对话框不随镜头缩放，遮挡区只能按占一屏的比例落位）；
  镜头视角模式铺满画布。遮挡区尺寸**实测 `VNStage.dialogue`**，量不到才退回默认布局。
...
- **撤销是窗口内独立栈**（`UndoDepth = 64`，快照 = `GenerateText()` 文本，
  `UndoIdleSeconds = 0.35` 让拖滑条合并成一次），`Ctrl+Z/Y` 走 ShortcutManager 窗口作用域，
  **不挂 Unity 全局 Undo**；换绑定行清空历史。
- 辅助文件：`VNCamWaypoint`（严格 `TryParse` / `Format`，与运行时 `ParseCamWaypoint` 同构；
...
### 17.4 静态校验器 `VNScenarioLinter`（1657 行）
...
### 17.5 素材浏览器 `VNAssetBrowserWindow` + 配套
...
| `VNAssetUi`（538 行） | **Sprite 缩略图不用 `AssetPreview`**（异步，几十张一起等会闪空白）——Sprite 自己知道在哪张 texture 的哪个 UV，`DrawTextureWithTexCoords` 同步画即可（texture 不必可读）。音频没这捷径只能异步 + 占位，但**不能无限等**（自给 3 秒窗口；`IsLoadingAssetPreview(int)` / `GetInstanceID()` 在 Unity 6.5 是 **error 级弃用**不能用）。试听走 `UnityEditor.AudioUtil` 反射（`PlayPreviewClip` / 老版 `PlayClip` 逐个探测，探不到就灰掉按钮） |
| `VNConfigEntryDrawers`（339 行） | 背景 · CG · 音频 · UI 皮肤四个条目的**紧凑单行 drawer**；**drawer 挂在类型上**，所以 `VNStage` / `VNAudio` 组件的同名列表也一并变紧凑 |
| `VNAssetTheme`（405 行） | 配色主题（默认 / 樱花粉白，顶栏 🌸 切换，进 EditorPrefs）。**Unity 编辑器整体只有 Light/Dark 改不了**（USS 打包在编辑器资源里不开放覆盖，第三方替换 skin 的做法升级即坏），但自己 IMGUI 画的窗口每一像素归自己管。圆角靠**程序化生成白色圆角贴图 + `GUI.color` 染色**（static 贴图域重载会丢，故 lazy 重建 + `HideFlags.DontSave`）。**换浅色底后必须显式覆盖每个 `GUIStyle` 的文字色**——Dark 主题下 EditorStyles 文字是浅色的，直接用就是白底白字；且要覆盖 normal/hover/active/focused 及 `on*` 共八个状态。主题是**叠加不是替换**：`Enabled == false` 时所有绘制原样退回 Unity 原生 |
| `VNTextureImportDefaults`（188 行） | 素材目录里**首次导入**的图自动设 `Sprite (2D and UI)` + `Single`（`OnPreprocessTexture` 在导入前跑）。**白名单目录**（`Roots`）——绝不能全项目一刀切（`Art/Models/**` 下 60+ 张法线 / 粗糙度贴图按 sRGB 的 Sprite 导入会让光照全错且极难联想到导入设置）。只在 `importSettingsMissing` 时设，所以手调过的 Pivot / MaxSize / Multiple 切图永不被打回。**坑**：`importSettingsMissing` ≠「没有 .meta」——meta 存在却缺完整设置块时也为 true；而想用「.meta 不存在」卡死新文件**也不行**，Unity 调 preprocessor 前就已写盘 |

### 17.6 AI 试聊台 `VNAiStudioWindow`（1089 行）
...
| 配套 | 职责 |
|---|---|
| `VNAiStudioDraft`（250 行） | **临时 SO 副本**当草稿层：`SerializedObject` 迭代画 = 零 UI 代码就有全部字段、加新字段自动跟上；写回逐属性 copy 而**不用 `CopySerialized`**（那会把 `m_Name` 一起抄成 `xxx(Clone)`） |
| `VNAiStudioSession`（349 行） | 会话驱动，域重载后靠轮次记录 `BuildRequest` + `RecordReply` 重建历史 |
| `VNAiStudioMemory`（370 行） | 可命名记忆预设（`<项目根>/AiTalkStudio/Memories/*.json`，不进 git）；两个独立开关：`注入记忆`（勾掉再跑一遍 = 直接对比有无记忆的差别）与 `结束时做总结`（结果先预览再决定收不收）。导入三源：试聊后总结 / 从 `AiTalkLogs` 日志（**要发一次总结请求**，日志里没有 summary/topics/facts）/ 从游戏存档槽（**自己读 JSON**） |
| `VNAiStudioLog`（94 行） | 导出到 `AiTalkLogs/Editor/`，**与游戏内同格式**所以两边能互相对比 |
| `VNAiEditorCoroutine`（106 行） | 编辑器协程泵，与自检菜单共用 |
| `VNAiCostReport`（414 行） | 花费累计报表：扫 `AiTalkLogs/`（含 `Editor/`）全部 json，按月 / 日 / 人格 / 模型 / 来源聚合，可导 CSV。**默认按当前单价重算**（日志存了 token 数、模型名与缓存命中数，能修正历史上按写死单价算出的错误金额；高峰倍率按日志里那场对话当时的时间判） |
| `VNAiConnectionTester`（245 行） | 测试连接（默认 / Gemini / DeepSeek）、查看 Key 状态、试聊 3 轮 |

### 17.7 场景生成器与装机器

**两种截然不同的策略**：

| | 生成器 `VNEffectsDemoSetup`（1490 行） | 装机器 `VN*Installer` |
|---|---|---|
...
装机器的典型内容（以 `VNLiquidInstaller.Install()` 为例）：Canvas 下补 `VNWetScreen`
+ 场外补 `VNLiquidSplash` + 两层互连并回填 `VNStage`。
**老场景不跑它的话 `liquid` 命令会静默无效果**。
`VNQuizInstaller` 补禁用的 `QuizTemplate`（**必须带 RectTransform**）+ 登记题库；
`VNPhotoBoothInstaller`（481 行）缺资产会自动铺一套默认的。

### 17.8 其余工具

| 工具 | 职责 |
|---|---|
| `VNGameConfigTools` | `CreateOrSelect()` 建 / 选配置资产；`ImportFromScene()` 场景绑定回填资产；`RescanAssetFolders()` 扫目录自动登记定义资产 / 章节（`ScanChapters`）/ CG（`ScanCg` + `EnsureSpriteImport`）；`HookPlayModeCacheClear()` 进出 Play Mode 清缓存 |
| `VNLocalizationTools` | `ExtractAll()` 增量抽取翻译表（已译保留）；`ValidateAll()` 查缺译 |
| `VNFontAssetBuilder` | `CreateMenu()` 预烘焙 TMP 动态字体资产；`RebakeChineseFont()`；`RepairFontMaterialReferences()`；`EnsureFontAsset()` |
| `VNUiSkinExporter` | 烘焙对话框 / 选项默认皮肤 prefab 并自动登记（默认 + 顶部 + 右列样例） |
| `VNSoftSkinExporter` | 一键出三套**无框渐变**皮肤（白 / 粉 / 黑：整屏底部渐变带 + 居中台词，`shineFrame` 留空即无边框） |
| `VNSystemUiSkinExporter`（661 行） | `ExportAll()` / `ExportEventPanels()` / `ExportConfigPanel()` / `ValidateAll()` |
| `VNCharacterVisualPreviewWindow`（1176 行） | 角色资产可视化调参（尺寸 / 站位 / 旋转 / 眨眼 / 口型 / 头像框取实时预览） |
| `VNWeatherPreviewWindow` | `VNWeatherDef` 调参实时预览 |
| `VNTouchZoneEditorWindow`（569 行） | 在立绘上拖框画部位（拖框体 = 移动、拖右下角 = 改尺寸）；点选用运行时同一个 `Contains`；继承框画灰线、本层实线、禁忌部位红色；**撤销是窗口内独立栈**；改完必须 `InvalidateCache()` |
| `VNFogTuneWindow`（331 行） | 上半按「每秒擦除面积 ≈ 笔刷直径 × 鼠标速度」**算出**预计通关秒数并给手感评语，下半是可拖鼠标试擦的掩码预览。**这个窗口是刚性需求不是加分项**——难度唯一来源是笔刷面积 vs 回雾速度，手感全压在参数上 |
| `VNGameConfigEditor`（619 行） | 九页分页 Inspector + 智能列表（见 11.2） |
| `VNTutorialSamples` | 导出羽毛球示例教程资产 |
| `VNTextPromptWindow` | 通用文本输入弹窗 |
...
2. **事件模块与菜单类 UI 用 unscaled 时间 + `SetUpdate(true)`**；dt 一律走 `VNTime.Delta`；
   `Update()` 首行 `if (VNPause.IsPaused) return;`（**必须在 `ReadInput` 之前**）。
3. **输入只用新 Input System**（`Keyboard.current` / `Mouse.current`），禁用旧版 `Input.` API。
4. **玩家可见字符串一律 `VNLocale.T(key)`**；TMP 字体一律 `VNFont.Asset`；禁止 legacy Text。
5. **调色走 `SetGrade(通道, ...)`**，缩放走 `DOScaleMultiplier` / `DOCamScaleMultiplier` 双通道。
6. **每张图独立材质实例**（`VNImageEffectController` 管理）；改 `sharedMaterial` = 污染全局。
7. **贴图全程序化生成**（`VNProceduralTextures` 1011 行 + `VNFoliageTextures` + `VNPhotoTextures`
   + `VNFogTextures`），零美术依赖。
8. **「旧场景自愈」**：引用为空自动 Find / 自动建（`VNStage.AutoWire`、Runner 的 `Ensure*` 族）——
   加新字段不必重建场景。
9. **防卡死兜底遍布交互路径**：choice 全隐藏 → 全显示；全付不起 → 全解禁；map 无可用地点 → 直接返回；
   sns reply 同理。
10. **静默重放（`silent` 参数）**贯穿 `ApplyFlagCommand` / `VNStatsHud.Apply` / `VNQuestLog.Apply` /
...
13. **纯逻辑层与 MonoBehaviour 分离**：`VNBadmintonBallistics` / `VNFogMask` / `VNFogScore` /
    `VNTouchScore` / `VNPhotoScore` / `VNAiConversation` / `VNExpression` / `VNEquipment` /
    `VNTouchZoneDef.Contains` / `VNCamera.CharacterScaleFor` 全都可单测，
    且**编辑器工具与运行时共用同一份**。
14. **模块自绘 UI 默认 `raycastTarget = false`**（EventLayer 60 在 ChoicePanel 45 之上）。
15. **`VNTouchCursor.Dispose()` 必须在四处调**（`Finish` / `CancelForDebug` / `OnDestroy` / `OnDisable`）。
...
| # | 位置 | 问题 |
|---|---|---|
| 1 | `VNScriptRunner.ApplyFlagCommand` | `flag 名 rand:1-100` 在调试重建时会**重新掷骰**（注释已自认），重建出的分支状态可能与实际游玩不同；`event` 结果同样不重放 |
| 2 | `VNStage.ShowInstant()` line 1020 | 重建 y 坐标用写死的 `-60f + def.positionOffset.y`，与 `SlotPosition()` 的 `-60f` 魔数耦合；`VNScriptRunner.DebugSlotX()` 也复制了同一组站位常数——**同一组数字散在三处** |
| 3 | `VNQteModule.cs` line 3 | 多余的 `using Unity.AppUI.UI;`（未使用） |
| 4 | `VNSaveSystem.Load()` | 直接在静态方法里改 `VNFlags`（读档失败时 flag 可能已被清）。目前由调用方 `LoadFrom()` 的顺序保证安全，但 `Load` 兼有副作用值得留意；`VNAiStudioMemory` 正因此绕开它自己读 JSON |
...
| 8 | `VNScriptRunner.RebuildStateBefore()` | 318 行的巨型 switch，每加一条有持续状态的命令都要改它——这是「三处同步」里最容易漏的一处 |
| 9 | `VNScenarioSchema.EventVariants` | 手工维护、与各模块 `OnLaunch` 的 `ctx.Kw(...)` 无编译期关联，加参数忘了补表就退回基础定义（不报错，静默降级） |
