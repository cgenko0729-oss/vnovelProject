...
| VNSecretPhotoMode / VNSecretPhotoUi / VNSecretPhotoRig / VNSecretAlbum / VNSecretPhotoDef | 秘密偷拍模式（解锁后右上角相机图标，任何台词处可进）：**不是事件模块但直接操控舞台镜头**（同 aitalk/interact 破铁律，边界=ZoomRoot 经 `VNCamera.BeginManual/SetManualView/EndManual`、UI 临时隐藏位、Ken Burns 开关，四条退出路径收口 `Restore()`）/ 手机相机风取景 HUD（主 Canvas 嵌套排序 70，教程锚点 `secretphoto.*`；圆形贴图自己生成硬边——`SoftCircle` 是发光用的软边，亮画面上看不见）/ **纯静态数学**：取景框钳制与 `VNCamera.ComputeOffset` 同构、框内权重、察觉涨速（只升不降；她不在框里不涨）/ 独立相册 `vn_secret_photos/`（照抄 VNPhotoAlbum，条目存拍摄信息；画廊「私密」页在解锁或相册非空时出现）/ 参数资产（单份 `VNGameConfig.secretPhoto`，没登记用 DontSave 默认值）。状态全在 flag（`秘密相机`/`道具_胶卷`/`偷拍_警惕_<角色>`），**VNSaveData 零改动**；被发现的台词走 `Runner.SayOutOfScript`（`RunInlineCo` 会嵌套 SayCo 清掉 `_waitingAtSay`）；防露边按背景实际尺寸取 `EffectiveOverscan()`（手搭场景背景常没那 60px 溢出）；装机 Tools → VN Effects → 场景装机 → **秘密偷拍 Secret Photo**（只建资产不动场景） |
...
| 秘密偷拍 | 不是命令：`flag 秘密相机=1` 解锁右上角相机图标、`道具_胶卷` 当胶卷（商店买）；台词处点进去滚轮缩放 1.0~1.6x + 拖动平移 + 快门存进独立私密相册（G 键画廊「私密」页）。察觉条只升不降，满了扣好感 + `偷拍_警惕_<角色>` 永久累积 + 她说一句 + 强制退出；数值全在 VNSecretPhotoDef | 一三五 |
