...
给立绘漫符系统（`mark` 命令 / [`VNCharacterMarks`](Assets/Project/Scripts/VNEffects/Script/VNCharacterMarks.cs)）生成符号图用的 prompt 库。
...
| 材质 | **共用角色立绘的材质实例** | 漫符会跟着立绘一起溶解/淡出/被 [`VNToneMatch`](Assets/Project/Scripts/VNEffects/VNToneMatch.cs) 色调匹配染色 → **别做 HDR 发光**，也别指望颜色绝对准 |
...
| 覆盖入口 | `VNCharacterDef.markSprites`（[`Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs:211`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs#L211)）（`name` 填英文正名 + `sprite`） | 每个角色资产都要配一遍（见第七节的批量工具建议） |
...
每个 [`VNCharacterDef`](Assets/Project/Scripts/VNEffects/Script/VNCharacterDef.cs) 的 `markSprites` 列表里加条目：`name` = 英文正名（`sweat` 等，
...
| 符号在游戏里泛白/发灰 | 被立绘的 [`VNToneMatch`](Assets/Project/Scripts/VNEffects/VNToneMatch.cs) 色调匹配染了 | 这是设计意图（漫符要跟着立绘融进背景光）；不能接受就调角色资产的色调匹配强度 |
